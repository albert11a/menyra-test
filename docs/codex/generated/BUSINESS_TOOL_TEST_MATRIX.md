...
| ------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Owner login                     | Manual required      | Seed actors exist; auth prompt reachable in browser.                                                             | Stable owner editor entrypoint required; current local route did not expose owner menu surface after login. |
...
| Menu editor price contract      | Automated green      | Unit/projection tests cover string editor price saved/published as numeric price.                                | Browser owner editor create/edit proof still required.                                                      |
| Owner menu management           | Blocked              | Rules tests cover own menu/public menu write and cross-business denial.                                          | Browser create/edit/delete/publish/QR settings blocked by unstable owner editor entrypoint.                 |
| Product/shop item management    | Manual required      | Shop seed and public menu browser smoke pass for public product menu.                                            | Add shop owner product create/edit/delete browser proof.                                                    |
| Hotel/travel offers             | Manual required      | Hotel seed and public profile/menu/offers projection coverage exists.                                            | Add hotel owner offer create/edit/delete browser proof.                                                     |
...
| Public startup console          | Blocked              | Public routes render, but interactive probe saw one denied Firestore `list`.                                     | Map and remove the denied list before P1.                                                                   |
| Bundle/performance              | Performance risk     | `npm run build` passed; the tracked `social-app.js` bundle changed with the menu save-path fix.                  | Public runtime extraction and bundle budgets remain later P3 work.                                          |
...
| Owner restaurant     | Manual required/blocked.                                                         | Login, menu create/edit/delete, publish public menu and QR table config after stable owner entrypoint exists. |
...
| Shop                 | Public browser smoke passed; owner tool manual required.                         | Shop owner product create/edit/delete.                                                                        |
| Hotel/travel         | Public browser smoke passed; owner tool manual required.                         | Hotel owner offer create/edit/delete.                                                                         |
