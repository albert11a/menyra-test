...
- Schritt 153 ist abgeschlossen: Public-Beitraege starten den ersten
  Posts-Read frueher, wenn eine belastbare canonical Restaurant-ID bereits im
  sichtbaren Public-Business-Context vorhanden ist.
- Bewertung von Schritt 153: `Tests/Build bestanden, Bundle-Budget gzip
  minimal rot, keine Production-Mutation, kein Deploy, keine Browser-/
  Smoke-Tests durch Codex`.
- Start-Commit Schritt 153:
  `da780a0648e4f9e142fe60113529705540eeb37b Document public focus menu loading step`.
- Schritt-153-Code-Commit:
  `9f615ab5 Start public posts read from canonical target`.
- Ursache Schritt 153:
  `ensurePostsDataForProfile` wartete bisher immer zuerst auf
  `resolveProfileRestaurantId`, bevor der sichtbare
  `restaurants/{restaurantId}/socialPosts`-Read gestartet wurde. In vielen
  Public-Routen ist die canonical Restaurant-ID aber schon im sichtbaren
  Context vorhanden. Dadurch konnte der Profilkopf bereits sichtbar sein,
  waehrend Beitraege erst nach der Identitaets-/Canonical-Absicherung ihren
  ersten echten Read starteten.
- Neue Regel Schritt 153:
  Wenn `buildVisiblePublicBusinessContext` bereits eine
  `canonicalRestaurantId` liefert, startet `ensurePostsDataForProfile` den
  bounded initial Posts-Read sofort mit `skipProfileResolve: true`. Der
  sichtbare Commit passiert weiterhin erst nach der normalen
  `resolveProfileRestaurantId`-Absicherung und nur, wenn das sichtbare Profil
  noch zu den akzeptierten IDs passt. Slugs/Hints aus `targetIds` werden nicht
  als frueher Posts-Read gestaerkt.
...
- Neue/erweiterte Tests Schritt 153:
  Regression fuer den Fall, dass die canonical Restaurant-ID bereits im
  sichtbaren Profil steht, die Identity-/Canonical-Absicherung aber noch
  pending ist: der Posts-Read startet sofort, waehrend der sichtbare
  Posts-Commit erst nach der Absicherung kommt. Der bestehende Alias-Test
  sichert weiter ab, dass Route-Aliase nicht als Profil-Post-Wahrheit gelesen
  werden.
- Verifikation Schritt 153:
  `node --test tests/profile-business-menu-runtime-cluster.test.mjs`
  bestanden (`5/5`),
  `node --test tests/*.test.mjs` bestanden (`127/127`),
  `npm run build` bestanden,
  `npm run check:social-bundle` nicht bestanden wegen bekanntem gzip-Budget:
  `social-app.js` raw `1050494` von `1052000`, gzip `285580` von `285000`
  (`+580` gzip). Keine riskante Bundle-Mikro-Optimierung in diesem Schritt.
- Browserpruefung Schritt 153:
  Nicht durch Codex ausgefuehrt, weil die aktuelle Repo-Regel keine Smoke-
  Tests oder Playwright-Laeufe durch Codex erlaubt, ausser der Nutzer fordert
  das spaeter ausdruecklich an. Der Nutzer testet manuell.
- Bewusst nicht geaendert in Schritt 153:
  keine UI-/Design-Aenderung, keine Posts-Renderer-Aenderung, keine
  Error-Text-Unterdrueckung, keine neue ID-Architektur, keine Staerkung von
  `socialFeed` oder `users/*/posts` als Profil-Post-Wahrheit, keine
...
- Rollback Schritt 153:
  `git revert 9f615ab5`.
  Wenn der Doku-Commit separat zurueckgenommen werden soll, den finalen
  Schritt-153-Doku-Commit ebenfalls per `git revert <commit>` rueckgaengig
  machen.
- Manuelle Testliste Schritt 153:
  `/casarita`, `/bro-pizza`, `/moa-risto-bar`, `/terrassiere`, `/moka-coffee`
  und mindestens ein Public-Profil aus dem Restaurant-Tab oeffnen. Mehrfach
  zwischen Profil, Menu und Beitraege wechseln, zurueck navigieren und dieselbe
  Route erneut oeffnen. Pruefen: `Beitraege werden geladen...` bleibt nicht
  unnoetig lange stehen, Beitraege erscheinen ohne kurzen falschen Empty- oder
  Fehlerzustand, vorhandene Profilkopf-/Menu-/Focus-Zustaende bleiben stabil,
  keine Console Errors, keine Schreibflows ausfuehren.
- Schritt 152 ist abgeschlossen: Public Menu und Focus werden weiter als
  gemeinsame Surface behandelt, aber ein vorhandener Public-Menu-Seed startet
  den passenden Focus-Load jetzt sofort im selben Ensure-Zyklus.
- Bewertung von Schritt 152: `Tests/Build bestanden, Bundle-Budget gzip
  minimal rot, keine Production-Mutation, kein Deploy, keine Browser-/
  Smoke-Tests durch Codex`.
- Start-Commit Schritt 152:
  `b662eb564502e1ce13dd13071d9668a85a948023 Revert "Harden public menu and posts loading states"`.
- Schritt-152-Code-Commit:
  `88463e38 Start public focus with settled menu truth`.
- Ursache Schritt 152:
  Die Public-Menu-Surface koordiniert Restaurant/Cafe-Menu und Focus bewusst
  gemeinsam, damit Focus nicht nachtraeglich oberhalb bereits gerenderter
  Produkte einspringt. Wenn Menu-Truth aber schon aus Cache/Seed vorhanden
  war, konnte `loadVisiblePublicMenuIds` frueh zurueckkehren, ohne den
  zugehoerigen Focus-Load zu starten. Danach musste der Renderer erst in
  einem spaeteren Pass `ensureFocusDataForProfile` ausloesen. Dadurch entstand
  bei normaler Navigation oder Tab-Wechsel ein zweiter sichtbarer Ladeabschnitt:
  Menu war fachlich schon bekannt, die koordinierte Surface wartete aber noch
  auf Focus-Settlement.
- Neue Regel Schritt 152:
  Wenn sichtbare Public-Menu-Truth fuer dieselbe Surface bereits settled ist
  und echte Menu-Items vorhanden sind, startet der passende Public-Focus-Load
  sofort. Bereits settled Focus-Truth oder ein laufender Focus-Ensure werden
  nicht erneut geladen. Die Renderer-Regel bleibt unveraendert: Menu und Focus
  bleiben gemeinsam koordiniert; Menu wird nicht isoliert vor Focus-Settlement
  freigegeben.
...
- Neue/erweiterte Tests Schritt 152:
  Regression fuer den Fall, dass Public-Menu-Items bereits als `seeded`
  sichtbar sind und Focus noch `unknown` ist: `ensureMenuDataForProfile`
  startet dann direkt den Focus-Load fuer dieselbe Restaurant-ID. Bestehende
  Surface-Tests sichern weiter ab, dass koordinierte Menu-Items bei
  `coordinateFocusWithMenu` bis zum Focus-Settlement warten.
- Verifikation Schritt 152:
  `node --test tests/profile-business-menu-runtime-cluster.test.mjs
  tests/public-menu-surface-state-utils.test.mjs` bestanden (`11/11`),
  `node --test tests/*.test.mjs` bestanden (`126/126`),
  `npm run build` bestanden,
  `npm run check:social-bundle` nicht bestanden wegen bekanntem gzip-Budget:
  `social-app.js` raw `1050494` von `1052000`, gzip `285579` von `285000`
  (`+579` gzip). Keine riskante Bundle-Mikro-Optimierung in diesem Schritt.
- Browserpruefung Schritt 152:
  Nicht durch Codex ausgefuehrt, weil die aktuelle Repo-Regel keine Smoke-
  Tests oder Playwright-Laeufe durch Codex erlaubt, ausser der Nutzer fordert
  das spaeter ausdruecklich an. Der Nutzer testet manuell.
- Bewusst nicht geaendert in Schritt 152:
  keine UI-/Design-Aenderung, keine Renderer-Entkopplung von Menu und Focus,
  keine Posts-State-Maschine, keine neue ID-Architektur, keine Spezialloesung
  fuer Casarita, keine PublicRoute-/Slug-Migration, keine Firestore-Regeln,
  keine Functions, keine Production-Firebase-Schreib-, Loesch- oder
  Mutationslaeufe, kein Deploy, keine Checkout-/Order-/QR-Schreibflows.
- Rollback Schritt 152:
  `git revert 88463e38`.
  Wenn der Doku-Commit separat zurueckgenommen werden soll, den finalen
  Schritt-152-Doku-Commit ebenfalls per `git revert <commit>` rueckgaengig
  machen.
- Offene Folgearbeit nach Schritt 152:
  Manuelle Browser-Matrix fuer Public-Profile und Public-Menu bei normaler
  Navigation, Tab-Wechsel, Hard Refresh und langsamer Verbindung; danach erst
  separat bewerten, ob Posts ebenfalls frueher/orchestrierter starten muessen.
  Sichere Bundle-Reduktion fuer `social-app.js` gzip `+579` bleibt
  Performance-Follow-up.
- Manuelle Testliste Schritt 152:
  `/casarita/menu`, `/bro-pizza/menu`, `/moa-risto-bar/menu`,
  `/terrassiere/menu`, `/moka-coffee` und mindestens ein Public-Profil aus dem
  Restaurant-Tab oeffnen. Mehrfach zwischen Profil und Menu wechseln, zurueck
  navigieren und dieselbe Route erneut oeffnen. Pruefen: Menu und Focus wirken
  gemeinsam stabil, kein zweiter langer `Menu wird geladen`-Abschnitt nachdem
  Produkte bereits bekannt waren, keine nachtraeglich einspringende Focus-
  Sektion, Profilkopf stabil, QR-Kontext stabil, keine Console Errors, keine
...
- Wichtigster Effekt aus Schritt 88:
  ...
  bleiben unveraendert.
- Schritt 89 ist abgeschlossen: Travel `Ofertat` zeigt nur noch Hotels/Motels
  mit aktiver Oferta und der Hotel-/Motel-Oferta-Editor pflegt die benoetigten
  Hotel-Card-Felder fuer Oferta.
- Bewertung von Schritt 89: `bestanden mit kleinem Rest-Risiko`.
- Wichtigster Effekt aus Schritt 89:
  Travel `Ofertat` wird aus aktiven Eintraegen in
  `restaurants/{restaurantId}/public/offers` gebaut. Hotels/Motels ohne aktive
  Oferta verschwinden aus diesem Tab. Oferta-Eintraege koennen jetzt Badge
...
- Wichtigster Effekt aus Schritt 95:
  ...
  unveraendert.
- Schritt 96 ist abgeschlossen: Travel-Businesses ohne bekannte Offer-Wahrheit
  gelten jetzt als cache-unvollstaendig und rohe Restaurantdaten ueberschreiben
  den Restaurant-Cache nicht mehr vor dem `public/offers`-Merge.
- Bewertung von Schritt 96: `bestanden mit kleinem Rest-Risiko`.
- Wichtigster Effekt aus Schritt 96:
  `Ofertat` kann beim Refresh die letzte bekannte angereicherte Restaurant- und
  Offer-Wahrheit aus dem Cache behalten. Der Cache wird erst nach
  `enrichRestaurantsWithPublicMeta()` mit Offer-Status geschrieben. Dadurch
...
- Schritt 113 ist abgeschlossen: Die Restaurant-Premium-Ad-Card fluchtet links
  mit den Restaurant-/Cafe-Cards darunter und wurde ueber kleinere Rating-/
  Preisspanne-Pills kompakter gemacht.
- Bewertung von Schritt 113: `bestanden mit kleinem Rest-Risiko`.
- Wichtigster Effekt aus Schritt 113:
  Der Ads-Wrapper und der horizontale Track haben keine zusaetzliche linke
  Einrueckung mehr; die Ad-Card ist von `26rem` auf `24rem` reduziert und die
...
- Schritt 134 ist abgeschlossen: Die Shopping-Brand-Intro-Card wurde in
  Breite, Startposition, Farbe und Headline-Schrift nachjustiert.
- Bewertung von Schritt 134: `bestanden mit kleinem Rest-Risiko`.
- Wichtigster Effekt aus Schritt 134:
  Die Intro-Card nutzt jetzt volle Spaltenbreite wie die anderen Shopping-Cards
  und startet rechts oben auf gleicher Hoehe wie die erste linke Card. Die
  leuchtende Mint-Farbe wurde durch ein ruhigeres Teal ersetzt, und die
  Headline-Schrift wurde naeher an die Feed-/Restaurant-Gate-Optik gezogen.
  Der App-Build-Token wurde auf
  `2026-06-27-shopping-brand-intro-card-02` aktualisiert und das Bundle wurde
...
Zusaetzlich fuer Schritt 89 manuell pruefen: Travel-Tab `Ofertat` zeigt keine
Hotels/Motels ohne aktive Oferta; neue Oferta erstellen und danach im Travel-
Tab als Hotel-Card sehen; Badge links, `Nete / dite`, Zentrum, Strand/See,
...
Zusaetzlich fuer Schritt 96 manuell pruefen: App einmal laden, bis Oferta-Cards
sichtbar sind; danach refreshen und Travel `Ofertat` beobachten; die letzte
...
Zusaetzlich fuer Schritt 113 manuell pruefen: Restaurant-Tab mit freigegebener
Ad oeffnen und pruefen, dass die erste Highlight-Ad links exakt mit den
Restaurant-/Cafe-Cards darunter fluchtet. Auf Mobile horizontal swipen und
...
Zusaetzlich fuer Schritt 134 manuell pruefen: App hart neu laden, bei Bedarf
mit `?sw-reset=1`, und bei `?debug-build=1` muss der Build-Stand
`2026-06-27-shopping-brand-intro-card-02` aktiv sein. Den Shopping-Tab oeffnen
und pruefen, dass die Brand-Intro-Card rechts oben dieselbe Breite wie die
