...
const allowedRoles = new Set(["owner", "admin", "manager", "waiter"]);
...
function renderOrders() {
  ...
  if (!rows.length) {
    ordersList.innerHTML = "<div class='empty'>No orders.</div>";
  ...
  ordersList.innerHTML = rows.map((order) => {
      ...
      return `
        <div class="item-row">
          <div>
            ...
            <div class="item-meta">${type}${category}</div>
          </div>
          <div class="item-meta">${Number(item.price || 0).toFixed(2)}</div>
        </div>
      `;
    }).join("");
    ...
    const actions = [];
    if (status === "new") {
      actions.push(`<button class="btn" data-action="accept" data-order-id="${order.id}">Accept</button>`);
      actions.push(`<button class="btn btn--danger" data-action="cancel" data-order-id="${order.id}">Cancel</button>`);
    }
    if (status === "ready") {
    ...
    return `
      <div class="order-card" data-order-id="${order.id}">
        ...
        <div class="item-list">${itemsHtml || "<div class='meta'>No items</div>"}</div>
        ${note}
  ...
  setStatusLine(`Orders: ${rows.length}`);
}
...
ordersList.addEventListener("click", async (e) => {
  ...
  try {
    if (action === "accept") await updateStatus(orderId, "accepted");
    if (action === "done") await updateStatus(orderId, "done");
    if (action === "cancel") await updateStatus(orderId, "cancelled");
  } catch (err) {
    console.error(err);
