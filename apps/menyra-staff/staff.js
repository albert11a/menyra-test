...
import { collection, query, where, orderBy, onSnapshot, addDoc, doc, setDoc, writeBatch } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
rows.forEach((row) => customersEl.appendChild(renderCustomerCard(row)));
    if (!snap.size) customersEl.innerHTML = "<div class='muted'>Noch keine Kunden (nur deine werden hier angezeigt).</div>";
  });
});
