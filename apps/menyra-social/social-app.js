...
const PUSH_SW_URL = "/apps/menyra-social/sw.js?v=2026-03-07-perf-7";
const PUSH_SW_SCOPE = "/apps/menyra-social/";
const PUSH_SW_READY_TIMEOUT_MS = 10000;
const CRM_LAZY_RENDERERS_MODULE_URL = "/apps/menyra-social/_shared/crm-lazy-renderers.js?v=2026-03-07-perf-8";
...
function ensureTabData(tab) {
  ...
  if ((tab === "leads" || tab === "staff") && isCeoUser()) {
...
function prefetchCrmLazyRenderers() {
  ...
  promise.then(() => {
    if (!state.user) return;
    if (state.activeTab === "leads" || state.activeTab === "staff") {
      render();
    }
  }).catch(() => {});
}
