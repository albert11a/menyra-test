...
export function createAppControllerBridge({
  ...
  function createProfileOpenFlowControllerConfig() {
    return {
      ...
      beginProfileLoad: (target = {}) => profile.beginProfileLoad(target),
      isCurrentProfileLoad: (loadId, targetKey = "") => profile.isCurrentProfileLoad(loadId, targetKey),
      cancelProfileLoad: (reason = "") => profile.cancelProfileLoad(reason),
      patchVisibleProfileIfCurrent: (targetKey = "", patch = {}) => profile.patchVisibleProfileIfCurrent(targetKey, patch),
