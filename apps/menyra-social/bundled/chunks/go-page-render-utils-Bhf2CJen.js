...
`,g=Object.freeze({brand:"Mnyra GO",partyQuestion:"Sa veta jeni?",partyOne:"person",partyMany:"veta",step:"Hapi",stepBack:"Kthehu një hap prapa",next:"Vazhdo",categoryQuestion:"Çka dëshironi?",whenQuestion:"Kur?",whenLaterLabel:"Zgjidh ditën dhe orën",placeQuestion:"Ku?",placeLabel:"Qyteti",placeEmpty:"Shto qytetin tënd",cityPlaceholder:"Shkruaj qytetin",locationChange:"Ndrysho",locationSave:"Ruaj",submit:"Shiko ofertat",searching:"Po kërkojmë...",resultsHeadline:"lokale kanë oferta për ju",offering:"po ju ofron",forGroup:"për grupin tuaj",accept:"Prano ofertën",confirming:"Po konfirmohet...",sponsored:"Sponsored",onlyGo:"Vetëm me Mnyra GO",tableIncluded:"Tavolinë",emptyTitle:"Nuk gjetëm ofertë GO që përputhet tani.",emptySubtitle:"Provo me një orë tjetër ose me një grup tjetër.",doneTitle:"U krye",reservationConfirmed:"Tavolina është konfirmuar",claimConfirmed:"Oferta është e juaja",menu:"Shiko menunë",directions:"Udhëzime",code:"Kodi",cancel:"Anulo",cancelConfirm:"Dëshiron ta anulosh?",cancelYes:"Po, anuloje",cancelNo:"Jo",saveToAccount:"Ruaje në llogarinë tënde",signIn:"Hyr",errorTitle:"Mnyra GO është përkohësisht i padisponueshëm.",errorRetry:"Provo prapë",alternatives:"alternativa për ju",peopleSuffix:"persona",now:"Tani",back:"Ndrysho kërkimin"}),w="/apps/menyra-social/assets/go/",y=Object.freeze([Object.freeze({file:"story-1-unt.webp",headline:"A je unt?",text:"Trego sa veta jeni edhe çka po ju hahet."}),Object.freeze({file:"story-2-ku-me-dal.webp",headline:"S’po din ku me dal?",text:"Mos lyp lokal — lokalet që t’përshtaten t’gjejnë ty."}),Object.freeze({file:"story-3-shtrejt.webp",headline:"Edhe shumë shtrejt?",text:"Lokalet t’çojnë oferta me zbritje direkt — ti veç zgjedh."}),Object.freeze({file:"story-4-knaqu.webp",headline:"Ofertat t’vijn. Ti veç shko, knaqu.",text:"Zgjedhe ofertën që t’pëlqen, shko aty edhe knaqu."})]),E=Object.freeze({now:"zap",in30:"timer",in60:"clock",later:"calendar-clock"}),c=Object.freeze(["party","category","when","place"]),k=Object.freeze({party:"users",category:"utensils",when:"clock",place:"map-pin"});function n(a=""){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function f(a=""){const e=String(a||"").trim();return c.includes(e)?e:c[0]}function ae(a=""){const e=c.indexOf(f(a));return c[Math.min(c.length-1,e+1)]}function ne(a=""){const e=c.indexOf(f(a));return c[Math.max(0,e-1)]}function u(a){const e=Math.trunc(Number(a));return Number.isFinite(e)?Math.min(m,Math.max(l,e)):A}function $(a,e=g){return u(a)===1?e.partyOne||g.partyOne:e.partyMany||g.partyMany}function S(a,e=g){return`${u(a)} ${$(a,e)}`}function G(a){const e=u(a),t=m-l;return Math.round((e-l)/t*100)}function z(a,{active:e=!1,attr:t="",value:r="",iconName:o="",wide:p=!1}={}){return`
  ...
  `}function M(a,{nowMs:e=Date.now()}={}){const t=Date.parse(String(a||""));if(!Number.isFinite(t))return"";if(Math.abs(t-e)<900*1e3)return g.now;const r=new Date(t);return`Rreth ${String(r.getHours()).padStart(2,"0")}:${String(r.getMinutes()).padStart(2,"0")}`}function B(a="",e={},t=g){if(a==="party")return S(e.partySize,t);if(a==="category"){const r=b.find(o=>o.key===String(e.category||"all"));return r?r.label:""}if(a==="when"){const r=x.find(o=>o.key===String(e.when||"now"));return r?r.label:""}return a==="place"?String(e.city||"").trim()||t.placeEmpty:""}function T(a="",e={},t=g){const r=c.indexOf(a),o=c.slice(0,Math.max(0,r));return o.length?`
    <div class="mnyra-go-page__ask-done">
      ${o.map(p=>`
        <button type="button" class="mnyra-go-page__ask-tag" data-go-goto="${n(p)}">
          ${i(k[p]||"check-check")}<span>${n(B(p,e,t))}</span>
        </button>
      `).join("")}
    </div>
  `:""}function D(a={},e=g){const t=u(a.partySize);return`
  ...
  `}function P(a={},e=g){const t=String(a.category||"all");return`
    <div class="mnyra-go-page__chips" role="group" aria-label="${n(e.categoryQuestion)}">
      ${b.map((r,o)=>z(r.label,{active:t===r.key,attr:"data-go-category",value:r.key,iconName:r.icon,wide:o===0})).join("")}
    </div>
  `}function L(a={},e=g){const t=String(a.when||"now");return`
    <div class="mnyra-go-page__chips" role="group" aria-label="${n(e.whenQuestion)}">
      ${x.map(r=>z(r.label,{active:t===r.key,attr:"data-go-when",value:r.key,iconName:E[r.key]||"clock"})).join("")}
    </div>
    ${t==="later"?`
  ...
  `}function N(a={},e=g){const t=f(a.step),r=c.indexOf(t),o=c.length,p=Math.round((r+1)/o*100);let s="",d="",h="";return t==="party"?(d=e.partyQuestion,s=D(a,e),h=`<button type="button" class="mnyra-go-page__cta" data-go-step-next>${n(e.next)}${i("chevron-right")}</button>`):t==="category"?(d=e.categoryQuestion,s=P(a,e)):t==="when"?(d=e.whenQuestion,s=L(a,e),String(a.when||"now")==="later"&&(h=`<button type="button" class="mnyra-go-page__cta" data-go-step-next>${n(e.next)}${i("chevron-right")}</button>`)):(d=e.placeQuestion,s=I(a,e),h=`<button type="button" class="mnyra-go-page__cta" data-go-submit>${i("search")}${n(e.submit)}</button>`),`
    <section class="mnyra-go-page__ask" data-go-step="${n(t)}">
      ...
      <h2 class="mnyra-go-page__q">${i(k[t]||"users")}${n(d)}</h2>
      ${s}
      ${h}
    </section>
  `}function j(a={},{texts:e=g,busyOfferId:t="",nowMs:r=Date.now()}={}){const o=t&&t===a.offerId,p=Number.isFinite(Number(a.distanceKm))&&a.distanceKm!==null?`${Number(a.distanceKm).toFixed(1)} km`:"",s=a.isNow?e.now:M(a.expectedArrivalAt,{nowMs:r});return`
  ...
  `}function R(a={},e=g){const t=Array.isArray(a.results)?a.results:[],r=Number(a.nowMs)||Date.now();return t.length?`
    <h2 class="mnyra-go-page__lead">${t.length} ${n(e.resultsHeadline)}</h2>
    ${a.notice?`<p class="mnyra-go-page__note">${n(a.notice)}</p>`:""}
    ${t.map(o=>j(o,{texts:e,busyOfferId:a.busyOfferId,nowMs:r})).join("")}
  ...
  `:`
    ...
    `}function C(a={},e=g){const t=a.booking||{},r=t.type==="reservation",o=Number(a.nowMs)||Date.now(),p=M(t.expectedArrivalAt,{nowMs:o}),s=String(a.statusLabel||"").trim()||(r?e.reservationConfirmed:e.claimConfirmed);return a.confirmCancel?`
  ...
  `}function q(a={},e=g){const t=String(a.error||"").trim()||e.errorTitle;return`
    ...
    `}
  `}function H(a=[]){const e=Array.isArray(a)?a:[],t=y.length;return`
  ...
  `}function K(a={}){return H(a.storyShown)}function te(a={}){const e={...g,...a.texts||{}},t=String(a.view||"search"),r=t==="search";let o="";return t==="loading"?o=`<div class="mnyra-go-page__empty">${n(e.searching)}</div>`:t==="results"?o=R(a,e):t==="booking"?o=C(a,e):t==="error"?o=q(a,e):o=K(a),`
    <div class="mnyra-go-page" data-go-page data-go-view="${n(t)}">
      ${r?`<div class="mnyra-go-page__top">${N(a.form||{},e)}</div>`:""}
      <div class="mnyra-go-page__bento" data-go-bento>${o}</div>
    </div>
  `}const re=g,oe=y,ie=w;export{ee as GO_PAGE_CSS,ie as GO_PAGE_STORY_BASE,oe as GO_PAGE_STORY_SLIDES,J as GO_PAGE_STYLE_ELEMENT_ID,re as GO_PAGE_TEXTS,c as GO_STEPS,u as clampGoPartySize,G as goPartyFillPercent,S as goPartyLabel,$ as goPartyWord,ae as nextGoStep,ne as previousGoStep,te as renderGoPageCore,f as resolveGoStep};
