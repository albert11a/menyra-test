...
`;function F(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Ma({businessName:e="",logoUrl:t="",imageUrl:a="",variant:n="",benefitLabel:r="",benefitView:i=null,sponsored:d=!1,meta:u=[],ctaLabel:b="",ctaIcon:x="check-check",ctaDisabled:p=!1,cardAttrs:m="",ctaAttrs:g="",texts:k=Ze}={}){const A={...Ze,...k||{}},O=(Array.isArray(u)?u:[]).filter(W=>W&&W.label),M=String(b||A.accept),_=i&&typeof i=="object"?i:{},E=String(_.headline||r||""),H=String(_.priceGo||""),v=String(_.priceRegular||""),C=String(a||"").trim(),G=Ca({imageUrl:C,variant:n}),J=G===mt,N=C?`<img class="mnyra-go-page__card-photo" src="${F(C)}" alt="" loading="lazy" decoding="async" />`:"",Z=`
    ...
    <p class="mnyra-go-page__card-benefit${H?" mnyra-go-page__card-benefit--title":""}">${F(E)}</p>
  ...
  `,V=`
    <div class="mnyra-go-page__card-meta">
      ${O.map(W=>`<span>${ce(W.icon||"")}${F(W.label)}</span>`).join("")}
    </div>
...
`;function Ga(e={},t={}){const a=t.escapeHtml,n=t.icon,r=e.imageUrl?`<img class="go-hl__media" src="${o(a,e.imageUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:"",i=e.action?`<span class="go-hl__action">${o(a,e.action)}</span>`:`
  ...
  `}function Na({stats:e={},deps:t={}}={}){return`
  ...
  `}function Ka({tab:e="active",deps:t={}}={}){const a=t.escapeHtml,n=t.icon;return`
    <div class="go-tabs" role="tablist" data-go-tabs>
      ${[["active",s.tabs.active,"zap"],["offers",s.tabs.offers,"tag"],["archive",s.tabs.archive,"archive"],["options",s.tabs.options,"settings"]].map(([i,d,u])=>`
        <button type="button" role="tab" aria-selected="${e===i?"true":"false"}" data-go-business-tab="${i}"
          class="go-tab">${U(n,u,"w-4 h-4")}<span class="go-tab-label">${o(a,d)}</span></button>
      `).join("")}
    </div>
  `}function _e(e={},t={},{found:a=!1}={}){const n=t.escapeHtml,r=e.type==="reservation",i=gt(e.expectedArrivalAt),d=e.benefitLabel||e.snapshot?.benefitLabel||"",u=!e.businessSeenAt,b=i?`${s.around} ${i}`:s.guestName;return`
    <div class="p-4 rounded-[1.6rem] border ${a?"bg-white border-indigo-300 ring-2 ring-indigo-100":u?"bg-indigo-50/50 border-indigo-100":"bg-slate-50 border-slate-100"}"
      ...
      ${e.commission?`
        ...
        <p class="mt-3 pt-3 border-t border-slate-200/70 text-[10px] font-black uppercase tracking-widest text-slate-400">
          ${o(n,s.commission)} · ${o(n,Ea(e.commission.amountCents))}
        </p>
      `:""}
      ${e.status==="checked_in"?`
        <div class="mt-3">
  ...
  `}function ue({eyebrow:e="",title:t="",sub:a="",action:n="",body:r="",deps:i={}}={}){const d=i.escapeHtml;return`
  ...
  `}function Ua({offer:e={},businessName:t="",previewImageUrl:a="",deps:n={}}={}){const r=n.escapeHtml;return`
    <div data-go-offer-preview>
      ...
      <div style="pointer-events:none;" aria-hidden="true">
        ${Ma({businessName:t,imageUrl:a||e.imageUrl||"",benefitLabel:e.benefitLabel||"",benefitView:ka(e.benefit||{}),meta:[{icon:"users",label:ut(e)},{icon:"clock",label:ct(e)}]})}
      </div>
    </div>
  `}function K(e,t="",a=""){return`<label class="text-[10px] font-black uppercase tracking-widest text-slate-400"${a?` for="${o(e,a)}"`:""}>${o(e,t)}</label>`}function he(e,{active:t=!1,attr:a="",value:n="",escapeHtml:r=null}={}){return`
...
`;function xe({attr:e="",unit:t="€",value:a="",placeholder:n="",mode:r="decimal",inputClass:i="",escapeHtml:d=null}={}){return`
  ...
  `}const Ve=Object.freeze([10,15,20,25]);function Za({benefit:e={},percentCustom:t=!1,errorFor:a=()=>"",inputClass:n="",inputBase:r="",escapeHtml:i=null}={}){const d=g=>K(i,g),u=g=>{const k=a(g);return k?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="${o(i,g)}">${o(i,k)}</p>`:""};if(!pa.includes(e.kind))return`
      <p class="go-offer-saving font-bold text-slate-400">${o(i,s.benefitLegacy)}</p>
      ${u("benefit")}
    `;const b=Number(e.percent)||0,x=t||b>0&&!Ve.includes(b),p=()=>{const g=me(e.savingCents);if(!g)return"";const k=Math.round(Number(e.savingPercent)||0);return`
      <p class="mt-3 go-offer-saving font-black text-emerald-600" data-go-benefit-saving>
        ${o(i,s.saving)} ${o(i,g)}${k>0?` &middot; -${k}%`:""}
      </p>
    `},m=()=>`
    <div class="mt-3">
      ${d(s.priceRegular)}
  ...
  `;if(e.kind===Y)return`
      ...
      <div class="mt-4">
        ${d(s.scopeQuestion)}
        <div class="mt-2 flex flex-wrap gap-2">
          ${[["all",s.scopeAll],["food",s.scopeFood],["drinks",s.scopeDrinks]].map(([g,k])=>se(k,{active:(e.scope||"all")===g,attr:"data-go-discount-scope",value:g,escapeHtml:i})).join("")}
        </div>
        ${u("benefitScope")}
      </div>
    `;if(e.kind===X)return`
    ...
    `;if(e.kind===ae){const g=String(e.conditionType||"");return`
      ...
      <div class="mt-4">
        ...
        <div class="mt-2 grid grid-cols-2 gap-2">
          ${[["food",s.conditionFood],["drink",s.conditionDrink],["any_order",s.conditionAny],["custom",s.conditionCustom]].map(([k,A])=>se(A,{active:g===k,attr:"data-go-benefit-condition",value:k,escapeHtml:i})).join("")}
        </div>
        ${g==="custom"?`
          <div class="mt-3">
    ...
    `}return e.kind===ne?`
      ...
      ${u("benefitItem")}
      ${m()}
    `:""}function Va({imageUrl:e="",photo:t={},escapeHtml:a=null,icon:n=null}={}){const r=String(t.status||""),i=r==="uploading",d=String(t.previewUrl||e||""),u=r==="error"?String(t.error||s.photoError):"",b=d?`
    ...
    <div data-go-section="photo">
      ...
      ${u?`<p class="mt-2 text-[11px] font-bold text-rose-500">${o(a,u)}</p>`:""}
    </div>
  `}function Wa(e=""){const t=String(e||"all").trim().toLowerCase();return t==="food"?["food"]:t==="coffee"||t==="drinks"||t==="dessert"?["drinks"]:["food","drinks"]}function Hn(e=[]){const t=Array.isArray(e)?e:[],a=t.includes("food"),n=t.includes("drinks");return a&&n?"all":a?"food":n?"drinks":""}function Zn({editor:e=null,businessName:t="",deps:a={}}={}){if(!e)return"";const n=a.escapeHtml,r=a.icon,i=e.draft||{},d=Array.isArray(e.errors)?e.errors:[],u=v=>d.find(C=>C.field===v)?.message||"",b=Array.isArray(i.partyRanges)?i.partyRanges:[],x=i.schedule?.mode==="windows"?"windows":"always",p=Array.isArray(i.schedule?.days)&&i.schedule.days.length?i.schedule.days:I.slice(),m=re.every(v=>b.includes(v.key)),g=Array.isArray(e.intents)?e.intents:Wa(i.category),k=i.benefit||{},A=e.mode==="edit",O="w-full go-offer-input bg-slate-50 border border-slate-100 rounded-2xl px-4 text-sm font-bold text-slate-900 outline-none focus:border-indigo-400",M=`mt-2 ${O}`,_='<div class="h-px bg-slate-100"></div>',E=v=>`<p class="mt-1 text-[11px] font-semibold text-slate-400">${o(n,v)}</p>`,H=ja(i).ok&&g.length>0;return`
      ...
      <div class="modal-frame">
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll px-6 py-5 space-y-5" data-go-editor-scroll>
          ...
          <div class="go-offer-section" data-go-section="benefit">
            ${K(n,s.benefitQuestion)}
            ${E(s.benefitHint)}
            <div class="mt-4 grid grid-cols-2 gap-2">
              ${[[Y,s.benefitPercent],[X,s.benefitBundle],[ae,s.benefitFree],[ne,s.benefitSpecial]].map(([v,C])=>`
                <button type="button" data-go-benefit-kind="${o(n,v)}"
                  aria-pressed="${k.kind===v?"true":"false"}"
                  class="go-offer-kind px-3 rounded-2xl text-xs font-black transition-colors ${k.kind===v?"bg-slate-900 text-white":"bg-slate-50 text-slate-600 border border-slate-100"}">
                  ${o(n,C)}
                </button>
              `).join("")}
            </div>
            <div class="mt-5 go-offer-form" data-go-benefit-form>
              ${Za({benefit:k,percentCustom:e.percentCustom===!0,errorFor:u,inputClass:M,inputBase:O,escapeHtml:n})}
            </div>
          </div>
          ...
          <div data-go-section="partyRanges">
            ...
            -->
            <div class="mt-3">
              ${he(s.partyAll,{active:m,attr:"data-go-offer-party",value:"all",escapeHtml:n})}
            </div>
            <div class="mt-2 flex flex-wrap gap-2">
              ${re.map(v=>he(v.label,{active:b.includes(v.key),attr:"data-go-offer-party",value:v.key,escapeHtml:n})).join("")}
            </div>
            ${u("partyRanges")?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="partyRanges">${o(n,u("partyRanges"))}</p>`:""}
          </div>

          ${_}

          <div data-go-section="category">
            ${K(n,s.categoryQuestion)}
            ${E(s.categoryHint)}
            <div class="mt-3 space-y-2">
              ${[{key:"food",label:s.ifFood},{key:"drinks",label:s.ifDrinks}].map(v=>{const C=g.includes(v.key),G=et.find(J=>J.key===v.key)?.hint||"";return`
                  <button type="button" data-go-offer-intent="${o(n,v.key)}" aria-pressed="${C?"true":"false"}"
                    class="w-full text-left go-offer-answer px-4 py-3 rounded-2xl border transition-colors ${C?"bg-slate-900 border-slate-900 text-white":"bg-slate-50 border-slate-100 text-slate-600"}">
                    <span class="block text-xs font-black">${o(n,v.label)}</span>
                    <span class="block mt-0.5 text-[11px] font-semibold ${C?"text-white/60":"text-slate-400"}">${o(n,G)}</span>
                  </button>
                `}).join("")}
            </div>
            ${u("category")?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="category">${o(n,u("category"))}</p>`:""}
          </div>

          ${_}

          <div data-go-section="schedule">
            ${K(n,s.scheduleQuestion)}
            ${E(s.scheduleHint)}
            <div class="mt-3 flex flex-wrap gap-2">
              ${he(s.always,{active:x==="always",attr:"data-go-offer-schedule",value:"always",escapeHtml:n})}
              ${he(s.specificHours,{active:x==="windows",attr:"data-go-offer-schedule",value:"windows",escapeHtml:n})}
            </div>
            ${x==="windows"?`
              <!--
        ...
        <div class="px-6 pb-6 pt-4 border-t border-slate-100 bg-white modal-footer-safe">
          ...
          <button type="button" data-go-offer-save ${e.saving?"disabled":""}
            ...
            class="w-full py-4 rounded-[1.8rem] text-white font-black text-xs uppercase tracking-widest active:scale-95 transition-all ${H?"bg-indigo-600 shadow-xl shadow-indigo-500/20":"bg-indigo-300"}">
  ...
  `}function Vn({restaurantName:e="",tab:t="active",stats:a={},search:n={},bookings:r=[],offers:i=[],settings:d={},paused:u=!1,loading:b=!1,error:x="",deps:p={}}={}){const m=p.escapeHtml,g=p.icon,k=r.filter(_=>["confirmed","checked_in"].includes(_.status)),A=r.filter(_=>!["confirmed","checked_in"].includes(_.status)),O=i.filter(_=>_.status!=="archived");let M="";if(t==="offers")M=ue({eyebrow:s.brand,title:s.tabs.offers,sub:`${O.length} ${O.length===1?"oferte":"oferta"}`,action:`
      ...
      `,body:O.length?`<div class="space-y-3">${O.map(_=>La(_,p)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(m,s.emptyTitle)}</div>`,deps:p});else if(t==="archive")M=ue({eyebrow:s.brand,title:s.tabs.archive,sub:`${A.length}`,body:A.length?`<div class="space-y-3">${A.map(_=>_e(_,p)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(m,s.noHistory)}</div>`,deps:p});else if(t==="options"){const _=gt(d?.pausedUntil);M=ue({eyebrow:s.brand,title:s.tabs.options,body:`
        <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
          <div>
            <p class="text-xs font-black text-slate-800">${o(m,s.goOn)}</p>
            <p class="text-[10px] font-bold text-slate-400">${o(m,u?`${s.pausedUntil} ${_}`:"ON")}</p>
          </div>
          <span class="text-[9px] font-black uppercase tracking-widest ${u?"text-amber-600":"text-emerald-600"}">
            ${o(m,u?s.paused:s.statActive)}
          </span>
        </div>
        <div class="mt-4 flex flex-wrap gap-2">
          ${u?`<button type="button" data-go-pause="0" class="min-h-[44px] px-4 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest">${o(m,s.resume)}</button>`:[{value:"30",label:"30 min"},{value:"60",label:"1 orë"},{value:"tomorrow",label:"Deri nesër"},{value:"-1",label:"Pa afat"}].map(E=>`
              <button type="button" data-go-pause="${E.value}"
                class="min-h-[44px] px-4 rounded-2xl bg-slate-50 border border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-600">${o(m,E.label)}</button>
            `).join("")}
        </div>
        <p class="mt-4 text-[10px] font-bold text-slate-400">${o(m,s.keepsRunning)}</p>
      `,deps:p})}else M=ue({eyebrow:s.brand,title:s.tabs.active,sub:`${k.length}`,body:`
        ${Ia({code:n.code,status:n.status,busy:n.busy,deps:p})}
        ${n.booking?`
          <div class="mb-4">${_e(n.booking,p,{found:!0})}</div>
        `:""}
        ${b?`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-400">${o(m,s.loading)}</div>`:k.length?`<div class="space-y-3">${k.filter(_=>_.id!==n.booking?.id).map(_=>_e(_,p)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(m,s.noBookings)}</div>`}
      `,deps:p});return`
    <div class="p-6 app-main-content-safe animate-in slide-in-from-right-10 duration-500" data-go-admin>
      ...
      -->
      <div class="mb-6">
        <h1 class="text-xl font-black tracking-tight text-slate-900 md:text-2xl">${o(m,s.brandMnyra)}<span class="text-indigo-600">${o(m,s.brandGo)}</span></h1>
        <p class="text-[11px] text-slate-400 font-semibold mt-0.5">${o(m,e?`${s.editor} ${e}`:s.editor)}</p>
      </div>
...
`;function Qa(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(We)))try{const t=e.createElement("style");t.id=We,t.textContent=qa,e.head?.appendChild(t)}catch{}}function S(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function j(e,t,a=""){if(typeof e!="function")return"";try{return e(t,a)||""}catch{return""}}const Ya=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function Xa({businessType:e="",isShopCatalog:t=!1}={}){if(t)return"shop";const a=String(e||"").trim().toLowerCase();return Ya.includes(a)?"hotel":"restaurant"}function Ja(e=new Date().getHours()){const t=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return t>=5&&t<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:t>=11&&t<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:t>=18&&t<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function en({name:e="",logoUrl:t="",hour:a=new Date().getHours(),iconFn:n}={}){const r=Ja(a),i=S(e||"Business");return`
    <div class="mnyra-dash__greet">
      <p class="mnyra-dash__greet-title">
        ...
        <span class="mnyra-dash__greet-logo">
          ${t?`<img src="${S(t)}" alt="${i}" title="${i}" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:`<span class="mnyra-dash__greet-logo-fallback" title="${i}">${j(n,"store","w-4 h-4")}</span>`}
        </span>
      </p>
  ...
  `:""}function ln({cards:e=[],iconFn:t}={}){const a=(Array.isArray(e)?e:[]).filter(r=>r&&r.key);if(!a.length)return"";const n=a.map((r,i)=>{const d=S(r.label||"");if(r.pending)return'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>';const u=i<2?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"';let b="";r.imageUrl?b=`<img class="mnyra-dash__hl-media" src="${S(r.imageUrl)}" alt="" ${u} decoding="async" onerror="this.style.display='none'" />`:r.videoUrl&&(b=`<video class="mnyra-dash__hl-media" src="${S(r.videoUrl)}#t=0.1" preload="metadata" muted playsinline disablepictureinpicture tabindex="-1" aria-hidden="true"></video>`);const x=`
      <span class="mnyra-dash__hl-plate">${j(t,r.iconName||"image","w-6 h-6")}</span>
      ${b}
    `,p=r.withEye?`<span class="mnyra-dash__hl-eye">${j(t,"eye","w-4 h-4")}</span>`:"";let m;r.locked?m=`<span class="mnyra-dash__hl-lock">${j(t,"lock","w-3 h-3")}Me pagesë</span>`:r.loading?m='<span class="mnyra-dash__hl-value mnyra-dash__hl-value--pending" aria-hidden="true"></span>':r.emptyText?m=`<span class="mnyra-dash__hl-empty">${S(r.emptyText)}</span>`:m=`<span class="mnyra-dash__hl-value">${p}${S(r.value||"0")}</span>`;let g;r.locked?g=`class="mnyra-dash__hl-card mnyra-dash__hl-card--locked" data-dashboard-metric-locked="${S(r.key)}"`:r.composer?g=`class="mnyra-dash__hl-card" data-dashboard-composer="${S(r.composer)}"`:g=`class="mnyra-dash__hl-card"${r.panelTab?` data-dashboard-panel-tab="${S(r.panelTab)}"`:""}`;const k=r.locked?`${d} – me pagesë`:`${d} ${r.emptyText||r.value||""}`.trim();return`
      <button type="button" ${g} data-dashboard-metric="${S(r.key)}" aria-label="${S(k)}">
        ${x}
  ...
  `}function cn(e=[]){return(Array.isArray(e)?e:[]).filter(t=>t&&t.key).map(t=>[t.key,t.label||"",t.value||"",t.emptyText||"",t.imageUrl||"",t.videoUrl||"",t.iconName||"",t.panelTab||"",t.composer||"",t.pending?"p":"",t.loading?"l":"",t.locked?"x":"",t.withEye?"e":""].join("~")).join("|")}const bt=Object.freeze([Object.freeze({id:"funksionet",label:"Funksionet",iconName:"layout-grid"}),Object.freeze({id:"analitika",label:"Analitika",iconName:"bar-chart-3"}),Object.freeze({id:"opsionet",label:"Opsionet",iconName:"settings"})]);function pe(e=""){const t=String(e||"").trim().toLowerCase();return bt.some(a=>a.id===t)?t:"funksionet"}function un({activeTab:e="funksionet",iconFn:t}={}){const a=pe(e);return`<div class="mnyra-dash__tabs" role="tablist" data-dashboard-panel-tabs>${bt.map(r=>{const i=r.id===a;return`
      ...
      >${j(t,r.iconName,"w-4 h-4")}<span class="mnyra-dash__tab-label">${S(r.label)}</span></button>
    `}).join("")}</div>`}function yt(e=""){return`
    <div class="mnyra-dash__bento" data-dashboard-bento>
      ${e}
    </div>
  `}function hn({posts:e=[],iconFn:t}={}){const a=Array.isArray(e)?e:[];let n="";return a.length?(n=a.map(r=>{const i=[r.dateLabel,`${te(r.likesCount||0)} Likes`,`${te(r.commentsCount||0)} Kommentare`];return Number(r.impressions||0)>0&&i.push(`${te(r.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${r.thumbUrl?`<img src="${S(r.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:j(t,r.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
  ...
  `}const _n="menyra_social_dashboard_cache_v1::",Qe="menyra_social_composer_products_v1::",Ye=2500,Xe=1200,xn=6,vn=3,kn=Object.freeze({menuImageUrl:"/apps/menyra-social/assets/panel/menu-scan.jpg",qrImageUrl:"/apps/menyra-social/assets/panel/qr-stand.jpg"}),wn=Object.freeze({menuOpens:"Menü-Aufrufe",qrScans:"QR-Scans"});function R(e){const t=Number(e);return Number.isFinite(t)?t:0}function $n(e={}){const t=String(e.createdAtClient||"").trim();if(t){const n=new Date(t);if(!Number.isNaN(n.getTime()))return n}const a=e.createdAt;if(a&&typeof a.toDate=="function")try{const n=a.toDate();if(n instanceof Date&&!Number.isNaN(n.getTime()))return n}catch{}return null}function Sn(e="",t={}){const a=Array.isArray(t.media)&&t.media.length?t.media[0]:{},n=String(a.type||t.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",r=String(a.thumbUrl||(n==="image"?a.url:"")||t.thumbUrl||"").trim(),i=n==="video"?String(a.url||t.mediaUrl||"").trim():"",d=$n(t);return{id:String(e||"").trim(),caption:String(t.caption||"").trim(),mediaType:n,thumbUrl:r,videoUrl:i,likesCount:R(t.likesCount),commentsCount:R(t.commentsCount),impressions:0,dateLabel:d?d.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:d?d.getTime():0}}function zn({days:e=[],todayKey:t="",rawPosts:a=[]}={}){const n=Array.isArray(e)?e:[],r=Re(n),i=n.find(p=>String(p?.date||p?.id||"").trim()===String(t||"").trim()),d=Re(i?[i]:[]),u=r.merged?.posts&&typeof r.merged.posts=="object"?r.merged.posts:{},b=(Array.isArray(a)?a:[]).map(p=>Sn(p?.id,p?.data||{})).filter(p=>p.id).map(p=>({...p,impressions:R(u[p.id]?.impressions)})),x=b.slice().sort((p,m)=>m.createdAtMs-p.createdAtMs).slice(0,vn);return{day:String(t||"").trim(),week:r.summary,today:d.summary,posts:x,latestPost:An(b)}}function An(e=[]){const t=(Array.isArray(e)?e:[]).filter(a=>a&&a.id);return t.length?t.slice().sort((a,n)=>R(n.createdAtMs)-R(a.createdAtMs)||R(n.impressions)-R(a.impressions)||R(n.likesCount)-R(a.likesCount))[0]:null}function Pn({profile:e={},restaurant:t={}}={}){return Gt({profile:e,restaurant:t,feature:"qr"})}function jn(e={}){const t=e&&typeof e=="object"?e:{};return String(t.titleImageUrl||t.coverImageUrl||t.coverUrl||t.heroUrl||t.bannerUrl||"").trim()}function Dn({model:e=null,coverUrl:t="",subscribed:a=!1,assets:n={}}={}){const r=e?.today||{},i=!e,d=e?.latestPost||null,u=[];if(i)u.push({key:"latestPost",label:"Postimi fundit",pending:!0});else if(!d)u.push({key:"latestPost",label:"Postimi fundit",emptyText:"S'ka postim",iconName:"image",composer:"post"});else{const b=String(d.thumbUrl||"").trim();u.push({key:"latestPost",label:"Postimi fundit",value:te(R(d.impressions)),withEye:!0,imageUrl:b,videoUrl:b?"":String(d.videoUrl||"").trim(),iconName:"image",panelTab:"analitika"})}return u.push({key:"profileViews",label:"Vizitor n'profil",value:te(R(r.profileViews)),withEye:!0,loading:i,imageUrl:String(t||"").trim(),iconName:"user",panelTab:"analitika"}),u.push({key:"menuOpens",label:"Vizitor n'meny",value:te(R(r.menuOpens)),withEye:!0,loading:i&&a,locked:!a,imageUrl:String(n.menuImageUrl||"").trim(),iconName:"book-open",panelTab:"analitika"}),u.push({key:"qrScans",label:"Skanime n'tavolina",value:te(R(r.qrScans)),withEye:!0,loading:i&&a,locked:!a,imageUrl:String(n.qrImageUrl||"").trim(),iconName:"layout-grid",panelTab:"analitika"}),u}function qn({state:e,renderFn:t,documentObj:a,firestoreApi:n={},profileApi:r={},composerApi:i={},viewApi:d={},iconFn:u,storageObj:b}={}){const x=a||(typeof document>"u"?null:document),p=x?.defaultView||(typeof window>"u"?null:window),m=typeof t=="function"?t:()=>{},g=b||(typeof localStorage>"u"?null:localStorage),k=typeof r.getBusinessProfileTypeFn=="function"?r.getBusinessProfileTypeFn:(()=>""),A=typeof r.isShopCatalogProfileFn=="function"?r.isShopCatalogProfileFn:(()=>!1),O=typeof r.getRestaurantMetaByIdFn=="function"?r.getRestaurantMetaByIdFn:(()=>null),M=typeof r.resolveRestaurantLogoFn=="function"?r.resolveRestaurantLogoFn:(()=>""),_=typeof r.resolveOwnAvatarUrlFn=="function"?r.resolveOwnAvatarUrlFn:(()=>""),E=typeof d.renderAnalyticsViewFn=="function"?d.renderAnalyticsViewFn:(()=>""),H=typeof d.renderSettingsViewFn=="function"?d.renderSettingsViewFn:(()=>""),v=typeof d.warmAnalyticsFn=="function"?d.warmAnalyticsFn:(()=>{});let C=!1,G=0,J=!1,N=null,Z=null,V="",W=!1,Pe=()=>null;const _t=300;function ge(){const l=e?.userProfile||{};return Xa({businessType:k(l),isShopCatalog:A(l)})}function xt(l=""){const c=O(l)||{};return qt(c).map(h=>({id:h.id,name:h.title,price:h.price??"",category:h.beds||h.tag||"",type:"room",imageUrl:h.imageUrl||""}))}function vt(l=""){if(!g)return null;try{const c=g.getItem(`${Qe}${l}`);if(!c)return null;const h=JSON.parse(c),f=Array.isArray(h?.items)?h.items:null;return f&&f.length?f:null}catch{return null}}function kt(l="",c=[]){if(g)try{g.setItem(`${Qe}${l}`,JSON.stringify({savedAt:Date.now(),items:c}))}catch{}}async function wt(l=""){const{db:c,collectionFn:h,queryFn:f,limitFn:w,getDocsFn:z}=n;if(!c||typeof h!="function"||typeof z!="function")throw new Error("Produktet nuk u ngarkuan.");const q=h(c,"restaurants",l,"menuItems"),D=typeof f=="function"&&typeof w=="function"?f(q,w(_t)):q,Q=await z(D),P=[];return Q.forEach(ee=>{const le=Pe(ee?.id,ee?.data?.()||{});le&&P.push(le)}),P.sort((ee,le)=>ee.name.localeCompare(le.name,"sq")),P}async function $t(l="",c){const h=String(l||"").trim();if(!h)throw new Error("Produktet nuk u ngarkuan.");if(ge()==="hotel")return xt(h);const f=wt(h).then(z=>(kt(h,z),z)),w=vt(h);return w?(typeof c=="function"?f.then(z=>c(z)).catch(()=>{}):f.catch(()=>{}),w):f}function je(){return N?Promise.resolve(N):(Z||(Z=Et(()=>import("./business-composer-controller-BXHaSNsS.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(l=>(Pe=typeof l?.normalizeComposerProductCore=="function"?l.normalizeComposerProductCore:(()=>null),N=l.createBusinessComposerController({documentObj:x,windowObj:x?.defaultView||null,api:{getRestaurantIdFn:()=>oe(),getBusinessMetaFn:()=>{const c=oe();if(!c)return{name:"",logoUrl:"",city:""};const h=Me(c),f=O(c)||{};return{name:h.name,logoUrl:h.logoUrl,city:String(f.city||"").trim()}},loadProductsFn:(c,h)=>$t(c,h),getBusinessKindFn:()=>ge(),uploadImageFn:i.uploadImageFn,uploadVideoFn:i.uploadVideoFn,captureVideoPosterFn:i.captureVideoPosterFn,createPostFn:i.createPostFn,createStoryFn:i.createStoryFn,formatPriceFn:i.formatPriceFn,getOptimizedImageUrlFn:i.getOptimizedImageUrlFn,escapeHtmlFn:i.escapeHtmlFn,iconFn:typeof u=="function"?u:void 0,afterPublishFn:async c=>{try{await de({force:!0})}catch{}typeof i.afterPublishFn=="function"&&await i.afterPublishFn(c)}}}),N)).catch(l=>{throw Z=null,console.error("[mnyra][dashboard] composer load failed",l),l})),Z)}function De(){const l=p?.navigator?.connection;return!l||typeof l!="object"?!1:l.saveData===!0?!0:/(^|-)2g$/.test(String(l.effectiveType||"").trim().toLowerCase())}function St(){if(W||N||!p||De())return;W=!0;const l=()=>{if(je().catch(()=>{}),typeof i.prewarmFn=="function")try{i.prewarmFn()}catch{}};if(typeof p.requestIdleCallback=="function"){p.requestIdleCallback(l,{timeout:Ye});return}p.setTimeout?.(l,Xe)}function zt(){if(C||!p||De())return;C=!0;const l=()=>{try{v()}catch{}};if(typeof p.requestIdleCallback=="function"){p.requestIdleCallback(l,{timeout:Ye});return}p.setTimeout?.(l,Xe)}function At(l="post"){const c=String(l||"").trim().toLowerCase(),h=c==="story"||c==="profile"?c:"post";if(typeof i.prewarmFn=="function")try{i.prewarmFn()}catch{}if(N){N.open(h);return}V=h,je().then(f=>{const w=V||h;V="",f?.open?.(w)}).catch(()=>{V=""})}function be(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:"",restaurantId:"",paywall:""}),e.dashboardView}function Fe(l=""){const c=be(),h=String(l||"").trim();return String(c.restaurantId||"")===h||(c.restaurantId=h,c.model=null,c.status="idle",c.error="",c.loadedSignature="",c.paywall="",G+=1),c}function oe(){const l=e?.userProfile||{};return String(l.restaurantId||l.staffRestaurantId||"").trim()}let Ce="";function Pt(){const l=String(e?.user?.uid||"").trim();!l||Ce===l||typeof r.ensureBusinessProfileFn=="function"&&(Ce=l,Promise.resolve().then(()=>r.ensureBusinessProfileFn()).catch(c=>{console.warn("[mnyra][panel] business profile could not be resolved",c)}).finally(()=>{String(e?.user?.uid||"").trim()===l&&m()}))}function jt(){const l=String(e?.user?.uid||"").trim();if(!l)return!1;const c=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||c===l}function Oe(l=""){return`${_n}${l}`}function Dt(l="",c=""){if(!g||!l)return null;try{const h=g.getItem(Oe(l));if(!h)return null;const f=JSON.parse(h);return!f||typeof f!="object"||String(f.day||"").trim()!==String(c||"").trim()||!f.model||typeof f.model!="object"?null:f.model}catch{return null}}function Ft(l="",c=null){if(!(!g||!l||!c))try{g.setItem(Oe(l),JSON.stringify({day:c.day,model:c}))}catch{}}async function Ct(l=""){const{db:c,collectionFn:h,queryFn:f,orderByFn:w,limitFn:z,getDocsFn:q}=n;if(!c||typeof h!="function"||typeof f!="function"||typeof w!="function"||typeof z!="function"||typeof q!="function")return[];const D=h(c,"restaurants",l,"socialPosts");return(await q(f(D,w("createdAt","desc"),z(xn)))).docs.map(P=>({id:P.id,data:P.data()||{}})).filter(P=>{const ee=String(P.data.status||"active").trim().toLowerCase();return ee!=="deleted"&&ee!=="hidden"})}async function de({force:l=!1}={}){const c=oe(),h=Fe(c);if(!c)return;const f=Bt({rangeKey:"7d"});if(!f)return;const w=`${c}::${f.toDay}`;if(!l&&h.loadedSignature===w&&h.status==="ready")return;if(!h.model){const D=Dt(c,f.toDay);D&&(h.model=D,h.status="ready",m())}G+=1;const z=G;h.model||(h.status="loading",h.error="",m());try{const D={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:c},[Q,P]=await Promise.allSettled([Tt({...D,fromDay:f.fromDay,toDay:f.toDay}),Ct(c)]);if(z!==G)return;if(Q.status==="rejected")throw Q.reason;P.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",P.reason),h.model=zn({days:Q.value,todayKey:f.toDay,rawPosts:P.status==="fulfilled"?P.value:[]}),h.status="ready",h.error="",h.loadedSignature=w,Ft(c,h.model)}catch(D){if(z!==G)return;console.error("[mnyra][dashboard] load failed",D),h.model||(h.status="error",h.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}m()}function Ot(){J||!x||(J=!0,x.addEventListener("click",l=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(l.target?.closest?.("[data-dashboard-retry]")){de({force:!0});return}if(l.target?.closest?.("[data-dashboard-paywall-close]")){l.preventDefault(),be().paywall="",m();return}const c=l.target?.closest?.("[data-dashboard-metric-locked]");if(c){l.preventDefault(),be().paywall=String(c.getAttribute("data-dashboard-metric-locked")||"").trim(),m();return}const h=l.target?.closest?.("[data-dashboard-composer]");if(h){l.preventDefault(),At(h.getAttribute("data-dashboard-composer"));return}const f=l.target?.closest?.("[data-dashboard-panel-tab]");if(f){l.preventDefault();const w=pe(f.getAttribute("data-dashboard-panel-tab"));if(w===pe(e?.dashboardPanelTab))return;e.dashboardPanelTab=w,m()}}catch{}}))}function Me(l=""){const c=e?.userProfile||{},h=l?O(l)||{}:{},f=String(h.name||h.restaurantName||c.name||"").trim()||"Business";let w="";try{w=String(_()||"").trim()}catch{}if(!w)try{w=String(M(h)||"").trim()}catch{}return{name:f,logoUrl:w,kind:ge(),coverUrl:jn(h),subscribed:Pn({profile:c,restaurant:h})}}function Mt(l=""){try{if(!Nt()||!l)return"";Kt({restaurantId:l,onBadgeFn:()=>m()});const c=It();return Ba({enabled:!0,unseenCount:c.unseen,activeOffers:c.activeOffers||0,todayBookings:c.today,iconFn:u})}catch{return""}}function Rt(){Qa(x),Ot();const l=oe(),c=Fe(l);let h="";if(!l)Pt(),h=jt()?gn():yn();else{St(),zt();const f=Me(l),w=pe(e?.dashboardPanelTab);c.status==="idle"&&(c.status="loading",queueMicrotask(()=>{de({force:!1})}));let z="";c.model?z=hn({posts:c.model.posts,iconFn:u}):c.status==="error"?z=bn({message:c.error}):z=pn();const q=`
        ${Mt(l)}
        ${tn({iconFn:u})}
        ${dn({iconFn:u,showEditor:!!l})}
        ${an({iconFn:u,showEditor:!!l})}
        ${nn({iconFn:u,showEditor:!!l})}
        ${sn({iconFn:u,kind:f.kind,showEditor:!!l})}
      `;let D;w==="analitika"?D=`
          <div class="mnyra-dash__embed">${E()}</div>
          ${z}
        `:w==="opsionet"?D=`<div class="mnyra-dash__embed">${H()}</div>`:D=q;const Q=Dn({model:c.model,coverUrl:f.coverUrl,subscribed:f.subscribed,assets:kn}),P=String(c.paywall||"").trim();h=`
        ${en({name:f.name,logoUrl:f.logoUrl,iconFn:u})}
        ${ln({cards:Q,iconFn:u})}
        ${yt(`
          ${un({activeTab:w,iconFn:u})}
          ${D}
        `)}
        ${P?mn({title:wn[P]||"Me pagesë"}):""}
      `}return`
      <section class="mnyra-dash" data-dashboard-root>
        ${h}
      </section>
    `}return Object.freeze({renderDashboardView:Rt,loadDashboard:de})}export{Kn as A,Nn as B,Ma as C,qt as D,Rn as E,En as F,re as G,Ut as M,qn as a,fe as b,Ht as c,Wa as d,Vn as e,Ge as f,Gn as g,I as h,Hn as i,Zn as j,Ua as k,Un as l,Ln as m,Wt as n,Oa as o,ya as p,Tn as q,Wn as r,Ee as s,In as t,Bn as u,ja as v,ce as w,et as x,mt as y,Ae as z};
