...
`,s=Object.freeze({brand:"Mnyra GO",partyQuestion:"Sa veta jeni?",partyOne:"person",partyMany:"veta",step:"Hapi",stepBack:"Kthehu një hap prapa",next:"Vazhdo",categoryQuestion:"Çka dëshironi?",whenQuestion:"Kur?",whenLaterLabel:"Zgjidh ditën dhe orën",placeQuestion:"Ku?",placeLabel:"Qyteti",placeEmpty:"Shto qytetin tënd",cityPlaceholder:"Shkruaj qytetin",locationChange:"Ndrysho",locationSave:"Ruaj",submit:"Shiko ofertat",searching:"Po kërkojmë...",resultsHeadline:"lokale kanë oferta për ju",offering:"po ju ofron",forGroup:"për grupin tuaj",accept:"Prano ofertën",confirming:"Po konfirmohet...",sponsored:"Sponsored",onlyGo:"Vetëm me Mnyra GO",tableIncluded:"Tavolinë",emptyTitle:"Nuk gjetëm ofertë GO që përputhet tani.",emptySubtitle:"Provo me një orë tjetër ose me një grup tjetër.",doneTitle:"U krye",reservationConfirmed:"Tavolina është konfirmuar",claimConfirmed:"Oferta është e juaja",menu:"Shiko menunë",directions:"Udhëzime",code:"Kodi",cancel:"Anulo",cancelConfirm:"Dëshiron ta anulosh?",cancelYes:"Po, anuloje",cancelNo:"Jo",saveToAccount:"Ruaje në llogarinë tënde",signIn:"Hyr",errorTitle:"Mnyra GO është përkohësisht i padisponueshëm.",errorRetry:"Provo prapë",alternatives:"alternativa për ju",peopleSuffix:"persona",now:"Tani",back:"Ndrysho kërkimin"}),v="/apps/menyra-social/assets/go/",w=Object.freeze([Object.freeze({file:"story-1-unt.webp",headline:"A je unt?",text:Object.freeze(["Trego sa veta jeni edhe ",Object.freeze({accent:"çka po ju hahet."})])}),Object.freeze({file:"story-2-ku-me-dal.webp",headline:"S’po din ku me dal?",text:Object.freeze(["Mos lyp lokal — lokalet që t’përshtaten ",Object.freeze({accent:"t’gjejnë ty."})])}),Object.freeze({file:"story-3-shtrejt.webp",headline:"Edhe shumë shtrejt?",text:Object.freeze(["Lokalet t’çojnë oferta me ",Object.freeze({accent:"zbritje direkt"})," — ti veç zgjedh."])}),Object.freeze({file:"story-4-knaqu.webp",headline:"Ofertat t’vijn. Ti veç shko, knaqu.",text:Object.freeze(["Zgjedhe ofertën që t’pëlqen, shko aty edhe ",Object.freeze({accent:"knaqu."})])})]);function B(a=[]){return(Array.isArray(a)?a:[a]).map(e=>{if(typeof e=="string")return n(e);const t=typeof e?.accent=="string"?e.accent:"";return t?`<span class="mnyra-go-page__story-accent">${n(t)}</span>`:""}).join("")}function ne(a=[]){return(Array.isArray(a)?a:[a]).map(e=>typeof e=="string"?e:String(e?.accent||"")).join("")}const E=Object.freeze({now:"zap",in30:"timer",in60:"clock",later:"calendar-clock"}),d=Object.freeze(["party","category","when","place"]),k=Object.freeze({party:"users",category:"utensils",when:"clock",place:"map-pin"});function n(a=""){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function f(a=""){const e=String(a||"").trim();return d.includes(e)?e:d[0]}function te(a=""){const e=d.indexOf(f(a));return d[Math.min(d.length-1,e+1)]}function re(a=""){const e=d.indexOf(f(a));return d[Math.max(0,e-1)]}function u(a){const e=Math.trunc(Number(a));return Number.isFinite(e)?Math.min(m,Math.max(l,e)):O}function $(a,e=s){return u(a)===1?e.partyOne||s.partyOne:e.partyMany||s.partyMany}function S(a,e=s){return`${u(a)} ${$(a,e)}`}function G(a){const e=u(a),t=m-l;return Math.round((e-l)/t*100)}function z(a,{active:e=!1,attr:t="",value:r="",iconName:i="",wide:g=!1}={}){return`
    <button
      type="button"
  ...
  `}function M(a,{nowMs:e=Date.now()}={}){const t=Date.parse(String(a||""));if(!Number.isFinite(t))return"";if(Math.abs(t-e)<900*1e3)return s.now;const r=new Date(t);return`Rreth ${String(r.getHours()).padStart(2,"0")}:${String(r.getMinutes()).padStart(2,"0")}`}function D(a="",e={},t=s){if(a==="party")return S(e.partySize,t);if(a==="category"){const r=_.find(i=>i.key===String(e.category||"all"));return r?r.label:""}if(a==="when"){const r=b.find(i=>i.key===String(e.when||"now"));return r?r.label:""}return a==="place"?String(e.city||"").trim()||t.placeEmpty:""}function T(a="",e={},t=s){const r=d.indexOf(a),i=d.slice(0,Math.max(0,r));return i.length?`
    <div class="mnyra-go-page__ask-done">
      ${i.map(g=>`
        <button type="button" class="mnyra-go-page__ask-tag" data-go-goto="${n(g)}">
          ${o(k[g]||"check-check")}<span>${n(D(g,e,t))}</span>
        </button>
      `).join("")}
    </div>
  `:""}function F(a={},e=s){const t=u(a.partySize);return`
    <output class="mnyra-go-page__party-value" data-go-party-value for="mnyraGoParty">
      ${n(String(t))} <span>${n($(t,e))}</span>
    </output>
    <input
      id="mnyraGoParty"
      type="range"
      ...
      step="1"
      value="${t}"
      style="--go-range-fill:${G(t)}%;"
      aria-label="${n(e.partyQuestion)}"
      aria-valuetext="${n(S(t,e))}"
    />
    <div class="mnyra-go-page__range-scale" aria-hidden="true">
      <span>${l}</span>
      <span>${m}</span>
    </div>
  `}function L(a={},e=s){const t=String(a.category||"all");return`
    <div class="mnyra-go-page__chips" role="group" aria-label="${n(e.categoryQuestion)}">
      ${_.map((r,i)=>z(r.label,{active:t===r.key,attr:"data-go-category",value:r.key,iconName:r.icon,wide:i===0})).join("")}
    </div>
  `}function P(a={},e=s){const t=String(a.when||"now");return`
    <div class="mnyra-go-page__chips" role="group" aria-label="${n(e.whenQuestion)}">
      ${b.map(r=>z(r.label,{active:t===r.key,attr:"data-go-when",value:r.key,iconName:E[r.key]||"clock"})).join("")}
    </div>
    ${t==="later"?`
      ...
      />
    `:""}
  `}function R(a={},e=s){const t=String(a.city||"").trim();return a.editCity?`
  ...
  `}function K(a={},e=s){const t=f(a.step),r=d.indexOf(t),i=d.length,g=Math.round((r+1)/i*100);let p="",c="",h="";return t==="party"?(c=e.partyQuestion,p=F(a,e),h=`<button type="button" class="mnyra-go-page__cta" data-go-step-next>${n(e.next)}${o("chevron-right")}</button>`):t==="category"?(c=e.categoryQuestion,p=L(a,e)):t==="when"?(c=e.whenQuestion,p=P(a,e),String(a.when||"now")==="later"&&(h=`<button type="button" class="mnyra-go-page__cta" data-go-step-next>${n(e.next)}${o("chevron-right")}</button>`)):(c=e.placeQuestion,p=R(a,e),h=`<button type="button" class="mnyra-go-page__cta" data-go-submit>${o("search")}${n(e.submit)}</button>`),`
    <section class="mnyra-go-page__ask" data-go-step="${n(t)}">
      ...
      <h2 class="mnyra-go-page__q">${o(k[t]||"users")}${n(c)}</h2>
      ${p}
      ${h}
    </section>
  `}function j(a={},{texts:e=s,busyOfferId:t="",nowMs:r=Date.now()}={}){const i=t&&t===a.offerId,g=Number.isFinite(Number(a.distanceKm))&&a.distanceKm!==null?`${Number(a.distanceKm).toFixed(1)} km`:"",p=a.isNow?e.now:M(a.expectedArrivalAt,{nowMs:r});return`
    <article class="mnyra-go-page__card" data-go-result="${n(a.offerId)}">
      ...
      <div class="mnyra-go-page__card-meta">
        ...
        ${p?`<span>${o("clock")}${n(p)}</span>`:""}
        ${g?`<span>${o("map-pin")}${n(g)}</span>`:""}
  ...
  `}function I(a={},e=s){const t=Array.isArray(a.results)?a.results:[],r=Number(a.nowMs)||Date.now();return t.length?`
    <h2 class="mnyra-go-page__lead">${t.length} ${n(e.resultsHeadline)}</h2>
    ${a.notice?`<p class="mnyra-go-page__note">${n(a.notice)}</p>`:""}
    ${t.map(i=>j(i,{texts:e,busyOfferId:a.busyOfferId,nowMs:r})).join("")}
  ...
  `:`
    ...
    `}function C(a={},e=s){const t=a.booking||{},r=t.type==="reservation",i=Number(a.nowMs)||Date.now(),g=M(t.expectedArrivalAt,{nowMs:i}),p=String(a.statusLabel||"").trim()||(r?e.reservationConfirmed:e.claimConfirmed);return a.confirmCancel?`
    ...
    <div class="mnyra-go-page__card-meta">
      ...
      ${g?`<span>${o("clock")}${n(g)}</span>`:""}
      ${t.city?`<span>${o("map-pin")}${n(t.city)}</span>`:""}
    </div>

    <p class="mnyra-go-page__ok">${o("circle-check-big")}${n(p)}</p>
  ...
  `}function N(a={},e=s){const t=String(a.error||"").trim()||e.errorTitle;return`
    ...
    <div class="mnyra-go-page__story" data-go-story>
      ${w.map((t,r)=>{const i=r===0;return`
        ...
        >
          ...
          <p class="mnyra-go-page__story-text">${B(t.text)}</p>
        </article>
      `}).join("")}
    </div>
  `}function q(a={}){return H(a.storyShown)}function ie(a={}){const e={...s,...a.texts||{}},t=String(a.view||"search"),r=t==="search";let i="";return t==="loading"?i=`<div class="mnyra-go-page__empty">${n(e.searching)}</div>`:t==="results"?i=I(a,e):t==="booking"?i=C(a,e):t==="error"?i=N(a,e):i=q(a),`
  ...
  `}const oe=s,se=w,ge=v;export{ae as GO_PAGE_CSS,ge as GO_PAGE_STORY_BASE,se as GO_PAGE_STORY_SLIDES,ee as GO_PAGE_STYLE_ELEMENT_ID,oe as GO_PAGE_TEXTS,d as GO_STEPS,u as clampGoPartySize,G as goPartyFillPercent,S as goPartyLabel,$ as goPartyWord,ne as goStoryPlainText,te as nextGoStep,re as previousGoStep,ie as renderGoPageCore,f as resolveGoStep};
