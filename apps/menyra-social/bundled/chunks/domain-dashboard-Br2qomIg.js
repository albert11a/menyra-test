...
`;function R(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Ie({businessName:e="",logoUrl:t="",imageUrl:a="",variant:n="",benefitLabel:r="",benefitView:s=null,sponsored:h=!1,meta:l=[],ctaLabel:p="",ctaIcon:b="check-check",ctaDisabled:f=!1,cardAttrs:y="",ctaAttrs:g="",texts:_=xe}={}){const k={...xe,..._||{}},C=(Array.isArray(l)?l:[]).filter(H=>H&&H.label),O=String(p||k.accept),$=s&&typeof s=="object"?s:{},F=String($.headline||r||""),M=String($.priceGo||""),w=String($.priceRegular||""),S=String(a||"").trim(),x=Mt({imageUrl:S,variant:n}),T=x===ce,G=S?`<img class="mnyra-go-page__card-photo" src="${R(S)}" alt="" loading="lazy" decoding="async" />`:"",L=`
    ...
    ${T?"":`<p class="mnyra-go-page__card-for">${R(k.forGroup)}</p>`}
  `,I=`
    <div class="mnyra-go-page__card-meta">
      ${C.map(H=>`<span>${ee(H.icon||"")}${R(H.label)}</span>`).join("")}
    </div>
    ...
    >${b?ee(b):""}${R(O)}</button>
  `;return T?`
      <article class="mnyra-go-page__card mnyra-go-page__card--compact"${y?` ${y}`:""}>
    ...
    `:x===le?`
      <article class="mnyra-go-page__card mnyra-go-page__card--hero"${y?` ${y}`:""}>
        ${G}
    ...
    `:`
    <article class="mnyra-go-page__card"${y?` ${y}`:""}>
      ${L}
      ${I}
    </article>
  `}function He(e=0){const t=Math.max(0,Math.trunc(Number(e)||0)),a=Math.trunc(t/100),n=String(t%100).padStart(2,"0");return`${a},${n} €`}const ze="mnyraWorkSurfaceStyles",Ze=`
...
`;function Tt(e={},t={}){const a=t.escapeHtml,n=t.icon;return`
  ...
  `}function Gt({overview:e={},deps:t={}}={}){const a=l=>Number.isFinite(Number(l))&&l!==null&&l!=="",n=l=>a(l)?String(Math.max(0,Math.trunc(Number(l)))):"",r=a(e?.openCents)?Math.max(0,Math.trunc(Number(e.openCents))):null,s=r===0;return`
  ...
  `}const ae=Object.freeze([Object.freeze({key:"shift",tabs:Object.freeze(["pending","active","finalized"])}),Object.freeze({key:"manage",tabs:Object.freeze(["stats","payments","offers"])})]),Nt=Object.freeze({pending:"clock-3",active:"zap",finalized:"circle-check",stats:"bar-chart-3",payments:"wallet",offers:"tag",options:"settings"});function Oa(e=""){return ae.findIndex(t=>t.tabs.includes(String(e||"")))}function Lt({tab:e="active",group:t=0,deps:a={}}={}){const n=a.escapeHtml,r=a.icon,s=Math.min(Math.max(Math.trunc(Number(t)||0),0),ae.length-1),h=s<ae.length-1?i.groupNext:i.groupBack;return`
    <div class="go-tabs" data-go-tabs data-go-tab-group="${s}">
      <div class="go-tabs__viewport">
        <div class="go-tabs__track">
          ${ae.map((l,p)=>`
            <div class="mnyra-work__pills go-tabs__pane" role="tablist" data-go-tab-pane="${p}"${p===s?"":' aria-hidden="true" inert'}>
              ${l.tabs.map(b=>`
                <button type="button" role="tab" aria-selected="${e===b?"true":"false"}" data-go-business-tab="${o(n,b)}"
                  ...
                  class="mnyra-work__pill">${E(r,Nt[b],"w-4 h-4")}<span class="mnyra-work__pill-label">${o(n,i.tabs[b])}</span></button>
              `).join("")}
            </div>
          `).join("")}
        </div>
  ...
  `}function Ve(e={}){return e.partySizeVerified||e.partySizeRequested||e.partySize||1}function Se(e={},t={}){const a=t.escapeHtml,n=e.benefitLabel||e.snapshot?.benefitLabel||"",r=!e.businessSeenAt,s=Ve(e),h=Ue(e.acceptedAt),l=h?`${i.around} ${h}`:i.guestName;return`
    <div class="p-4 rounded-[1.6rem] border ${r?"bg-indigo-50/50 border-indigo-100":"bg-slate-50 border-slate-100"}"
      ...
      ${e.commission?`
        ...
        <p class="mt-3 pt-3 border-t border-slate-200/70 text-[10px] font-black uppercase tracking-widest text-slate-400">
          ${o(a,i.commission)} · ${o(a,He(e.commission.amountCents))}
        </p>
      `:""}
    </div>
  `}function It({code:e="",status:t="",busy:a=!1,cameraOpen:n=!1,cameraError:r="",booking:s=null,bookingEntering:h=!1,deps:l={}}={}){const p=l.escapeHtml,b=l.icon,f=s&&typeof s=="object"?s:null,y=!!f&&!h,g=String(t||"").trim()||String(r||"").trim();return`
    <div class="go-activate" data-go-activate data-go-camera="${n?"1":"0"}"
      data-go-found="${y?"1":"0"}" data-go-code-search>
  ...
  `}function Ht({booking:e={},code:t="",busy:a=!1,note:n="",deps:r={}}={}){const s=r.escapeHtml,h=Ve(e),l=e.benefitLabel||e.snapshot?.benefitLabel||"",p=de(e.status),b=String(t||"").trim().toUpperCase();return`
    <div class="go-activate__done" data-go-activate-done data-go-booking="${o(s,e.id)}">
      <div class="go-activate__done-head">
        <p class="go-activate__done-code">${o(s,b?`${i.dealCode}: ${b}`:i.dealCode)}</p>
      ...
      ${p==="activated"?`
        ...
        ${n?`<p class="go-activate__done-status" role="status">${o(s,n)}</p>`:""}
        <button type="button" data-go-booking-finalize data-go-booking-id="${o(s,e.id)}"
  ...
  `}function q({eyebrow:e="",title:t="",sub:a="",action:n="",body:r="",deps:s={}}={}){const h=s.escapeHtml;return`
  ...
  `}function $e({title:e="",note:t="",iconName:a="",deps:n={}}={}){const r=n.escapeHtml,s=n.icon;return q({eyebrow:i.brand,title:e,sub:i.soonHint,body:`
    ...
    `,deps:n})}function Zt(e={},t={}){const a=t.escapeHtml,n=e.status==="paused"?i.paused:e.status==="archived"?i.archived:"";return`
    <div class="p-4 rounded-[1.6rem] bg-slate-50 border border-slate-100" data-go-offer="${o(a,e.id)}">
      ...
      <div style="pointer-events:none;" aria-hidden="true">
        ${Ie({businessName:"",imageUrl:e.imageUrl||"",variant:ce,benefitLabel:e.benefitLabel||"",benefitView:Te(e.benefit||{}),meta:[{icon:"users",label:Ce(e)},{icon:"clock",label:Oe(e)}]})}
      </div>
  ...
  `}function N(e,t="",a=""){return`<label class="text-[10px] font-black uppercase tracking-widest text-slate-400"${a?` for="${o(e,a)}"`:""}>${o(e,t)}</label>`}function te(e,{active:t=!1,attr:a="",value:n="",escapeHtml:r=null}={}){return`
...
`;function oe({attr:e="",unit:t="€",value:a="",placeholder:n="",mode:r="decimal",inputClass:s="",escapeHtml:h=null}={}){return`
  ...
  `}const Pe=Object.freeze([10,15,20,25]);function Wt({benefit:e={},percentCustom:t=!1,errorFor:a=()=>"",inputClass:n="",inputBase:r="",escapeHtml:s=null}={}){const h=g=>N(s,g),l=g=>{const _=a(g);return _?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="${o(s,g)}">${o(s,_)}</p>`:""};if(!kt.includes(e.kind))return`
      <p class="go-offer-saving font-bold text-slate-400">${o(s,i.benefitLegacy)}</p>
      ${l("benefit")}
    `;const p=Number(e.percent)||0,b=t||p>0&&!Pe.includes(p),f=()=>{const g=vt(e.savingCents);if(!g)return"";const _=Math.round(Number(e.savingPercent)||0);return`
      <p class="mt-3 go-offer-saving font-black text-emerald-600" data-go-benefit-saving>
        ${o(s,i.saving)} ${o(s,g)}${_>0?` &middot; -${_}%`:""}
      </p>
    `},y=()=>`
    <div class="mt-3">
      ${h(i.priceRegular)}
      ${oe({attr:"data-go-benefit-regular",value:ke(e.regularPriceCents),placeholder:i.pricePlaceholder,inputClass:r,escapeHtml:s})}
      ${l("regularPrice")}
    </div>
    <div class="mt-3">
      ${h(i.priceGo)}
      ${oe({attr:"data-go-benefit-go",value:ke(e.goPriceCents),placeholder:i.pricePlaceholder,inputClass:r,escapeHtml:s})}
      ${l("goPrice")}
    </div>
    ${f()}
  `;if(e.kind===Ee)return`
      ...
      <div class="mt-2 flex flex-wrap gap-2">
        ${Pe.map(g=>Q(`${g}%`,{active:!b&&p===g,attr:"data-go-discount",value:String(g),escapeHtml:s})).join("")}
        ${Q(i.discountOther,{active:b,attr:"data-go-discount",value:"other",escapeHtml:s})}
      </div>
      ${b?`
        <div class="mt-3">
          ${oe({attr:"data-go-benefit-percent",unit:"%",mode:"numeric",value:p>0?String(p):"",placeholder:i.discountPlaceholder,inputClass:r,escapeHtml:s})}
        </div>
      `:""}
      ${l("benefitPercent")}
      ...
      <div class="mt-4">
        ${h(i.scopeQuestion)}
        <div class="mt-2 flex flex-wrap gap-2">
          ${[["all",i.scopeAll],["food",i.scopeFood],["drinks",i.scopeDrinks]].map(([g,_])=>Q(_,{active:(e.scope||"all")===g,attr:"data-go-discount-scope",value:g,escapeHtml:s})).join("")}
        </div>
        ${l("benefitScope")}
      </div>
    `;if(e.kind===Fe)return`
    ...
    `;if(e.kind===Me){const g=String(e.conditionType||"");return`
      ...
      <div class="mt-4">
        ...
        <div class="mt-2 grid grid-cols-2 gap-2">
          ${[["food",i.conditionFood],["drink",i.conditionDrink],["any_order",i.conditionAny],["custom",i.conditionCustom]].map(([_,k])=>Q(k,{active:g===_,attr:"data-go-benefit-condition",value:_,escapeHtml:s})).join("")}
        </div>
        ${g==="custom"?`
          <div class="mt-3">
    ...
    `}return e.kind===je?`
      ...
      ${l("benefitItem")}
      ${y()}
    `:""}function qt({imageUrl:e="",photo:t={},escapeHtml:a=null,icon:n=null}={}){const r=String(t.status||""),s=r==="uploading",h=String(t.previewUrl||e||""),l=r==="error"?String(t.error||i.photoError):"",p=h?`
  ...
  `}function Qt(e=""){const t=String(e||"all").trim().toLowerCase();return t==="food"?["food"]:t==="coffee"||t==="drinks"||t==="dessert"?["drinks"]:["food","drinks"]}function Ta(e=[]){const t=Array.isArray(e)?e:[],a=t.includes("food"),n=t.includes("drinks");return a&&n?"all":a?"food":n?"drinks":""}function Ga({editor:e=null,businessName:t="",deps:a={}}={}){if(!e)return"";const n=a.escapeHtml,r=a.icon,s=e.draft||{},h=Array.isArray(e.errors)?e.errors:[],l=w=>h.find(S=>S.field===w)?.message||"",p=Array.isArray(s.partyRanges)?s.partyRanges:[],b=s.schedule?.mode==="windows"?"windows":"always",f=Array.isArray(s.schedule?.days)&&s.schedule.days.length?s.schedule.days:ye.slice(),y=we.every(w=>p.includes(w.key)),g=Array.isArray(e.intents)?e.intents:Qt(s.category),_=s.benefit||{},k=e.mode==="edit",C="w-full go-offer-input bg-slate-50 border border-slate-100 rounded-2xl px-4 text-sm font-bold text-slate-900 outline-none focus:border-indigo-400",O=`mt-2 ${C}`,$='<div class="h-px bg-slate-100"></div>',F=w=>`<p class="mt-1 text-[11px] font-semibold text-slate-400">${o(n,w)}</p>`,M=bt(s).ok&&g.length>0;return`
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll px-6 py-5 space-y-5" data-go-editor-scroll>
          ...
          <div class="go-offer-section" data-go-section="benefit">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
              ${[[Ee,i.benefitPercent],[Fe,i.benefitBundle],[Me,i.benefitFree],[je,i.benefitSpecial]].map(([w,S])=>`
                <button type="button" data-go-benefit-kind="${o(n,w)}"
                  aria-pressed="${_.kind===w?"true":"false"}"
                  class="go-offer-kind px-3 rounded-2xl text-xs font-black transition-colors ${_.kind===w?"bg-slate-900 text-white":"bg-slate-50 text-slate-600 border border-slate-100"}">
                  ${o(n,S)}
                </button>
              `).join("")}
            </div>
            <div class="mt-5" data-go-benefit-form>
              ${Wt({benefit:_,percentCustom:e.percentCustom===!0,errorFor:l,inputClass:O,inputBase:C,escapeHtml:n})}
            </div>
          </div>
          ...
          -->
          ${qt({imageUrl:s.imageUrl||"",photo:e.photo||{},escapeHtml:n,icon:r})}
          ...
          <div data-go-section="partyRanges">
            ...
            -->
            <div class="mt-3">
              ${te(i.partyAll,{active:y,attr:"data-go-offer-party",value:"all",escapeHtml:n})}
            </div>
            <div class="mt-2 flex flex-wrap gap-2">
              ${we.map(w=>te(w.label,{active:p.includes(w.key),attr:"data-go-offer-party",value:w.key,escapeHtml:n})).join("")}
            </div>
          ...
          <div data-go-section="category">
            ...
            <div class="mt-3 space-y-2">
              ${[{key:"food",label:i.ifFood},{key:"drinks",label:i.ifDrinks}].map(w=>{const S=g.includes(w.key),x=_t.find(T=>T.key===w.key)?.hint||"";return`
                  <button type="button" data-go-offer-intent="${o(n,w.key)}" aria-pressed="${S?"true":"false"}"
          ...
          <div data-go-section="schedule">
            ...
            ${b==="windows"?`
              ...
              <div class="mt-4">
                ${N(n,i.daysQuestion)}
                <div class="mt-2 flex flex-wrap gap-2">
                  ${ye.map(w=>Q(yt(w),{active:f.includes(w),attr:"data-go-offer-day",value:w,escapeHtml:n})).join("")}
                </div>
              </div>
              <div class="mt-4">
                ${N(n,i.hoursQuestion)}
          ...
          ${Ut({offer:s,businessName:t,previewImageUrl:e.photo?.previewUrl||"",deps:a})}
        </div>
  ...
  `}function Na({restaurantName:e="",tab:t="active",group:a=0,overview:n={},search:r={},camera:s={},bookingEntering:h=!1,bookings:l=[],offers:p=[],settings:b={},paused:f=!1,loading:y=!1,error:g="",deps:_={}}={}){const k=_.escapeHtml,C=_.icon,O=x=>["accepted","activated"].includes(de(x.status)),F=l.filter(O).filter(x=>de(x.status)==="accepted"),M=l.filter(x=>!O(x)),w=p.filter(x=>x.status!=="archived");let S="";if(t==="offers")S=q({eyebrow:i.brand,title:i.tabs.offers,sub:`${w.length} ${w.length===1?"oferte":"oferta"}`,action:`
      ...
      `,body:w.length?`<div class="space-y-3">${w.map(x=>Zt(x,_)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(k,i.emptyTitle)}</div>`,deps:_});else if(t==="finalized")S=q({eyebrow:i.brand,title:i.tabs.finalized,sub:`${M.length}`,body:M.length?`<div class="space-y-3">${M.map(x=>Se(x,_)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(k,i.noHistory)}</div>`,deps:_});else if(t==="pending")S=q({eyebrow:i.brand,title:i.tabs.pending,sub:`${F.length}`,body:y?`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-400">${o(k,i.loading)}</div>`:F.length?`<div class="space-y-3">${F.map(x=>Se(x,_)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(k,i.noBookings)}</div>`,deps:_});else if(t==="stats")S=$e({title:i.tabs.stats,note:i.soonStats,iconName:"bar-chart-3",deps:_});else if(t==="payments")S=$e({title:i.tabs.payments,note:i.soonPayments,iconName:"wallet",deps:_});else if(t==="options"){const x=Ue(b?.pausedUntil);S=q({eyebrow:i.brand,title:i.tabs.options,body:`
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          ${f?`<button type="button" data-go-pause="0" class="go-pause px-4 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest">${o(k,i.resume)}</button>`:[{value:"30",label:"30 min"},{value:"60",label:"1 orë"},{value:"tomorrow",label:"Deri nesër"},{value:"-1",label:"Pa afat"}].map(T=>`
              <button type="button" data-go-pause="${T.value}"
                class="go-pause px-4 rounded-2xl bg-slate-50 border border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-600">${o(k,T.label)}</button>
            `).join("")}
        </div>
        <p class="mt-4 text-[10px] font-bold text-slate-400">${o(k,i.keepsRunning)}</p>
      `,deps:_})}else S=It({code:r.code,status:r.status,busy:r.busy,cameraOpen:s.open===!0,cameraError:s.error,booking:r.booking,bookingEntering:h===!0,deps:_});return`
  ...
  `}const Ia=i,De="mnyraDashboardStyles",Yt=`
.mnyra-dash {
...
`;function Jt(e=typeof document>"u"?null:document){if(jt(e),!(!e||e.getElementById(De)))try{const t=e.createElement("style");t.id=De,t.textContent=Yt,e.head?.appendChild(t)}catch{}}function v(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function K(e,t,a=""){if(typeof e!="function")return"";try{return e(t,a)||""}catch{return""}}const Xt=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function ea({businessType:e="",isShopCatalog:t=!1}={}){if(t)return"shop";const a=String(e||"").trim().toLowerCase();return Xt.includes(a)?"hotel":"restaurant"}function ta(e=new Date().getHours()){const t=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return t>=5&&t<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:t>=11&&t<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:t>=18&&t<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function aa({name:e="",logoUrl:t="",hour:a=new Date().getHours(),iconFn:n}={}){const r=ta(a),s=v(e||"Business");return`
    <div class="mnyra-work__head mnyra-dash__greet">
      <p class="mnyra-dash__greet-title">
        ...
        <span class="mnyra-dash__greet-logo">
          ${t?`<img src="${v(t)}" alt="${s}" title="${s}" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:`<span class="mnyra-dash__greet-logo-fallback" title="${s}">${K(n,"store","w-4 h-4")}</span>`}
        </span>
      </p>
      <p class="mnyra-dash__greet-sub">${v(r.text)}</p>
    </div>
  `}function na({iconFn:e}={}){return`
  ...
  `}function ia({iconFn:e,showEditor:t=!0}={}){return t?`
  ...
  `}const da="/waiter?from=panel";function la({iconFn:e,showEditor:t=!0}={}){return t?`
    <a href="${da}" class="mnyra-dash__composer mnyra-dash__composer--tap mnyra-dash__composer--waiter" data-dashboard-waiter-card>
  ...
  `:""}function ca({cards:e=[],iconFn:t}={}){const a=(Array.isArray(e)?e:[]).filter(r=>r&&r.key);if(!a.length)return"";const n=a.map((r,s)=>{const h=v(r.label||"");if(r.pending)return'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>';const l=s<2?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"';let p="";r.imageUrl?p=`<img class="mnyra-dash__hl-media" src="${v(r.imageUrl)}" alt="" ${l} decoding="async" onerror="this.style.display='none'" />`:r.videoUrl&&(p=`<video class="mnyra-dash__hl-media" src="${v(r.videoUrl)}#t=0.1" preload="metadata" muted playsinline disablepictureinpicture tabindex="-1" aria-hidden="true"></video>`);const b=`
      <span class="mnyra-dash__hl-plate">${K(t,r.iconName||"image","w-6 h-6")}</span>
      ${p}
    `,f=r.withEye?`<span class="mnyra-dash__hl-eye">${K(t,"eye","w-4 h-4")}</span>`:"";let y;r.locked?y=`<span class="mnyra-dash__hl-lock">${K(t,"lock","w-3 h-3")}Me pagesë</span>`:r.loading?y='<span class="mnyra-dash__hl-value mnyra-dash__hl-value--pending" aria-hidden="true"></span>':r.emptyText?y=`<span class="mnyra-dash__hl-empty">${v(r.emptyText)}</span>`:y=`<span class="mnyra-dash__hl-value">${f}${v(r.value||"0")}</span>`;let g;r.locked?g=`class="mnyra-dash__hl-card mnyra-dash__hl-card--locked" data-dashboard-metric-locked="${v(r.key)}"`:r.composer?g=`class="mnyra-dash__hl-card" data-dashboard-composer="${v(r.composer)}"`:g=`class="mnyra-dash__hl-card"${r.panelTab?` data-dashboard-panel-tab="${v(r.panelTab)}"`:""}`;const _=r.locked?`${h} – me pagesë`:`${h} ${r.emptyText||r.value||""}`.trim();return`
      <button type="button" ${g} data-dashboard-metric="${v(r.key)}" aria-label="${v(_)}">
        ${b}
    ...
    `}).join("");return`
    <div class="mnyra-work__cards" data-dashboard-metrics="${v(ha(a))}">
      ${n}
      <span class="mnyra-dash__hl-tail" aria-hidden="true"></span>
    </div>
  `}function ha(e=[]){return(Array.isArray(e)?e:[]).filter(t=>t&&t.key).map(t=>[t.key,t.label||"",t.value||"",t.emptyText||"",t.imageUrl||"",t.videoUrl||"",t.iconName||"",t.panelTab||"",t.composer||"",t.pending?"p":"",t.loading?"l":"",t.locked?"x":"",t.withEye?"e":""].join("~")).join("|")}const We=Object.freeze([Object.freeze({id:"funksionet",label:"Funksionet",iconName:"layout-grid"}),Object.freeze({id:"analitika",label:"Analitika",iconName:"bar-chart-3"}),Object.freeze({id:"opsionet",label:"Opsionet",iconName:"settings"})]);function ne(e=""){const t=String(e||"").trim().toLowerCase();return We.some(a=>a.id===t)?t:"funksionet"}function ua({activeTab:e="funksionet",iconFn:t}={}){const a=ne(e);return`<div class="mnyra-work__pills mnyra-dash__tabs" role="tablist" data-dashboard-panel-tabs>${We.map(r=>{const s=r.id===a;return`
      <button
        type="button"
        role="tab"
      ...
      >${K(t,r.iconName,"w-4 h-4")}<span class="mnyra-work__pill-label">${v(r.label)}</span></button>
    `}).join("")}</div>`}function qe(e=""){return`
    <div class="mnyra-work__bento mnyra-dash__bento" data-dashboard-bento>
      ${e}
    </div>
  `}function pa({posts:e=[],iconFn:t}={}){const a=Array.isArray(e)?e:[];let n="";return a.length?(n=a.map(r=>{const s=[r.dateLabel,`${W(r.likesCount||0)} Likes`,`${W(r.commentsCount||0)} Kommentare`];return Number(r.impressions||0)>0&&s.push(`${W(r.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${r.thumbUrl?`<img src="${v(r.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:K(t,r.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
          <div class="mnyra-dash__post-main">
            <p class="mnyra-dash__post-caption">${v(r.caption||"Pa tekst")}</p>
            <p class="mnyra-dash__post-meta">${v(s.filter(Boolean).join(" · "))}</p>
          </div>
        </div>
  ...
  `}const wa="menyra_social_dashboard_cache_v1::",Ae="menyra_social_composer_products_v1::",Re=2500,Be=1200,ka=6,va=3,xa=Object.freeze({menuImageUrl:"/apps/menyra-social/assets/panel/menu-scan.jpg",qrImageUrl:"/apps/menyra-social/assets/panel/qr-stand.jpg"}),za=Object.freeze({menuOpens:"Menü-Aufrufe",qrScans:"QR-Scans"});function B(e){const t=Number(e);return Number.isFinite(t)?t:0}function Sa(e={}){const t=String(e.createdAtClient||"").trim();if(t){const n=new Date(t);if(!Number.isNaN(n.getTime()))return n}const a=e.createdAt;if(a&&typeof a.toDate=="function")try{const n=a.toDate();if(n instanceof Date&&!Number.isNaN(n.getTime()))return n}catch{}return null}function $a(e="",t={}){const a=Array.isArray(t.media)&&t.media.length?t.media[0]:{},n=String(a.type||t.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",r=String(a.thumbUrl||(n==="image"?a.url:"")||t.thumbUrl||"").trim(),s=n==="video"?String(a.url||t.mediaUrl||"").trim():"",h=Sa(t);return{id:String(e||"").trim(),caption:String(t.caption||"").trim(),mediaType:n,thumbUrl:r,videoUrl:s,likesCount:B(t.likesCount),commentsCount:B(t.commentsCount),impressions:0,dateLabel:h?h.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:h?h.getTime():0}}function Pa({days:e=[],todayKey:t="",rawPosts:a=[]}={}){const n=Array.isArray(e)?e:[],r=_e(n),s=n.find(f=>String(f?.date||f?.id||"").trim()===String(t||"").trim()),h=_e(s?[s]:[]),l=r.merged?.posts&&typeof r.merged.posts=="object"?r.merged.posts:{},p=(Array.isArray(a)?a:[]).map(f=>$a(f?.id,f?.data||{})).filter(f=>f.id).map(f=>({...f,impressions:B(l[f.id]?.impressions)})),b=p.slice().sort((f,y)=>y.createdAtMs-f.createdAtMs).slice(0,va);return{day:String(t||"").trim(),week:r.summary,today:h.summary,posts:b,latestPost:Da(p)}}function Da(e=[]){const t=(Array.isArray(e)?e:[]).filter(a=>a&&a.id);return t.length?t.slice().sort((a,n)=>B(n.createdAtMs)-B(a.createdAtMs)||B(n.impressions)-B(a.impressions)||B(n.likesCount)-B(a.likesCount))[0]:null}function Ka({profile:e={},restaurant:t={}}={}){return ft({profile:e,restaurant:t,feature:"qr"})}function Aa(e={}){const t=e&&typeof e=="object"?e:{};return String(t.titleImageUrl||t.coverImageUrl||t.coverUrl||t.heroUrl||t.bannerUrl||"").trim()}function Ra({model:e=null,coverUrl:t="",subscribed:a=!1,assets:n={}}={}){const r=e?.today||{},s=!e,h=e?.latestPost||null,l=[];if(s)l.push({key:"latestPost",label:"Postimi fundit",pending:!0});else if(!h)l.push({key:"latestPost",label:"Postimi fundit",emptyText:"S'ka postim",iconName:"image",composer:"post"});else{const p=String(h.thumbUrl||"").trim();l.push({key:"latestPost",label:"Postimi fundit",value:W(B(h.impressions)),withEye:!0,imageUrl:p,videoUrl:p?"":String(h.videoUrl||"").trim(),iconName:"image",panelTab:"analitika"})}return l.push({key:"profileViews",label:"Vizitor n'profil",value:W(B(r.profileViews)),withEye:!0,loading:s,imageUrl:String(t||"").trim(),iconName:"user",panelTab:"analitika"}),l.push({key:"menuOpens",label:"Vizitor n'meny",value:W(B(r.menuOpens)),withEye:!0,loading:s&&a,locked:!a,imageUrl:String(n.menuImageUrl||"").trim(),iconName:"book-open",panelTab:"analitika"}),l.push({key:"qrScans",label:"Skanime n'tavolina",value:W(B(r.qrScans)),withEye:!0,loading:s&&a,locked:!a,imageUrl:String(n.qrImageUrl||"").trim(),iconName:"layout-grid",panelTab:"analitika"}),l}function Ha({state:e,renderFn:t,documentObj:a,firestoreApi:n={},profileApi:r={},composerApi:s={},viewApi:h={},iconFn:l,storageObj:p}={}){const b=a||(typeof document>"u"?null:document),f=b?.defaultView||(typeof window>"u"?null:window),y=typeof t=="function"?t:()=>{},g=p||(typeof localStorage>"u"?null:localStorage),_=typeof r.getBusinessProfileTypeFn=="function"?r.getBusinessProfileTypeFn:(()=>""),k=typeof r.isShopCatalogProfileFn=="function"?r.isShopCatalogProfileFn:(()=>!1),C=typeof r.getRestaurantMetaByIdFn=="function"?r.getRestaurantMetaByIdFn:(()=>null),O=typeof r.resolveRestaurantLogoFn=="function"?r.resolveRestaurantLogoFn:(()=>""),$=typeof r.resolveOwnAvatarUrlFn=="function"?r.resolveOwnAvatarUrlFn:(()=>""),F=typeof h.renderAnalyticsViewFn=="function"?h.renderAnalyticsViewFn:(()=>""),M=typeof h.renderSettingsViewFn=="function"?h.renderSettingsViewFn:(()=>""),w=typeof h.warmAnalyticsFn=="function"?h.warmAnalyticsFn:(()=>{});let S=!1,x=0,T=!1,G=null,L=null,I="",H=!1,he=()=>null;const Qe=300;function ie(){const d=e?.userProfile||{};return ea({businessType:_(d),isShopCatalog:k(d)})}function Ye(d=""){const c=C(d)||{};return Bt(c).map(u=>({id:u.id,name:u.title,price:u.price??"",category:u.beds||u.tag||"",type:"room",imageUrl:u.imageUrl||""}))}function Je(d=""){if(!g)return null;try{const c=g.getItem(`${Ae}${d}`);if(!c)return null;const u=JSON.parse(c),m=Array.isArray(u?.items)?u.items:null;return m&&m.length?m:null}catch{return null}}function Xe(d="",c=[]){if(g)try{g.setItem(`${Ae}${d}`,JSON.stringify({savedAt:Date.now(),items:c}))}catch{}}async function et(d=""){const{db:c,collectionFn:u,queryFn:m,limitFn:z,getDocsFn:P}=n;if(!c||typeof u!="function"||typeof P!="function")throw new Error("Produktet nuk u ngarkuan.");const Z=u(c,"restaurants",d,"menuItems"),A=typeof m=="function"&&typeof z=="function"?m(Z,z(Qe)):Z,U=await P(A),D=[];return U.forEach(V=>{const X=he(V?.id,V?.data?.()||{});X&&D.push(X)}),D.sort((V,X)=>V.name.localeCompare(X.name,"sq")),D}async function tt(d="",c){const u=String(d||"").trim();if(!u)throw new Error("Produktet nuk u ngarkuan.");if(ie()==="hotel")return Ye(u);const m=et(u).then(P=>(Xe(u,P),P)),z=Je(u);return z?(typeof c=="function"?m.then(P=>c(P)).catch(()=>{}):m.catch(()=>{}),z):m}function ue(){return G?Promise.resolve(G):(L||(L=pt(()=>import("./business-composer-controller-40IKWTQ1.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(d=>(he=typeof d?.normalizeComposerProductCore=="function"?d.normalizeComposerProductCore:(()=>null),G=d.createBusinessComposerController({documentObj:b,windowObj:b?.defaultView||null,api:{getRestaurantIdFn:()=>Y(),getBusinessMetaFn:()=>{const c=Y();if(!c)return{name:"",logoUrl:"",city:""};const u=be(c),m=C(c)||{};return{name:u.name,logoUrl:u.logoUrl,city:String(m.city||"").trim()}},loadProductsFn:(c,u)=>tt(c,u),getBusinessKindFn:()=>ie(),uploadImageFn:s.uploadImageFn,uploadVideoFn:s.uploadVideoFn,captureVideoPosterFn:s.captureVideoPosterFn,createPostFn:s.createPostFn,createStoryFn:s.createStoryFn,formatPriceFn:s.formatPriceFn,getOptimizedImageUrlFn:s.getOptimizedImageUrlFn,escapeHtmlFn:s.escapeHtmlFn,iconFn:typeof l=="function"?l:void 0,afterPublishFn:async c=>{try{await J({force:!0})}catch{}typeof s.afterPublishFn=="function"&&await s.afterPublishFn(c)}}}),G)).catch(d=>{throw L=null,console.error("[mnyra][dashboard] composer load failed",d),d})),L)}function pe(){const d=f?.navigator?.connection;return!d||typeof d!="object"?!1:d.saveData===!0?!0:/(^|-)2g$/.test(String(d.effectiveType||"").trim().toLowerCase())}function at(){if(H||G||!f||pe())return;H=!0;const d=()=>{if(ue().catch(()=>{}),typeof s.prewarmFn=="function")try{s.prewarmFn()}catch{}};if(typeof f.requestIdleCallback=="function"){f.requestIdleCallback(d,{timeout:Re});return}f.setTimeout?.(d,Be)}function nt(){if(S||!f||pe())return;S=!0;const d=()=>{try{w()}catch{}};if(typeof f.requestIdleCallback=="function"){f.requestIdleCallback(d,{timeout:Re});return}f.setTimeout?.(d,Be)}function it(d="post"){const c=String(d||"").trim().toLowerCase(),u=c==="story"||c==="profile"?c:"post";if(typeof s.prewarmFn=="function")try{s.prewarmFn()}catch{}if(G){G.open(u);return}I=u,ue().then(m=>{const z=I||u;I="",m?.open?.(z)}).catch(()=>{I=""})}function re(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:"",restaurantId:"",paywall:""}),e.dashboardView}function ge(d=""){const c=re(),u=String(d||"").trim();return String(c.restaurantId||"")===u||(c.restaurantId=u,c.model=null,c.status="idle",c.error="",c.loadedSignature="",c.paywall="",x+=1),c}function Y(){const d=e?.userProfile||{};return String(d.restaurantId||d.staffRestaurantId||"").trim()}let me="";function rt(){const d=String(e?.user?.uid||"").trim();!d||me===d||typeof r.ensureBusinessProfileFn=="function"&&(me=d,Promise.resolve().then(()=>r.ensureBusinessProfileFn()).catch(c=>{console.warn("[mnyra][panel] business profile could not be resolved",c)}).finally(()=>{String(e?.user?.uid||"").trim()===d&&y()}))}function st(){const d=String(e?.user?.uid||"").trim();if(!d)return!1;const c=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||c===d}function fe(d=""){return`${wa}${d}`}function ot(d="",c=""){if(!g||!d)return null;try{const u=g.getItem(fe(d));if(!u)return null;const m=JSON.parse(u);return!m||typeof m!="object"||String(m.day||"").trim()!==String(c||"").trim()||!m.model||typeof m.model!="object"?null:m.model}catch{return null}}function dt(d="",c=null){if(!(!g||!d||!c))try{g.setItem(fe(d),JSON.stringify({day:c.day,model:c}))}catch{}}async function lt(d=""){const{db:c,collectionFn:u,queryFn:m,orderByFn:z,limitFn:P,getDocsFn:Z}=n;if(!c||typeof u!="function"||typeof m!="function"||typeof z!="function"||typeof P!="function"||typeof Z!="function")return[];const A=u(c,"restaurants",d,"socialPosts");return(await Z(m(A,z("createdAt","desc"),P(ka)))).docs.map(D=>({id:D.id,data:D.data()||{}})).filter(D=>{const V=String(D.data.status||"active").trim().toLowerCase();return V!=="deleted"&&V!=="hidden"})}async function J({force:d=!1}={}){const c=Y(),u=ge(c);if(!c)return;const m=gt({rangeKey:"7d"});if(!m)return;const z=`${c}::${m.toDay}`;if(!d&&u.loadedSignature===z&&u.status==="ready")return;if(!u.model){const A=ot(c,m.toDay);A&&(u.model=A,u.status="ready",y())}x+=1;const P=x;u.model||(u.status="loading",u.error="",y());try{const A={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:c},[U,D]=await Promise.allSettled([mt({...A,fromDay:m.fromDay,toDay:m.toDay}),lt(c)]);if(P!==x)return;if(U.status==="rejected")throw U.reason;D.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",D.reason),u.model=Pa({days:U.value,todayKey:m.toDay,rawPosts:D.status==="fulfilled"?D.value:[]}),u.status="ready",u.error="",u.loadedSignature=z,dt(c,u.model)}catch(A){if(P!==x)return;console.error("[mnyra][dashboard] load failed",A),u.model||(u.status="error",u.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}y()}function ct(){T||!b||(T=!0,b.addEventListener("click",d=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(d.target?.closest?.("[data-dashboard-retry]")){J({force:!0});return}if(d.target?.closest?.("[data-dashboard-paywall-close]")){d.preventDefault(),re().paywall="",y();return}const c=d.target?.closest?.("[data-dashboard-metric-locked]");if(c){d.preventDefault(),re().paywall=String(c.getAttribute("data-dashboard-metric-locked")||"").trim(),y();return}const u=d.target?.closest?.("[data-dashboard-composer]");if(u){d.preventDefault(),it(u.getAttribute("data-dashboard-composer"));return}const m=d.target?.closest?.("[data-dashboard-panel-tab]");if(m){d.preventDefault();const z=ne(m.getAttribute("data-dashboard-panel-tab"));if(z===ne(e?.dashboardPanelTab))return;e.dashboardPanelTab=z,y()}}catch{}}))}function be(d=""){const c=e?.userProfile||{},u=d?C(d)||{}:{},m=String(u.name||u.restaurantName||c.name||"").trim()||"Business";let z="";try{z=String($()||"").trim()}catch{}if(!z)try{z=String(O(u)||"").trim()}catch{}return{name:m,logoUrl:z,kind:ie(),coverUrl:Aa(u),subscribed:Ka({profile:c,restaurant:u})}}function ht(d=""){try{if(!xt()||!d)return"";zt({restaurantId:d,onBadgeFn:()=>y()});const c=St();return Ct({enabled:!0,unseenCount:c.unseen,activeOffers:c.activeOffers||0,todayBookings:c.today,iconFn:l})}catch{return""}}function ut(){Jt(b),ct();const d=Y(),c=ge(d);let u="";if(!d)rt(),u=st()?ba():ya();else{at(),nt();const m=be(d),z=ne(e?.dashboardPanelTab);c.status==="idle"&&(c.status="loading",queueMicrotask(()=>{J({force:!1})}));let P="";c.model?P=pa({posts:c.model.posts,iconFn:l}):c.status==="error"?P=_a({message:c.error}):P=ga();const Z=`
        ${ht(d)}
        ${na({iconFn:l})}
        ${la({iconFn:l,showEditor:!!d})}
        ${ia({iconFn:l,showEditor:!!d})}
        ${ra({iconFn:l,showEditor:!!d})}
        ${oa({iconFn:l,kind:m.kind,showEditor:!!d})}
      `;let A;z==="analitika"?A=`
          <div class="mnyra-dash__embed">${F()}</div>
          ${P}
        `:z==="opsionet"?A=`<div class="mnyra-dash__embed">${M()}</div>`:A=Z;const U=Ra({model:c.model,coverUrl:m.coverUrl,subscribed:m.subscribed,assets:xa}),D=String(c.paywall||"").trim();u=`
        ${aa({name:m.name,logoUrl:m.logoUrl,iconFn:l})}
        ${ca({cards:U,iconFn:l})}
        ${qe(`
          ${ua({activeTab:z,iconFn:l})}
          ${A}
        `)}
        ${D?ma({title:za[D]||"Me pagesë"}):""}
      `}return`
      <section class="mnyra-work mnyra-dash" data-dashboard-root>
        ${u}
      </section>
    `}return Object.freeze({renderDashboardView:ut,loadDashboard:J})}export{Ia as B,ae as G,Pt as M,Ha as a,Na as b,Dt as c,Oa as d,Ta as e,Ga as f,Qt as g,Ut as h,Le as i,ee as j,ce as k,le as l,Ie as m,Rt as n,Bt as o,ja as p,Ca as q,La as r};
