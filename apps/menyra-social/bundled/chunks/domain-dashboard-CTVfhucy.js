...
`;function Nt(e={},t={}){const a=t.escapeHtml,n=t.icon;return`
  ...
  `}function Vt({booking:e={},code:t="",busy:a=!1,note:n="",deps:r={}}={}){const s=r.escapeHtml,h=Ve(e),l=e.benefitLabel||e.snapshot?.benefitLabel||"",g=Re(e.status),_=String(t||"").trim().toUpperCase();return`
  ...
  `}function Se({title:e="",note:t="",iconName:a="",deps:n={}}={}){const r=n.escapeHtml,s=n.icon;return oe({eyebrow:i.brand,title:e,sub:i.soonHint,body:`
    ...
    <div class="p-4 rounded-[1.6rem] bg-slate-50 border border-slate-100" data-go-offer="${o(a,e.id)}">
      ...
      <div style="pointer-events:none;" aria-hidden="true">
        ${Le({businessName:"",imageUrl:e.imageUrl||"",variant:le,benefitLabel:e.benefitLabel||"",benefitView:Te(e.benefit||{}),meta:[{icon:"users",label:Ce(e)},{icon:"clock",label:Ge(e)}]})}
      </div>
...
`;function se({attr:e="",unit:t="€",value:a="",placeholder:n="",mode:r="decimal",inputClass:s="",escapeHtml:h=null}={}){return`
  ...
  `}const $e=Object.freeze([10,15,20,25]);function Jt({benefit:e={},percentCustom:t=!1,errorFor:a=()=>"",inputClass:n="",inputBase:r="",escapeHtml:s=null}={}){const h=p=>Z(s,p),l=p=>{const x=a(p);return x?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="${o(s,p)}">${o(s,x)}</p>`:""};if(!xt.includes(e.kind))return`
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll px-6 py-5 space-y-5" data-go-editor-scroll>
          ...
          <div data-go-section="category">
            ...
            <div class="mt-3 space-y-2">
              ${[{key:"food",label:i.ifFood},{key:"drinks",label:i.ifDrinks}].map(w=>{const $=p.includes(w.key),y=wt.find(G=>G.key===w.key)?.hint||"";return`
                  <button type="button" data-go-offer-intent="${o(n,w.key)}" aria-pressed="${$?"true":"false"}"
                    ...
                    <span class="block text-[11px] font-semibold go-offer-answer__hint">${o(n,y)}</span>
                  </button>
                `}).join("")}
            </div>
  ...
  `}function Ha({restaurantName:e="",tab:t="active",group:a=0,overview:n={},search:r={},camera:s={},bookingEntering:h=!1,bookings:l=[],nowMs:g=Date.now(),offers:_=[],settings:b={},paused:m=!1,loading:p=!1,error:x="",deps:k={}}={}){const S=k.escapeHtml,O=k.icon,K=y=>vt(y,g),M=l.filter(K),j=l.filter(y=>!K(y)),w=_.filter(y=>y.status!=="archived");let $="";if(t==="offers")$=oe({eyebrow:i.brand,title:i.tabs.offers,sub:`${w.length} ${w.length===1?"oferte":"oferta"}`,action:`
      ...
      `,body:w.length?`<div class="space-y-3">${w.map(y=>qt(y,k)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(S,i.emptyTitle)}</div>`,deps:k});else if(t==="finalized")$=j.length?`<div class="go-cards">${j.map(y=>ze(Tt.includes(Re(y.status))?{...y,status:"expired"}:y,{done:!0,deps:k})).join("")}</div>`:`<p class="go-cards__note">${o(S,i.noHistory)}</p>`;else if(t==="pending")$=p?`<p class="go-cards__note go-cards__note--loading" role="status">${o(S,i.loading)}</p>`:M.length?`<div class="go-cards">${M.map(y=>ze(y,{deps:k})).join("")}</div>`:`<p class="go-cards__note">${o(S,i.noBookings)}</p>`;else if(t==="stats")$=Se({title:i.tabs.stats,note:i.soonStats,iconName:"bar-chart-3",deps:k});else if(t==="payments")$=Se({title:i.tabs.payments,note:i.soonPayments,iconName:"wallet",deps:k});else if(t==="options"){const y=We(b?.pausedUntil);$=oe({eyebrow:i.brand,title:i.tabs.options,body:`
        <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
          <div>
            <p class="text-xs font-black text-slate-800">${o(S,i.goOn)}</p>
            <p class="text-[10px] font-bold text-slate-400">${o(S,m?`${i.pausedUntil} ${y}`:"ON")}</p>
          </div>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          ${m?`<button type="button" data-go-pause="0" class="go-pause px-4 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest">${o(S,i.resume)}</button>`:[{value:"30",label:"30 min"},{value:"60",label:"1 orë"},{value:"tomorrow",label:"Deri nesër"},{value:"-1",label:"Pa afat"}].map(G=>`
              <button type="button" data-go-pause="${G.value}"
                class="go-pause px-4 rounded-2xl bg-slate-50 border border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-600">${o(S,G.label)}</button>
            `).join("")}
        </div>
...
`;function aa(e=typeof document>"u"?null:document){if(Gt(e),!(!e||e.getElementById(Ke)))try{const t=e.createElement("style");t.id=Ke,t.textContent=ta,e.head?.appendChild(t)}catch{}}function v(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function A(e,t,a=""){if(typeof e!="function")return"";try{return e(t,a)||""}catch{return""}}const na=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function ia({businessType:e="",isShopCatalog:t=!1}={}){if(t)return"shop";const a=String(e||"").trim().toLowerCase();return na.includes(a)?"hotel":"restaurant"}function ra(e=new Date().getHours()){const t=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return t>=5&&t<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:t>=11&&t<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:t>=18&&t<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function sa({name:e="",logoUrl:t="",hour:a=new Date().getHours(),iconFn:n}={}){const r=ra(a),s=v(e||"Business");return`
    <div class="mnyra-work__head mnyra-dash__greet">
      <p class="mnyra-dash__greet-title">
        ...
        <span class="mnyra-dash__greet-logo">
          ${t?`<img src="${v(t)}" alt="${s}" title="${s}" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:`<span class="mnyra-dash__greet-logo-fallback" title="${s}">${A(n,"store","w-4 h-4")}</span>`}
        </span>
      </p>
  ...
  `:""}function pa({cards:e=[],iconFn:t}={}){const a=(Array.isArray(e)?e:[]).filter(r=>r&&r.key);if(!a.length)return"";const n=a.map((r,s)=>{const h=v(r.label||"");if(r.pending)return'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>';const l=s<2?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"';let g="";r.imageUrl?g=`<img class="mnyra-dash__hl-media" src="${v(r.imageUrl)}" alt="" ${l} decoding="async" onerror="this.style.display='none'" />`:r.videoUrl&&(g=`<video class="mnyra-dash__hl-media" src="${v(r.videoUrl)}#t=0.1" preload="metadata" muted playsinline disablepictureinpicture tabindex="-1" aria-hidden="true"></video>`);const _=`
    ...
    `,b=r.withEye?`<span class="mnyra-dash__hl-eye">${A(t,"eye","w-4 h-4")}</span>`:"";let m;r.locked?m=`<span class="mnyra-dash__hl-lock">${A(t,"lock","w-3 h-3")}Me pagesë</span>`:r.loading?m='<span class="mnyra-dash__hl-value mnyra-dash__hl-value--pending" aria-hidden="true"></span>':r.emptyText?m=`<span class="mnyra-dash__hl-empty">${v(r.emptyText)}</span>`:m=`<span class="mnyra-dash__hl-value">${b}${v(r.value||"0")}</span>`;let p;r.locked?p=`class="mnyra-dash__hl-card mnyra-dash__hl-card--locked" data-dashboard-metric-locked="${v(r.key)}"`:r.composer?p=`class="mnyra-dash__hl-card" data-dashboard-composer="${v(r.composer)}"`:p=`class="mnyra-dash__hl-card"${r.panelTab?` data-dashboard-panel-tab="${v(r.panelTab)}"`:""}`;const x=r.locked?`${h} – me pagesë`:`${h} ${r.emptyText||r.value||""}`.trim();return`
      <button type="button" ${p} data-dashboard-metric="${v(r.key)}" aria-label="${v(x)}">
        ${_}
  ...
  `}function ma(e=[]){return(Array.isArray(e)?e:[]).filter(t=>t&&t.key).map(t=>[t.key,t.label||"",t.value||"",t.emptyText||"",t.imageUrl||"",t.videoUrl||"",t.iconName||"",t.panelTab||"",t.composer||"",t.pending?"p":"",t.loading?"l":"",t.locked?"x":"",t.withEye?"e":""].join("~")).join("|")}const qe=Object.freeze([Object.freeze({id:"funksionet",label:"Funksionet",iconName:"layout-grid"}),Object.freeze({id:"analitika",label:"Analitika",iconName:"bar-chart-3"}),Object.freeze({id:"opsionet",label:"Opsionet",iconName:"settings"})]);function ae(e=""){const t=String(e||"").trim().toLowerCase();return qe.some(a=>a.id===t)?t:"funksionet"}function fa({activeTab:e="funksionet",iconFn:t}={}){const a=ae(e);return`<div class="mnyra-work__pills mnyra-dash__tabs" role="tablist" data-dashboard-panel-tabs>${qe.map(r=>{const s=r.id===a;return`
      ...
      >${A(t,r.iconName,"w-4 h-4")}<span class="mnyra-work__pill-label">${v(r.label)}</span></button>
  ...
  `}function ba({posts:e=[],iconFn:t}={}){const a=Array.isArray(e)?e:[];let n="";return a.length?(n=a.map(r=>{const s=[r.dateLabel,`${V(r.likesCount||0)} Likes`,`${V(r.commentsCount||0)} Kommentare`];return Number(r.impressions||0)>0&&s.push(`${V(r.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${r.thumbUrl?`<img src="${v(r.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:A(t,r.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
  ...
  `}const za="menyra_social_dashboard_cache_v1::",Pe="menyra_social_composer_products_v1::",Ae=2500,Ee=1200,Sa=6,$a=3,Ka=Object.freeze({menuImageUrl:"/apps/menyra-social/assets/panel/menu-scan.jpg",qrImageUrl:"/apps/menyra-social/assets/panel/qr-stand.jpg"}),Da=Object.freeze({menuOpens:"Menü-Aufrufe",qrScans:"QR-Scans"});function B(e){const t=Number(e);return Number.isFinite(t)?t:0}function Pa(e={}){const t=String(e.createdAtClient||"").trim();if(t){const n=new Date(t);if(!Number.isNaN(n.getTime()))return n}const a=e.createdAt;if(a&&typeof a.toDate=="function")try{const n=a.toDate();if(n instanceof Date&&!Number.isNaN(n.getTime()))return n}catch{}return null}function Aa(e="",t={}){const a=Array.isArray(t.media)&&t.media.length?t.media[0]:{},n=String(a.type||t.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",r=String(a.thumbUrl||(n==="image"?a.url:"")||t.thumbUrl||"").trim(),s=n==="video"?String(a.url||t.mediaUrl||"").trim():"",h=Pa(t);return{id:String(e||"").trim(),caption:String(t.caption||"").trim(),mediaType:n,thumbUrl:r,videoUrl:s,likesCount:B(t.likesCount),commentsCount:B(t.commentsCount),impressions:0,dateLabel:h?h.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:h?h.getTime():0}}function Ea({days:e=[],todayKey:t="",rawPosts:a=[]}={}){const n=Array.isArray(e)?e:[],r=be(n),s=n.find(b=>String(b?.date||b?.id||"").trim()===String(t||"").trim()),h=be(s?[s]:[]),l=r.merged?.posts&&typeof r.merged.posts=="object"?r.merged.posts:{},g=(Array.isArray(a)?a:[]).map(b=>Aa(b?.id,b?.data||{})).filter(b=>b.id).map(b=>({...b,impressions:B(l[b.id]?.impressions)})),_=g.slice().sort((b,m)=>m.createdAtMs-b.createdAtMs).slice(0,$a);return{day:String(t||"").trim(),week:r.summary,today:h.summary,posts:_,latestPost:Ra(g)}}function Ra(e=[]){const t=(Array.isArray(e)?e:[]).filter(a=>a&&a.id);return t.length?t.slice().sort((a,n)=>B(n.createdAtMs)-B(a.createdAtMs)||B(n.impressions)-B(a.impressions)||B(n.likesCount)-B(a.likesCount))[0]:null}function Fa({profile:e={},restaurant:t={}}={}){return bt({profile:e,restaurant:t,feature:"qr"})}function Ba(e={}){const t=e&&typeof e=="object"?e:{};return String(t.titleImageUrl||t.coverImageUrl||t.coverUrl||t.heroUrl||t.bannerUrl||"").trim()}function Ma({model:e=null,coverUrl:t="",subscribed:a=!1,assets:n={}}={}){const r=e?.today||{},s=!e,h=e?.latestPost||null,l=[];if(s)l.push({key:"latestPost",label:"Postimi fundit",pending:!0});else if(!h)l.push({key:"latestPost",label:"Postimi fundit",emptyText:"S'ka postim",iconName:"image",composer:"post"});else{const g=String(h.thumbUrl||"").trim();l.push({key:"latestPost",label:"Postimi fundit",value:V(B(h.impressions)),withEye:!0,imageUrl:g,videoUrl:g?"":String(h.videoUrl||"").trim(),iconName:"image",panelTab:"analitika"})}return l.push({key:"profileViews",label:"Vizitor n'profil",value:V(B(r.profileViews)),withEye:!0,loading:s,imageUrl:String(t||"").trim(),iconName:"user",panelTab:"analitika"}),l.push({key:"menuOpens",label:"Vizitor n'meny",value:V(B(r.menuOpens)),withEye:!0,loading:s&&a,locked:!a,imageUrl:String(n.menuImageUrl||"").trim(),iconName:"book-open",panelTab:"analitika"}),l.push({key:"qrScans",label:"Skanime n'tavolina",value:V(B(r.qrScans)),withEye:!0,loading:s&&a,locked:!a,imageUrl:String(n.qrImageUrl||"").trim(),iconName:"layout-grid",panelTab:"analitika"}),l}function Va({state:e,renderFn:t,documentObj:a,firestoreApi:n={},profileApi:r={},composerApi:s={},viewApi:h={},iconFn:l,storageObj:g}={}){const _=a||(typeof document>"u"?null:document),b=_?.defaultView||(typeof window>"u"?null:window),m=typeof t=="function"?t:()=>{},p=g||(typeof localStorage>"u"?null:localStorage),x=typeof r.getBusinessProfileTypeFn=="function"?r.getBusinessProfileTypeFn:(()=>""),k=typeof r.isShopCatalogProfileFn=="function"?r.isShopCatalogProfileFn:(()=>!1),S=typeof r.getRestaurantMetaByIdFn=="function"?r.getRestaurantMetaByIdFn:(()=>null),O=typeof r.resolveRestaurantLogoFn=="function"?r.resolveRestaurantLogoFn:(()=>""),K=typeof r.resolveOwnAvatarUrlFn=="function"?r.resolveOwnAvatarUrlFn:(()=>""),M=typeof h.renderAnalyticsViewFn=="function"?h.renderAnalyticsViewFn:(()=>""),j=typeof h.renderSettingsViewFn=="function"?h.renderSettingsViewFn:(()=>""),w=typeof h.warmAnalyticsFn=="function"?h.warmAnalyticsFn:(()=>{});let $=!1,y=0,G=!1,T=null,N=null,L="",I=!1,ce=()=>null;const Ye=300;function ne(){const d=e?.userProfile||{};return ia({businessType:x(d),isShopCatalog:k(d)})}function Je(d=""){const c=S(d)||{};return Bt(c).map(u=>({id:u.id,name:u.title,price:u.price??"",category:u.beds||u.tag||"",type:"room",imageUrl:u.imageUrl||""}))}function Xe(d=""){if(!p)return null;try{const c=p.getItem(`${Pe}${d}`);if(!c)return null;const u=JSON.parse(c),f=Array.isArray(u?.items)?u.items:null;return f&&f.length?f:null}catch{return null}}function et(d="",c=[]){if(p)try{p.setItem(`${Pe}${d}`,JSON.stringify({savedAt:Date.now(),items:c}))}catch{}}async function tt(d=""){const{db:c,collectionFn:u,queryFn:f,limitFn:z,getDocsFn:D}=n;if(!c||typeof u!="function"||typeof D!="function")throw new Error("Produktet nuk u ngarkuan.");const H=u(c,"restaurants",d,"menuItems"),E=typeof f=="function"&&typeof z=="function"?f(H,z(Ye)):H,U=await D(E),P=[];return U.forEach(W=>{const J=ce(W?.id,W?.data?.()||{});J&&P.push(J)}),P.sort((W,J)=>W.name.localeCompare(J.name,"sq")),P}async function at(d="",c){const u=String(d||"").trim();if(!u)throw new Error("Produktet nuk u ngarkuan.");if(ne()==="hotel")return Je(u);const f=tt(u).then(D=>(et(u,D),D)),z=Xe(u);return z?(typeof c=="function"?f.then(D=>c(D)).catch(()=>{}):f.catch(()=>{}),z):f}function he(){return T?Promise.resolve(T):(N||(N=pt(()=>import("./business-composer-controller-lrI_ziPG.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(d=>(ce=typeof d?.normalizeComposerProductCore=="function"?d.normalizeComposerProductCore:(()=>null),T=d.createBusinessComposerController({documentObj:_,windowObj:_?.defaultView||null,api:{getRestaurantIdFn:()=>Q(),getBusinessMetaFn:()=>{const c=Q();if(!c)return{name:"",logoUrl:"",city:""};const u=fe(c),f=S(c)||{};return{name:u.name,logoUrl:u.logoUrl,city:String(f.city||"").trim()}},loadProductsFn:(c,u)=>at(c,u),getBusinessKindFn:()=>ne(),uploadImageFn:s.uploadImageFn,uploadVideoFn:s.uploadVideoFn,captureVideoPosterFn:s.captureVideoPosterFn,createPostFn:s.createPostFn,createStoryFn:s.createStoryFn,formatPriceFn:s.formatPriceFn,getOptimizedImageUrlFn:s.getOptimizedImageUrlFn,escapeHtmlFn:s.escapeHtmlFn,iconFn:typeof l=="function"?l:void 0,afterPublishFn:async c=>{try{await Y({force:!0})}catch{}typeof s.afterPublishFn=="function"&&await s.afterPublishFn(c)}}}),T)).catch(d=>{throw N=null,console.error("[mnyra][dashboard] composer load failed",d),d})),N)}function ue(){const d=b?.navigator?.connection;return!d||typeof d!="object"?!1:d.saveData===!0?!0:/(^|-)2g$/.test(String(d.effectiveType||"").trim().toLowerCase())}function nt(){if(I||T||!b||ue())return;I=!0;const d=()=>{if(he().catch(()=>{}),typeof s.prewarmFn=="function")try{s.prewarmFn()}catch{}};if(typeof b.requestIdleCallback=="function"){b.requestIdleCallback(d,{timeout:Ae});return}b.setTimeout?.(d,Ee)}function it(){if($||!b||ue())return;$=!0;const d=()=>{try{w()}catch{}};if(typeof b.requestIdleCallback=="function"){b.requestIdleCallback(d,{timeout:Ae});return}b.setTimeout?.(d,Ee)}function rt(d="post"){const c=String(d||"").trim().toLowerCase(),u=c==="story"||c==="profile"?c:"post";if(typeof s.prewarmFn=="function")try{s.prewarmFn()}catch{}if(T){T.open(u);return}L=u,he().then(f=>{const z=L||u;L="",f?.open?.(z)}).catch(()=>{L=""})}function ie(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:"",restaurantId:"",paywall:""}),e.dashboardView}function ge(d=""){const c=ie(),u=String(d||"").trim();return String(c.restaurantId||"")===u||(c.restaurantId=u,c.model=null,c.status="idle",c.error="",c.loadedSignature="",c.paywall="",y+=1),c}function Q(){const d=e?.userProfile||{};return String(d.restaurantId||d.staffRestaurantId||"").trim()}let pe="";function st(){const d=String(e?.user?.uid||"").trim();!d||pe===d||typeof r.ensureBusinessProfileFn=="function"&&(pe=d,Promise.resolve().then(()=>r.ensureBusinessProfileFn()).catch(c=>{console.warn("[mnyra][panel] business profile could not be resolved",c)}).finally(()=>{String(e?.user?.uid||"").trim()===d&&m()}))}function ot(){const d=String(e?.user?.uid||"").trim();if(!d)return!1;const c=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||c===d}function me(d=""){return`${za}${d}`}function dt(d="",c=""){if(!p||!d)return null;try{const u=p.getItem(me(d));if(!u)return null;const f=JSON.parse(u);return!f||typeof f!="object"||String(f.day||"").trim()!==String(c||"").trim()||!f.model||typeof f.model!="object"?null:f.model}catch{return null}}function lt(d="",c=null){if(!(!p||!d||!c))try{p.setItem(me(d),JSON.stringify({day:c.day,model:c}))}catch{}}async function ct(d=""){const{db:c,collectionFn:u,queryFn:f,orderByFn:z,limitFn:D,getDocsFn:H}=n;if(!c||typeof u!="function"||typeof f!="function"||typeof z!="function"||typeof D!="function"||typeof H!="function")return[];const E=u(c,"restaurants",d,"socialPosts");return(await H(f(E,z("createdAt","desc"),D(Sa)))).docs.map(P=>({id:P.id,data:P.data()||{}})).filter(P=>{const W=String(P.data.status||"active").trim().toLowerCase();return W!=="deleted"&&W!=="hidden"})}async function Y({force:d=!1}={}){const c=Q(),u=ge(c);if(!c)return;const f=mt({rangeKey:"7d"});if(!f)return;const z=`${c}::${f.toDay}`;if(!d&&u.loadedSignature===z&&u.status==="ready")return;if(!u.model){const E=dt(c,f.toDay);E&&(u.model=E,u.status="ready",m())}y+=1;const D=y;u.model||(u.status="loading",u.error="",m());try{const E={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:c},[U,P]=await Promise.allSettled([ft({...E,fromDay:f.fromDay,toDay:f.toDay}),ct(c)]);if(D!==y)return;if(U.status==="rejected")throw U.reason;P.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",P.reason),u.model=Ea({days:U.value,todayKey:f.toDay,rawPosts:P.status==="fulfilled"?P.value:[]}),u.status="ready",u.error="",u.loadedSignature=z,lt(c,u.model)}catch(E){if(D!==y)return;console.error("[mnyra][dashboard] load failed",E),u.model||(u.status="error",u.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}m()}function ht(){G||!_||(G=!0,_.addEventListener("click",d=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(d.target?.closest?.("[data-dashboard-retry]")){Y({force:!0});return}if(d.target?.closest?.("[data-dashboard-paywall-close]")){d.preventDefault(),ie().paywall="",m();return}const c=d.target?.closest?.("[data-dashboard-metric-locked]");if(c){d.preventDefault(),ie().paywall=String(c.getAttribute("data-dashboard-metric-locked")||"").trim(),m();return}const u=d.target?.closest?.("[data-dashboard-composer]");if(u){d.preventDefault(),rt(u.getAttribute("data-dashboard-composer"));return}const f=d.target?.closest?.("[data-dashboard-panel-tab]");if(f){d.preventDefault();const z=ae(f.getAttribute("data-dashboard-panel-tab"));if(z===ae(e?.dashboardPanelTab))return;e.dashboardPanelTab=z,m()}}catch{}}))}function fe(d=""){const c=e?.userProfile||{},u=d?S(d)||{}:{},f=String(u.name||u.restaurantName||c.name||"").trim()||"Business";let z="";try{z=String(K()||"").trim()}catch{}if(!z)try{z=String(O(u)||"").trim()}catch{}return{name:f,logoUrl:z,kind:ne(),coverUrl:Ba(u),subscribed:Fa({profile:c,restaurant:u})}}function ut(d=""){try{if(!St()||!d)return"";$t({restaurantId:d,onBadgeFn:()=>m()});const c=Kt();return Zt({enabled:!0,unseenCount:c.unseen,activeOffers:c.activeOffers||0,todayBookings:c.today,iconFn:l})}catch{return""}}function gt(){aa(_),ht();const d=Q(),c=ge(d);let u="";if(!d)st(),u=ot()?va():xa();else{nt(),it();const f=fe(d),z=ae(e?.dashboardPanelTab);c.status==="idle"&&(c.status="loading",queueMicrotask(()=>{Y({force:!1})}));let D="";c.model?D=ba({posts:c.model.posts,iconFn:l}):c.status==="error"?D=ya({message:c.error}):D=_a();const H=`
    ...
    `}return Object.freeze({renderDashboardView:gt,loadDashboard:Y})}export{Wa as B,te as G,Pt as M,Va as a,Ha as b,At as c,Na as d,La as e,Ia as f,ea as g,Qt as h,Ne as i,X as j,le as k,de as l,Le as m,Ft as n,Bt as o,Za as p,Oa as q,Ua as r};
