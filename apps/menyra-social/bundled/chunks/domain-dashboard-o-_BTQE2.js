const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["chunks/business-composer-controller-Dh7_Ikb_.js","chunks/domain-feed-social-eager-DpTyrZVV.js","chunks/domain-auth-B1kS5TG-.js","chunks/domain-public-profile-mLQti0eH.js","chunks/domain-media-eager-DAUyCk2O.js","chunks/domain-menu-eager-DVNGPNZT.js","chunks/profile-post-card-markup-utils-HwqIiXgP.js"])))=>i.map(i=>d[i]);
import{_ as it}from"./domain-auth-B1kS5TG-.js";import{f as K,r as ot,l as dt,s as me}from"./domain-analytics-oyJYW1vv.js";import{b as lt}from"./domain-business-accounts-D8NpUhi6.js";import{i as ct,e as ut,a as ht}from"./domain-feed-social-eager-DpTyrZVV.js";const pt=20,mt=8;function F(e=""){return e==null?"":String(e).trim()}function ae(e){if(e==null||e==="")return null;const a=Number(String(e).replace(",","."));return Number.isFinite(a)&&a>0?a:null}function ft(e=Date.now(),a=Math.random()){const t=Math.max(0,Number(e)||0).toString(36),n=Math.floor(Math.max(0,Math.min(.999999,Number(a)||0))*36**6).toString(36).padStart(6,"0");return`room_${t}_${n}`}function bt(e={}){const a=e&&typeof e=="object"?e:{},t=[...Array.isArray(a.images)?a.images:[],F(a.imageUrl??a.image??a.photoUrl)],n=[];return t.forEach(r=>{const s=F(r);s&&!n.includes(s)&&n.push(s)}),n.slice(0,mt)}function gt(e={},{index:a=0}={}){const t=e&&typeof e=="object"?e:{},n=ae(t.persons??t.guests??t.capacity),r=ae(t.size??t.sizeSqm??t.area),s=bt(t);return{id:F(t.id)||ft(Date.now()+a),title:F(t.title??t.name),description:F(t.description??t.text).slice(0,400),imageUrl:s[0]||"",images:s,price:ae(t.price??t.pricePerNight),currency:F(t.currency??t.currencyCode).toUpperCase()||"EUR",persons:n==null?null:Math.min(20,Math.round(n)),beds:F(t.beds??t.bedsLabel).slice(0,60),size:r==null?null:Math.min(500,Math.round(r)),tag:F(t.tag??t.badge).slice(0,40),active:t.active!==!1}}function yt(e=[]){return(Array.isArray(e)?e:[]).slice(0,pt).map((a,t)=>gt(a,{index:t}))}function _t(e={}){return yt((e&&typeof e=="object"?e:{}).hotelRooms).filter(t=>t.title)}function Ca(e={}){const a=[];return Number.isFinite(e?.persons)&&e.persons>0&&a.push({icon:"users",label:`${e.persons} persona`}),F(e?.beds)&&a.push({icon:"bed",label:F(e.beds)}),Number.isFinite(e?.size)&&e.size>0&&a.push({icon:"size",label:`${e.size} m²`}),a}function Ta(e={}){const a=Number(e?.price);if(!Number.isFinite(a)||a<=0)return"";const t=F(e?.currency).toUpperCase()||"EUR",n=Number.isInteger(a)?String(a):a.toFixed(2);return t==="EUR"?`€${n}`:`${n} ${t}`}const Ae="all",Pe=Object.freeze([{key:Ae,label:"Krejt",icon:"sparkles"},{key:"coffee",label:"Kafe",icon:"coffee"},{key:"drinks",label:"Pije",icon:"cup-soda"},{key:"food",label:"Ushqim",icon:"utensils"},{key:"dessert",label:"Ëmbëlsira",icon:"cake-slice"}]),xt=Object.freeze(Pe.map(e=>e.key)),vt="food",wt="drinks",kt="unsure",$t=Object.freeze([Object.freeze({key:vt,label:"Ushqim",hint:"Mëngjes, drekë, darkë etj.",icon:"utensils",categories:Object.freeze(["food"])}),Object.freeze({key:wt,label:"Pije",hint:"Kafe, ëmbëlsira, lëngje etj.",icon:"cup-soda",categories:Object.freeze(["coffee","drinks","dessert"])}),Object.freeze({key:kt,label:"Nuk e di",hint:"Gjitha ofertat për rreth teje.",icon:"sparkles",categories:Object.freeze([])})]);Object.freeze($t.map(e=>e.key));const Ea=1,St=10,Na=2,ne=Object.freeze([{key:"1-2",min:1,max:2,label:"1–2"},{key:"2-4",min:2,max:4,label:"2–4"},{key:"4-6",min:4,max:6,label:"4–6"},{key:"6+",min:6,max:99,label:"6+"}]),Ia=Object.freeze([{key:"now",label:"Tani",offsetMinutes:0},{key:"in30",label:"+30 min",offsetMinutes:30},{key:"in60",label:"+1 orë",offsetMinutes:60},{key:"later",label:"Më vonë",offsetMinutes:-1}]),zt="claim",fe="reservation",At=Object.freeze([{key:"percent",label:"Zbritje %"},{key:"freeItem",label:"Produkt falas"},{key:"bundle",label:"Paket / Çmim special"},{key:"table",label:"Tavolinë"},{key:"custom",label:"Oferta ime"}]),Ga=7;function De(e=""){const a=String(e||"").trim().toLowerCase();return ne.find(t=>t.key===a)||null}const Pt="Europe/Belgrade",je=1440,Q=["mon","tue","wed","thu","fri","sat","sun"];function Be(e){if(e==null||e==="")return 0;if(typeof e=="number")return Number.isFinite(e)?e:0;if(e instanceof Date){const n=e.getTime();return Number.isNaN(n)?0:n}if(typeof e=="object"){if(typeof e.toDate=="function")try{const n=e.toDate(),r=n instanceof Date?n.getTime():NaN;return Number.isNaN(r)?0:r}catch{return 0}return Number.isFinite(Number(e.seconds))?Math.round(Number(e.seconds)*1e3):Number.isFinite(Number(e._seconds))?Math.round(Number(e._seconds)*1e3):0}const t=new Date(String(e)).getTime();return Number.isNaN(t)?0:t}function R(e){const a=Be(e);return a?new Date(a).toISOString():""}function Dt(e){const a=String(e??"").trim();if(!a)return-1;const t=a.match(/^(\d{1,2})\s*[:.：]?\s*(\d{2})?$/);if(!t)return-1;const n=Number(t[1]),r=t[2]===void 0?0:Number(t[2]);return!Number.isFinite(n)||!Number.isFinite(r)||n>24||r>59?-1:n*60+r}function be(e){const a=Math.max(0,Math.round(Number(e)||0))%je,t=Math.floor(a/60),n=a%60;return`${String(t).padStart(2,"0")}:${String(n).padStart(2,"0")}`}function ge(e){return typeof e=="number"?Number.isFinite(e)?Math.max(0,Math.round(e)):-1:Dt(e)}function jt(e,a){const t=ge(e);let n=ge(a);return t<0||n<0||n===t?null:(n<t&&(n+=je),{start:t,end:n})}function Bt(e=[]){const a=Array.isArray(e)?e:[],t=[];return a.forEach(n=>{if(!n||typeof n!="object")return;const r=jt(n.start??n.from,n.end??n.to);r&&t.push(r)}),Ft(t)}function Ft(e=[]){const a=(Array.isArray(e)?e:[]).filter(n=>n&&Number.isFinite(n.start)&&Number.isFinite(n.end)&&n.end>n.start).slice().sort((n,r)=>n.start-r.start||n.end-r.end),t=[];return a.forEach(n=>{const r=t[t.length-1];if(r&&n.start<=r.end){r.end=Math.max(r.end,n.end);return}t.push({start:n.start,end:n.end})}),t}const Rt="active",ye="paused",_e="archived",xe="go",Ot="public";function f(e="",a=240){return String(e??"").trim().slice(0,a)}function N(e,a=0){const t=Math.trunc(Number(e));return!Number.isFinite(t)||t<0?a:t}function Ct(e=""){const a=String(e||"").trim().toLowerCase();return a===ye?ye:a===_e?_e:Rt}function Tt(e=""){const a=String(e||"").trim().toLowerCase();return xt.includes(a)?a:Ae}function Fe(e=""){return String(e||"").trim().toLowerCase()===fe?fe:zt}function Et(e={}){const a=e&&typeof e=="object"?e:{},t=String(a.kind||a.type||"").trim().toLowerCase(),n=Math.min(90,Math.max(0,N(a.percent??a.discountPercent,0))),r={kind:["percent","freeItem","bundle","table","custom"].includes(t)?t:n>0?"percent":"custom",percent:n,itemId:f(a.itemId,180),itemName:f(a.itemName||a.item,160),priceText:f(a.priceText||a.price,60),text:f(a.text,160)};return r.label=Nt(r),r}function Nt(e={}){const a=e&&typeof e=="object"?e:{},t=f(a.text,160);if(t)return t;const n=N(a.percent,0);if(a.kind==="percent"||n>0)return n>0?`–${n} %`:"";if(a.kind==="freeItem"){const r=f(a.itemName,160);return r?`${r} falas`:"Produkt falas"}if(a.kind==="bundle"){const r=f(a.itemName,160),s=f(a.priceText,60);return r&&s?`${r} ${s}`:r||s||"Paket special"}return a.kind==="table"?"Tavolinë e rezervuar":""}function Re(e){const a=Array.isArray(e)?e:e?[e]:[],t=[];return a.forEach(n=>{const r=String(n||"").trim().toLowerCase();De(r)&&!t.includes(r)&&t.push(r)}),t.length?t:ne.map(n=>n.key)}function It(e=[]){const a=Re(e);let t=Number.POSITIVE_INFINITY,n=0;return a.forEach(r=>{const s=De(r);s&&(t=Math.min(t,s.min),n=Math.max(n,s.max))}),!Number.isFinite(t)||!n?{min:1,max:99}:{min:t,max:n}}function Gt(e={}){const a=e&&typeof e=="object"?e:{},t=String(a.mode||"").trim().toLowerCase()==="windows"?"windows":"always",n=[];(Array.isArray(a.days)?a.days:[]).forEach(u=>{const m=String(u||"").trim().toLowerCase();Q.includes(m)&&!n.includes(m)&&n.push(m)});const s=Bt((Array.isArray(a.windows)?a.windows:[]).map(u=>({start:u?.start??u?.from,end:u?.end??u?.to})));return t==="always"||!n.length&&!s.length?{mode:"always",days:Q.slice(),windows:[]}:{mode:"windows",days:n.length?n:Q.slice(),windows:s}}function Kt(e={}){const a=e&&typeof e=="object"?e:{},t=s=>{const u=f(s,10);return/^\d{4}-\d{2}-\d{2}$/.test(u)?u:""},n=t(a.startDate||a.from),r=t(a.endDate||a.to);return n&&r&&r<n?{startDate:r,endDate:n}:{startDate:n,endDate:r}}function Mt(e={}){const a=e&&typeof e=="object"?e:{};return{slotGroups:N(a.slotGroups??a.maxGroupsPerSlot,0),slotGuests:N(a.slotGuests??a.maxGuestsPerSlot,0),dailyGroups:N(a.dailyGroups??a.maxGroupsPerDay,0),totalRedemptions:N(a.totalRedemptions??a.maxRedemptions,0)}}function Lt(e){const a=Array.isArray(e)?e:[],t=[];return a.forEach(n=>{const r=String(n||"").trim().toLowerCase();(r===xe||r===Ot)&&!t.includes(r)&&t.push(r)}),t.length?t:[xe]}function X(e={},a=""){const t=e&&typeof e=="object"?e:{},n=Re(t.partyRanges||t.partySizes),r=It(n),s=Et(t.benefit),u=Fe(t.bookingType);return{id:f(t.id||a,180),restaurantId:f(t.restaurantId,180),locationId:f(t.locationId,180)||"main",title:f(t.title,120),description:f(t.description||t.text,400),terms:f(t.terms||t.conditions,400),benefit:s,benefitLabel:s.label,category:Tt(t.category),partyRanges:n,minParty:r.min,maxParty:r.max,schedule:Gt(t.schedule),dateRange:Kt(t.dateRange),bookingType:u,limits:Mt(t.limits),channels:Lt(t.channels),status:Ct(t.status),sponsored:t.sponsored===!0||t.sponsored?.active===!0,sponsoredUntil:R(t.sponsored?.until),priceLevel:Math.min(4,Math.max(0,N(t.priceLevel,0))),redeemedCount:N(t.redeemedCount,0),createdAt:R(t.createdAt),updatedAt:R(t.updatedAt)}}function Ka(e={},{serverTimestamp:a=null}={}){const t=X(e),n={restaurantId:t.restaurantId,locationId:t.locationId,title:t.title,description:t.description,terms:t.terms,benefit:t.benefit,benefitLabel:t.benefitLabel,category:t.category,partyRanges:t.partyRanges,minParty:t.minParty,maxParty:t.maxParty,schedule:t.schedule,dateRange:t.dateRange,bookingType:t.bookingType,limits:t.limits,channels:t.channels,status:t.status,sponsored:t.sponsored,priceLevel:t.priceLevel};return a&&(n.updatedAt=a,t.createdAt||(n.createdAt=a)),n}function Ma(e={}){const a=X(e),t=[];return a.restaurantId||t.push({field:"restaurantId",message:"Lokali mungon."}),a.benefitLabel||t.push({field:"benefit",message:"Shkruaj çka po ofron."}),a.partyRanges.length||t.push({field:"partyRanges",message:"Zgjidh sa persona."}),a.schedule.mode==="windows"&&(a.schedule.days.length||t.push({field:"schedule",message:"Zgjidh ditët."}),a.schedule.windows.length||t.push({field:"schedule",message:"Zgjidh orarin."})),a.benefit.kind==="percent"&&a.benefit.percent<=0&&t.push({field:"benefit",message:"Zbritja duhet të jetë mbi 0 %."}),{ok:t.length===0,errors:t,offer:a}}function Oe(e={}){const t=(e&&e.schedule?e:X(e)).schedule;if(t.mode==="always")return"Gjithmonë";const n={mon:"Hën",tue:"Mar",wed:"Mër",thu:"Enj",fri:"Pre",sat:"Sht",sun:"Die"},r=t.days.map(u=>n[u]||u).join(", "),s=t.windows.map(u=>`${be(u.start)}-${be(u.end)}`).join(", ");return[r,s].filter(Boolean).join(" · ")}function Ce(e={}){const a=e&&e.partyRanges?e:X(e);return`${a.maxParty>=St?`${a.minParty}+`:`${a.minParty}–${a.maxParty}`} persona`}const _=Object.freeze({confirmed:"confirmed",checkedIn:"checked_in",completed:"completed",cancelledByUser:"cancelled_by_user",cancelledByBusiness:"cancelled_by_business",notArrived:"not_arrived",expired:"expired"});Object.freeze([_.confirmed,_.checkedIn]);Object.freeze({[_.confirmed]:[_.checkedIn,_.completed,_.cancelledByUser,_.cancelledByBusiness,_.notArrived,_.expired],[_.checkedIn]:[_.completed,_.cancelledByBusiness],[_.completed]:[],[_.cancelledByUser]:[],[_.cancelledByBusiness]:[],[_.notArrived]:[],[_.expired]:[]});function Te(e=""){const a=String(e||"").trim().toLowerCase();return Object.values(_).includes(a)?a:_.confirmed}function La(e={},a=""){const t=e&&typeof e=="object"?e:{},n=t.snapshot&&typeof t.snapshot=="object"?t.snapshot:{};return{id:f(t.id||a,180),restaurantId:f(t.restaurantId||n.restaurantId,180),locationId:f(t.locationId||n.locationId,180)||"main",offerId:f(t.offerId||n.offerId,180),guestId:f(t.guestId,180),uid:f(t.uid,180),shortCode:f(t.shortCode,12).toUpperCase(),type:Fe(t.type||n.bookingType),status:Te(t.status),partySize:Math.max(1,Math.trunc(Number(t.partySize||n.partySize)||1)),expectedArrivalAt:R(t.expectedArrivalAt||n.expectedArrivalAt),expectedArrivalMs:Be(t.expectedArrivalAt||n.expectedArrivalAt),dayKey:f(t.dayKey,10),slotKey:f(t.slotKey,240),timeZone:f(t.timeZone,60)||Pt,snapshot:n,businessName:f(n.businessName,120),benefitLabel:f(n.benefitLabel,160),logoUrl:f(n.logoUrl,500),businessSeenAt:R(t.businessSeenAt),checkedInAt:R(t.checkedInAt),completedAt:R(t.completedAt),cancelledAt:R(t.cancelledAt),cancelReason:f(t.cancelReason,200),createdAt:R(t.createdAt),updatedAt:R(t.updatedAt)}}function Ut(e={}){const a=Te(e?.status);return a===_.confirmed?"Po vijnë":a===_.checkedIn?"Këtu":a===_.completed?"Përfunduar":a===_.cancelledByUser?"Anuluar nga klienti":a===_.cancelledByBusiness?"Anuluar nga ju":a===_.notArrived?"Nuk erdhën":"Skaduar"}const l=Object.freeze({brand:"Mnyra GO",mark:"⚡",editor:"Editor",emptyTitle:"Merr klientë kur ata janë gati të dalin.",emptyAction:"Aktivizo ofertën e parë",cardIdle:"Krijo oferta për klientët që kërkojnë tani.",cardManage:"Menaxho GO",tabs:{active:"Aktiv",offers:"Ofertat",history:"Historiku",options:"Opsionet"},statNew:"Të reja",statActive:"Aktive",statToday:"Sot",guests:"Mysafirë",goOn:"GO Aktiv",pause:"Pauzo GO",resume:"Aktivizo GO",pausedUntil:"Pauzuar deri",createOffer:"Ofertë e re GO",editOffer:"Ndrysho ofertën",preview:"Kështu e sheh klienti",activate:"Aktivizo",save:"Ruaj ofertën",saving:"Po ruhet...",cancel:"Anulo",edit:"Edit",offering:"po ju ofron",forGroup:"për grupin tuaj",accept:"Prano ofertën",benefitQuestion:"Çka po ofron?",benefitCustom:"Teksti yt (opsionale)",partyQuestion:"Për sa persona?",categoryQuestion:"Për kë âsht kjo ofertë?",categoryHint:"Gastet zgjedhin mes «Ushqim» edhe «Pije».",scheduleQuestion:"Kur vlen?",always:"Gjithmonë",specificHours:"Orar specifik",dateFrom:"Prej datës (opsionale)",dateTo:"Deri me datën (opsionale)",actionQuestion:"Kur klienti e zgjedh",onlyOffer:"Vetëm oferta",offerAndTable:"Oferta + tavolinë",limitsTitle:"Kufijtë",slotGroups:"Grupe për 30 min",slotGuests:"Mysafirë për 30 min",dailyGroups:"Grupe në ditë",totalRedemptions:"Sa herë gjithsej",noLimit:"0 = pa kufi",paused:"Pauzuar",archived:"Arkivuar",archive:"Arkivo",noBookings:"Ende asnjë klient sot.",noHistory:"Ende asnjë histori.",loading:"Po ngarkohet...",guestName:"Mnyra Guest",table:"Tavolinë",markArrived:"Erdhën",markNotArrived:"Nuk erdhën",markDone:"Përfundo",keepsRunning:"Rezervimet ekzistuese mbeten. Vetëm të rejat ndalen.",onlyBusiness:"Ky funksion eshte vetem per profile biznesi.",loadingBusiness:"Biznesi po ngarkohet..."});function o(e,a=""){return typeof e=="function"?e(a):String(a??"")}function M(e,a="",t="w-4 h-4"){return typeof e=="function"?e(a,t):""}function Ee(e=""){const a=Date.parse(String(e||""));if(!Number.isFinite(a))return"";const t=new Date(a);return`${String(t.getHours()).padStart(2,"0")}:${String(t.getMinutes()).padStart(2,"0")}`}function Ht({enabled:e=!1,unseenCount:a=0,activeOffers:t=0,todayBookings:n=0,iconFn:r=null,texts:s={}}={}){if(!e)return"";const u={...l,...s||{}},m=Math.max(0,Math.trunc(Number(a)||0)),k=t>0||n>0,g=k?`${t} oferta aktive · ${n} rezervime sot`:u.cardIdle;return`
  ...
  `}function Zt({summary:e={},deps:a={}}={}){const t=a.escapeHtml,n=a.icon;return`
  ...
  `}function Vt({tab:e="active",deps:a={}}={}){const t=a.escapeHtml;return`
  ...
  `}function ve(e={},a={}){const t=a.escapeHtml,n=e.type==="reservation",r=Ee(e.expectedArrivalAt),s=e.benefitLabel||e.snapshot?.benefitLabel||"",u=`${l.guestName} · ${e.shortCode||""}`.trim();return`
  ...
  `}function Wt(e={},a={}){const t=a.escapeHtml,n=e.status==="paused"?l.paused:e.status==="archived"?l.archived:"";return`
  ...
  `}function qt({offer:e={},businessName:a="",deps:t={}}={}){const n=t.escapeHtml;return`
    <div class="rounded-[1.8rem] border border-slate-200 bg-white p-5" data-go-offer-preview>
      ...
      <p class="mt-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
        ${o(n,Ce(e))} &middot; ${o(n,Oe(e))}
      </p>
      <span class="mt-4 inline-flex rounded-2xl bg-slate-900 px-5 py-3 text-[10px] font-black uppercase tracking-widest text-white">
        ${o(n,l.accept)}
      </span>
    </div>
  `}function T(e,a="",t=""){return`<label class="text-[10px] font-black uppercase tracking-widest text-slate-400"${t?` for="${o(e,t)}"`:""}>${o(e,a)}</label>`}function E(e,{active:a=!1,attr:t="",value:n="",escapeHtml:r=null}={}){return`
  ...
  `}function Ua({editor:e=null,businessName:a="",deps:t={}}={}){if(!e)return"";const n=t.escapeHtml,r=t.icon,s=e.draft||{},u=Array.isArray(e.errors)?e.errors:[],m=b=>u.find(B=>B.field===b)?.message||"",k=Array.isArray(s.partyRanges)?s.partyRanges:[],g=Array.isArray(s.schedule?.days)?s.schedule.days:[],h=s.schedule?.mode==="windows"?"windows":"always",v={mon:"Hën",tue:"Mar",wed:"Mër",thu:"Enj",fri:"Pre",sat:"Sht",sun:"Die"},w=e.mode==="edit",S="mt-2 w-full bg-slate-50 border border-slate-100 rounded-2xl px-4 py-3.5 text-sm font-bold text-slate-900 outline-none focus:border-indigo-400";return`
    <div class="p-6 app-main-content-safe animate-in slide-in-from-right-10 duration-500" data-go-offer-editor>
      ...
      <div class="bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-sm space-y-5">
        <div>
          ${T(n,l.benefitQuestion)}
          <div class="mt-2 flex flex-wrap gap-2">
            ${At.map(b=>E(b.label,{active:(s.benefit?.kind||"percent")===b.key,attr:"data-go-benefit-kind",value:b.key,escapeHtml:n})).join("")}
          </div>
        ...
        <div>
          ${T(n,l.partyQuestion)}
          <div class="mt-2 flex flex-wrap gap-2">
            ${ne.map(b=>E(b.label,{active:k.includes(b.key),attr:"data-go-offer-party",value:b.key,escapeHtml:n})).join("")}
          </div>
        </div>

        <div>
          ${T(n,l.categoryQuestion)}
          <p class="mt-1 text-[11px] font-semibold text-slate-400">${n(l.categoryHint)}</p>
          <div class="mt-2 flex flex-wrap gap-2">
            ${Pe.map(b=>E(b.label,{active:(s.category||"all")===b.key,attr:"data-go-offer-category",value:b.key,escapeHtml:n})).join("")}
          </div>
        </div>

        <div>
          ${T(n,l.scheduleQuestion)}
          <div class="mt-2 flex flex-wrap gap-2">
            ${E(l.always,{active:h==="always",attr:"data-go-offer-schedule",value:"always",escapeHtml:n})}
            ${E(l.specificHours,{active:h==="windows",attr:"data-go-offer-schedule",value:"windows",escapeHtml:n})}
          </div>
          ${h==="windows"?`
            <div class="mt-3 flex flex-wrap gap-2">
              ${Q.map(b=>E(v[b],{active:g.includes(b),attr:"data-go-offer-day",value:b,escapeHtml:n})).join("")}
            </div>
        ...
        <div class="grid grid-cols-1 gap-4">
          <div>
            ${T(n,l.dateFrom,"goOfferStart")}
            <input id="goOfferStart" type="date" data-go-offer-start value="${o(n,s.dateRange?.startDate||"")}" class="${S}" />
          </div>
          <div>
            ${T(n,l.dateTo,"goOfferEnd")}
            <input id="goOfferEnd" type="date" data-go-offer-end value="${o(n,s.dateRange?.endDate||"")}" class="${S}" />
          </div>
        </div>

        <div>
          ${T(n,l.actionQuestion)}
          <div class="mt-2 flex flex-wrap gap-2">
            ${E(l.onlyOffer,{active:s.bookingType!=="reservation",attr:"data-go-offer-type",value:"claim",escapeHtml:n})}
            ${E(l.offerAndTable,{active:s.bookingType==="reservation",attr:"data-go-offer-type",value:"reservation",escapeHtml:n})}
          </div>
        </div>
        ...
        ${qt({offer:s,businessName:a,deps:t})}
  ...
  `}function Ha({restaurantName:e="",tab:a="active",summary:t={},bookings:n=[],offers:r=[],settings:s={},paused:u=!1,loading:m=!1,error:k="",deps:g={}}={}){const h=g.escapeHtml,v=g.icon,w=n.filter(P=>["confirmed","checked_in"].includes(P.status)),S=n.filter(P=>!["confirmed","checked_in"].includes(P.status)),b=r.filter(P=>P.status!=="archived");let B="";if(a==="offers")B=Y({eyebrow:l.brand,title:l.tabs.offers,sub:`${b.length} ${b.length===1?"oferte":"oferta"}`,action:`
      ...
      `,body:b.length?`<div class="space-y-3">${b.map(P=>Wt(P,g)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(h,l.emptyTitle)}</div>`,deps:g});else if(a==="history")B=Y({eyebrow:l.brand,title:l.tabs.history,sub:`${S.length}`,body:S.length?`<div class="space-y-3">${S.map(P=>ve(P,g)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(h,l.noHistory)}</div>`,deps:g});else if(a==="options"){const P=Ee(s?.pausedUntil);B=Y({eyebrow:l.brand,title:l.tabs.options,body:`
      ...
      `,deps:g})}else B=Y({eyebrow:l.brand,title:l.tabs.active,sub:`${w.length}`,body:m?`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-400">${o(h,l.loading)}</div>`:w.length?`<div class="space-y-3">${w.map(P=>ve(P,g)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(h,l.noBookings)}</div>`,deps:g});return`
  ...
  `}function Za({deps:e={},resolving:a=!1}={}){const t=e.icon,n=e.escapeHtml;return a?`
  ...
  `}const we="mnyraDashboardStyles",Yt=`
.mnyra-dash {
...
`;function Qt(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(we)))try{const a=e.createElement("style");a.id=we,a.textContent=Yt,e.head?.appendChild(a)}catch{}}function x(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function A(e,a,t=""){if(typeof e!="function")return"";try{return e(a,t)||""}catch{return""}}const Jt=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function Xt({businessType:e="",isShopCatalog:a=!1}={}){if(a)return"shop";const t=String(e||"").trim().toLowerCase();return Jt.includes(t)?"hotel":"restaurant"}function ea(e=new Date().getHours()){const a=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return a>=5&&a<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:a>=11&&a<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:a>=18&&a<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function ta({name:e="",logoUrl:a="",hour:t=new Date().getHours(),iconFn:n}={}){const r=ea(t),s=x(e||"Business");return`
  ...
  `:""}function ra({iconFn:e,showEditor:a=!0}={}){return a?`
  ...
  `}const oa="/waiter?from=panel";function da({iconFn:e,showEditor:a=!0}={}){return a?`
    <a href="${oa}" class="mnyra-dash__composer mnyra-dash__composer--tap mnyra-dash__composer--waiter" data-dashboard-waiter-card>
  ...
  `:""}function la({cards:e=[],iconFn:a}={}){const t=(Array.isArray(e)?e:[]).filter(r=>r&&r.key);if(!t.length)return"";const n=t.map((r,s)=>{const u=x(r.label||"");if(r.pending)return'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>';const m=s<2?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"';let k="";r.imageUrl?k=`<img class="mnyra-dash__hl-media" src="${x(r.imageUrl)}" alt="" ${m} decoding="async" onerror="this.style.display='none'" />`:r.videoUrl&&(k=`<video class="mnyra-dash__hl-media" src="${x(r.videoUrl)}#t=0.1" preload="metadata" muted playsinline disablepictureinpicture tabindex="-1" aria-hidden="true"></video>`);const g=`
    ...
    `}).join("");return`
    <div class="mnyra-dash__hl" data-dashboard-metrics="${x(ca(t))}">
      ${n}
      <span class="mnyra-dash__hl-tail" aria-hidden="true"></span>
    </div>
  `}function ca(e=[]){return(Array.isArray(e)?e:[]).filter(a=>a&&a.key).map(a=>[a.key,a.label||"",a.value||"",a.emptyText||"",a.imageUrl||"",a.videoUrl||"",a.iconName||"",a.panelTab||"",a.composer||"",a.pending?"p":"",a.loading?"l":"",a.locked?"x":"",a.withEye?"e":""].join("~")).join("|")}const Ne=Object.freeze([Object.freeze({id:"funksionet",label:"Funksionet",iconName:"layout-grid"}),Object.freeze({id:"analitika",label:"Analitika",iconName:"bar-chart-3"}),Object.freeze({id:"opsionet",label:"Opsionet",iconName:"settings"})]);function J(e=""){const a=String(e||"").trim().toLowerCase();return Ne.some(t=>t.id===a)?a:"funksionet"}function ua({activeTab:e="funksionet",iconFn:a}={}){const t=J(e);return`<div class="mnyra-dash__tabs" role="tablist" data-dashboard-panel-tabs>${Ne.map(r=>{const s=r.id===t;return`
      <button
        type="button"
        role="tab"
  ...
  `}function ha({posts:e=[],iconFn:a}={}){const t=Array.isArray(e)?e:[];let n="";return t.length?(n=t.map(r=>{const s=[r.dateLabel,`${K(r.likesCount||0)} Likes`,`${K(r.commentsCount||0)} Kommentare`];return Number(r.impressions||0)>0&&s.push(`${K(r.impressions)} shtrirje (7 dite)`),`
  ...
  `}const _a="menyra_social_dashboard_cache_v1::",$e="menyra_social_composer_products_v1::",Se=2500,ze=1200,xa=6,va=3,wa=Object.freeze({menuImageUrl:"/apps/menyra-social/assets/panel/menu-scan.jpg",qrImageUrl:"/apps/menyra-social/assets/panel/qr-stand.jpg"}),ka=Object.freeze({menuOpens:"Menü-Aufrufe",qrScans:"QR-Scans"});function j(e){const a=Number(e);return Number.isFinite(a)?a:0}function $a(e={}){const a=String(e.createdAtClient||"").trim();if(a){const n=new Date(a);if(!Number.isNaN(n.getTime()))return n}const t=e.createdAt;if(t&&typeof t.toDate=="function")try{const n=t.toDate();if(n instanceof Date&&!Number.isNaN(n.getTime()))return n}catch{}return null}function Sa(e="",a={}){const t=Array.isArray(a.media)&&a.media.length?a.media[0]:{},n=String(t.type||a.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",r=String(t.thumbUrl||(n==="image"?t.url:"")||a.thumbUrl||"").trim(),s=n==="video"?String(t.url||a.mediaUrl||"").trim():"",u=$a(a);return{id:String(e||"").trim(),caption:String(a.caption||"").trim(),mediaType:n,thumbUrl:r,videoUrl:s,likesCount:j(a.likesCount),commentsCount:j(a.commentsCount),impressions:0,dateLabel:u?u.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:u?u.getTime():0}}function za({days:e=[],todayKey:a="",rawPosts:t=[]}={}){const n=Array.isArray(e)?e:[],r=me(n),s=n.find(h=>String(h?.date||h?.id||"").trim()===String(a||"").trim()),u=me(s?[s]:[]),m=r.merged?.posts&&typeof r.merged.posts=="object"?r.merged.posts:{},k=(Array.isArray(t)?t:[]).map(h=>Sa(h?.id,h?.data||{})).filter(h=>h.id).map(h=>({...h,impressions:j(m[h.id]?.impressions)})),g=k.slice().sort((h,v)=>v.createdAtMs-h.createdAtMs).slice(0,va);return{day:String(a||"").trim(),week:r.summary,today:u.summary,posts:g,latestPost:Aa(k)}}function Aa(e=[]){const a=(Array.isArray(e)?e:[]).filter(t=>t&&t.id);return a.length?a.slice().sort((t,n)=>j(n.createdAtMs)-j(t.createdAtMs)||j(n.impressions)-j(t.impressions)||j(n.likesCount)-j(t.likesCount))[0]:null}function Pa({profile:e={},restaurant:a={}}={}){return lt({profile:e,restaurant:a,feature:"qr"})}function Da(e={}){const a=e&&typeof e=="object"?e:{};return String(a.titleImageUrl||a.coverImageUrl||a.coverUrl||a.heroUrl||a.bannerUrl||"").trim()}function ja({model:e=null,coverUrl:a="",subscribed:t=!1,assets:n={}}={}){const r=e?.today||{},s=!e,u=e?.latestPost||null,m=[];if(s)m.push({key:"latestPost",label:"Postimi fundit",pending:!0});else if(!u)m.push({key:"latestPost",label:"Postimi fundit",emptyText:"S'ka postim",iconName:"image",composer:"post"});else{const k=String(u.thumbUrl||"").trim();m.push({key:"latestPost",label:"Postimi fundit",value:K(j(u.impressions)),withEye:!0,imageUrl:k,videoUrl:k?"":String(u.videoUrl||"").trim(),iconName:"image",panelTab:"analitika"})}return m.push({key:"profileViews",label:"Vizitor n'profil",value:K(j(r.profileViews)),withEye:!0,loading:s,imageUrl:String(a||"").trim(),iconName:"user",panelTab:"analitika"}),m.push({key:"menuOpens",label:"Vizitor n'meny",value:K(j(r.menuOpens)),withEye:!0,loading:s&&t,locked:!t,imageUrl:String(n.menuImageUrl||"").trim(),iconName:"book-open",panelTab:"analitika"}),m.push({key:"qrScans",label:"Skanime n'tavolina",value:K(j(r.qrScans)),withEye:!0,loading:s&&t,locked:!t,imageUrl:String(n.qrImageUrl||"").trim(),iconName:"layout-grid",panelTab:"analitika"}),m}function Va({state:e,renderFn:a,documentObj:t,firestoreApi:n={},profileApi:r={},composerApi:s={},viewApi:u={},iconFn:m,storageObj:k}={}){const g=t||(typeof document>"u"?null:document),h=g?.defaultView||(typeof window>"u"?null:window),v=typeof a=="function"?a:()=>{},w=k||(typeof localStorage>"u"?null:localStorage),S=typeof r.getBusinessProfileTypeFn=="function"?r.getBusinessProfileTypeFn:(()=>""),b=typeof r.isShopCatalogProfileFn=="function"?r.isShopCatalogProfileFn:(()=>!1),B=typeof r.getRestaurantMetaByIdFn=="function"?r.getRestaurantMetaByIdFn:(()=>null),P=typeof r.resolveRestaurantLogoFn=="function"?r.resolveRestaurantLogoFn:(()=>""),U=typeof r.resolveOwnAvatarUrlFn=="function"?r.resolveOwnAvatarUrlFn:(()=>""),Ge=typeof u.renderAnalyticsViewFn=="function"?u.renderAnalyticsViewFn:(()=>""),Ke=typeof u.renderSettingsViewFn=="function"?u.renderSettingsViewFn:(()=>""),Me=typeof u.warmAnalyticsFn=="function"?u.warmAnalyticsFn:(()=>{});let re=!1,L=0,se=!1,I=null,H=null,Z="",ie=!1,oe=()=>null;const Le=300;function ee(){const i=e?.userProfile||{};return Xt({businessType:S(i),isShopCatalog:b(i)})}function Ue(i=""){const d=B(i)||{};return _t(d).map(c=>({id:c.id,name:c.title,price:c.price??"",category:c.beds||c.tag||"",type:"room",imageUrl:c.imageUrl||""}))}function He(i=""){if(!w)return null;try{const d=w.getItem(`${$e}${i}`);if(!d)return null;const c=JSON.parse(d),p=Array.isArray(c?.items)?c.items:null;return p&&p.length?p:null}catch{return null}}function Ze(i="",d=[]){if(w)try{w.setItem(`${$e}${i}`,JSON.stringify({savedAt:Date.now(),items:d}))}catch{}}async function Ve(i=""){const{db:d,collectionFn:c,queryFn:p,limitFn:y,getDocsFn:$}=n;if(!d||typeof c!="function"||typeof $!="function")throw new Error("Produktet nuk u ngarkuan.");const O=c(d,"restaurants",i,"menuItems"),D=typeof p=="function"&&typeof y=="function"?p(O,y(Le)):O,C=await $(D),z=[];return C.forEach(G=>{const q=oe(G?.id,G?.data?.()||{});q&&z.push(q)}),z.sort((G,q)=>G.name.localeCompare(q.name,"sq")),z}async function We(i="",d){const c=String(i||"").trim();if(!c)throw new Error("Produktet nuk u ngarkuan.");if(ee()==="hotel")return Ue(c);const p=Ve(c).then($=>(Ze(c,$),$)),y=He(c);return y?(typeof d=="function"?p.then($=>d($)).catch(()=>{}):p.catch(()=>{}),y):p}function de(){return I?Promise.resolve(I):(H||(H=it(()=>import("./business-composer-controller-Dh7_Ikb_.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(i=>(oe=typeof i?.normalizeComposerProductCore=="function"?i.normalizeComposerProductCore:(()=>null),I=i.createBusinessComposerController({documentObj:g,windowObj:g?.defaultView||null,api:{getRestaurantIdFn:()=>V(),getBusinessMetaFn:()=>{const d=V();if(!d)return{name:"",logoUrl:"",city:""};const c=pe(d),p=B(d)||{};return{name:c.name,logoUrl:c.logoUrl,city:String(p.city||"").trim()}},loadProductsFn:(d,c)=>We(d,c),getBusinessKindFn:()=>ee(),uploadImageFn:s.uploadImageFn,uploadVideoFn:s.uploadVideoFn,captureVideoPosterFn:s.captureVideoPosterFn,createPostFn:s.createPostFn,createStoryFn:s.createStoryFn,formatPriceFn:s.formatPriceFn,getOptimizedImageUrlFn:s.getOptimizedImageUrlFn,escapeHtmlFn:s.escapeHtmlFn,iconFn:typeof m=="function"?m:void 0,afterPublishFn:async d=>{try{await W({force:!0})}catch{}typeof s.afterPublishFn=="function"&&await s.afterPublishFn(d)}}}),I)).catch(i=>{throw H=null,console.error("[mnyra][dashboard] composer load failed",i),i})),H)}function le(){const i=h?.navigator?.connection;return!i||typeof i!="object"?!1:i.saveData===!0?!0:/(^|-)2g$/.test(String(i.effectiveType||"").trim().toLowerCase())}function qe(){if(ie||I||!h||le())return;ie=!0;const i=()=>{if(de().catch(()=>{}),typeof s.prewarmFn=="function")try{s.prewarmFn()}catch{}};if(typeof h.requestIdleCallback=="function"){h.requestIdleCallback(i,{timeout:Se});return}h.setTimeout?.(i,ze)}function Ye(){if(re||!h||le())return;re=!0;const i=()=>{try{Me()}catch{}};if(typeof h.requestIdleCallback=="function"){h.requestIdleCallback(i,{timeout:Se});return}h.setTimeout?.(i,ze)}function Qe(i="post"){const d=String(i||"").trim().toLowerCase(),c=d==="story"||d==="profile"?d:"post";if(typeof s.prewarmFn=="function")try{s.prewarmFn()}catch{}if(I){I.open(c);return}Z=c,de().then(p=>{const y=Z||c;Z="",p?.open?.(y)}).catch(()=>{Z=""})}function te(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:"",restaurantId:"",paywall:""}),e.dashboardView}function ce(i=""){const d=te(),c=String(i||"").trim();return String(d.restaurantId||"")===c||(d.restaurantId=c,d.model=null,d.status="idle",d.error="",d.loadedSignature="",d.paywall="",L+=1),d}function V(){const i=e?.userProfile||{};return String(i.restaurantId||i.staffRestaurantId||"").trim()}let ue="";function Je(){const i=String(e?.user?.uid||"").trim();!i||ue===i||typeof r.ensureBusinessProfileFn=="function"&&(ue=i,Promise.resolve().then(()=>r.ensureBusinessProfileFn()).catch(d=>{console.warn("[mnyra][panel] business profile could not be resolved",d)}).finally(()=>{String(e?.user?.uid||"").trim()===i&&v()}))}function Xe(){const i=String(e?.user?.uid||"").trim();if(!i)return!1;const d=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||d===i}function he(i=""){return`${_a}${i}`}function et(i="",d=""){if(!w||!i)return null;try{const c=w.getItem(he(i));if(!c)return null;const p=JSON.parse(c);return!p||typeof p!="object"||String(p.day||"").trim()!==String(d||"").trim()||!p.model||typeof p.model!="object"?null:p.model}catch{return null}}function tt(i="",d=null){if(!(!w||!i||!d))try{w.setItem(he(i),JSON.stringify({day:d.day,model:d}))}catch{}}async function at(i=""){const{db:d,collectionFn:c,queryFn:p,orderByFn:y,limitFn:$,getDocsFn:O}=n;if(!d||typeof c!="function"||typeof p!="function"||typeof y!="function"||typeof $!="function"||typeof O!="function")return[];const D=c(d,"restaurants",i,"socialPosts");return(await O(p(D,y("createdAt","desc"),$(xa)))).docs.map(z=>({id:z.id,data:z.data()||{}})).filter(z=>{const G=String(z.data.status||"active").trim().toLowerCase();return G!=="deleted"&&G!=="hidden"})}async function W({force:i=!1}={}){const d=V(),c=ce(d);if(!d)return;const p=ot({rangeKey:"7d"});if(!p)return;const y=`${d}::${p.toDay}`;if(!i&&c.loadedSignature===y&&c.status==="ready")return;if(!c.model){const D=et(d,p.toDay);D&&(c.model=D,c.status="ready",v())}L+=1;const $=L;c.model||(c.status="loading",c.error="",v());try{const D={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:d},[C,z]=await Promise.allSettled([dt({...D,fromDay:p.fromDay,toDay:p.toDay}),at(d)]);if($!==L)return;if(C.status==="rejected")throw C.reason;z.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",z.reason),c.model=za({days:C.value,todayKey:p.toDay,rawPosts:z.status==="fulfilled"?z.value:[]}),c.status="ready",c.error="",c.loadedSignature=y,tt(d,c.model)}catch(D){if($!==L)return;console.error("[mnyra][dashboard] load failed",D),c.model||(c.status="error",c.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}v()}function nt(){se||!g||(se=!0,g.addEventListener("click",i=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(i.target?.closest?.("[data-dashboard-retry]")){W({force:!0});return}if(i.target?.closest?.("[data-dashboard-paywall-close]")){i.preventDefault(),te().paywall="",v();return}const d=i.target?.closest?.("[data-dashboard-metric-locked]");if(d){i.preventDefault(),te().paywall=String(d.getAttribute("data-dashboard-metric-locked")||"").trim(),v();return}const c=i.target?.closest?.("[data-dashboard-composer]");if(c){i.preventDefault(),Qe(c.getAttribute("data-dashboard-composer"));return}const p=i.target?.closest?.("[data-dashboard-panel-tab]");if(p){i.preventDefault();const y=J(p.getAttribute("data-dashboard-panel-tab"));if(y===J(e?.dashboardPanelTab))return;e.dashboardPanelTab=y,v()}}catch{}}))}function pe(i=""){const d=e?.userProfile||{},c=i?B(i)||{}:{},p=String(c.name||c.restaurantName||d.name||"").trim()||"Business";let y="";try{y=String(U()||"").trim()}catch{}if(!y)try{y=String(P(c)||"").trim()}catch{}return{name:p,logoUrl:y,kind:ee(),coverUrl:Da(c),subscribed:Pa({profile:d,restaurant:c})}}function rt(i=""){try{if(!ct()||!i)return"";ut({restaurantId:i,onBadgeFn:()=>v()});const d=ht();return Ht({enabled:!0,unseenCount:d.unseen,activeOffers:d.activeOffers||0,todayBookings:d.today,iconFn:m})}catch{return""}}function st(){Qt(g),nt();const i=V(),d=ce(i);let c="";if(!i)Je(),c=Xe()?ba():ya();else{qe(),Ye();const p=pe(i),y=J(e?.dashboardPanelTab);d.status==="idle"&&(d.status="loading",queueMicrotask(()=>{W({force:!1})}));let $="";d.model?$=ha({posts:d.model.posts,iconFn:m}):d.status==="error"?$=ga({message:d.error}):$=pa();const O=`
        ${rt(i)}
        ${aa({iconFn:m})}
        ${da({iconFn:m,showEditor:!!i})}
        ${na({iconFn:m,showEditor:!!i})}
        ${ra({iconFn:m,showEditor:!!i})}
        ${ia({iconFn:m,kind:p.kind,showEditor:!!i})}
      `;let D;y==="analitika"?D=`
          <div class="mnyra-dash__embed">${Ge()}</div>
          ${$}
        `:y==="opsionet"?D=`<div class="mnyra-dash__embed">${Ke()}</div>`:D=O;const C=ja({model:d.model,coverUrl:p.coverUrl,subscribed:p.subscribed,assets:wa}),z=String(d.paywall||"").trim();c=`
        ${ta({name:p.name,logoUrl:p.logoUrl,iconFn:m})}
        ${la({cards:C,iconFn:m})}
        ${Ie(`
          ${ua({activeTab:y,iconFn:m})}
          ${D}
        `)}
        ${z?ma({title:ka[z]||"Me pagesë"}):""}
      `}return`
      <section class="mnyra-dash" data-dashboard-root>
        ${c}
      </section>
    `}return Object.freeze({renderDashboardView:st,loadDashboard:W})}export{Na as G,mt as M,Va as a,X as b,ft as c,Ua as d,Ha as e,be as f,La as g,St as h,Ea as i,$t as j,Ga as k,Ia as l,_t as m,yt as n,Ca as o,Ta as p,Za as r,Ka as t,Ma as v};
