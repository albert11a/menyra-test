import{i as zt,a as Tt,t as Dt}from"../entry/social-app.js";import"./startup-route-runtime-context-6Co7bthZ.js";import"./vendor-firebase-DbBtOzVQ.js";const Ft=Object.freeze(["Fruehstueck","Brunch","Vorspeise","Suppe","Salat","Pasta","Pizza","Burger","Sandwich","Wrap","Grill","Fleisch","Fisch","Vegetarisch","Vegan","Beilage","Kinder","Dessert","Kuchen","Eis","Kaffee","Tee","Softdrink","Saft","Smoothie","Bier","Wein","Cocktail","Spirituose","Sonstiges"]);function dt(s){if(s==null)return null;const g=typeof s=="string"?s.trim():s;if(g==="")return null;const I=Number(g);return Number.isFinite(I)?Math.max(0,I):null}function ct(s=""){const g=String(s||"").trim().toLowerCase();return["speisen","food","getraenke","getränke","drink","drinks","beverage","beverages"].includes(g)}function ut(s=""){const g=String(s||"").trim();return g?ct(g)?"Produkte":g:""}function Ut({state:s,isShopCatalogProfile:g,getBusinessProfileType:I,getOptimizedImageUrl:q,PLACEHOLDER_IMAGE:Q,isPlaceholderUrl:Y,normalizeMenuType:X,getMenuModalCrop:J,escapeHtml:ee,icon:te}={}){if(!s||!s.menuModal?.open)return"";const me=typeof g=="function"?g:(()=>!1),fe=typeof I=="function"?I:(()=>""),ne=typeof q=="function"?q:(e=>String(e||"")),be=typeof Y=="function"?Y:(()=>!1),ge=typeof X=="function"?X:(e=>String(e||"food")),xe=typeof J=="function"?J:(()=>({x:50,y:50})),r=typeof ee=="function"?ee:(e=>String(e||"")),P=typeof te=="function"?te:(()=>""),he=e=>{const a=Number(e);return Number.isFinite(a)?`${a.toFixed(2).replace(".",",")} EUR`:String(e||"").trim()||"0,00 EUR"},ve=(e,{excludeId:a=""}={})=>{const d=String(a||"").trim(),f=new Set,j=x=>{if(x==null)return;if(Array.isArray(x)){x.forEach(j);return}const ue=typeof x=="object"?x.id||x.itemId||x.productId||x.menuItemId||"":x,R=String(ue||"").trim();R&&R.split(",").forEach(pe=>{const B=String(pe||"").trim();!B||B===d||f.has(B)||f.add(B)})};return j(e),Array.from(f)},o=s.menuModal.item||{},se=s.menuModal.mode==="edit",p=me(s.userProfile),we=se?"Produkt bearbeiten":"Produkt hinzufuegen",ye=Array.isArray(s.menuModal.existingImages)?s.menuModal.existingImages:[],oe=Array.isArray(s.menuModal.imagePreviews)?s.menuModal.imagePreviews:[],ke=String(s.menuModal.imageUrlDraft||"").trim(),z=[...ye.map((e,a)=>({src:e,kind:"existing",idx:a})),...oe.map((e,a)=>({src:e,kind:"new",idx:a}))].filter(e=>e.src),le=z[0]?.src||ke||o.imageUrl||"",ae=le?ne(le,"large"):Q,$e=be(ae)?Q:ae,T=ge(o.type||"food"),Le=T==="food"||T==="drink",ie=o.available!==!1?"available":"unavailable",Be=s.menuModal.status||"",Ve=Array.isArray(o.sizes)?o.sizes.join(", "):"",Ne=Array.isArray(o.colors)?o.colors.join(", "):"",Se=dt(o.stock),Ke=Se===null?"":String(Se),k=xe(),We=String(fe(s.userProfile)||"").trim().toLowerCase(),K=!p&&zt(We),C=(()=>{const e=s?.userProfile?.specialEnabled;if(typeof e=="boolean")return e;const a=String(s?.userProfile?.restaurantId||"").trim();return a?(Array.isArray(s?.restaurants)?s.restaurants.find(f=>String(f?.id||"").trim()===a):null)?.specialEnabled===!0:!1})(),D=Tt(o.cardStyle||"",T),i=D==="testfirst_special"||String(o.category||"").trim().toLowerCase()==="special",Ee=K&&!i&&D==="testfirst_drink",re=(e=>{const a=Number(e);return Number.isFinite(a)?a<=33?"left":a>=67?"right":"center":"center"})(k.x),Ie=String(o.category||(p?"Produkte":"Sonstiges")).trim()||(p?"Produkte":"Sonstiges"),Ce=p?ut(Ie):Ie,t=e=>String(e||"").trim().toLowerCase()==="special",u=Array.from(new Set([Ce,...Array.isArray(s.menu?.items)?s.menu.items.map(e=>String(e?.category||"").trim()):[],...p?["Produkte"]:Ft].filter(e=>!(!e||p&&ct(e)||!C&&t(e))))),l="menuItemCategoryOptions",w=String(o.specialSize||o.specialCardSize||"").trim().toLowerCase()==="food"?"food":"default",$=String(o.specialActionType||o.actionType||"").trim().toLowerCase()==="link"?"link":"product",A=String(o.specialActionUrl||o.linkUrl||o.actionUrl||"").trim(),Oe=String(o.specialActionProductId||o.targetProductId||"").trim(),F=String(o.ingredients||o.ingredient||o.inhaltsstoffe||"").trim(),Ae=String(o.woltUrl||o.woltLink||"").trim(),S=String(o.id||"").trim(),je=(e,a=0)=>{const d=Number(e);return Number.isFinite(d)?Math.max(0,Math.floor(d)):Math.max(0,Number(a)||0)},W=Array.isArray(s.menu?.items)?s.menu.items.slice().map((e,a)=>({entry:e,idx:a,order:je(e?.orderIndex,a)})).sort((e,a)=>e.order-a.order||e.idx-a.idx).map(e=>e.entry):[],U=W.filter(e=>String(e?.id||"").trim()!==S),E=W.findIndex(e=>String(e?.id||"").trim()===S),m=Math.max(1,U.length+1),Pe=E>=0?Math.min(m,Math.max(1,E+1)):m,O=Math.min(m,Math.max(1,Number.isFinite(Number(o.orderIndex))?je(o.orderIndex,Pe-1)+1:Pe)),de=Array.isArray(s.menu?.items)?s.menu.items.filter(e=>{const a=String(e?.id||"").trim();return!(!a||a===S)}):[],_e=ve(o.crossSellItemIds||o.crossSellIds||o.crossSell||o.crossSelling,{excludeId:S}),_=W.filter(e=>{const a=String(e?.id||"").trim();if(!a||a===S)return!1;const d=String(e?.type||"").trim().toLowerCase(),f=String(e?.menuSection||"").trim().toLowerCase();return d==="food"||d==="drink"||f==="food"||f==="drink"}),G=`
    ...
    <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll px-6 py-5 space-y-4">
      ...
      <div class="p-5 rounded-[2rem] border border-slate-100 bg-white space-y-4">
        ...
        <div>
          ...
          <input id="menuItemAllergens" type="text" value="${r(o.allergens||"")}" placeholder="${p?"z.B. limitierte Edition, ohne Rueckgabe":"z.B. Milch, Gluten"}" class="w-full px-5 py-4 bg-slate-50 rounded-2xl text-sm font-bold border-none outline-none focus:ring-2 focus:ring-indigo-100" />
        </div>
        ${!p&&Le?`
          <div>
  ...
  `}function Et({state:s,getMenuItemImages:g,getOptimizedImageUrl:I,isPlaceholderUrl:q,PLACEHOLDER_IMAGE:Q,getFirebaseStorageUrl:Y,isDirectImageUrl:X,formatPrice:J,getMenuDetailRestaurantId:ee,getMenuDetailCatalogProfile:te,isShopCatalogProfile:me,normalizeMenuType:fe,canAddToShopCart:ne,getMenuItemSocialId:be,menuItemMetaKey:ge,ensureMenuItemMeta:xe,resolveMenuItemCounts:r,currentUserBadge:P,ensureCommentShape:he,getCartCountForRestaurant:ve,renderMenuDetailComments:o,formatCount:se,getMenuItemObjectPosition:p,escapeHtml:we,icon:ye}={}){if(!s||!s.menuDetail?.open||!s.menuDetail?.item)return"";const oe=typeof g=="function"?g:(()=>[]),ke=typeof I=="function"?I:(n=>String(n||"")),z=typeof q=="function"?q:(()=>!1),le=typeof Y=="function"?Y:(n=>String(n||"")),ae=typeof X=="function"?X:(()=>!1),$e=typeof J=="function"?J:(n=>String(n||"")),T=typeof ee=="function"?ee:(()=>""),Le=typeof te=="function"?te:(()=>null),Xe=typeof me=="function"?me:(()=>!1),ie=typeof fe=="function"?fe:(n=>String(n||"food")),Be=typeof ne=="function"?ne:(()=>!1),Ve=typeof be=="function"?be:(()=>""),Ne=typeof ge=="function"?ge:(()=>""),Se=typeof xe=="function"?xe:(()=>({likes:[],comments:[],counts:{likes:0,comments:0}})),Ke=typeof r=="function"?r:(()=>({likes:0,comments:0})),k=typeof P=="function"?P:(()=>({uid:"",handle:""})),We=typeof he=="function"?he:(n=>n),K=typeof ve=="function"?ve:(()=>0),Ue=typeof o=="function"?o:(()=>""),C=typeof se=="function"?se:(n=>String(n||"0")),D=typeof p=="function"?p:(()=>"50% 50%"),i=(n,c=n,v={})=>Dt(n,{fallback:c,params:v}),Ee=(n="")=>{const c=String(n||"").trim();return c?/^(https?:\/\/|mailto:|tel:)/i.test(c)?c:`https://${c.replace(/^\/+/,"")}`:""},Je=(n,{excludeId:c=""}={})=>{const v=String(c||"").trim(),y=new Set,b=h=>{if(h==null)return;if(Array.isArray(h)){h.forEach(b);return}const Fe=typeof h=="object"?h.id||h.itemId||h.productId||h.menuItemId||"":h,Me=String(Fe||"").trim();Me&&Me.split(",").forEach(Ye=>{const N=String(Ye||"").trim();!N||N===v||y.has(N)||y.add(N)})};return b(n),Array.from(y)},re=(n,c=[])=>{const v=String(n||"").trim();if(!v)return null;const y=Array.isArray(s?.menu?.items)?s.menu.items.find(b=>String(b?.id||"").trim()===v):null;return y||Array.isArray(c)&&c.find(b=>String(b?.id||b?.itemId||b?.productId||b?.menuItemId||"").trim()===v)||null},Ie=(n={},c=0,{showAdd:v=!1}={})=>{const b=(Array.isArray(oe(n))?oe(n):[])[0]||n.imageUrl||n.image||"",h=ke(b,"thumb"),Fe=z(h)?Q:h,Me=le(b),Ye=ae(b)&&b!==Fe?b:Me,N=String(n.name||`Empfehlung ${c+1}`).trim()||`Empfehlung ${c+1}`,jt=String(n.category||"Passt dazu").trim()||"Passt dazu",Pt=$e(n.price,n);return`
    ...
    `:"",t=typeof we=="function"?we:(n=>String(n||"")),u=typeof ye=="function"?ye:(()=>""),l=s.menuDetail.item,w=oe(l),et=w.length?w.length-1:0,$=Math.max(0,Math.min(s.menuDetail.index||0,et)),A=T(l),Oe=l?.id?`menu-detail:${String(A||"")}:${String(l.id)}:${$}`:"",F=w[$]||"",Ae=ke(F,"large",{stableKey:Oe,variantGroup:"menu-detail"}),S=z(Ae)?Q:Ae,je=le(F),W=ae(F)&&F!==S?F:je,U=$e(l.price,l),E=Le(l),m=Xe(E),Pe=m?i("menu.product","Produkt"):ie(l.type)==="drink"?i("menu.drinkItem","Getraenk"):i("menu.foodItem","Speise"),O=m?ut(l.category):l.category||"",de=l.longDescription||l.description||"",_e=String(l.ingredients||l.ingredient||l.inhaltsstoffe||"").trim(),_=l.allergens||"",ce=String(l.brand||"").trim(),G=String(l.sku||"").trim(),M=Array.isArray(l.sizes)?l.sizes:[],L=Array.isArray(l.colors)?l.colors:[],tt=dt(l.stock),e=String(ie(l.type||"")||"").trim().toLowerCase(),a=e==="food"||e==="drink",d=m?l.available===!1||tt===0:l.available===!1,f=M.length?String(s.menuDetail.selectedSize||M[0]).trim()||String(M[0]):"",j=L.length?String(s.menuDetail.selectedColor||L[0]).trim()||String(L[0]):"",x=String(s.profileView?.menuAccessSource||"").trim().toLowerCase(),ue=String(s.profileTopTab||"").trim().toLowerCase()==="menu",R=!m&&a&&ue&&x==="qr",pe=m?Be(E):R,B=Ee(l.woltUrl||l.woltLink||""),H=!m&&a&&!R&&!!B,Ge=!m&&a&&!R&&!H,pt=!!pe,mt=Je(l.crossSellItemIds||l.crossSellIds||l.crossSell||l.crossSelling,{excludeId:l.id}),nt=Array.isArray(l.crossSell)?l.crossSell:[],st=mt.map(n=>re(n,nt)).filter(Boolean),ot=st.length?st:nt.filter(Boolean),ze=Ve(l),lt=Ne(A,ze),Re=lt?Se(lt):{likes:[],comments:[],counts:{likes:0,comments:0}},Z=Ke(Re),at=k(),Te=Re.likes?.some(n=>n.uid===at.uid||n.handle===at.handle),ft=(Array.isArray(s.favoriteMenuItems?.items)?s.favoriteMenuItems.items:[]).some(n=>{if(!n||typeof n!="object")return!1;const c=String(n.restaurantId||"").trim();if(A&&c&&c!==A)return!1;const v=[n.itemId,n.menuSocialId,n.menuItemId,n.id].map(y=>String(y||"").trim()).filter(Boolean);return!!ze&&v.includes(ze)}),it=(Re.comments||[]).map(We),V=!!A&&!!ze&&!!s.user,bt=!!String(s.user?.uid||"").trim(),gt="menuDetailTitle",He=m?K(A||E?.restaurantId||""):0,rt=(String(s.menuDetail.footerView||"cart").trim().toLowerCase()==="comment"?"comment":"cart")==="comment",Ze=i("menu.noInfo","Keine Informationen, bitte an das Lokal oder den Kellner wenden."),xt=String(de||"").trim()||Ze,ht=_e||Ze,vt=String(_||"").trim()||Ze,qe=String(s.menuDetail?.infoTab||"info").trim().toLowerCase(),De=qe==="ingredients"||qe==="allergens"?qe:"info",Qe=n=>["h-10 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center cursor-pointer select-none",De===n?"bg-slate-900 text-white border-slate-900":"bg-white text-slate-500 border-slate-200"].join(" "),wt=m?`
    ...
    `:`
      <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll modal-handoff-scroll px-7 py-6 bg-white/98">
        ...
        <div class="mt-6 space-y-5">
          ...
          <div class="border-t border-slate-100"></div>
          ${ot.length?`<div class="pt-1">${Ce(ot,{showAdd:pt})}</div><div class="border-t border-slate-100"></div>`:""}
    ...
    <div class="modal-handoff-chrome px-7 pb-6 pt-4 border-t border-slate-100 bg-white/98 backdrop-blur-sm modal-footer-safe relative z-10">
      ...
      <div id="footer-comment-view" class="flex gap-3 items-center w-full transition-all duration-300 ${rt?"":"hidden opacity-0"}">
        ...
        <div class="flex-1 flex gap-2">
          <textarea id="menuDetailCommentInput" placeholder="${t(V?i("menu.commentPlaceholder","Schreib einen Kommentar..."):i("menu.loginRequired","Bitte einloggen"))}" class="flex-1 px-5 py-3.5 rounded-[1.65rem] border border-slate-100 bg-slate-50 text-sm font-medium outline-none resize-none leading-relaxed ${V?"":"opacity-60"}" rows="1" ${V?"":"disabled"}>${t(s.menuDetail.commentText||"")}</textarea>
  ...
  `}export{Et as renderMenuDetailModalCore,Ut as renderMenuItemModalCore};
