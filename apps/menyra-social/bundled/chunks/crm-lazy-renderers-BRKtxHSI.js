import{m as le}from"./loading-diagnostics-utils-D9lcQexT.js";function ne(o=""){const e=String(o||"").trim();if(!e)return"i panjohur";const l=new Date(e);return Number.isFinite(l.getTime())?l.toLocaleString("de-DE",{dateStyle:"short",timeStyle:"short"}):e}function J(o="",e="#111827"){const l=String(o||"").trim();return/^#[0-9a-fA-F]{6}$/.test(l)?l:e}function oe(o={}){const e=o?.landingScreenOne&&typeof o.landingScreenOne=="object"?o.landingScreenOne:{},l=J(o?.businessNameColor||o?.landingBusinessNameColor||e.businessNameColor||"",""),t=l&&l.toLowerCase()!=="#111827"?l:"";return{part1:J(o?.businessNameColorPart1||o?.landingBusinessNameColorPart1||e.businessNameColorPart1||l||"","#111827"),part2:J(o?.businessNameColorPart2||o?.landingBusinessNameColorPart2||e.businessNameColorPart2||t||"","#4f46e5")}}function de(o={}){const{state:e,icon:l,escapeHtml:t,getLeadSettingsConfig:P,CEO_COUNTRIES:w,LEAD_TYPE_ORDER:h,LEAD_TYPE_LABELS:y}=o,p=P();return`
  ...
  `}function ce(o={}){const{state:e,icon:l,escapeHtml:t,getLeadSettingsConfig:P,getOptimizedImageUrl:w,PLACEHOLDER_IMAGE:h,resolveCustomerType:y,normalizeLeadLocations:p,getLeadCountryCenter:c,getLeadMonthlyPrice:b,LEAD_TYPE_ORDER:g,LEAD_TYPE_LABELS:r,buildLeadAccountEmail:d,hasLeadLocationCoords:$,CEO_COUNTRIES:x,normalizeLeadCountry:I,resolveCurrencyCodeFromLeadCountry:C,buildLeadContactName:S}=o,a=e.leadModal.lead||{},M=P(),m=e.leadModal.logoPreview||a.logoUrl||"",L=m?w(m,"avatar"):h,U=e.leadModal.bestSpotLogoPreview||a.bestSpotLogoUrl||a.spotLogoUrl||m,D=U?w(U,"avatar"):h,T=e.leadModal.titleImagePreview||a.titleImageUrl||a.coverImageUrl||a.coverUrl||a.heroUrl||"",B=T?w(T,"medium"):h,n=e.leadModal.mode==="edit"&&!!a.id,O=!!e.leadModal.actionsOpen,k=!!e.leadModal.deleting,v=y(a.customerType||"cafe"),A=a.billingCycle==="yearly"?"yearly":"monthly",i=I(a.country||M.defaultCountry),u=String(typeof C=="function"&&C(i,"EUR")||"EUR").trim().toUpperCase()||"EUR",_=p(e.leadModal.locations,a.address||"",e.leadModal.coords||c(i)),E=b(v,M),R=E*12,N=A==="yearly"?R:E,z=oe(a),K=a.specialEnabled===!0,V=e.leadModal.coords&&Number.isFinite(Number(e.leadModal.coords.lat))&&Number.isFinite(Number(e.leadModal.coords.lng))?{lat:Number(e.leadModal.coords.lat),lng:Number(e.leadModal.coords.lng)}:null;return`
      ...
      <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div class="mb-6 flex items-start justify-between">
          <div>
            ...
            <h2 class="text-2xl font-black italic uppercase tracking-tighter">${t(n?"Ndrysho lead-in":"Lead i ri")}</h2>
          </div>
          ${n?`
            <div class="relative">
          ...
          <div>
            ...
            <div class="relative mt-2">
              <select id="leadCustomerType" class="w-full px-5 py-4 pr-12 bg-slate-50 rounded-2xl text-sm font-bold border-none outline-none appearance-none focus:ring-2 focus:ring-indigo-100">
                ${g.map(j=>`<option value="${j}" ${v===j?"selected":""}>${t(r[j])}</option>`).join("")}
              </select>
              <div class="absolute inset-y-0 right-5 flex items-center text-slate-400 pointer-events-none">${l("chevron-down","w-4 h-4")}</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="relative mt-2">
              <select id="leadBillingCycle" class="w-full px-5 py-4 pr-12 bg-white rounded-2xl text-sm font-bold border border-slate-100 outline-none appearance-none focus:ring-2 focus:ring-indigo-100">
                <option value="monthly" ${A==="monthly"?"selected":""}>Monatlich</option>
                <option value="yearly" ${A==="yearly"?"selected":""}>Jaehrlich</option>
              </select>
              <div class="absolute inset-y-0 right-5 flex items-center text-slate-400 pointer-events-none">${l("chevron-down","w-4 h-4")}</div>
            </div>
          </div>
        ...
        <div class="mt-6 p-5 rounded-[2rem] bg-slate-50 border border-slate-100 space-y-4">
          ...
          ${_.map((j,F)=>{const s=$(j);return`
              <div class="bg-white p-4 rounded-2xl border border-slate-100 space-y-3">
                ...
                <input id="leadLocationAddress_${F}" data-lead-location-address="${F}" type="text" value="${t(j.address||"")}" placeholder="Plus Code ose adresa" class="w-full px-5 py-4 bg-slate-50 rounded-2xl text-sm font-bold border-none outline-none focus:ring-2 focus:ring-indigo-100" />
            ...
            `}).join("")}
          <div id="leadCoordsDisplay" class="${V?"":"hidden"} text-[9px] font-bold text-emerald-600 flex items-center gap-1">
            ${l("check-circle-2","w-3 h-3")} ${V?t(`${V.lat.toFixed(4)}, ${V.lng.toFixed(4)}`):""}
          </div>
        </div>
        <div class="mt-6 space-y-4">
          <div>
            ...
            <div class="relative mt-2">
              <select id="leadCountry" class="w-full px-5 py-4 pr-12 bg-slate-50 rounded-2xl text-sm font-bold border-none outline-none appearance-none focus:ring-2 focus:ring-indigo-100">
                ${x.map(j=>`<option value="${t(j)}" ${i===j?"selected":""}>${t(j)}</option>`).join("")}
              </select>
              <div class="absolute inset-y-0 right-5 flex items-center text-slate-400 pointer-events-none">${l("chevron-down","w-4 h-4")}</div>
            </div>
          </div>
          ...
          <label class="flex items-center justify-between gap-4 px-4 py-3 rounded-2xl bg-slate-50 border border-slate-100">
            ...
            <input id="leadSpecialEnabled" type="checkbox" class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-200" ${K?"checked":""} />
          </label>
        </div>
        ...
        <button id="leadInlineSaveBtn" type="button" class="w-full mt-5 py-4 rounded-[1.8rem] bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-500/20 active:scale-95 transition-transform" ${e.leadModal.loading?"disabled":""}>${t(e.leadModal.loading?"Duke ruajtur...":n?"Ruaj lead-in":"Krijo lead")}</button>
      </div>
    </div>
  `}function pe(o={}){const{state:e,icon:l,escapeHtml:t,getCurrentCeoMeta:P,getStaffFormEmail:w,getOptimizedImageUrl:h,PLACEHOLDER_IMAGE:y,isPlaceholderUrl:p,CEO_COUNTRIES:c,normalizeCeoCountry:b}=o,g=P(),r=e.staff.form||{},d=!!e.staff.editorUid,$=d&&String(e.staff.editorUid||"")===String(g.uid||""),x=r.coords&&Number.isFinite(Number(r.coords.lat))&&Number.isFinite(Number(r.coords.lng))?{lat:Number(r.coords.lat),lng:Number(r.coords.lng)}:null,I=w(r,{preferStored:d}),C=r.avatarPreview||r.avatarUrl||"",S=C?h(C,"avatar"):y,a=!S||p(S)?y:S,M=e.staff.saving?d?"Duke ruajtur...":"Erstelle CEO...":d?"Ruaj CEO-n":"Krijo CEO";return`
  ...
  `}function ue(o={}){const{state:e,icon:l,escapeHtml:t,isCeoUser:P,renderCeoGuard:w,renderLeadSettingsView:h,renderLeadCreationView:y,normalizeSearchKey:p,normalizeLeadStatusKey:c,normalizeLeadScopeKey:b,createLeadScopeMap:g,sanitizeCeoCrmCounts:r,hasStoredCeoCrmCounts:d,resolveKnownScopeCountLabel:$,leadMatchesQuery:x,leadStatusTone:I,leadStatusLabel:C,buildLeadLandingPageUrl:S,getOptimizedImageUrl:a,PLACEHOLDER_IMAGE:M,renderOwnershipPills:m,renderCeoScopeTabs:L,LEAD_STATUS_ORDER:U,LEAD_STATUS_LABELS:D}=o;if(!P())return w("Leads");if(e.leads.view==="settings")return h();if(e.leads.view==="create")return y();const T=p(e.leads.query||""),B=c(e.leads.status||""),n=b(e.leads.scope),O=e.leads.pages||g(()=>[]);e.leads.hasMore||g(()=>!1);const k=e.leads.loaded||g(()=>!1),v=e.leads.knownCount||g(()=>0),A=e.leads.countExact||g(()=>!1),i=r(e.userProfile?.crmCounts||{}),u=d(e.userProfile?.crmCounts)?String(i.ownLeads):$(v.own,!!A.own,!!k.own),_=d(e.userProfile?.crmCounts)?String(i.staffLeads):$(v.staff,!!A.staff,!!k.staff),E=d(e.userProfile?.crmCounts)?String(i.archivedLeads):$(v.archived,!!A.archived,!!k.archived),R=(()=>{try{return typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}catch{return Date.now()}})();let N=Array.isArray(O[n])?O[n].slice():[];B&&n!=="archived"&&(N=N.filter(s=>c(s.status)===B));const z=(s,f=null)=>{const H=Array.isArray(s?.locations)?s.locations.map(G=>G?.address||G?.city||"").join(" "):"";return String(s?._searchKey||"").trim()||p([s?.businessName,s?.restaurantName,s?.name,f?.name,f?.restaurantName,s?.contactName,s?.phone,s?.email,s?.socialEmail,s?.instagram,s?.city,s?.address,s?.publicSlug,s?.landingSlug,f?.publicSlug,f?.landingSlug,s?.canonicalPublicPath,s?.status,C(s?.status),s?.customerType,H].filter(Boolean).join(" "))},K=N.map(s=>{const f=s.restaurantId?e.restaurants.find(G=>String(G.id)===String(s.restaurantId)):null,H=z(s,f);return{lead:s,rest:f,searchKey:H,matches:!T||x({...s,_searchKey:H},T)}}),V=K.filter(s=>s.matches).length,j=e.leads.loading?'<div class="text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 py-16">Leads laden...</div>':K.length?`
      ${K.map(({lead:s,rest:f,searchKey:H,matches:G})=>{const Q=I(s.status),te=C(s.status),Y=s.logoUrl||s.logo||f?.logoUrl||f?.logo||"",ae=Y?a(Y,"avatar"):M,Z=s.businessName||f?.name||f?.restaurantName||"Business",W=s.email||s.socialEmail||"",X=String(s.landingRestaurantId||s.restaurantId||f?.id||"").trim(),q=String(s.publicSlug||s.landingSlug||f?.publicSlug||f?.landingSlug||"").trim(),ee=!!String(X||q).trim()&&typeof S=="function"?String(S(X,{publicSlug:q,landingSlug:q,businessName:Z})||"").trim():"",se=m(s,{hideOwn:n==="own"});return`
        <div data-lead-row="true" data-lead-search-key="${t(H)}" class="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm" ${G?"":"hidden"}>
          <div class="flex items-center gap-3">
            ...
            <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${Q.bg} ${Q.text}">${t(te)}</span>
          </div>
          ${se}
      ...
      `}).join("")}
      <div id="leadsNoResults" class="text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 py-16" ${V?"hidden":""}>Nuk ka leads</div>
    `:'<div class="text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 py-16">Nuk ka leads</div>',F=(()=>{try{return typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}catch{return Date.now()}})();return le("lead list render",{scope:n,count:K.length,items:V,elapsedMs:F-R}),`
    <div id="leadsView" class="p-6 animate-in slide-in-from-right-10 duration-500">
      ...
      ${L({idPrefix:"lead-scope",active:n,tabs:[{key:"own",label:"Meine Leads",count:u},{key:"staff",label:"Staff Leads",count:_},{key:"archived",label:"Archiviert",count:E}]})}
      ...
      <div id="leadsList" class="space-y-4">${j}</div>
      ${e.leads.hasMore?.[n]?`
  ...
  `}function be(o={}){const{state:e,icon:l,escapeHtml:t,isCeoUser:P,renderCeoGuard:w,normalizeSearchKey:h,normalizeCustomerScopeKey:y,createCustomerScopeMap:p,sanitizeCeoCrmCounts:c,hasStoredCeoCrmCounts:b,resolveKnownScopeCountLabel:g,customerMatchesQuery:r,toDateSafe:d,getOptimizedImageUrl:$,PLACEHOLDER_IMAGE:x,leadTypeLabel:I,customerStatusLabel:C,isCustomerRestaurant:S,renderOwnershipPills:a,renderCeoScopeTabs:M}=o;if(!P())return w("Kunden");const m=h(e.customers.query||""),L=y(e.customers.scope),U=e.customers.pages||p(()=>[]);e.customers.hasMore||p(()=>!1);const D=e.customers.loaded||p(()=>!1),T=e.customers.knownCount||p(()=>0),B=e.customers.countExact||p(()=>!1),n=c(e.userProfile?.crmCounts||{}),O=b(e.userProfile?.crmCounts)?String(n.ownCustomers):g(T.own,!!B.own,!!D.own),k=b(e.userProfile?.crmCounts)?String(n.staffCustomers):g(T.staff,!!B.staff,!!D.staff),v=(Array.isArray(U[L])?U[L].slice():[]).filter(i=>r(i,m)).sort((i,u)=>(d(u?.createdAt)?.getTime()||0)-(d(i?.createdAt)?.getTime()||0)),A=e.customers.loading?'<div class="text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 py-16">Kunden laden...</div>':v.length?v.map(i=>{const u=i.logoUrl||i.logo||"",_=u?$(u,"avatar"):x,E=i.name||i.restaurantName||"Business",R=I(i.type||i.customerType||""),N=i.city||"",z=C(S(i)?"kunde":i.status),K=a(i,{hideOwn:L==="own"});return`
    ...
    <div id="customersView" class="p-6 animate-in slide-in-from-right-10 duration-500">
      ...
      ${M({idPrefix:"customer-scope",active:L,ownLabel:"Meine Kunden",ownCount:O,staffLabel:"Staff Kunden",staffCount:k})}
      <div class="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm mb-4 flex items-center gap-3">
        ${l("search","w-4 h-4 text-slate-400")}
  ...
  `}function xe(o={}){const{state:e,icon:l,escapeHtml:t,isCeoUser:P,renderCeoGuard:w,renderStaffEditorView:h,getCurrentCeoMeta:y,getOptimizedImageUrl:p,isPlaceholderUrl:c,PLACEHOLDER_IMAGE:b,normalizeHandle:g,staffBuildStatus:r,staffBuildStatusLoading:d,staffBuildStatusError:$}=o;if(!P())return w("Staff");if(e.staff.view==="form")return h();const x=r&&typeof r=="object"?r:e?.staff?.buildStatus||{},I=String(x.commitShort||"").trim()||"i panjohur",C=ne(x.buildTimestamp||""),S=String(x.branch||"").trim()||"i panjohur",a=String(x.environment||"").trim()||"i panjohur",M=!!(d??e?.staff?.buildStatusLoading),m=String($??e?.staff?.buildStatusError??"").trim(),L=y(),U=Array.isArray(e.staff.items)?e.staff.items.slice():[],D=[...Array.isArray(e.leads.pages?.own)?e.leads.pages.own:[],...Array.isArray(e.leads.pages?.staff)?e.leads.pages.staff:[],...Array.isArray(e.leads.pages?.archived)?e.leads.pages.archived:[]],T=[...Array.isArray(e.customers.pages?.own)?e.customers.pages.own:[],...Array.isArray(e.customers.pages?.staff)?e.customers.pages.staff:[]],B=e.staff.loading?'<div class="text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 py-16">Staff laden...</div>':U.length?U.map(n=>{const O=String(n.uid||"")===String(L.uid||""),k=O?"Du":String(n.ceoParentUid||"")===String(L.uid||"")?"Direkt":"Unterstaff",v=n.crmCounts&&typeof n.crmCounts=="object"?n.crmCounts:{},A=Number.isFinite(Number(v.ownLeads))?Number(v.ownLeads):D.filter(N=>String(N.createdByUid||"")===String(n.uid||"")).length,i=Number.isFinite(Number(v.ownCustomers))?Number(v.ownCustomers):T.filter(N=>String(N.createdByUid||"")===String(n.uid||"")).length,u=n.locationLabel||n.location||n.city||n.country||"-",_=n.avatarPreview||n.avatarUrl||n.avatar||"",E=_?p(_,"avatar"):b,R=!E||c(E)?b:E;return`
    ...
    <div id="staffView" class="p-6 animate-in slide-in-from-right-10 duration-500 pb-24">
      ...
      <div class="mb-4 rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
        ...
        ${M?'<p class="mt-2 text-[9px] font-bold text-slate-400 uppercase tracking-widest">Build info po ngarkohet...</p>':""}
