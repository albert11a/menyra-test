...
import{_ as oe}from"./domain-auth-Aq-4Vdvh.js";import{f as z,r as ie,l as de,s as H}from"./domain-analytics-jv5B-kA2.js";const M="mnyraDashboardStyles",le=`
.mnyra-dash {
...
`;function ce(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(M)))try{const a=e.createElement("style");a.id=M,a.textContent=le,e.head?.appendChild(a)}catch{}}function u(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function $(e,a,o=""){if(typeof e!="function")return"";try{return e(a,o)||""}catch{return""}}const me=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function ue({businessType:e="",isShopCatalog:a=!1}={}){if(a)return"shop";const o=String(e||"").trim().toLowerCase();return me.includes(o)?"hotel":"restaurant"}function pe({kind:e="restaurant",isOwner:a=!1,canAccessOrders:o=!1}={}){const n=[{nav:"upload",uploadIntent:"chooser",iconName:"plus",label:"Neuer Beitrag",sub:"Posto foto ose video"},{nav:"upload",uploadIntent:"story",iconName:"camera",label:"Story",sub:"E dukshme 24h"}];return e==="hotel"?n.push({nav:"menu",iconName:"bed-double",label:"Hotel & Dhoma",sub:"Detaje, dhoma, oferta"}):e==="shop"?n.push({nav:"menu",iconName:"shopping-bag",label:"Ndrysho dyqanin",sub:"Produkte & Stok"}):n.push({nav:"menu",iconName:"utensils",label:"Ndrysho menune",sub:"Produkte & Kategorien"}),n.push({nav:"menu",iconName:"megaphone",label:"Oferta & Reklama",sub:"Im Editor verwalten"}),e!=="hotel"&&o&&n.push({nav:"orders",iconName:"shopping-cart",label:"Porosite",sub:"Hyrje & Status"}),n.push({nav:"analytics",iconName:"bar-chart-3",label:"Analytics",sub:"Te gjitha statistikat"}),a&&n.push({nav:"businessAccounts",iconName:"users-round",label:"Team & Staff",sub:"Zugänge verwalten"}),n.push({nav:"settings",iconName:"settings",label:"Cilesimet",sub:"Profili & Kontakti"}),n}function he(e="restaurant"){const a=[{key:"profileViews",label:"Profilaufrufe"},{key:"postImpressions",label:"Shtrirja e postimeve"},{key:"contactClicks",label:"Kontakt-Klicks"}];return e==="shop"?a.concat([{key:"ordersCompleted",label:"Porosite"},{key:"revenue",label:"Umsatz",unit:"€"},{key:"productViews",label:"Produkt-Aufrufe"}]):e==="hotel"?a.concat([{key:"uniqueVisitors",label:"Vizitore"},{key:"postLikes",label:"Likes"},{key:"feedImpressions",label:"Shtrirja ne feed"}]):a.concat([{key:"ordersCompleted",label:"Porosite"},{key:"revenue",label:"Umsatz",unit:"€"},{key:"qrScans",label:"QR-Scans"}])}function U(e=0,a=""){const o=z(e);return a?`${o} ${a}`:o}function fe(e=new Date().getHours()){const a=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return a>=5&&a<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:a>=11&&a<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:a>=18&&a<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function ye({name:e="",logoUrl:a="",hour:o=new Date().getHours(),iconFn:n}={}){const s=fe(o);return`
    <div class="mnyra-dash__greet">
      ...
      <div class="mnyra-dash__greet-text">
        ...
        <p class="mnyra-dash__greet-sub">${u(s.text)}</p>
      </div>
    </div>
  `}const ge='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="9"></circle><circle cx="12" cy="12" r="4" fill="currentColor" stroke="none"></circle></svg>';function be({iconFn:e}={}){return`
    ...
    <button type="button" class="mnyra-dash__composer-btn${o.primary?" mnyra-dash__composer-btn--primary":""}" data-dashboard-composer="${u(o.mode)}">
      <span class="mnyra-dash__composer-btn-icon">${o.iconHtml}</span>
      <span class="mnyra-dash__composer-btn-label">${u(o.label)}</span>
    </button>
  `).join("")}</div>
    </div>
  `}function _e({actions:e=[],iconFn:a}={}){return`
    <div class="mnyra-dash__section">
      <div class="mnyra-dash__section-head">
        <p class="mnyra-dash__section-title">Schnellzugriff</p>
      </div>
      <div class="mnyra-dash__actions">${(Array.isArray(e)?e:[]).map(n=>{const s=n.uploadIntent?` data-upload-intent="${u(n.uploadIntent)}"`:"";return`
      <button type="button" class="mnyra-dash__action" data-nav="${u(n.nav)}"${s}>
        <span class="mnyra-dash__action-icon">${$(a,n.iconName,"w-4 h-4")}</span>
        <span>
          <span class="mnyra-dash__action-label" style="display:block;">${u(n.label)}</span>
          <span class="mnyra-dash__action-sub" style="display:block;">${u(n.sub||"")}</span>
        </span>
      </button>
    `}).join("")}</div>
    </div>
  `}function ve({kpiDefs:e=[],week:a={},today:o={}}={}){return`
    ...
    <div class="mnyra-dash__kpi">
      <p class="mnyra-dash__kpi-label">${u(s.label)}</p>
      <p class="mnyra-dash__kpi-value">${u(U(a?.[s.key]||0,s.unit||""))}</p>
      <p class="mnyra-dash__kpi-today">Heute: ${u(U(o?.[s.key]||0,s.unit||""))}</p>
    </div>
  `).join("")}</div>
    </div>
  `}function xe({posts:e=[],iconFn:a}={}){const o=Array.isArray(e)?e:[];let n="";return o.length?(n=o.map(s=>{const l=[s.dateLabel,`${z(s.likesCount||0)} Likes`,`${z(s.commentsCount||0)} Kommentare`];return Number(s.impressions||0)>0&&l.push(`${z(s.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${s.thumbUrl?`<img src="${u(s.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:$(a,s.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
          <div class="mnyra-dash__post-main">
            <p class="mnyra-dash__post-caption">${u(s.caption||"Pa tekst")}</p>
  ...
  `}const Pe="menyra_social_dashboard_cache_v1::",Ce=6,De=3;function N(e){const a=Number(e);return Number.isFinite(a)?a:0}function Fe(e={}){const a=String(e.createdAtClient||"").trim();if(a){const n=new Date(a);if(!Number.isNaN(n.getTime()))return n}const o=e.createdAt;if(o&&typeof o.toDate=="function")try{const n=o.toDate();if(n instanceof Date&&!Number.isNaN(n.getTime()))return n}catch{}return null}function je(e="",a={}){const o=Array.isArray(a.media)&&a.media.length?a.media[0]:{},n=String(o.type||a.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",s=String(o.thumbUrl||(n==="image"?o.url:"")||a.thumbUrl||"").trim(),l=Fe(a);return{id:String(e||"").trim(),caption:String(a.caption||"").trim(),mediaType:n,thumbUrl:s,likesCount:N(a.likesCount),commentsCount:N(a.commentsCount),impressions:0,dateLabel:l?l.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:l?l.getTime():0}}function ze({days:e=[],todayKey:a="",rawPosts:o=[]}={}){const n=Array.isArray(e)?e:[],s=H(n),l=n.find(c=>String(c?.date||c?.id||"").trim()===String(a||"").trim()),g=H(l?[l]:[]),I=s.merged?.posts&&typeof s.merged.posts=="object"?s.merged.posts:{},b=(Array.isArray(o)?o:[]).map(c=>je(c?.id,c?.data||{})).filter(c=>c.id).sort((c,_)=>_.createdAtMs-c.createdAtMs).slice(0,De).map(c=>({...c,impressions:N(I[c.id]?.impressions)}));return{day:String(a||"").trim(),week:s.summary,today:g.summary,posts:b}}function Ne({state:e,renderFn:a,documentObj:o,firestoreApi:n={},profileApi:s={},composerApi:l={},iconFn:g,storageObj:I}={}){const b=o||(typeof document>"u"?null:document),c=typeof a=="function"?a:()=>{},_=I||(typeof localStorage>"u"?null:localStorage),K=typeof s.getBusinessProfileTypeFn=="function"?s.getBusinessProfileTypeFn:(()=>""),A=typeof s.isShopCatalogProfileFn=="function"?s.isShopCatalogProfileFn:(()=>!1),q=typeof s.isBusinessOwnerProfileFn=="function"?s.isBusinessOwnerProfileFn:(()=>!1),J=typeof s.canAccessRestaurantOrdersFn=="function"?s.canAccessRestaurantOrdersFn:(()=>!1),T=typeof s.getRestaurantMetaByIdFn=="function"?s.getRestaurantMetaByIdFn:(()=>null),Q=typeof s.resolveRestaurantLogoFn=="function"?s.resolveRestaurantLogoFn:(()=>""),G=typeof s.resolveOwnAvatarUrlFn=="function"?s.resolveOwnAvatarUrlFn:(()=>"");let w=0,R=!1,v=null,S=null,P="",B=()=>null;const Y=300;async function Z(r=""){const{db:t,collectionFn:d,queryFn:i,limitFn:p,getDocsFn:m}=n,h=String(r||"").trim();if(!h||!t||typeof d!="function"||typeof m!="function")throw new Error("Produktet nuk u ngarkuan.");const f=d(t,"restaurants",h,"menuItems"),x=typeof i=="function"&&typeof p=="function"?i(f,p(Y)):f,y=await m(x),k=[];return y.forEach(F=>{const j=B(F?.id,F?.data?.()||{});j&&k.push(j)}),k.sort((F,j)=>F.name.localeCompare(j.name,"sq")),k}function W(){return v?Promise.resolve(v):(S||(S=oe(()=>import("./business-composer-controller-DSnLaCwi.js"),__vite__mapDeps([0,1,2,3,4,5])).then(r=>(B=typeof r?.normalizeComposerProductCore=="function"?r.normalizeComposerProductCore:(()=>null),v=r.createBusinessComposerController({documentObj:b,windowObj:b?.defaultView||null,api:{getRestaurantIdFn:()=>C(),getBusinessMetaFn:()=>{const t=C();if(!t)return{name:"",logoUrl:"",city:""};const d=O(t),i=T(t)||{};return{name:d.name,logoUrl:d.logoUrl,city:String(i.city||"").trim()}},loadProductsFn:t=>Z(t),uploadImageFn:l.uploadImageFn,uploadVideoFn:l.uploadVideoFn,captureVideoPosterFn:l.captureVideoPosterFn,createPostFn:l.createPostFn,createStoryFn:l.createStoryFn,formatPriceFn:l.formatPriceFn,getOptimizedImageUrlFn:l.getOptimizedImageUrlFn,escapeHtmlFn:l.escapeHtmlFn,iconFn:typeof g=="function"?g:void 0,afterPublishFn:async t=>{try{await D({force:!0})}catch{}typeof l.afterPublishFn=="function"&&await l.afterPublishFn(t)}}}),v)).catch(r=>{throw S=null,console.error("[mnyra][dashboard] composer load failed",r),r})),S)}function X(r="post"){const t=String(r||"").trim().toLowerCase()==="story"?"story":"post";if(typeof l.prewarmFn=="function")try{l.prewarmFn()}catch{}if(v){v.open(t);return}P=t,W().then(d=>{const i=P||t;P="",d?.open?.(i)}).catch(()=>{P=""})}function E(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:""}),e.dashboardView}function C(){const r=e?.userProfile||{};return String(r.restaurantId||r.staffRestaurantId||"").trim()}function ee(){const r=String(e?.user?.uid||"").trim();if(!r)return!1;const t=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||t===r}function L(r=""){return`${Pe}${r}`}function ae(r="",t=""){if(!_||!r)return null;try{const d=_.getItem(L(r));if(!d)return null;const i=JSON.parse(d);return!i||typeof i!="object"||String(i.day||"").trim()!==String(t||"").trim()||!i.model||typeof i.model!="object"?null:i.model}catch{return null}}function te(r="",t=null){if(!(!_||!r||!t))try{_.setItem(L(r),JSON.stringify({day:t.day,model:t}))}catch{}}async function ne(r=""){const{db:t,collectionFn:d,queryFn:i,orderByFn:p,limitFn:m,getDocsFn:h}=n;if(!t||typeof d!="function"||typeof i!="function"||typeof p!="function"||typeof m!="function"||typeof h!="function")return[];const f=d(t,"restaurants",r,"socialPosts");return(await h(i(f,p("createdAt","desc"),m(Ce)))).docs.map(y=>({id:y.id,data:y.data()||{}})).filter(y=>{const k=String(y.data.status||"active").trim().toLowerCase();return k!=="deleted"&&k!=="hidden"})}async function D({force:r=!1}={}){const t=E(),d=C();if(!d)return;const i=ie({rangeKey:"7d"});if(!i)return;const p=`${d}::${i.toDay}`;if(!r&&t.loadedSignature===p&&t.status==="ready")return;if(!t.model){const f=ae(d,i.toDay);f&&(t.model=f,t.status="ready",c())}w+=1;const m=w;t.model||(t.status="loading",t.error="",c());try{const f={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:d},[x,y]=await Promise.allSettled([de({...f,fromDay:i.fromDay,toDay:i.toDay}),ne(d)]);if(m!==w)return;if(x.status==="rejected")throw x.reason;y.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",y.reason),t.model=ze({days:x.value,todayKey:i.toDay,rawPosts:y.status==="fulfilled"?y.value:[]}),t.status="ready",t.error="",t.loadedSignature=p,te(d,t.model)}catch(f){if(m!==w)return;console.error("[mnyra][dashboard] load failed",f),t.model||(t.status="error",t.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}c()}function re(){R||!b||(R=!0,b.addEventListener("click",r=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(r.target?.closest?.("[data-dashboard-retry]")){D({force:!0});return}const t=r.target?.closest?.("[data-dashboard-composer]");t&&(r.preventDefault(),X(t.getAttribute("data-dashboard-composer")))}catch{}}))}function O(r=""){const t=e?.userProfile||{},d=r?T(r)||{}:{},i=K(t),p=String(d.name||d.restaurantName||t.name||"").trim()||"Business";let m="";try{m=String(G()||"").trim()}catch{}if(!m)try{m=String(Q(d)||"").trim()}catch{}return{name:p,logoUrl:m,kind:ue({businessType:i,isShopCatalog:A(t)})}}function se(){ce(b),re();const r=E(),t=C();let d="";if(!t)d=ee()?`${ke()}${V({kpiCount:6})}`:Se();else{const i=O(t),p=pe({kind:i.kind,isOwner:q(e?.userProfile),canAccessOrders:J(e?.userProfile)}),m=he(i.kind);r.status==="idle"&&(r.status="loading",queueMicrotask(()=>{D({force:!1})}));let h="";r.model?h=`
          ${ve({kpiDefs:m,week:r.model.week,today:r.model.today})}
          ${xe({posts:r.model.posts,iconFn:g})}
        `:r.status==="error"?h=we({message:r.error}):h=V({kpiCount:m.length}),d=`
    ...
    `}return Object.freeze({renderDashboardView:se,loadDashboard:D})}export{Ne as c};
