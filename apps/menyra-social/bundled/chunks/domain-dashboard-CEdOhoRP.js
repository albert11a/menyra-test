const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["chunks/business-composer-controller-tM-zhMKO.js","chunks/domain-feed-social-eager-DLDUm3QL.js","chunks/domain-auth-CdW0dNSS.js","chunks/domain-public-profile-mLQti0eH.js","chunks/domain-media-eager-DAUyCk2O.js","chunks/domain-menu-eager-DgcaTsW0.js","chunks/profile-post-card-markup-utils-HwqIiXgP.js"])))=>i.map(i=>d[i]);
import{_ as Pe}from"./domain-auth-CdW0dNSS.js";import{f as B,r as Be,l as Ce,s as X}from"./domain-analytics-YvFkDAlU.js";import{b as Re}from"./domain-business-accounts-D8NpUhi6.js";const Ke=20,$e=8;function x(e=""){return e==null?"":String(e).trim()}function L(e){if(e==null||e==="")return null;const a=Number(String(e).replace(",","."));return Number.isFinite(a)&&a>0?a:null}function Ae(e=Date.now(),a=Math.random()){const n=Math.max(0,Number(e)||0).toString(36),o=Math.floor(Math.max(0,Math.min(.999999,Number(a)||0))*36**6).toString(36).padStart(6,"0");return`room_${n}_${o}`}function je(e={}){const a=e&&typeof e=="object"?e:{},n=[...Array.isArray(a.images)?a.images:[],x(a.imageUrl??a.image??a.photoUrl)],o=[];return n.forEach(r=>{const d=x(r);d&&!o.includes(d)&&o.push(d)}),o.slice(0,$e)}function Ee(e={},{index:a=0}={}){const n=e&&typeof e=="object"?e:{},o=L(n.persons??n.guests??n.capacity),r=L(n.size??n.sizeSqm??n.area),d=je(n);return{id:x(n.id)||Ae(Date.now()+a),title:x(n.title??n.name),description:x(n.description??n.text).slice(0,400),imageUrl:d[0]||"",images:d,price:L(n.price??n.pricePerNight),currency:x(n.currency??n.currencyCode).toUpperCase()||"EUR",persons:o==null?null:Math.min(20,Math.round(o)),beds:x(n.beds??n.bedsLabel).slice(0,60),size:r==null?null:Math.min(500,Math.round(r)),tag:x(n.tag??n.badge).slice(0,40),active:n.active!==!1}}function Te(e=[]){return(Array.isArray(e)?e:[]).slice(0,Ke).map((a,n)=>Ee(a,{index:n}))}function Ue(e={}){return Te((e&&typeof e=="object"?e:{}).hotelRooms).filter(n=>n.title)}function xa(e={}){const a=[];return Number.isFinite(e?.persons)&&e.persons>0&&a.push({icon:"users",label:`${e.persons} persona`}),x(e?.beds)&&a.push({icon:"bed",label:x(e.beds)}),Number.isFinite(e?.size)&&e.size>0&&a.push({icon:"size",label:`${e.size} m²`}),a}function Sa(e={}){const a=Number(e?.price);if(!Number.isFinite(a)||a<=0)return"";const n=x(e?.currency).toUpperCase()||"EUR",o=Number.isInteger(a)?String(a):a.toFixed(2);return n==="EUR"?`€${o}`:`${o} ${n}`}const Q="mnyraDashboardStyles",Me=`
.mnyra-dash {
...
`;function Ie(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(Q)))try{const a=e.createElement("style");a.id=Q,a.textContent=Me,e.head?.appendChild(a)}catch{}}function h(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function f(e,a,n=""){if(typeof e!="function")return"";try{return e(a,n)||""}catch{return""}}const Le=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function Ne({businessType:e="",isShopCatalog:a=!1}={}){if(a)return"shop";const n=String(e||"").trim().toLowerCase();return Le.includes(n)?"hotel":"restaurant"}function Oe(e=new Date().getHours()){const a=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return a>=5&&a<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:a>=11&&a<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:a>=18&&a<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function He({name:e="",logoUrl:a="",hour:n=new Date().getHours(),iconFn:o}={}){const r=Oe(n),d=h(e||"Business");return`
    <div class="mnyra-dash__greet">
      <p class="mnyra-dash__greet-title">
        ...
        <span class="mnyra-dash__greet-logo">
          ${a?`<img src="${h(a)}" alt="${d}" title="${d}" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:`<span class="mnyra-dash__greet-logo-fallback" title="${d}">${f(o,"store","w-4 h-4")}</span>`}
        </span>
      </p>
  ...
  `}const Je="/waiter?from=panel";function Ye({iconFn:e,showEditor:a=!0}={}){return a?`
    <a href="${Je}" class="mnyra-dash__composer mnyra-dash__composer--tap mnyra-dash__composer--waiter" data-dashboard-waiter-card>
  ...
  `:""}function Xe({cards:e=[],iconFn:a}={}){const n=(Array.isArray(e)?e:[]).filter(r=>r&&r.key);if(!n.length)return"";const o=n.map((r,d)=>{const p=h(r.label||"");if(r.pending)return'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>';const m=d<2?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"';let k="";r.imageUrl?k=`<img class="mnyra-dash__hl-media" src="${h(r.imageUrl)}" alt="" ${m} decoding="async" onerror="this.style.display='none'" />`:r.videoUrl&&(k=`<video class="mnyra-dash__hl-media" src="${h(r.videoUrl)}#t=0.1" preload="metadata" muted playsinline disablepictureinpicture tabindex="-1" aria-hidden="true"></video>`);const S=`
      <span class="mnyra-dash__hl-plate">${f(a,r.iconName||"image","w-6 h-6")}</span>
      ${k}
    `,u=r.withEye?`<span class="mnyra-dash__hl-eye">${f(a,"eye","w-4 h-4")}</span>`:"";let y;r.locked?y=`<span class="mnyra-dash__hl-lock">${f(a,"lock","w-3 h-3")}Me pagesë</span>`:r.loading?y='<span class="mnyra-dash__hl-value mnyra-dash__hl-value--pending" aria-hidden="true"></span>':r.emptyText?y=`<span class="mnyra-dash__hl-empty">${h(r.emptyText)}</span>`:y=`<span class="mnyra-dash__hl-value">${u}${h(r.value||"0")}</span>`;let w;r.locked?w=`class="mnyra-dash__hl-card mnyra-dash__hl-card--locked" data-dashboard-metric-locked="${h(r.key)}"`:r.composer?w=`class="mnyra-dash__hl-card" data-dashboard-composer="${h(r.composer)}"`:w=`class="mnyra-dash__hl-card"${r.panelTab?` data-dashboard-panel-tab="${h(r.panelTab)}"`:""}`;const T=r.locked?`${p} – me pagesë`:`${p} ${r.emptyText||r.value||""}`.trim();return`
    ...
    `}).join("");return`
    <div class="mnyra-dash__hl" data-dashboard-metrics="${h(Qe(n))}">
      ${o}
      <span class="mnyra-dash__hl-tail" aria-hidden="true"></span>
    </div>
  `}function Qe(e=[]){return(Array.isArray(e)?e:[]).filter(a=>a&&a.key).map(a=>[a.key,a.label||"",a.value||"",a.emptyText||"",a.imageUrl||"",a.videoUrl||"",a.iconName||"",a.panelTab||"",a.composer||"",a.pending?"p":"",a.loading?"l":"",a.locked?"x":"",a.withEye?"e":""].join("~")).join("|")}const re=Object.freeze([Object.freeze({id:"funksionet",label:"Funksionet",iconName:"layout-grid"}),Object.freeze({id:"analitika",label:"Analitika",iconName:"bar-chart-3"}),Object.freeze({id:"opsionet",label:"Opsionet",iconName:"settings"})]);function E(e=""){const a=String(e||"").trim().toLowerCase();return re.some(n=>n.id===a)?a:"funksionet"}function ea({activeTab:e="funksionet",iconFn:a}={}){const n=E(e);return`<div class="mnyra-dash__tabs" role="tablist" data-dashboard-panel-tabs>${re.map(r=>{const d=r.id===n;return`
      <button
        type="button"
        role="tab"
      ...
      >${f(a,r.iconName,"w-4 h-4")}<span class="mnyra-dash__tab-label">${h(r.label)}</span></button>
    `}).join("")}</div>`}function se(e=""){return`
    <div class="mnyra-dash__bento" data-dashboard-bento>
      ${e}
    </div>
  `}function aa({posts:e=[],iconFn:a}={}){const n=Array.isArray(e)?e:[];let o="";return n.length?(o=n.map(r=>{const d=[r.dateLabel,`${B(r.likesCount||0)} Likes`,`${B(r.commentsCount||0)} Kommentare`];return Number(r.impressions||0)>0&&d.push(`${B(r.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${r.thumbUrl?`<img src="${h(r.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:f(a,r.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
  ...
  `}function ra(){return'<div class="mnyra-dash__skeleton" style="min-height:44px; border-radius:14px; margin: 4px 0 16px;"></div>'}function sa(){const e=Array.from({length:4},()=>'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>').join(""),a=Array.from({length:4},(n,o)=>`<div class="mnyra-dash__skeleton" style="min-height:132px; border-radius:var(--dash-card-radius); margin-top:${o===0?32:22}px;"></div>`).join("");return`
    ${ra()}
  ...
  `}const da="menyra_social_dashboard_cache_v1::",ae="menyra_social_composer_products_v1::",ne=2500,te=1200,la=6,ca=3,ha=Object.freeze({menuImageUrl:"/apps/menyra-social/assets/panel/menu-scan.jpg",qrImageUrl:"/apps/menyra-social/assets/panel/qr-stand.jpg"}),ua=Object.freeze({menuOpens:"Menü-Aufrufe",qrScans:"QR-Scans"});function v(e){const a=Number(e);return Number.isFinite(a)?a:0}function ma(e={}){const a=String(e.createdAtClient||"").trim();if(a){const o=new Date(a);if(!Number.isNaN(o.getTime()))return o}const n=e.createdAt;if(n&&typeof n.toDate=="function")try{const o=n.toDate();if(o instanceof Date&&!Number.isNaN(o.getTime()))return o}catch{}return null}function pa(e="",a={}){const n=Array.isArray(a.media)&&a.media.length?a.media[0]:{},o=String(n.type||a.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",r=String(n.thumbUrl||(o==="image"?n.url:"")||a.thumbUrl||"").trim(),d=o==="video"?String(n.url||a.mediaUrl||"").trim():"",p=ma(a);return{id:String(e||"").trim(),caption:String(a.caption||"").trim(),mediaType:o,thumbUrl:r,videoUrl:d,likesCount:v(a.likesCount),commentsCount:v(a.commentsCount),impressions:0,dateLabel:p?p.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:p?p.getTime():0}}function ba({days:e=[],todayKey:a="",rawPosts:n=[]}={}){const o=Array.isArray(e)?e:[],r=X(o),d=o.find(u=>String(u?.date||u?.id||"").trim()===String(a||"").trim()),p=X(d?[d]:[]),m=r.merged?.posts&&typeof r.merged.posts=="object"?r.merged.posts:{},k=(Array.isArray(n)?n:[]).map(u=>pa(u?.id,u?.data||{})).filter(u=>u.id).map(u=>({...u,impressions:v(m[u.id]?.impressions)})),S=k.slice().sort((u,y)=>y.createdAtMs-u.createdAtMs).slice(0,ca);return{day:String(a||"").trim(),week:r.summary,today:p.summary,posts:S,latestPost:ga(k)}}function ga(e=[]){const a=(Array.isArray(e)?e:[]).filter(n=>n&&n.id);return a.length?a.slice().sort((n,o)=>v(o.createdAtMs)-v(n.createdAtMs)||v(o.impressions)-v(n.impressions)||v(o.likesCount)-v(n.likesCount))[0]:null}function fa({profile:e={},restaurant:a={}}={}){return Re({profile:e,restaurant:a,feature:"qr"})}function ya(e={}){const a=e&&typeof e=="object"?e:{};return String(a.titleImageUrl||a.coverImageUrl||a.coverUrl||a.heroUrl||a.bannerUrl||"").trim()}function _a({model:e=null,coverUrl:a="",subscribed:n=!1,assets:o={}}={}){const r=e?.today||{},d=!e,p=e?.latestPost||null,m=[];if(d)m.push({key:"latestPost",label:"Postimi fundit",pending:!0});else if(!p)m.push({key:"latestPost",label:"Postimi fundit",emptyText:"S'ka postim",iconName:"image",composer:"post"});else{const k=String(p.thumbUrl||"").trim();m.push({key:"latestPost",label:"Postimi fundit",value:B(v(p.impressions)),withEye:!0,imageUrl:k,videoUrl:k?"":String(p.videoUrl||"").trim(),iconName:"image",panelTab:"analitika"})}return m.push({key:"profileViews",label:"Vizitor n'profil",value:B(v(r.profileViews)),withEye:!0,loading:d,imageUrl:String(a||"").trim(),iconName:"user",panelTab:"analitika"}),m.push({key:"menuOpens",label:"Vizitor n'meny",value:B(v(r.menuOpens)),withEye:!0,loading:d&&n,locked:!n,imageUrl:String(o.menuImageUrl||"").trim(),iconName:"book-open",panelTab:"analitika"}),m.push({key:"qrScans",label:"Skanime n'tavolina",value:B(v(r.qrScans)),withEye:!0,loading:d&&n,locked:!n,imageUrl:String(o.qrImageUrl||"").trim(),iconName:"layout-grid",panelTab:"analitika"}),m}function za({state:e,renderFn:a,documentObj:n,firestoreApi:o={},profileApi:r={},composerApi:d={},viewApi:p={},iconFn:m,storageObj:k}={}){const S=n||(typeof document>"u"?null:document),u=S?.defaultView||(typeof window>"u"?null:window),y=typeof a=="function"?a:()=>{},w=k||(typeof localStorage>"u"?null:localStorage),T=typeof r.getBusinessProfileTypeFn=="function"?r.getBusinessProfileTypeFn:(()=>""),ie=typeof r.isShopCatalogProfileFn=="function"?r.isShopCatalogProfileFn:(()=>!1),U=typeof r.getRestaurantMetaByIdFn=="function"?r.getRestaurantMetaByIdFn:(()=>null),oe=typeof r.resolveRestaurantLogoFn=="function"?r.resolveRestaurantLogoFn:(()=>""),de=typeof r.resolveOwnAvatarUrlFn=="function"?r.resolveOwnAvatarUrlFn:(()=>""),le=typeof p.renderAnalyticsViewFn=="function"?p.renderAnalyticsViewFn:(()=>""),ce=typeof p.renderSettingsViewFn=="function"?p.renderSettingsViewFn:(()=>""),he=typeof p.warmAnalyticsFn=="function"?p.warmAnalyticsFn:(()=>{});let N=!1,C=0,O=!1,F=null,R=null,K="",H=!1,V=()=>null;const ue=300;function M(){const t=e?.userProfile||{};return Ne({businessType:T(t),isShopCatalog:ie(t)})}function me(t=""){const s=U(t)||{};return Ue(s).map(i=>({id:i.id,name:i.title,price:i.price??"",category:i.beds||i.tag||"",type:"room",imageUrl:i.imageUrl||""}))}function pe(t=""){if(!w)return null;try{const s=w.getItem(`${ae}${t}`);if(!s)return null;const i=JSON.parse(s),l=Array.isArray(i?.items)?i.items:null;return l&&l.length?l:null}catch{return null}}function be(t="",s=[]){if(w)try{w.setItem(`${ae}${t}`,JSON.stringify({savedAt:Date.now(),items:s}))}catch{}}async function ge(t=""){const{db:s,collectionFn:i,queryFn:l,limitFn:c,getDocsFn:b}=o;if(!s||typeof i!="function"||typeof b!="function")throw new Error("Produktet nuk u ngarkuan.");const z=i(s,"restaurants",t,"menuItems"),_=typeof l=="function"&&typeof c=="function"?l(z,c(ue)):z,D=await b(_),g=[];return D.forEach(P=>{const j=V(P?.id,P?.data?.()||{});j&&g.push(j)}),g.sort((P,j)=>P.name.localeCompare(j.name,"sq")),g}async function fe(t="",s){const i=String(t||"").trim();if(!i)throw new Error("Produktet nuk u ngarkuan.");if(M()==="hotel")return me(i);const l=ge(i).then(b=>(be(i,b),b)),c=pe(i);return c?(typeof s=="function"?l.then(b=>s(b)).catch(()=>{}):l.catch(()=>{}),c):l}function W(){return F?Promise.resolve(F):(R||(R=Pe(()=>import("./business-composer-controller-tM-zhMKO.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(t=>(V=typeof t?.normalizeComposerProductCore=="function"?t.normalizeComposerProductCore:(()=>null),F=t.createBusinessComposerController({documentObj:S,windowObj:S?.defaultView||null,api:{getRestaurantIdFn:()=>$(),getBusinessMetaFn:()=>{const s=$();if(!s)return{name:"",logoUrl:"",city:""};const i=Y(s),l=U(s)||{};return{name:i.name,logoUrl:i.logoUrl,city:String(l.city||"").trim()}},loadProductsFn:(s,i)=>fe(s,i),getBusinessKindFn:()=>M(),uploadImageFn:d.uploadImageFn,uploadVideoFn:d.uploadVideoFn,captureVideoPosterFn:d.captureVideoPosterFn,createPostFn:d.createPostFn,createStoryFn:d.createStoryFn,formatPriceFn:d.formatPriceFn,getOptimizedImageUrlFn:d.getOptimizedImageUrlFn,escapeHtmlFn:d.escapeHtmlFn,iconFn:typeof m=="function"?m:void 0,afterPublishFn:async s=>{try{await A({force:!0})}catch{}typeof d.afterPublishFn=="function"&&await d.afterPublishFn(s)}}}),F)).catch(t=>{throw R=null,console.error("[mnyra][dashboard] composer load failed",t),t})),R)}function Z(){const t=u?.navigator?.connection;return!t||typeof t!="object"?!1:t.saveData===!0?!0:/(^|-)2g$/.test(String(t.effectiveType||"").trim().toLowerCase())}function ye(){if(H||F||!u||Z())return;H=!0;const t=()=>{if(W().catch(()=>{}),typeof d.prewarmFn=="function")try{d.prewarmFn()}catch{}};if(typeof u.requestIdleCallback=="function"){u.requestIdleCallback(t,{timeout:ne});return}u.setTimeout?.(t,te)}function _e(){if(N||!u||Z())return;N=!0;const t=()=>{try{he()}catch{}};if(typeof u.requestIdleCallback=="function"){u.requestIdleCallback(t,{timeout:ne});return}u.setTimeout?.(t,te)}function we(t="post"){const s=String(t||"").trim().toLowerCase(),i=s==="story"||s==="profile"?s:"post";if(typeof d.prewarmFn=="function")try{d.prewarmFn()}catch{}if(F){F.open(i);return}K=i,W().then(l=>{const c=K||i;K="",l?.open?.(c)}).catch(()=>{K=""})}function I(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:"",restaurantId:"",paywall:""}),e.dashboardView}function G(t=""){const s=I(),i=String(t||"").trim();return String(s.restaurantId||"")===i||(s.restaurantId=i,s.model=null,s.status="idle",s.error="",s.loadedSignature="",s.paywall="",C+=1),s}function $(){const t=e?.userProfile||{};return String(t.restaurantId||t.staffRestaurantId||"").trim()}let q="";function ve(){const t=String(e?.user?.uid||"").trim();!t||q===t||typeof r.ensureBusinessProfileFn=="function"&&(q=t,Promise.resolve().then(()=>r.ensureBusinessProfileFn()).catch(s=>{console.warn("[mnyra][panel] business profile could not be resolved",s)}).finally(()=>{String(e?.user?.uid||"").trim()===t&&y()}))}function ke(){const t=String(e?.user?.uid||"").trim();if(!t)return!1;const s=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||s===t}function J(t=""){return`${da}${t}`}function xe(t="",s=""){if(!w||!t)return null;try{const i=w.getItem(J(t));if(!i)return null;const l=JSON.parse(i);return!l||typeof l!="object"||String(l.day||"").trim()!==String(s||"").trim()||!l.model||typeof l.model!="object"?null:l.model}catch{return null}}function Se(t="",s=null){if(!(!w||!t||!s))try{w.setItem(J(t),JSON.stringify({day:s.day,model:s}))}catch{}}async function ze(t=""){const{db:s,collectionFn:i,queryFn:l,orderByFn:c,limitFn:b,getDocsFn:z}=o;if(!s||typeof i!="function"||typeof l!="function"||typeof c!="function"||typeof b!="function"||typeof z!="function")return[];const _=i(s,"restaurants",t,"socialPosts");return(await z(l(_,c("createdAt","desc"),b(la)))).docs.map(g=>({id:g.id,data:g.data()||{}})).filter(g=>{const P=String(g.data.status||"active").trim().toLowerCase();return P!=="deleted"&&P!=="hidden"})}async function A({force:t=!1}={}){const s=$(),i=G(s);if(!s)return;const l=Be({rangeKey:"7d"});if(!l)return;const c=`${s}::${l.toDay}`;if(!t&&i.loadedSignature===c&&i.status==="ready")return;if(!i.model){const _=xe(s,l.toDay);_&&(i.model=_,i.status="ready",y())}C+=1;const b=C;i.model||(i.status="loading",i.error="",y());try{const _={db:o.db,collectionFn:o.collectionFn,queryFn:o.queryFn,whereFn:o.whereFn,documentIdFn:o.documentIdFn,getDocsFn:o.getDocsFn,restaurantId:s},[D,g]=await Promise.allSettled([Ce({..._,fromDay:l.fromDay,toDay:l.toDay}),ze(s)]);if(b!==C)return;if(D.status==="rejected")throw D.reason;g.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",g.reason),i.model=ba({days:D.value,todayKey:l.toDay,rawPosts:g.status==="fulfilled"?g.value:[]}),i.status="ready",i.error="",i.loadedSignature=c,Se(s,i.model)}catch(_){if(b!==C)return;console.error("[mnyra][dashboard] load failed",_),i.model||(i.status="error",i.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}y()}function De(){O||!S||(O=!0,S.addEventListener("click",t=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(t.target?.closest?.("[data-dashboard-retry]")){A({force:!0});return}if(t.target?.closest?.("[data-dashboard-paywall-close]")){t.preventDefault(),I().paywall="",y();return}const s=t.target?.closest?.("[data-dashboard-metric-locked]");if(s){t.preventDefault(),I().paywall=String(s.getAttribute("data-dashboard-metric-locked")||"").trim(),y();return}const i=t.target?.closest?.("[data-dashboard-composer]");if(i){t.preventDefault(),we(i.getAttribute("data-dashboard-composer"));return}const l=t.target?.closest?.("[data-dashboard-panel-tab]");if(l){t.preventDefault();const c=E(l.getAttribute("data-dashboard-panel-tab"));if(c===E(e?.dashboardPanelTab))return;e.dashboardPanelTab=c,y()}}catch{}}))}function Y(t=""){const s=e?.userProfile||{},i=t?U(t)||{}:{},l=String(i.name||i.restaurantName||s.name||"").trim()||"Business";let c="";try{c=String(de()||"").trim()}catch{}if(!c)try{c=String(oe(i)||"").trim()}catch{}return{name:l,logoUrl:c,kind:M(),coverUrl:ya(i),subscribed:fa({profile:s,restaurant:i})}}function Fe(){Ie(S),De();const t=$(),s=G(t);let i="";if(!t)ve(),i=ke()?sa():oa();else{ye(),_e();const l=Y(t),c=E(e?.dashboardPanelTab);s.status==="idle"&&(s.status="loading",queueMicrotask(()=>{A({force:!1})}));let b="";s.model?b=aa({posts:s.model.posts,iconFn:m}):s.status==="error"?b=ia({message:s.error}):b=na();const z=`
        ${Ve({iconFn:m})}
        ${Ye({iconFn:m,showEditor:!!t})}
        ${We({iconFn:m,showEditor:!!t})}
        ${Ze({iconFn:m,showEditor:!!t})}
        ${qe({iconFn:m,kind:l.kind,showEditor:!!t})}
      `;let _;c==="analitika"?_=`
          <div class="mnyra-dash__embed">${le()}</div>
          ${b}
        `:c==="opsionet"?_=`<div class="mnyra-dash__embed">${ce()}</div>`:_=z;const D=_a({model:s.model,coverUrl:l.coverUrl,subscribed:l.subscribed,assets:ha}),g=String(s.paywall||"").trim();i=`
        ${He({name:l.name,logoUrl:l.logoUrl,iconFn:m})}
        ${Xe({cards:D,iconFn:m})}
        ${se(`
          ${ea({activeTab:c,iconFn:m})}
          ${_}
        `)}
        ${g?ta({title:ua[g]||"Me pagesë"}):""}
      `}return`
      <section class="mnyra-dash" data-dashboard-root>
        ${i}
      </section>
    `}return Object.freeze({renderDashboardView:Fe,loadDashboard:A})}export{$e as M,za as a,Ue as b,Ae as c,xa as d,Sa as f,Te as n};
