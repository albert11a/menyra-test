...
`;function ct(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(se)))try{const t=e.createElement("style");t.id=se,t.textContent=ot,e.head?.appendChild(t)}catch{}}function b(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function ye(e=""){const t=/^(\d{4})-(\d{2})-(\d{2})$/.exec(String(e||""));return t?`${t[3]}.${t[2]}.`:e}function lt({rangeKey:e="7d",customFrom:t="",customTo:r=""}={}){const n=H.map(s=>`
    <button
      type="button"
      class="mnyra-an__chip ${s.key===e?"mnyra-an__chip--active":""}"
      data-analytics-range="${b(s.key)}"
    >${b(s.label)}</button>
  `).join(""),a=e==="custom"?`
      <div class="mnyra-an__custom">
        <input type="date" data-analytics-custom-from value="${b(t)}" aria-label="Nga" />
        <span style="color:var(--an-muted); font-size:11px; font-weight:700;">deri</span>
        <input type="date" data-analytics-custom-to value="${b(r)}" aria-label="Deri" />
        <button type="button" class="mnyra-an__chip mnyra-an__chip--active" data-analytics-custom-apply>Anwenden</button>
      </div>
    `:"";return`<div class="mnyra-an__filters" role="group" aria-label="Periudha">${n}</div>${a}`}function ut(e,{moreIsGood:t=!0}={}){if(e===void 0)return"";const r=Xe(e);let n="";if(e!==null&&e!==0){const a=e>0;n=(t?a:!a)?"mnyra-an__kpi-delta--up":"mnyra-an__kpi-delta--down"}return`<p class="mnyra-an__kpi-delta ${n}">${b(r)} <span style="color:var(--an-muted); font-weight:600;">vs. Vorperiode</span></p>`}function D({label:e="",value:t=0,delta:r,suffix:n="",moreIsGood:a=!0}={}){return`
    <div class="mnyra-an__card">
      <p class="mnyra-an__kpi-label">${b(e)}</p>
      <p class="mnyra-an__kpi-value">${b(L(t))}${n?` <span style="font-size:12px; color:var(--an-muted); font-weight:700;">${b(n)}</span>`:""}</p>
      ${ut(r,{moreIsGood:a})}
    </div>
  `}function dt({series:e=[],chartId:t="trend",height:r=150}={}){const n=(Array.isArray(e)?e:[]).filter(h=>Array.isArray(h?.points)&&h.points.length);if(!n.length)return"";const a=640,s={top:12,right:12,bottom:24,left:34},i=a-s.left-s.right,o=r-s.top-s.bottom,l=n[0].points.map(h=>h.day),m=l.length,_=Math.max(1,...n.flatMap(h=>h.points.map(k=>Number(k.value)||0))),c=Math.max(1,Math.ceil(_/4)*4),d=h=>s.left+(m<=1?i/2:h/(m-1)*i),v=h=>s.top+o-Math.max(0,Number(h)||0)/c*o,S=h=>String(h)==="2"?"var(--an-series-2)":"var(--an-series-1)",w=[0,.25,.5,.75,1].map(h=>{const k=s.top+o-h*o,A=Math.round(c*h);return`
      <line x1="${s.left}" y1="${k}" x2="${a-s.right}" y2="${k}" stroke="var(--an-grid)" stroke-width="1" />
      <text x="${s.left-6}" y="${k+3}" text-anchor="end" font-size="9" fill="var(--an-muted)" style="font-variant-numeric: tabular-nums;">${A.toLocaleString("de-DE")}</text>
    `}).join(""),I=Math.max(1,Math.ceil(m/6)),y=l.map((h,k)=>k%I!==0&&k!==m-1?"":`<text x="${d(k)}" y="${r-6}" text-anchor="middle" font-size="9" fill="var(--an-muted)">${b(ye(h))}</text>`).join(""),f=n.map(h=>{const k=h.points.map((he,G)=>`${G===0?"M":"L"}${d(G).toFixed(1)},${v(he.value).toFixed(1)}`).join(" "),A=`${k} L${d(m-1).toFixed(1)},${(s.top+o).toFixed(1)} L${d(0).toFixed(1)},${(s.top+o).toFixed(1)} Z`,q=m-1,R=`<circle cx="${d(q).toFixed(1)}" cy="${v(h.points[q].value).toFixed(1)}" r="4" fill="${S(h.color)}" stroke="var(--an-surface)" stroke-width="2" />`;return`
      <path d="${A}" fill="${S(h.color)}" opacity="0.10" />
      <path d="${k}" fill="none" stroke="${S(h.color)}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" />
      ${R}
    `}).join(""),g=n.length>1?`
      <div class="mnyra-an__legend">
        ${n.map(h=>`
          <span class="mnyra-an__legend-item">
            <span class="mnyra-an__legend-swatch" style="background:${S(h.color)};"></span>${b(h.label)}
          </span>
        `).join("")}
      </div>
    `:"",$=b(JSON.stringify({days:l,pad:s,width:a,height:r,series:n.map(h=>({label:h.label,color:String(h.color)==="2"?"2":"1",values:h.points.map(k=>Number(k.value)||0)}))}));return`
    ${g}
    <div class="mnyra-an__chart-wrap" data-analytics-chart="${b(t)}" data-analytics-chart-payload="${$}">
  ...
  `}function ie({rows:e=[],label:t="",chartId:r="hourly"}={}){const n=Array.isArray(e)?e:[];if(n.reduce((w,I)=>w+(Number(I?.value)||0),0)<=0)return`<p class="mnyra-an__empty-note">Ende nuk ka te dhena per „${b(t)}“ ne kete periudhe.</p>`;const s=640,i=120,o={top:8,right:8,bottom:20,left:8},l=s-o.left-o.right,m=i-o.top-o.bottom,_=Math.max(1,...n.map(w=>Number(w?.value)||0)),c=l/24,d=Math.min(20,Math.max(6,c-2)),v=n.map(w=>{const I=Number(w?.value)||0,y=I<=0?0:Math.max(2,I/_*m),f=o.left+w.hour*c+(c-d)/2,g=o.top+m-y;return I<=0?"":`<path d="M${f.toFixed(1)},${(o.top+m).toFixed(1)} L${f.toFixed(1)},${(g+4).toFixed(1)} Q${f.toFixed(1)},${g.toFixed(1)} ${(f+4).toFixed(1)},${g.toFixed(1)} L${(f+d-4).toFixed(1)},${g.toFixed(1)} Q${(f+d).toFixed(1)},${g.toFixed(1)} ${(f+d).toFixed(1)},${(g+4).toFixed(1)} L${(f+d).toFixed(1)},${(o.top+m).toFixed(1)} Z"
          fill="var(--an-series-1)"><title>${b(`${w.hour}:00 Uhr – ${I.toLocaleString("de-DE")}`)}</title></path>`}).join(""),S=[0,6,12,18,23].map(w=>`
    <text x="${(o.left+w*c+c/2).toFixed(1)}" y="${i-5}" text-anchor="middle" font-size="9" fill="var(--an-muted)">${w} Uhr</text>
  `).join("");return`
    <div class="mnyra-an__chart-wrap" data-analytics-chart="${b(r)}">
      <svg viewBox="0 0 ${s} ${i}" width="100%" height="auto" role="img" aria-label="${b(t)}">
  ...
  `}function pt(e=[]){const t=Array.isArray(e)?e:[];if(!t.length)return"";const r=Math.max(1,...t.map(n=>Number(n?.value)||0));return t.map((n,a)=>{const s=Number(n?.value)||0,i=Math.max(2,Math.round(s/r*100)),o=a>0&&n.dropFromPrev>0?` <span style="color:var(--an-down);">(−${n.dropFromPrev.toLocaleString("de-DE")} %)</span>`:"";return`
      <div class="mnyra-an__bar-row">
        <span class="mnyra-an__bar-label">${b(n.label)}</span>
        <div class="mnyra-an__bar-track"><div class="mnyra-an__bar-fill" style="width:${i}%; opacity:${1-a*.15};"></div></div>
        <span class="mnyra-an__bar-value">${b(L(s))}${o}</span>
      </div>
    `}).join("")}function B(e=[],{labelKey:t="label",valueKey:r="count",suffix:n=""}={}){const a=Array.isArray(e)?e:[];if(!a.length)return'<p class="mnyra-an__empty-note">Ende nuk ka te dhena ne kete periudhe.</p>';const s=Math.max(1,...a.map(i=>Number(i?.[r])||0));return a.map(i=>{const o=Number(i?.[r])||0,l=Math.max(2,Math.round(o/s*100)),m=n==="%"&&Number.isFinite(Number(i?.share))?` · ${Number(i.share).toLocaleString("de-DE")} %`:"";return`
      <div class="mnyra-an__bar-row">
        <span class="mnyra-an__bar-label">${b(String(i?.[t]||""))}</span>
        <div class="mnyra-an__bar-track"><div class="mnyra-an__bar-fill" style="width:${l}%;"></div></div>
        <span class="mnyra-an__bar-value">${b(L(o))}${b(m)}</span>
      </div>
    ...
    <div class="mnyra-an__table-scroll">
      <table class="mnyra-an__table">
        <thead><tr>${e.map(n=>`<th>${b(n.label)}</th>`).join("")}</tr></thead>
        <tbody>
          ${t.map(n=>`
            <tr>${e.map(a=>`<td>${b(String(a.render?a.render(n):n[a.key]??""))}</td>`).join("")}</tr>
          `).join("")}
        </tbody>
      </table>
    </div>
  `:`<p class="mnyra-an__empty-note">${b(r)}</p>`}function mt(){return`
  ...
  `}function yt(e=null,{currency:t="€"}={}){if(!e)return J({});const{summary:r,deltas:n}=e;if(!e.hasAnyData)return J({body:`Ne periudhen „${e.range.label}“ ende nuk ka te dhena. Ndaj profilin tend, QR kodet e tua ose posto ne feed per te rritur shtrirjen.`});const a=[D({label:"Vizitore te profilit",value:r.profileViews,delta:n.profileViews}),D({label:"Vizitore unike",value:r.uniqueVisitors,delta:n.uniqueVisitors}),D({label:"Shikime te menuse",value:r.menuOpens,delta:n.menuOpens}),D({label:"Produkt-Ansichten",value:r.productViews,delta:n.productViews}),D({label:"Beitrags-Impressionen",value:r.postImpressions,delta:n.postImpressions}),D({label:"QR-Scans",value:r.qrScans,delta:n.qrScans}),D({label:"Thirrje kamarieri",value:r.waiterCalls,delta:n.waiterCalls}),D({label:"Porosite",value:r.ordersCompleted,delta:n.ordersCompleted}),D({label:"Umsatz",value:r.revenue,delta:n.revenue,suffix:t}),D({label:"Ø Bestellwert",value:r.avgOrderValue,suffix:t}),D({label:"Shtrirja ne feed",value:r.feedImpressions,delta:n.feedImpressions}),D({label:"Feed-Klickrate",value:r.feedCtr,suffix:"%"})].join(""),s=dt({chartId:"profileTrend",series:[{key:"profileViews",label:"Vizitore te profilit",color:"1",points:e.trend.profileViews},{key:"menuOpens",label:"Shikime te menuse",color:"2",points:e.trend.menuOpens}]}),i=K({columns:[{key:"name",label:"Produkt"},{key:"views",label:"Ansichten"},{key:"orders",label:"Porosite"},{key:"revenue",label:`Umsatz (${t})`,render:d=>d.revenue>0?d.revenue.toLocaleString("de-DE"):"–"}],rows:e.topProducts,emptyLabel:"Ende nuk ka te dhena produktesh ne kete periudhe."}),o=e.lowProducts.length&&e.topProducts.length>e.lowProducts.length?`
    ...
    `:"",_=e.categories.length?`
      <div class="mnyra-an__section">
        ...
        <div class="mnyra-an__card">
          ${B(e.categories,{labelKey:"name",valueKey:"opens"})}
        </div>
      </div>
    `:"",c=[{label:"Telefon",count:e.contacts.phone},{label:"Adresa",count:e.contacts.address},{label:"Harta",count:e.contacts.map},{label:"Orari i punes",count:e.contacts.hours},{label:"Social Links",count:e.contacts.social}].filter(d=>d.count>0);return`
    ...
    <div class="mnyra-an__section">
      ...
      <div class="mnyra-an__card">
        ${pt(e.funnel)}
        <p class="mnyra-an__empty-note">Konvertimi menu → porosi: ${b(String(r.orderConversion.toLocaleString("de-DE")))} % · Porosi me QR: ${b(L(r.ordersQr))} · Te jashtme: ${b(L(r.ordersExternal))}</p>
      </div>
    </div>

    <div class="mnyra-an__section">
      <h3 class="mnyra-an__section-title">Klickquellen</h3>
      <div class="mnyra-an__card">${B(e.sources,{suffix:"%"})}</div>
    </div>
    ...
    <div class="mnyra-an__section">
      ...
      <div class="mnyra-an__card">${ie({rows:e.hourlyWaiter,label:"Thirrje kamarieri",chartId:"hourlyWaiter"})}</div>
    </div>

    ${c.length?`
      <div class="mnyra-an__section">
        <h3 class="mnyra-an__section-title">Kontakt-Klicks</h3>
        <div class="mnyra-an__card">${B(c)}</div>
      </div>
    `:""}

    ${m}
  `}function ht(e=typeof document>"u"?null:document){e?.querySelectorAll&&e.querySelectorAll("[data-analytics-chart-payload]").forEach(t=>{if(t.__mnyraAnalyticsChartBound)return;t.__mnyraAnalyticsChartBound=!0;let r=null;try{r=JSON.parse(t.getAttribute("data-analytics-chart-payload")||"null")}catch{}if(!r||!Array.isArray(r.days)||!r.days.length)return;const n=t.querySelector("svg"),a=t.querySelector("[data-analytics-crosshair]"),s=t.querySelector("[data-analytics-tooltip]");if(!n||!a||!s)return;const{pad:i,width:o}=r,l=o-i.left-i.right,m=r.days.length,_=()=>{a.setAttribute("opacity","0"),s.classList.remove("mnyra-an__tooltip--visible")},c=d=>{const v=n.getBoundingClientRect();if(!v.width)return;const S=o/v.width,w=(d.clientX-v.left)*S,I=Math.min(1,Math.max(0,(w-i.left)/Math.max(1,l))),y=Math.round(I*(m-1)),f=i.left+(m<=1?l/2:y/(m-1)*l);a.setAttribute("x1",String(f)),a.setAttribute("x2",String(f)),a.setAttribute("opacity","0.6"),s.textContent="";const g=e.createElement?e.createElement("div"):document.createElement("div");g.textContent=ye(r.days[y]),g.style.fontWeight="800",s.appendChild(g),r.series.forEach($=>{const h=document.createElement("div"),k=document.createElement("span");k.className="mnyra-an__tooltip-key",k.style.background=$.color==="2"?"var(--an-series-2)":"var(--an-series-1)";const A=document.createElement("span");A.className="mnyra-an__tooltip-value",A.textContent=Number($.values[y]||0).toLocaleString("de-DE");const q=document.createElement("span");q.textContent=` ${$.label}`,h.appendChild(k),h.appendChild(A),h.appendChild(q),s.appendChild(h)}),s.style.left=`${f/o*100}%`,s.style.top="0px",s.classList.add("mnyra-an__tooltip--visible")};n.addEventListener("pointermove",c),n.addEventListener("pointerdown",c),n.addEventListener("pointerleave",_)})}function vt({state:e,renderFn:t,documentObj:r,firestoreApi:n={}}={}){const a=r||(typeof document>"u"?null:document),s=typeof t=="function"?t:()=>{};let i=0,o=!1;function l(){return(!e.analyticsView||typeof e.analyticsView!="object")&&(e.analyticsView={status:"idle",error:"",rangeKey:"7d",customFrom:"",customTo:"",loadedRangeSignature:"",model:null}),e.analyticsView}function m(){const y=e?.userProfile||{};return String(y.restaurantId||y.staffRestaurantId||"").trim()}function _(y,f){return`${f}::${y.fromDay}::${y.toDay}`}async function c({force:y=!1}={}){const f=l(),g=m();if(!g){f.status="empty-business";return}const $=He({rangeKey:f.rangeKey,customFrom:f.customFrom,customTo:f.customTo});if(!$){f.status="error",f.error="Ju lutem zgjidhni nje periudhe te vlefshme (data e fillimit para dates se mbarimit).",s();return}const h=_($,g);if(!y&&f.loadedRangeSignature===h&&f.status==="ready")return;i+=1;const k=i;f.status="loading",f.error="",s();try{const A={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:g},[q,R]=await Promise.all([ae({...A,fromDay:$.fromDay,toDay:$.toDay}),ae({...A,fromDay:$.prevFromDay,toDay:$.prevToDay})]);if(k!==i)return;f.model=it({range:$,currentDays:q,previousDays:R}),f.loadedRangeSignature=h,f.status="ready"}catch(A){if(k!==i)return;console.error("[mnyra][analytics] dashboard load failed",A),f.status="error",f.error="Analitika nuk mund te ngarkohej."}s()}function d(y="7d"){const f=l();f.rangeKey=String(y||"7d").trim()||"7d",f.rangeKey!=="custom"&&c({force:!1}),s()}function v(){const y=l(),f=a?.querySelector?.("[data-analytics-custom-from]"),g=a?.querySelector?.("[data-analytics-custom-to]");y.customFrom=String(f?.value||y.customFrom||"").trim(),y.customTo=String(g?.value||y.customTo||"").trim(),c({force:!0})}function S(){o||!a||(o=!0,a.addEventListener("click",y=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="analytics")return;const f=y.target?.closest?.("[data-analytics-range]");if(f){d(f.getAttribute("data-analytics-range"));return}if(y.target?.closest?.("[data-analytics-custom-apply]")){v();return}y.target?.closest?.("[data-analytics-retry]")&&c({force:!0})}catch{}}))}function w(){if(!a)return;((a.defaultView||(typeof window>"u"?null:window))?.requestAnimationFrame||(g=>setTimeout(g,0)))(()=>{try{ht(a)}catch{}})}function I(){ct(a),S();const y=l(),f=m();let g="";if(!f)g=J({title:"Nuk ka profil biznesi te lidhur",body:"Analitika eshte e disponueshme vetem per llogari biznesi. Sapo llogaria jote te lidhet me nje restorant ose dyqan, statistikat e tua shfaqen ketu."});else if(y.status==="idle"||y.status==="loading"&&!y.model)y.status==="idle"&&(queueMicrotask(()=>{c({force:!1})}),y.status="loading"),g=mt();else if(y.status==="error")g=ft({message:y.error});else{const $=yt(y.model);g=y.status==="loading"?`<div style="opacity:0.55; pointer-events:none;">${$}</div>`:$}return w(),`
    ...
    `}return Object.freeze({renderAnalyticsView:I,loadAnalytics:c,setRange:d})}export{vt as c,L as f,gt as i,ae as l,_t as o,He as r,ne as s,x as t};
