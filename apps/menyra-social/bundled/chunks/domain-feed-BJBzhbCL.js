function Xo(c={}){const m=String(c?.id||"").trim();if(!m)return"";const C=String(c?.ownerType||(c?.restaurantId||c?.rid?"restaurant":"")||(c?.uid||c?.userId?"user":"")||"").trim(),A=String(c?.ownerId||c?.restaurantId||c?.rid||c?.uid||c?.userId||"").trim();return C&&A?`${C}:${A}:${m}`:`post:${m}`}function Tr(c=null){return c?(c.postEntityMap instanceof Map||(c.postEntityMap=new Map),c.postEntityMap):new Map}function sn(c=null){const m=Tr(c);return m.clear(),m}function Jo(c=null,m=[]){const C=Tr(c),A=new Set;return(Array.isArray(m)?m:[]).map(N=>{const z=Xo(N);if(!z||A.has(z))return null;A.add(z);const B=C.get(z);if(!B){const V={...N||{}};return C.set(z,V),V}return Object.assign(B,N||{}),B}).filter(Boolean)}function cn({state:c=null,toDateSafeFn:m=f=>f,getStoriesRowSignatureFn:C=()=>"",setStoriesRowSignatureFn:A=()=>{},FAST_MODE:N=!1,buildStoriesFromFeedFn:z=()=>[],updateStoryLogoNodesFn:B=()=>{},updateStoryMetaNodesFn:V=()=>{},updateFeedLogoNodesFn:ee=()=>{},updatePostCountNodesFn:de=()=>{},ensureFeedRestaurantMetaListenersFn:tt=()=>{},preloadFeedHeroImagesFn:rt=()=>{},buildStoriesRowSignatureFn:Bt=()=>"",documentObj:ot=null,windowObj:nt=null,isLocalBusinessProfileFn:ze=()=>!1,iconFn:F=()=>"",escapeHtmlFn:p=f=>String(f||""),buildUrlFn:te=()=>"",buildStoryViewerUrlFn:be=(f="",b={})=>te("apps/menyra-social/index.html",{r:f,tab:"profile"}),resolveRestaurantLogoFn:ue=()=>"",resolveStoryRenderIdentityFn:Pe=null,getOptimizedImageUrlFn:h=()=>"",getVerifiedMapLocationFn:_=()=>null,setVerifiedMapLocationFn:E=null,buildUploadStateForIntentFn:j=(f="",b={})=>b,setStateFn:U=()=>{},openGuestAuthPromptFn:W=()=>!1,openOwnBusinessProfileFn:k=null,openProfileViewFromBusinessFn:re=()=>{},openPostModalFn:ke=async()=>{},togglePostLikeFn:Ee=async()=>{},setTimeoutFn:q=(f,b)=>setTimeout(f,b)}={}){if(!c)return{renderHomeView:()=>"",renderFeedView:()=>"",renderStoryItem:()=>"",renderStoriesRow:()=>"",renderFeedItem:()=>"",renderFeedList:()=>"",patchFeedList:()=>!1,patchStoriesRow:()=>!1,updateFeedDom:()=>!1,bindFeedDelegation:()=>{}};const f=ot||(typeof document<"u"?document:null),b=nt||(typeof window<"u"?window:null),K=typeof HTMLVideoElement=="function"?HTMLVideoElement:null,Re="mnyra_story_viewer_hint_v1:",it=()=>!!String(c.userProfile?.uid||"").trim(),at=()=>!!String(c.userProfile?.restaurantId||"").trim(),Ge=()=>!!ze(c.userProfile)||at()&&(!!c.user||it()),st=(e="")=>{const t=String(e||"").trim();return t?t.toLowerCase()==="business"?"":t:""},Ft=(e={})=>String(e?.truthSource||e?.storyTruthSource||e?.storyTruth||"").trim().toLowerCase()==="feed-fallback"?"feed-fallback":"canonical",Ut=15,jr=6,Or=15,Nr=35,zr=Object.freeze(["city","locationCity","primaryCity","place","locationPlace","primaryPlace","postalCity","address","primaryAddress","formattedAddress","fullAddress","addressText","streetAddress","street","locationLabel","displayLocation","locality","town","municipality","village","neighborhood","neighbourhood","area","district","county","region","state","province"]),Pr=Object.freeze(["location","primaryLocation","businessLocation","venueLocation","addressInfo","geo","coords","coordinates","geoPoint"]),Rr="flex:0 0 29%;width:29%;max-width:120px;",Gr="height:13rem;",Br="border-radius:1rem;",Q=new WeakMap;let Be=null,Dt=!1,Vt=!1;const Fe=({withMarginLeft:e=!1}={})=>`${Rr}${e?"margin-left:1.25rem;":""}`,Fr=()=>"margin-left:calc(var(--app-content-inline,1.5rem) * -1);margin-right:calc(var(--app-content-inline,1.5rem) * -1);scroll-padding-left:1.25rem;overscroll-behavior-x:contain;touch-action:pan-x;-webkit-overflow-scrolling:touch;",ct=(e="")=>`${Gr}${Br}position:relative;overflow:hidden;${e}`,w=(e=0)=>{const t=Number(e);return Number.isFinite(t)?t:0},Ur=(e={})=>De(e?.createdAt||e?.updatedAt||e?.timestamp||e?.ts||e?.publishedAt||null),Dr=(e="")=>{const t=String(e||"").trim().toLowerCase();return t?!!(/\.m3u8($|\?)/.test(t)||/\.mpd($|\?)/.test(t)||/\.mp4($|\?)/.test(t)||/\.webm($|\?)/.test(t)||/\.mov($|\?)/.test(t)||/\.m4v($|\?)/.test(t)||/\.ogv($|\?)/.test(t)):!1},lt=(e={})=>{const t=String(e?.mediaType||e?.type||"").trim().toLowerCase(),r=String(e?.imageUrl||e?.thumbUrl||"").trim(),o=String(e?.videoUrl||e?.playbackUrl||"").trim(),n=String(e?.mediaUrl||e?.url||"").trim(),i=String(e?.embedUrl||"").trim(),a=String(e?.image||e?.thumbnail||e?.thumbnailUrl||e?.previewImage||e?.previewUrl||e?.coverImage||e?.poster||e?.posterUrl||"").trim(),l=Dr(n),d=o||(t==="video"||l?n:""),u=r||(t==="image"?n:"");return d?{kind:"video",src:d,poster:u||a,signature:`video:${d}|${u||a||""}`}:u?{kind:"image",src:u,poster:u,signature:`image:${u}`}:a?{kind:"image",src:a,poster:a,signature:`fallback:${a}`}:i?{kind:"embed",src:i,poster:"",signature:`embed:${i}`}:{kind:"none",src:"",poster:"",signature:"none"}},qt=(e={})=>{const t=Ft(e),r=lt(e);return[String(e?.restaurantId||e?.id||"").trim(),t,e?.isLive?"1":"0",r.signature].join("|")},Kt=(e={})=>[String(e?.id||"").trim(),String(e?.business||"").trim(),String(e?.location||"").trim(),String(e?.content||e?.caption||"").trim(),String(e?.image||e?.url||"").trim(),e?.isLive?"1":"0"].join("|"),Vr=(e=[])=>(Array.isArray(e)?e:[]).slice(0,10).map(t=>[String(t?.id||"").trim(),String(t?.image||t?.url||"").trim()].join("|")).join(","),Ue=typeof Pe=="function"?(e={})=>Pe(e):(e={})=>{const t=String(e?.restaurantId||"").trim(),r=Ft(e),o=r==="feed-fallback";if(!t)return{storyRestaurantId:"",hasCanonicalRestaurant:!1,storyLabel:"",logoSource:"",borderClass:e?.isLive?"border-red-500 animate-pulse":o?"border-amber-300 border-dashed":"border-slate-200",truthSource:r};const n=c.restaurants.find(x=>String(x?.id||"").trim()===t)||null,i=String(c.userProfile?.restaurantId||"").trim(),a=i&&i===t,l=!!n?.id,d=String(n?.logoUrl||n?.logo||n?.logoURL||"").trim(),u=st(n?.name||n?.restaurantName||n?.displayName||n?.businessName||""),g=st(e?.name||e?.businessName||e?.restaurantName||""),y=a?st(c.userProfile?.name||""):"",v=a?String(c.userProfile?.avatar||"").trim():"",L=l?u||g||y||"":y||g||u||"",S=l?d||String(e?.img||e?.logo||e?.logoUrl||"").trim():v||String(e?.img||e?.logo||e?.logoUrl||"").trim();return{storyRestaurantId:t,hasCanonicalRestaurant:l,storyLabel:L,logoSource:S,borderClass:e?.isLive?"border-red-500 animate-pulse":o?"border-amber-300 border-dashed":"border-slate-200",truthSource:r}},Yt=(e={})=>!!Ue(e).storyRestaurantId,Ht=(e="")=>{const t=String(e||"").trim();return t&&(Array.isArray(c?.feedPosts)?c.feedPosts:[]).find(o=>String(o?.id||"").trim()===t)||null},De=(e=null)=>{const t=m(e),r=Number(t?.getTime?.()||0);return Number.isFinite(r)?r:0},dt=()=>{const e=new Map;return(Array.isArray(c?.restaurants)?c.restaurants:[]).forEach(r=>{const o=String(r?.id||r?.restaurantId||"").trim();!o||e.has(o)||e.set(o,r)}),e},Wt=(e={})=>String(e?.restaurantId||(String(e?.ownerType||"").trim().toLowerCase()==="restaurant"?e?.ownerId:"")||e?.rid||"").trim(),ut=(e=[],t="")=>{if(typeof t=="string"||typeof t=="number"){const r=String(t||"").trim();r&&e.push(r)}},ft=(e=[],t={})=>{!t||typeof t!="object"||(zr.forEach(r=>ut(e,t[r])),(typeof t.location=="string"||typeof t.location=="number")&&ut(e,t.location))},Ve=(e="")=>P(e).replace(/-/g," ").replace(/\s+/g," ").trim(),Qt=()=>ht.map(e=>[e?.id,e?.label,e?.city,...Array.isArray(e?.aliases)?e.aliases:[]].map(Ve).filter(Boolean)).filter(e=>e.length),qr=(e="")=>{const t=Ve(e);if(!t)return[];const r=new Set([t]);return Qt().forEach(o=>{o.includes(t)&&o.forEach(n=>r.add(n))}),Array.from(r)},Xt=(e=[],t=[])=>{const r=(Array.isArray(e)?e:[]).map(Ve).filter(Boolean);if(!r.length||!t.length)return!1;const o=r.join(" ");return t.some(n=>{const i=n.split(" ").filter(Boolean);return r.some(a=>a===n||a.includes(n))||i.length>0&&i.every(a=>o.includes(a))})},Kr=(e=[])=>{const t=(Array.isArray(e)?e:[]).map(Ve).filter(Boolean);if(!t.length)return!1;const r=t.join(" ");return Qt().some(o=>o.some(n=>{const i=n.split(" ").filter(Boolean);return t.some(a=>a===n||a.includes(n))||i.length>0&&i.every(a=>r.includes(a))}))},Yr=(e={})=>{const t=se(e);if(!t)return"";const r=ht.map(o=>{const n=G(o);return{label:String(o?.label||"").trim(),distanceKm:n?_e(t,n):Number.POSITIVE_INFINITY}}).filter(o=>o.label&&Number.isFinite(o.distanceKm)).sort((o,n)=>o.distanceKm-n.distanceKm)[0];return r&&r.distanceKm<=Nr?r.label:""},Hr=(...e)=>{const t=[];return e.forEach(r=>{!r||typeof r!="object"||(ft(t,r),Pr.forEach(o=>ft(t,r[o])),Array.isArray(r.locations)&&r.locations.forEach(o=>ft(t,o)))}),t},Wr=(...e)=>{const t=[];return e.forEach(r=>ut(t,Yr(r))),t},qe=({entry:e={},restaurant:t=null,viewerCity:r=""}={})=>{const o=qr(r);if(!o.length)return!0;const n=Hr(t,e);return Xt(n,o)?!0:Kr(n)?!1:Xt(Wr(t,e),o)},Jt=({feedPosts:e=[],stories:t=[]}={})=>{const r=Array.isArray(e)?e:[],o=Array.isArray(t)?t:[],n=X(xe()),i=wt(n),a=we(n),l=G(n),d=dt(),u=!!a,g=!!i,y=(S={},{type:x="post",fallbackIndex:I=0}={})=>{const T=Wt(S),O=T&&d.get(T)||null,H=we({...O&&typeof O=="object"?O:{},...S&&typeof S=="object"?S:{}}),ne=x==="story"&&!!S?.isLive;if(u&&!ne&&(!H||H!==a)||g&&!ne&&!qe({entry:S,restaurant:O,viewerCity:i}))return null;const Z=se(O)||se(S),s=l&&Z?_e(l,Z):Number.POSITIVE_INFINITY;return{entry:S,fallbackIndex:I,distanceKm:s,createdAtMs:x==="post"?De(S?.createdAt||S?.updatedAt):0}},v=r.map((S,x)=>y(S,{type:"post",fallbackIndex:x})).filter(Boolean).sort((S,x)=>{const I=Number.isFinite(S.distanceKm),T=Number.isFinite(x.distanceKm);return I&&T&&Math.abs(S.distanceKm-x.distanceKm)>.001?S.distanceKm-x.distanceKm:I!==T?I?-1:1:S.createdAtMs!==x.createdAtMs?x.createdAtMs-S.createdAtMs:S.fallbackIndex-x.fallbackIndex}).map(S=>S.entry),L=o.map((S,x)=>y(S,{type:"story",fallbackIndex:x})).filter(Boolean).sort((S,x)=>{const I=Number.isFinite(S.distanceKm),T=Number.isFinite(x.distanceKm);return I&&T&&Math.abs(S.distanceKm-x.distanceKm)>.001?S.distanceKm-x.distanceKm:I!==T?I?-1:1:S.fallbackIndex-x.fallbackIndex}).map(S=>S.entry);return{feedPosts:v,stories:L}},gt=(e=Number.POSITIVE_INFINITY,t=Number.POSITIVE_INFINITY)=>{const r=Number.isFinite(e),o=Number.isFinite(t);return r&&o&&Math.abs(e-t)>.001?e-t:r!==o?r?-1:1:0},mt=(e={},t={})=>{if(e.likes!==t.likes)return t.likes-e.likes;if(e.visitors!==t.visitors)return t.visitors-e.visitors;if(e.comments!==t.comments)return t.comments-e.comments;if(e.createdAtMs!==t.createdAtMs)return t.createdAtMs-e.createdAtMs;const r=gt(e.distanceKm,t.distanceKm);return r!==0?r:e.fallbackIndex-t.fallbackIndex},Qr=(e=null)=>{const t=new Set;return(e instanceof Set?Array.from(e):Array.isArray(e)?e:[]).forEach(o=>{const n=String(o||"").trim();n&&t.add(n)}),t},Zt=(...e)=>{const t=new Set;return e.forEach(r=>{(Array.isArray(r)?r:[]).forEach(o=>{const n=Ue(o),i=String(o?.restaurantId||o?.storyId||n.storyRestaurantId||"").trim();i&&t.add(i)})}),t},er=(e=[],t=[],r=null)=>{const o=Array.isArray(e)?e:[],n=Array.isArray(t)?t:[],i=o.length?o:n,a=X(xe()),l=wt(a),d=G(a),u=we(a),g=dt(),y=Qr(r),v=new Map,L=(s=null)=>{if(!s||!s.spotId)return;const $=v.get(s.spotId);if(!$){v.set(s.spotId,s);return}mt(s,$)<0&&v.set(s.spotId,s)},S=(s={},$=0)=>{const M=Wt(s),D=String(s?.id||"").trim(),ce=M||(D?`post:${D}`:`idx:${$}`),R=M&&g.get(M)||null,Je=we({...R&&typeof R=="object"?R:{},...s&&typeof s=="object"?s:{}});if(u&&(!Je||Je!==u)||l&&!qe({entry:s,restaurant:R,viewerCity:l}))return null;const Ze=se(R)||se(s),$t=d&&Ze?_e(d,Ze):Number.POSITIVE_INFINITY,jt=Math.max(0,w(s?.likes),w(s?.likesCount)),Ot=Math.max(0,w(s?.comments),w(s?.commentsCount)),Nt=Math.max(0,w(s?.visitors),w(s?.visitorCount),w(s?.visitorsCount),w(s?.views),w(s?.viewCount),w(s?.viewsCount),w(s?.reach),w(s?.reachCount),w(s?.impressions),w(s?.impressionsCount)),zt=Math.max(0,w(s?.rating),w(s?.score),w(s?.stars),w(R?.rating),w(R?.score),w(R?.stars)),Pt=De(s?.createdAt||s?.updatedAt),Rt=String(s?.business||s?.restaurantName||R?.name||R?.restaurantName||"Best Spot").trim()||"Best Spot",et=String(R?.bestSpotLogoUrl||R?.spotLogoUrl||R?.bestSpotLogo||R?.spotLogo||R?.logoUrl||R?.logo||R?.logoURL||s?.bestSpotLogoUrl||s?.spotLogoUrl||s?.logo||s?.image||s?.url||"").trim(),Wo=M?ue(M,et,"avatar",!1):et,Qo=M?te("apps/menyra-social/index.html",{r:M,tab:"profile",source:"best-spot"}):te("apps/menyra-social/index.html",{tab:"feed",post:D,source:"best-spot"});return{spotId:ce,postId:D,restaurantId:M,displayName:Rt,avatarUrl:Wo,profileUrl:Qo,likes:jt,comments:Ot,visitors:Nt,rating:zt,distanceKm:$t,createdAtMs:Pt,fallbackIndex:$}},x=(s={},$=0)=>{const M=String(s?.id||s?.restaurantId||"").trim();if(!M)return null;const D=String(s?.status||"").trim().toLowerCase();if(D==="archived"||D==="deleted"||D==="blocked"||D==="disabled")return null;const ce=we(s);if(u&&(!ce||ce!==u)||l&&!qe({entry:s,restaurant:s,viewerCity:l}))return null;const R=se(s),Je=d&&R?_e(d,R):Number.POSITIVE_INFINITY,Ze=Math.max(0,w(s?.likes),w(s?.likesCount),w(s?.likeCount),w(s?.socialLikes),w(s?.socialLikesCount),w(s?.followersCount),w(s?.followers)),$t=Math.max(0,w(s?.comments),w(s?.commentsCount),w(s?.reviewCount),w(s?.reviews),w(s?.reviewsCount),w(s?.ratingsCount)),jt=Math.max(0,w(s?.visitors),w(s?.visitorCount),w(s?.visitorsCount),w(s?.views),w(s?.viewCount),w(s?.viewsCount),w(s?.reach),w(s?.reachCount),w(s?.impressions),w(s?.impressionsCount),w(s?.ordersCount),w(s?.orders)),Ot=Math.max(0,w(s?.rating),w(s?.score),w(s?.stars)),Nt=De(s?.createdAt||s?.updatedAt||s?.truthUpdatedAt),zt=String(s?.name||s?.restaurantName||s?.displayName||s?.businessName||"Best Spot").trim()||"Best Spot",Pt=String(s?.bestSpotLogoUrl||s?.spotLogoUrl||s?.bestSpotLogo||s?.spotLogo||s?.logoUrl||s?.logo||s?.logoURL||s?.image||s?.coverImage||"").trim(),Rt=ue(M,Pt,"avatar",!1),et=te("apps/menyra-social/index.html",{r:M,tab:"profile",source:"best-spot"});return{spotId:M,postId:"",restaurantId:M,displayName:zt,avatarUrl:Rt,profileUrl:et,likes:Ze,comments:$t,visitors:jt,rating:Ot,distanceKm:Je,createdAtMs:Nt,fallbackIndex:$}};if(i.forEach((s,$)=>{L(S(s,$))}),!i.length||v.size<Ut){let s=i.length+1e3;Array.from(g.values()).forEach($=>{const M=x($,s);s+=1,M&&(v.has(M.spotId)||L(M))})}const I=Array.from(v.values()).filter(s=>{if(!y.size)return!0;const $=String(s?.restaurantId||"").trim();return!$||!y.has($)});if(!I.length)return[];const O=I.some(s=>Number.isFinite(s?.distanceKm))?I.filter(s=>Number.isFinite(s?.distanceKm)):I;if(!O.length)return[];const H=[...O].sort(mt).slice(0,jr),ne=new Set(H.map(s=>s.spotId)),Z=O.filter(s=>!ne.has(s.spotId)).sort((s,$)=>{const M=gt(s.distanceKm,$.distanceKm);return M!==0?M:mt(s,$)});return[...H,...Z].slice(0,Ut).map((s,$)=>({ratingDisplay:(()=>{const M=w(s?.rating);if(M>0)return Math.max(1,Math.min(5,M)).toFixed(1);const D=Math.max(1,w(s?.likes)*1.2+w(s?.visitors)*.05+w(s?.comments)*.6);return(4.2+Math.min(.75,Math.log10(1+D)/4.5)).toFixed(1)})(),...s,rank:$+1,renderSignature:[s.spotId,s.postId,s.displayName,s.avatarUrl,String($+1),String(s.rating||0),String(s.likes||0),String(s.visitors||0),Number(s.distanceKm).toFixed(3),String(s.createdAtMs||0)].join("|")}))},tr=(e={},t={})=>{const r=gt(e.distanceKm,t.distanceKm);if(r!==0)return r;const o=!!e.isLive,n=!!t.isLive;return o!==n?o?-1:1:e.createdAtMs!==t.createdAtMs?t.createdAtMs-e.createdAtMs:e.fallbackIndex-t.fallbackIndex},Xr=(e={},t={})=>{const r=!!e.isLive,o=!!t.isLive;return r!==o?r?-1:1:r&&o?e.createdAtMs!==t.createdAtMs?t.createdAtMs-e.createdAtMs:e.fallbackIndex-t.fallbackIndex:tr(e,t)},rr=(e=[],t=[])=>{const r=Array.isArray(e)?e:[],o=Array.isArray(t)?t:[],n=r.length?r:o;if(!n.length)return[];const i=X(xe()),a=wt(i),l=G(i),d=dt(),u=new Map;n.forEach((L,S)=>{const x=Ue(L),I=x.storyRestaurantId;if(!I)return;const T=d.get(I)||null;if(a&&!qe({entry:L,restaurant:T,viewerCity:a}))return;const O=se(T)||se(L),H=l&&O?_e(l,O):Number.POSITIVE_INFINITY,ne=Ur(L),Z=lt(L),s=String(x.storyLabel||"").trim()||"Story",$=be(I),M=ue(I,String(x.logoSource||"").trim(),"thumb",!1),D={...L,storyId:I,restaurantId:I,storyLabel:s,storyUrl:$,profileImageUrl:M,preview:Z,distanceKm:H,createdAtMs:ne,isLive:!!L?.isLive,fallbackIndex:S},ce=u.get(I);if(!ce){u.set(I,D);return}tr(D,ce)<0&&u.set(I,D)});const g=Array.from(u.values());return(g.some(L=>Number.isFinite(L?.distanceKm)&&!L?.isLive)?g.filter(L=>!!L?.isLive||Number.isFinite(L?.distanceKm)):g).sort(Xr).slice(0,Or)},or=({spots:e=[],stories:t=[]}={})=>{const r=Array.isArray(e)?e:[],o=Array.isArray(t)?t:[];if(!r.length&&!o.length)return[];const n=o.filter(g=>!!g?.isLive),i=o.filter(g=>!g?.isLive),a=n.map(g=>({type:"story",story:g}));if(!r.length&&!i.length)return a;if(!i.length)return[...a,...r.map(g=>({type:"spot",spot:g}))];if(!r.length)return[...a,...i.map(g=>({type:"story",story:g}))];const l=[];let d=0,u=0;for(;d<r.length||u<i.length;)d<r.length&&(l.push({type:"spot",spot:r[d]}),d+=1),u<i.length&&(l.push({type:"story",story:i[u]}),u+=1);return[...a,...l]},Jr=({spots:e=[],stories:t=[]}={})=>or({spots:e,stories:t}).map(r=>{if(r.type==="spot"){const o=r?.spot||{};return["spot",String(o?.spotId||"").trim(),String(o?.postId||"").trim(),String(o?.displayName||"").trim(),String(o?.avatarUrl||"").trim(),String(o?.rank||"").trim()].join(":")}return`story:${qt(r?.story||{})}`}).join(","),nr=async(e="")=>{const t=String(e||"");if(!t)return!1;try{if(b?.navigator?.clipboard?.writeText)return await b.navigator.clipboard.writeText(t),!0}catch{}if(!f?.body)return!1;const r=f.createElement("textarea");r.value=t,r.setAttribute("readonly","readonly"),r.style.position="fixed",r.style.opacity="0",r.style.pointerEvents="none",f.body.appendChild(r),r.select();let o=!1;try{o=!!f.execCommand?.("copy")}catch{}return r.remove(),o},pt=(e,t="Link kopiert")=>{if(!(e instanceof HTMLElement))return;const r=e.querySelector("[data-feed-share-label]");if(!r)return;const o=e.dataset.shareDefaultLabel||r.textContent||"Share";e.dataset.shareDefaultLabel=o,r.textContent=t,e.classList.add("text-white"),e.classList.remove("text-white/70"),e._shareFeedbackTimer&&clearTimeout(e._shareFeedbackTimer),e._shareFeedbackTimer=q(()=>{r.textContent=o,e.classList.add("text-white/70"),e.classList.remove("text-white"),e._shareFeedbackTimer=null},1800)},Zr=(e={})=>{const t={post:e?.id||""},r=String(e?.restaurantId||(String(e?.ownerType||"").trim()==="restaurant"?e?.ownerId:"")||"").trim();return r?(t.r=r,t.tab="profile"):t.tab="feed",te("apps/menyra-social/index.html",t)},eo=(e="")=>{const t=String(e||"").trim();if(!t)return null;const r=c.restaurants.find(l=>String(l?.id||"").trim()===t)||null,o=String(c.userProfile?.restaurantId||"").trim(),n=o&&o===t,i=String(r?.name||r?.restaurantName||r?.displayName||r?.businessName||(n?c.userProfile?.name:"")||"").trim(),a=String(r?.logoUrl||r?.logo||r?.logoURL||(n?c.userProfile?.avatar:"")||"").trim();return{id:t,restaurantName:i,name:i,logoUrl:a,logo:a}},to=(e="",t="")=>{const r=String(e||"").trim();if(!r||!b||!f)return;const o=eo(r);if(o&&b.sessionStorage)try{b.sessionStorage.setItem(`${Re}${r}`,JSON.stringify({restaurantId:r,meta:o,savedAt:Date.now()}))}catch{}const n=String(t||be(r)||"").trim();if(!n||!f.head||Array.from(f.head.querySelectorAll("link[data-story-prefetch]")).find(l=>String(l?.getAttribute?.("href")||"").trim()===n))return;const a=f.createElement("link");a.rel="prefetch",a.href=n,a.as="document",a.crossOrigin="anonymous",a.dataset.storyPrefetch="1",f.head.appendChild(a)},ro=()=>{q(()=>{const e=f?.getElementById("postCommentInput");if(e instanceof HTMLElement){try{e.focus({preventScroll:!1})}catch{try{e.focus()}catch{}}try{e.scrollIntoView({block:"nearest",behavior:"smooth"})}catch{}if(typeof e.setSelectionRange=="function"){const t=String(e.value||"").length;try{e.setSelectionRange(t,t)}catch{}}}},90)},ve="mnyra_social_feed_viewer_location_v1",oo=14,ir=3,no=72,io=620,ao=4200,so=12e4,co=new Set(["xk","al","rs"]),lo=new Set(["city","town","village","hamlet","municipality"]),uo=new Map([["xk","Kosove"],["kosove","Kosove"],["kosova","Kosove"],["kosovo","Kosove"],["al","Shqiperi"],["shqiperi","Shqiperi"],["shqiperia","Shqiperi"],["albania","Shqiperi"],["rs","Serbi"],["serbi","Serbi"],["serbia","Serbi"],["srbija","Serbi"]]),fo=new Map([["xk","xk"],["kosove","xk"],["kosova","xk"],["kosovo","xk"],["al","al"],["shqiperi","al"],["shqiperia","al"],["albania","al"],["rs","rs"],["serbi","rs"],["serbia","rs"],["srbija","rs"]]),go=["rruga","street","bulevard","boulevard","lagj","district","neighborhood","quarter","park","mall","plaza"],ht=Object.freeze([{id:"prishtina",label:"Prishtina",lat:42.6629,lng:21.1655,aliases:["prishtine","prishtin","pristina"]},{id:"prizren",label:"Prizren",lat:42.2139,lng:20.7397,aliases:["prizr","prizreni"]},{id:"peja",label:"Peja",lat:42.6591,lng:20.2883,aliases:["peje","pec"]},{id:"gjakova",label:"Gjakova",lat:42.3803,lng:20.4308,aliases:["gjakove","djakova"]},{id:"ferizaj",label:"Ferizaj",lat:42.3706,lng:21.1553,aliases:["feri","ferizaji","uroshevac"]},{id:"gjilan",label:"Gjilan",lat:42.4635,lng:21.4699,aliases:["gjilani"]},{id:"mitrovica",label:"Mitrovica",lat:42.8914,lng:20.866,aliases:["mitrovice","mitro"]},{id:"vushtrria",label:"Vushtrria",lat:42.8231,lng:20.9675,aliases:["vushtrri"]},{id:"podujeva",label:"Podujeva",lat:42.9106,lng:21.193,aliases:["podujeve","podu"]},{id:"tirana",label:"Tirana",lat:41.3275,lng:19.8187,aliases:["tirane"],country:"Shqiperi"},{id:"kukes",label:"Kukes",lat:42.0769,lng:20.4219,aliases:["kukes albania"],country:"Shqiperi"},{id:"smederevo",label:"Smederevo",lat:44.6644,lng:20.9276,aliases:["smederevo serbia"],country:"Serbi"}]),yt=Object.freeze({xk:Object.freeze({minLat:41.85,maxLat:43.35,minLng:20,maxLng:21.85}),al:Object.freeze({minLat:39.55,maxLat:42.75,minLng:19,maxLng:21.1}),rs:Object.freeze({minLat:42.2,maxLat:46.3,minLng:18.7,maxLng:23.1})}),mo=Object.freeze({xk:Object.freeze(["prishtina","prishtine","prizren","peja","peje","gjakova","gjakove","ferizaj","gjilan","mitrovica","mitrovice","vushtrria","vushtrri","podujeva","podujeve"]),al:Object.freeze(["tirana","tirane","kukes","durres","vlore","shkoder","elbasan","fier","korce","sarande"]),rs:Object.freeze(["smederevo","beograd","belgrade","novi sad","nis","kragujevac","subotica","pancevo"])});let Ce=null,Y=!1,fe="idle",Ke="",Ae=null,ie=null,ge="",Te=null,bt=0,me=!1,Me=null,Ye=!1,ar="",Se=!1,oe=null,vt=null,sr=!1,St=null;const ae=new Map,He=new Map,P=(e="")=>String(e||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9\s-]/g," ").replace(/\s+/g," ").trim(),Lt=(e="")=>{const t=P(e);return t==="current location"||t==="vendndodhja aktuale"||t==="trenutna lokacija"||t==="standort"||t==="aktueller standort"},wt=(e=null)=>{const t=String(e?.city||"").trim();if(t&&!Lt(t))return t;const r=String(e?.label||"").trim();return r&&!Lt(r)?r:""},Le=(e="")=>{const t=P(e);return t&&fo.get(t)||""},pe=(e="")=>{const t=P(e);return t&&uo.get(t)||""},cr=(e=null)=>{const t=G(e);if(!t)return"";const r=Number(t.lat),o=Number(t.lng),n=(i=null)=>!!i&&r>=Number(i.minLat)&&r<=Number(i.maxLat)&&o>=Number(i.minLng)&&o<=Number(i.maxLng);return n(yt.xk)?"xk":n(yt.al)?"al":n(yt.rs)?"rs":""},po=()=>{if(St instanceof Map)return St;const e=new Map,t=(r="",o="")=>{const n=P(r),i=Le(o);!n||!i||e.set(n,i)};return Object.entries(mo).forEach(([r,o])=>{(Array.isArray(o)?o:[]).forEach(n=>t(n,r))}),It().forEach(r=>{const o=Le(r?.countryCode||r?.country);o&&(t(r?.label,o),t(r?.city,o),(Array.isArray(r?.searchTerms)?r.searchTerms:[]).forEach(n=>t(n,o)))}),St=e,e},ho=(...e)=>{const t=po();for(const r of e){const o=Le(r);if(o)return o;const n=P(r);if(!n)continue;const i=t.get(n);if(i)return i;const a=n.split(" ").filter(Boolean);for(const l of a){const d=t.get(l);if(d)return d}}return""},we=(e=null)=>!e||typeof e!="object"?"":Le(e?.countryCode||e?.country_code||e?.country||e?.geo?.countryCode||e?.geo?.country_code||e?.geo?.country||e?.coords?.countryCode||e?.coords?.country||e?.location?.countryCode||e?.location?.country||"")||ho(e?.country,e?.geo?.country,e?.coords?.country,e?.location?.country,e?.city,e?.geo?.city,e?.coords?.city,e?.location?.city,e?.label,e?.address,e?.location)||cr(e),se=(e=null)=>!e||typeof e!="object"?null:G(e)||G({lat:e?.latitude,lng:e?.longitude})||G({lat:e?.gpsLat,lng:e?.gpsLng})||G({lat:e?.geo?.lat,lng:e?.geo?.lng})||G({lat:e?.geo?.latitude,lng:e?.geo?.longitude})||G({lat:e?.coords?.lat,lng:e?.coords?.lng})||G({lat:e?.coords?.latitude,lng:e?.coords?.longitude})||G({lat:e?.location?.lat,lng:e?.location?.lng}),_e=(e=null,t=null)=>{const r=G(e),o=G(t);if(!r||!o)return Number.POSITIVE_INFINITY;const n=6371,i=S=>S*Math.PI/180,a=i(Number(o.lat)-Number(r.lat)),l=i(Number(o.lng)-Number(r.lng)),d=i(Number(r.lat)),u=i(Number(o.lat)),g=Math.sin(a/2),y=Math.sin(l/2),v=g*g+Math.cos(d)*Math.cos(u)*y*y,L=2*Math.atan2(Math.sqrt(v),Math.sqrt(1-v));return n*L},G=(e=null)=>{const t=Number(e?.lat??e?.latitude??e?.y),r=Number(e?.lng??e?.lon??e?.longitude??e?.x);return!Number.isFinite(t)||!Number.isFinite(r)?null:{lat:t,lng:r}},X=(e=null)=>{const t=G(e);if(!t)return null;const r=String(e?.label||e?.city||"").trim(),n=String(e?.city||"").trim()||(Lt(r)?"":r),i=String(e?.source||"").trim().toLowerCase(),a=we({...e&&typeof e=="object"?e:{},lat:t.lat,lng:t.lng,label:r,city:n}),l=pe(e?.country||e?.countryCode||e?.country_code||a)||pe(a)||"";return{lat:t.lat,lng:t.lng,label:r,city:n,country:l,countryCode:a,source:i||"manual",savedAt:Number(e?.savedAt||Date.now())||Date.now()}},yo=()=>{if(!b?.localStorage)return null;try{const e=b.localStorage.getItem(ve);if(!e)return null;const t=X(JSON.parse(e));if(t)return t;try{b.localStorage.removeItem(ve)}catch{}return null}catch{try{b.localStorage.removeItem(ve)}catch{}return null}},bo=()=>{if(!b?.localStorage)return null;try{return b.localStorage.getItem(ve)!==null}catch{return null}},lr=(e=null)=>{if(b?.localStorage){if(!e){try{b.localStorage.removeItem(ve)}catch{}return}try{b.localStorage.setItem(ve,JSON.stringify(e))}catch{}}},xe=()=>{const e=X(Ce),t=yo();if(t){if(Ce=t,typeof E=="function")try{E({lat:t.lat,lng:t.lng,label:t.label,city:t.city,source:t.source,savedAt:t.savedAt})}catch{}return t}const r=bo();if(e&&r!==!1)return e;if(r===!1&&(Ce=null),typeof _!="function")return null;let o=null;try{o=_()}catch{}const n=X({...o&&typeof o=="object"?o:{},source:"gps-map"});return n?(Ce=n,lr(n),n):null},vo=(e=null)=>{const t=X(e);if(!t)return!1;if(Ce=t,lr(t),typeof E=="function")try{E({lat:t.lat,lng:t.lng,label:t.label,city:t.city,source:t.source,savedAt:t.savedAt})}catch{}return!0},dr=()=>{if(Ae){try{clearTimeout(Ae)}catch{}Ae=null}},ur=()=>{if(dr(),ie)try{ie.abort()}catch{}Te=null,ie=null,ge=""},fr=(e="",t=[])=>{const r=P(e);if(!r)return;const o=Array.isArray(t)?t:[];for(ae.set(r,o);ae.size>no;){const n=ae.keys().next().value;if(!n)break;ae.delete(n)}He.size>io&&He.clear(),o.forEach(n=>{He.set(String(n?.id||"").trim().toLowerCase(),n)})},xt=()=>{if(Me){try{clearTimeout(Me)}catch{}Me=null}},We=(e=!1)=>{const t=!!e,r=f?.documentElement||null,o=f?.body||null;r?.classList?.toggle?.("feed-location-gate-resolving",t),o?.classList?.toggle?.("feed-location-gate-resolving",t);try{const n=b?.__MENYRA_SOCIAL_SET_UI_CHROME_MODE__;t&&typeof n=="function"&&n("app");const i=b?.__MENYRA_SOCIAL_FORCE_UI_CHROME__;typeof i=="function"&&i()}catch{}},$e=()=>{if(oe){if(typeof b?.cancelAnimationFrame=="function")try{b.cancelAnimationFrame(oe)}catch{}else try{clearTimeout(oe)}catch{}oe=null}},So=()=>{if(!b||!f)return null;const e=f.querySelector("main.app-main-scroll");if(!(e instanceof HTMLElement))return null;const t=b.getComputedStyle?.(e),r=String(t?.overflowY||t?.overflow||"").trim().toLowerCase(),o=r==="auto"||r==="scroll"||r==="overlay",n=Number(e.scrollHeight||0)-Number(e.clientHeight||0)>1;return!o||!n?null:e},Lo=({behavior:e="smooth"}={})=>{if(!b||!f)return!1;const t=f.getElementById("feedLocationGate");if(!(t instanceof HTMLElement))return!1;const r=t.querySelector(".feed-bento-pin-outline");if(!(r instanceof HTMLElement))return!1;const o=So(),n=o instanceof HTMLElement,i=Math.max(0,Math.round(parseFloat(b.getComputedStyle?.(r)?.top||"0")||0)),a=n?Math.round(Number(o.getBoundingClientRect().top||0)):0,l=n?Math.max(0,Number(o.scrollTop||0)):Math.max(0,Number(b.scrollY||b.pageYOffset||f?.documentElement?.scrollTop||0)),d=Math.round(Number(r.getBoundingClientRect().top||0)),u=Math.max(0,l+(d-(a+i)));if(Math.abs(u-l)<2)return!0;if(n){try{o.scrollTo({top:u,behavior:e})}catch{o.scrollTop=u}return!0}try{b.scrollTo({top:u,behavior:e})}catch{b.scrollTo(0,u)}return!0},gr=()=>{if(!Se)return;Se=!1,$e();let e=0;const t=4,r=4,o=()=>{if(e+=1,Lo({behavior:e===1?"smooth":"auto"}),e>=t){$e();return}oe=q(o,e===1?420:130)};if(typeof b?.requestAnimationFrame=="function"){let n=0;const i=()=>{oe=b.requestAnimationFrame(()=>{if(n+=1,n<r){i();return}oe=null,o()})};i();return}oe=q(()=>{oe=null,o()},80)},mr=()=>{if($e(),typeof vt=="function")try{vt()}catch{}vt=null},wo=(e=null)=>{mr()},pr=(e={})=>{const t=pe(e?.countryCode||e?.country_code||"");if(t)return t;const r=pe(e?.country||"");return r||(String(e?.source||"").trim().toLowerCase()==="local"?"Kosove":String(e?.country||"").trim())},hr=({id:e="",label:t="",lat:r=null,lng:o=null,aliases:n=[],source:i="local",country:a="",metaLabel:l="",importance:d=0}={})=>{const u=G({lat:r,lng:o}),g=String(t||"").trim();if(!u||!g)return null;const y=String(i||"local").trim().toLowerCase(),v=Le(a),L=pe(a)||pe(v)||(y==="local"?"Kosove":""),S=Array.from(new Set([g,L,v,...Array.isArray(n)?n:[]].map(x=>P(x)).filter(Boolean)));return Object.freeze({id:String(e||g).trim().toLowerCase(),label:g,city:g,lat:u.lat,lng:u.lng,source:y,country:L,countryCode:v,metaLabel:String(l||"").trim(),importance:Number(d)||0,searchTerms:S})},It=()=>ht.map(e=>hr(e)).filter(Boolean),yr=(e="")=>{const t=String(e||"").trim().toLowerCase();if(!t)return null;const r=It().find(o=>o.id===t)||null;return r||He.get(t)||null},xo=(e="")=>{const t=P(e);if(!t||/[0-9]/.test(t))return!1;const r=t.split(" ").filter(Boolean);return!r.length||r.length>3?!1:!r.some(o=>go.some(n=>o.startsWith(n)))},Io=(e={},t="")=>{if(!e||!t)return-1;const r=Array.isArray(e.searchTerms)?e.searchTerms:[];let o=-1;return r.forEach(n=>{if(!n)return;if(n===t){o=Math.max(o,420);return}if(n.startsWith(t)){o=Math.max(o,260-Math.max(0,n.length-t.length));return}const i=n.indexOf(t);i>=0&&(o=Math.max(o,170-i))}),o<0?-1:(String(e?.source||"").toLowerCase()==="remote"&&(o+=12+Math.round(Math.max(0,Number(e.importance||0)*10))),o)},ko=(...e)=>{const t=new Map;return e.forEach(r=>{(Array.isArray(r)?r:[]).forEach(o=>{if(!o||typeof o!="object")return;const n=`${P(o.label)}|${P(pr(o))}`;if(!n.trim())return;if(!t.has(n)){t.set(n,o);return}const i=t.get(n);String(i?.source||"").trim().toLowerCase()!=="local"&&t.set(n,o)})}),Array.from(t.values())},Eo=(e=[])=>Array.isArray(e)?e.map((t,r)=>{const o=t?.properties&&typeof t.properties=="object"?t.properties:{},n=G({lat:t?.geometry?.coordinates?.[1],lng:t?.geometry?.coordinates?.[0]}),i=String(o.name||o.city||o.locality||"").trim();if(!n||!i||!xo(i))return null;const a=Le(o.countrycode||o.country_code||o.country||"");if(!a||!co.has(a))return null;const l=P(o.osm_value||o.type||"");if(!lo.has(l))return null;const d=pe(a)||"",g=[String(o.state||o.county||"").trim(),d].filter(Boolean).join(" · ");return hr({id:`remote-${String(o.osm_id||`${i}-${a}-${r}`).trim().toLowerCase()}`,label:i,lat:n.lat,lng:n.lng,aliases:[o.city,o.state,o.county,d].filter(Boolean),source:"remote",country:a,metaLabel:g,importance:Number(o.importance||t?.importance||0)})}).filter(Boolean):[],Co=async(e="")=>{const t=String(e||"").trim(),r=P(t);if(r.length<ir)return[];if(ae.has(r))return ae.get(r)||[];if(Te&&ge===r)return Te;if(Date.now()<bt)return[];const o=typeof b?.fetch=="function"?b.fetch.bind(b):null;if(!o)return[];if(ie&&ge&&ge!==r)try{ie.abort()}catch{}const n=typeof AbortController=="function"?new AbortController:null;let i=null;ie=n,ge=r,n&&(i=q(()=>{try{n.abort()}catch{}},ao));const a=new URL("https://photon.komoot.io/api/");a.searchParams.set("limit",String(oo)),a.searchParams.set("lang","en"),a.searchParams.set("osm_tag","place"),a.searchParams.set("q",t);const l=(async()=>{try{const d=await o(a.toString(),{method:"GET",signal:n?.signal,headers:{"Accept-Language":"sq,sr,de,en"}});if(!d?.ok)throw new Error(`photon_${Number(d?.status||0)}`);const u=await d.json(),g=Array.isArray(u?.features)?u.features:[],y=Eo(g);return fr(r,y),bt=0,y}catch(d){return String(d?.name||"")!=="AbortError"&&(bt=Date.now()+so,fr(r,[])),[]}finally{if(i)try{clearTimeout(i)}catch{}ie===n&&ge===r&&(ie=null,ge="",Te=null)}})();return Te=l,l},kt=(e="",t=6)=>{const r=P(e);if(r.length<2)return[];const o=It(),n=ae.get(r)||[];return ko(o,n).map(a=>{const l=Io(a,r);return l<0?null:{...a,score:l}}).filter(Boolean).sort((a,l)=>Number(l.score||0)-Number(a.score||0)||String(a.label||"").localeCompare(String(l.label||""),"de")).slice(0,Math.max(1,Number(t)||6))},Ao=(e=[])=>{const t=Array.isArray(e)?e:[];return t.length?t.map(r=>`
      <button
        type="button"
        role="option"
        aria-selected="false"
  ...
  `,No=(e=je())=>{const t=e?.socialBlock||{};return`
    ...
    `};function zo({mode:e=Sr(),bentoContentHtml:t="",showSearchControls:r=!0,showTopSection:o=!0}={}){const n=je(),i=xe(),a=String(i?.label||i?.city||"").trim(),l=String(e||"feed-gate").trim().toLowerCase()||"feed-gate",d=r!==!1,u=o!==!1,g=String(t||xr(n)).trim();return`
    ...
    `}function Po(){return Ar()}function Ir(){return Ge()?`
    ...
    `}function Go(e={},t=0){const r=String(e?.spotId||e?.postId||"").trim(),o=p(r),n=o?`data-best-spot-avatar="${o}"`:"",i=o?`data-img-key="best-spot-avatar:${o}"`:"",a=o?`data-best-spot-name="${o}"`:"",l=String(e?.profileUrl||te("apps/menyra-social/index.html",{tab:"feed",source:"best-spot"})).trim(),d=String(e?.restaurantId||"").trim(),u=String(e?.avatarUrl||"").trim(),g=String(e?.displayName||"Best Spot").trim()||"Best Spot",y=t<4,v=String(e?.ratingDisplay||"4.8").trim()||"4.8",L=y?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"',S='<svg viewBox="0 0 24 24" width="8" height="8" aria-hidden="true" focusable="false" style="display:block;color:#fbbf24;fill:currentColor;stroke:currentColor;stroke-width:1.5;"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>',x=u?`<img src="${p(h(u,"small"))}" ${L} decoding="async" width="120" height="208" ${n} ${i} class="absolute inset-0 w-full h-full object-cover" />`:`<div class="absolute inset-0 flex items-center justify-center text-white/85" style="background:linear-gradient(145deg,#1f2937 0%,#0f172a 60%,#020617 100%);">${F("map-pin","w-6 h-6")}</div>`,I=d?`<button type="button" data-profile-business="${p(g)}" data-profile-id="${p(d)}" data-best-spot-item="${o}" class="flex-none w-[29%] sm:w-[120px] snap-start cursor-pointer text-left" style="${Fe()}">`:`<a href="${p(l)}" data-best-spot-item="${o}" class="flex-none w-[29%] sm:w-[120px] snap-start cursor-pointer" style="${Fe()}">`,T=d?"</button>":"</a>";return`
      ${I}
      ...
      ${T}
    `}function Bo(e={},t=0,r=""){const o=lt(e),n=t<5;if(o.kind==="video"&&o.src){const i=n?'preload="auto" fetchpriority="high"':'preload="metadata" fetchpriority="low"',a=o.poster?`poster="${p(h(o.poster,"small"))}"`:"",l=r?`data-story-preview-id="${p(r)}"`:"";return`
        ...
        <div class="relative h-52 rounded-2xl overflow-hidden shadow-md" style="${ct()}">
          ...
          <div class="absolute top-2 right-2" style="position:absolute;top:0.5rem;right:0.5rem;z-index:12;">
            <div class="w-7 h-7 rounded-full p-[2px] bg-gradient-to-tr from-amber-500 to-fuchsia-600 shadow-sm" ${L} style="padding:2px;background:linear-gradient(135deg,#f59e0b 0%,#db2777 100%);">
              <img src="${p(u)}" ${O} decoding="async" width="28" height="28" ${y} ${v} class="w-full h-full rounded-full border-[1.5px] border-black/60 object-cover bg-white" style="border:1.5px solid rgba(0,0,0,0.6);" />
            </div>
          </div>
          <div class="absolute bottom-2 left-2 right-2" style="position:absolute;left:0.5rem;right:0.5rem;bottom:0.5rem;z-index:12;">
    ...
    `}function At(e,t=[],{fallbackFeedPosts:r=[],fallbackStories:o=[]}={}){const n=rr(e,o),i=er(t,r,Zt(n,e,o)),l=or({spots:i,stories:n}).map((d,u)=>d.type==="spot"?Go(d.spot,u):kr(d.story,u)).join("");return`
    ...
    `}function Qe(e,t){const r=e.id?String(e.id):"",o=r?`data-post-like-count="${p(r)}"`:"",n=r?`data-post-comment-count="${p(r)}"`:"",i=r?`data-feed-id="${p(r)}"`:'data-feed-id=""',a=r?`data-feed-logo="${p(r)}"`:"",l=r?`data-img-key="feed-logo:${p(r)}"`:"",d=r?`data-img-key="feed-hero:${p(r)}"`:"",u=`data-feed-render-sig="${p(Kt(e))}"`,g=t<2,y=g?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"',v=g?'loading="eager"':'loading="lazy" fetchpriority="low"',L=c.restaurants.find($=>$.id===(e.restaurantId||e.ownerId))||{},S=L.logoUrl||L.logo||e.logo||"",x=ue(e.restaurantId||e.ownerId,S,"avatar"),I=h(e.image,"medium",{stableKey:r?`feed-hero:${r}`:""}),T=p(`${h(e.image,"small")} 480w, ${I} 768w, ${h(e.image,"large")} 1280w`),O="(max-width: 640px) 100vw, 600px",H=e.poster?h(e.poster,"medium",{stableKey:r?`feed-hero-poster:${r}`:""}):I,ne=e.isVideo&&e.videoUrl?`<video src="${p(e.videoUrl)}" poster="${p(H)}" autoplay muted loop playsinline preload="none" ${d} class="w-full h-full block object-cover group-hover:scale-105 transition-transform duration-1000"></video>`:`<img src="${p(I)}" srcset="${T}" sizes="${O}" ${y} ${d} decoding="async" class="w-full h-full block object-cover group-hover:scale-105 transition-transform duration-1000" />`,Z=e.restaurantId?String(be(e.restaurantId,e.isVideo&&e.videoUrl&&r?{postId:r}:{})||"").trim():"",s=Z?`<a href="${p(Z)}" data-feed-post-open="${p(e.restaurantId)}" data-story-url="${p(Z)}" aria-label="Stories von ${p(e.business)} ansehen" class="block w-full h-full">${ne}</a>`:ne;return`
  ...
  `}function Tt(e){return e.length?e.slice(0,10).map((t,r)=>Qe(t,r)).join(""):'<div class="text-center py-20 text-slate-400 font-bold text-xs uppercase">Keine Posts vorhanden</div>'}function Er(e){const t=f?.getElementById("feedList");if(!t)return!1;if(!e.length){const l=Tt(e);return t.innerHTML!==l?(t.innerHTML=l,!0):!1}const r=Array.from(t.querySelectorAll("[data-feed-id]")),o=r.map(l=>l.dataset.feedId||""),n=e.map(l=>String(l.id||""));if(o.join("|")===n.join("|")){let l=!1;return e.forEach((d,u)=>{const g=r[u];if(!g)return;const y=Kt(d);if(String(g.getAttribute("data-feed-render-sig")||"").trim()===y)return;const L=f.createElement("template");L.innerHTML=Qe(d,u);const S=L.content.firstElementChild;S&&(g.replaceWith(S),l=!0)}),e.forEach(de),e.forEach(ee),l}const i=new Map;r.forEach(l=>i.set(l.dataset.feedId||"",l));const a=f.createDocumentFragment();return e.forEach((l,d)=>{const u=String(l.id||""),g=u?i.get(u):null;if(g)i.delete(u),a.appendChild(g);else{const y=f.createElement("template");y.innerHTML=Qe(l,d);const v=y.content.firstElementChild;v&&a.appendChild(v)}}),t.replaceChildren(a),e.forEach(de),e.forEach(ee),!0}function Fo(e){if(!K||!(e instanceof K)||e.dataset.storyBoomerangBound==="1")return;e.dataset.storyBoomerangBound="1",e.defaultMuted=!0,e.muted=!0,e.setAttribute("muted",""),e.autoplay=!0,e.loop=!1,e.controls=!1,e.disablePictureInPicture=!0,e.preload="metadata",e.playsInline=!0,e.setAttribute("playsinline",""),e.setAttribute("webkit-playsinline","");const t=String(b?.navigator?.userAgent||"").toLowerCase(),r=!!b?.matchMedia?.("(pointer: coarse)")?.matches,o=/android|iphone|ipad|ipod|mobile/.test(t),n=!!b?.matchMedia?.("(display-mode: standalone)")?.matches||!!b?.navigator?.standalone,i=r||o||n;let a=1.35;const l=.05;Q.set(e,{previewStart:l,previewEnd:a,active:!1,forceLightweightLoop:i});const d=(y=!1)=>{const v=Q.get(e);v&&(v.active=!!y,v.previewEnd=a,e.dataset.storyPreviewActive=y?"1":"0")},u=()=>{const y=Number(e.duration||0);if(!Number.isFinite(y)||y<=.05){a=1.35;const L=Q.get(e);L&&(L.previewEnd=a);return}a=Math.max(l+.45,Math.min(y,1.45));const v=Q.get(e);v&&(v.previewEnd=a)},g=()=>{if(Q.get(e)?.active){u();{e.currentTime>=a&&(e.currentTime=l+.01,e.paused&&e.play().catch(()=>{}));return}}};e.addEventListener("loadedmetadata",u),e.addEventListener("timeupdate",g),e.addEventListener("stalled",()=>{Q.get(e)?.active&&e.play().catch(()=>{})}),e.addEventListener("waiting",()=>{Q.get(e)?.active&&e.play().catch(()=>{})}),e.addEventListener("ended",()=>{Q.get(e)?.active&&(e.currentTime=l,e.play().catch(()=>{}))});try{e.currentTime=l}catch{}d(!1)}function Xe(e,{reset:t=!1}={}){if(!K||!(e instanceof K))return;const r=Q.get(e);if(r){if(t||!Number.isFinite(e.currentTime)||e.currentTime>r.previewEnd+.05)try{e.currentTime=r.previewStart}catch{}r.active=!0,e.dataset.storyPreviewActive="1",e.play().catch(()=>{try{!e.dataset.storyPreviewFrameSeek&&Number(e.currentTime||0)<=.01&&(e.dataset.storyPreviewFrameSeek="1",e.currentTime=Math.max(.05,Number(r.previewStart||0)))}catch{}Uo()})}}function Uo(){if(Vt||!f)return;Vt=!0;const e=()=>{f.querySelectorAll("video[data-story-preview-video]").forEach(t=>{String(t.dataset.storyPreviewVisible||"0")==="1"&&Xe(t)})};f.addEventListener("touchend",e,{once:!0,passive:!0}),f.addEventListener("pointerdown",e,{once:!0,passive:!0})}function Mt(e){if(!K||!(e instanceof K))return;const t=Q.get(e);t&&(t.active=!1),e.dataset.storyPreviewActive="0";try{e.pause()}catch{}}function Do(){return!b?.IntersectionObserver||!K?null:Be||(Be=new b.IntersectionObserver(e=>{e.forEach(t=>{const r=t.target;if(!(r instanceof K))return;const n=(t.isIntersecting?Number(t.intersectionRatio||0):0)>=.3;r.dataset.storyPreviewVisible=n?"1":"0",n?Xe(r):Mt(r)})},{threshold:[0,.15,.3,.55,.85,1]}),Be)}function Vo(){!f||Dt||(Dt=!0,f.addEventListener("visibilitychange",()=>{const e=f.getElementById("storiesRow");if(!e)return;const t=Array.from(e.querySelectorAll("video[data-story-preview-video]"));if(f.hidden){t.forEach(r=>Mt(r));return}t.forEach(r=>{String(r.dataset.storyPreviewVisible||"0")==="1"&&Xe(r)})}),b?.addEventListener?.("pagehide",()=>{const e=f.getElementById("storiesRow");e&&e.querySelectorAll("video[data-story-preview-video]")?.forEach?.(t=>Mt(t))}))}function qo(e=null){if(!f)return;const r=(e&&typeof e.querySelector=="function"?e:f).querySelector?.("[data-spot-story-track]");if(!(r instanceof HTMLElement)||r.dataset.edgeSwipeGuardBound==="1")return;r.dataset.edgeSwipeGuardBound="1";let o=!1,n=0,i=0,a=0;const l=g=>{const y=g.touches?.[0];y&&(o=!0,n=y.clientX,i=y.clientY,a=Number(r.scrollLeft||0))},d=g=>{if(!o)return;const y=g.touches?.[0];if(!y)return;const v=y.clientX-n,L=y.clientY-i;if(Math.abs(v)<=Math.abs(L))return;const S=Math.max(0,Number(r.scrollWidth||0)-Number(r.clientWidth||0)),x=a<=.5||Number(r.scrollLeft||0)<=.5,I=Math.abs(S-Number(r.scrollLeft||0))<=.5,T=v>0&&x,O=v<0&&I;(T||O)&&g.preventDefault()},u=()=>{o=!1};r.addEventListener("touchstart",l,{passive:!0}),r.addEventListener("touchmove",d,{passive:!1}),r.addEventListener("touchend",u,{passive:!0}),r.addEventListener("touchcancel",u,{passive:!0})}function Ne(e=null){const t=e&&typeof e.querySelectorAll=="function"?e:f?.getElementById("storiesRow");if(!t)return;const r=Array.from(t.querySelectorAll?.("video[data-story-preview-video]")||[]),o=Do();o&&o.disconnect(),r.forEach(n=>{Fo(n),o?o.observe(n):Xe(n,{reset:!0})}),qo(t),Vo()}function Cr(e,t=[],{fallbackFeedPosts:r=[],fallbackStories:o=[]}={}){const n=f?.getElementById("storiesRow");return n?(n.innerHTML=At(e,t,{fallbackFeedPosts:r,fallbackStories:o}),Ne(n),!0):!1}function Ko(e){if(!f||!e)return!1;const t=f.getElementById("feedList");if(!t)return!1;const r=e.querySelector("[data-feed-composer-wrap]");if(!Ge())return r?(r.remove(),!0):!1;if(r)return!1;const n=f.createElement("template");n.innerHTML=Ir();const i=n.content.firstElementChild;return i?(t.parentNode?.insertBefore(i,t),!0):!1}function Yo(){const e=f?.getElementById("feedView");if(!e)return!1;if(String(e.dataset.feedViewMode||"feed").trim().toLowerCase()!=="feed")return _t(),Ie(),b?.lucide?.createIcons&&b.lucide.createIcons(),!0;const o=(Array.isArray(c?.feedPosts)?c.feedPosts:[]).filter(I=>c.feedCategory==="all"||I.category===c.feedCategory).sort((I,T)=>(m(T.createdAt)?.getTime()||0)-(m(I.createdAt)?.getTime()||0)),n=(Array.isArray(c.stories)?c.stories:[]).filter(I=>Yt(I)),{feedPosts:i,stories:a}=Jt({feedPosts:o,stories:n}),l=a,d=rr(l,l),u=er(i,i,Zt(d,l)),g=f.getElementById("storiesRow"),y=Jr({spots:u,stories:d});let v=!1;if(g){const I=!!g.querySelector("[data-spot-story-track]");(C()!==y||!I)&&(Cr(l,i,{fallbackFeedPosts:i,fallbackStories:l}),A(y),v=!0),d.forEach(H=>{B(H),V(H)});const T=`motion:${y}|${d.length}`;(v||String(g.dataset.storyPreviewMotionSig||"")!==T)&&(Ne(g),g.dataset.storyPreviewMotionSig=T)}const L=Ko(e),S=Er(i);tt(i),_t();const x=Vr(i);return x!==ar&&(ar=x,rt(i)),(v||L||S)&&b?.lucide?.createIcons&&b.lucide.createIcons(),!0}function Ar(){const e=!!X(xe()),t=e?"feed":"feed-gate";let r=xr();if(e){const i=(Array.isArray(c?.feedPosts)?c.feedPosts:[]).filter(g=>c.feedCategory==="all"||g.category===c.feedCategory).sort((g,y)=>(m(y.createdAt)?.getTime()||0)-(m(g.createdAt)?.getTime()||0)),a=(Array.isArray(c.stories)?c.stories:[]).filter(g=>Yt(g)),{feedPosts:l,stories:d}=Jt({feedPosts:i,stories:a}),u=d;r=`
  ...
  `}function Ho(){if(!f||sr)return;sr=!0;const e=()=>Lr();f.addEventListener("input",t=>{if(!e())return;const r=t.target;if(!(r instanceof Element))return;const o=r.closest("[data-feed-location-city-input]");o instanceof HTMLInputElement&&Et(o.value)}),f.addEventListener("pointerdown",t=>{if(!e())return;const r=t.target;if(!(r instanceof Element))return;const o=r.closest("[data-feed-location-city-input]");if(o instanceof HTMLInputElement&&f?.activeElement!==o){t.preventDefault();try{o.focus({preventScroll:!0})}catch{o.focus()}}}),f.addEventListener("touchstart",t=>{if(!e())return;const r=t.target;if(!(r instanceof Element))return;const o=r.closest("[data-feed-location-city-input]");if(o instanceof HTMLInputElement&&f?.activeElement!==o){t.preventDefault();try{o.focus({preventScroll:!0})}catch{o.focus()}}},{passive:!1}),f.addEventListener("focusin",t=>{if(!e())return;const r=t.target;if(!(r instanceof Element))return;const o=r.closest("[data-feed-location-city-input]");o instanceof HTMLInputElement&&Et(o.value)}),f.addEventListener("focusout",t=>{if(!e())return;const r=t.target;!(r instanceof Element)||!r.closest("[data-feed-location-city-input]")||q(()=>{const n=f?.activeElement;n instanceof Element&&(n.closest("[data-feed-location-city-input]")||n.closest("[data-feed-city-suggestion]"))||J()},120)}),f.addEventListener("change",t=>{if(!e())return;const r=t.target;if(!(r instanceof Element))return;const o=r.closest("[data-feed-location-city-input]");if(!(o instanceof HTMLInputElement))return;const n=String(o.value||"").trim();if(!n)return;const i=kt(n,1)[0];if(!i||P(n)!==P(i.label))return;J(),Oe({lat:i.lat,lng:i.lng,label:i.label,city:i.city||i.label,country:i.country,countryCode:i.countryCode,source:"city-search"})||ye({fallbackCity:i})}),f.addEventListener("keydown",t=>{if(!e())return;const r=t.target;if(!(r instanceof Element))return;const o=r.closest("[data-feed-location-city-input]");if(!(o instanceof HTMLInputElement))return;if(t.key==="Escape"){J();return}if(t.key!=="Enter")return;const n=kt(o.value,1)[0];if(!n)return;t.preventDefault(),o.value=n.label,J(),Oe({lat:n.lat,lng:n.lng,label:n.label,city:n.city||n.label,country:n.country,countryCode:n.countryCode,source:"city-search"})||ye({fallbackCity:n})}),f.addEventListener("click",t=>{if(!e())return;const r=t.target;if(!(r instanceof Element))return;const o=r.closest("[data-feed-city-suggestion]");if(o){t.preventDefault(),t.stopPropagation();const i=yr(o.getAttribute("data-feed-city-suggestion")||"");if(i){const a=f?.getElementById("feedLocationCityInput");a instanceof HTMLInputElement&&(a.value=i.label),J(),Oe({lat:i.lat,lng:i.lng,label:i.label,city:i.city||i.label,country:i.country,countryCode:i.countryCode,source:"city-search"})||ye({fallbackCity:i})}return}r.closest("[data-feed-location-request]")&&(t.preventDefault(),t.stopPropagation(),ye({forceExact:!0}))})}function _t(){Ho(),Lr()&&Ie();const e=f?.getElementById("feedView");if(!e){mr();return}if(wo(e),String(e.dataset.feedViewMode||"").trim().toLowerCase()==="feed"?(gr(),Ne()):(Se=!1,$e()),e.dataset.bound==="true"){Ne(),Ie();return}const r=()=>String(e.dataset.feedViewMode||"").trim().toLowerCase()!=="feed",o=n=>{if(r()||!(n instanceof Element))return;const i=n.closest("[data-story-item]")||n.closest("[data-feed-post-open]");!(i instanceof Element)||String(i.getAttribute("data-story-truth")||"").trim().toLowerCase()==="feed-fallback"||to(i.getAttribute("data-story-item")||i.getAttribute("data-feed-post-open")||"",i.getAttribute("data-story-url")||i.getAttribute("href")||"")};e.addEventListener("pointerdown",n=>{o(n.target)},{passive:!0}),e.addEventListener("touchstart",n=>{o(n.target)},{passive:!0}),e.addEventListener("click",n=>{const i=n.target;if(!(i instanceof Element))return;const a=i.closest("[data-story-item]");if(a){o(a);return}const l=i.closest("[data-feed-post-like]");if(l){const v=l.dataset.feedPostLike||"";v&&Ee(v);return}const d=i.closest("[data-feed-post-comment]");if(d){const v=d.dataset.feedPostComment||"",L=Ht(v);if(L){const x=d.closest("[data-feed-id]")?.querySelector?.(`[data-img-key="feed-hero:${v}"]`)||null,I=String(x?.currentSrc||x?.getAttribute?.("src")||"").trim();Promise.resolve(ke(L,{previewImageEl:x,previewImageSrc:I})).then(()=>{ro()})}return}const u=i.closest("[data-feed-post-share]");if(u){const v=u.dataset.feedPostShare||"",L=Ht(v);if(!L)return;const S=Zr(L),x=String(L.business||"Menyra").trim()||"Menyra",I=[x,String(L.content||L.caption||"").trim()].filter(Boolean).join(`
`);b?.navigator?.share?b.navigator.share({title:x,text:I,url:S}).then(()=>{pt(u,"Geteilt")}).catch(async T=>{if(String(T?.name||"").trim()==="AbortError")return;const O=await nr(S);pt(u,O?"Kopiert":"Link")}):nr(S).then(T=>{pt(u,T?"Kopiert":"Link")});return}const g=i.closest("[data-nav]");if(g){const v=g.dataset.nav;if(v){if(v==="favorites"&&!String(c.user?.uid||"").trim()){W("Bitte registrieren oder einloggen, um Favoriten zu nutzen.");return}const L=v==="upload"?{upload:j(g.dataset.uploadIntent||"",c.upload)}:{},S=v==="favorites"?"profile":v,x=v==="favorites"?"favorites":v==="profile"?"profile":c.profileTopTab;if(v==="profile"&&typeof k=="function"){c.chatSettingsOpen=!1,c.chatListScope="inbox",c.chatThreadMenuId="",c.settingsView="main",c.selectedBusiness=null,c.postModal={open:!1,post:null,commentText:"",replyTo:null,loading:!1,animate:!1,sending:!1},c.likesModal={open:!1,postId:"",animate:!1},c.leadModal={open:!1,mode:"create",lead:null,status:"",loading:!1,deleting:!1,actionsOpen:!1,logoFile:null,logoPreview:"",bestSpotLogoFile:null,bestSpotLogoPreview:"",coords:null,locations:[]},c.customerModal={open:!1,mode:"edit",customer:null,status:"",loading:!1,logoFile:null,logoPreview:""},k({showBack:!1,topTab:"profile"});return}U({activeTab:S,profileTopTab:x,drawerOpen:!1,chatSettingsOpen:!1,chatListScope:"inbox",chatThreadMenuId:"",settingsView:"main",selectedBusiness:null,profileView:null,profileModal:{open:!1,profile:null},postModal:{open:!1,post:null,commentText:"",replyTo:null,loading:!1,animate:!1,sending:!1},likesModal:{open:!1,postId:"",animate:!1},leadModal:{open:!1,mode:"create",lead:null,status:"",loading:!1,deleting:!1,actionsOpen:!1,logoFile:null,logoPreview:"",bestSpotLogoFile:null,bestSpotLogoPreview:"",coords:null,locations:[]},customerModal:{open:!1,mode:"edit",customer:null,status:"",loading:!1,logoFile:null,logoPreview:""},...L})}return}const y=i.closest("[data-profile-business]");y&&re({id:y.dataset.profileId||"",name:y.dataset.profileBusiness||""},{showBack:!0})}),Ne(),Ie(),e.dataset.bound="true"}return{renderHomeView:Po,renderFeedView:Ar,renderStoryItem:kr,renderStoriesRow:At,renderFeedItem:Qe,renderFeedList:Tt,patchFeedList:Er,patchStoriesRow:Cr,updateFeedDom:Yo,bindFeedDelegation:_t}}function Mr(c={}){return String(c.url||c.mediaUrl||c.media?.[0]?.url||c.media?.[0]?.thumbUrl||c.imageUrl||c.image||c.photoUrl||c.pictureUrl||"").trim()}function _r(c={}){const m=String(c.media?.[0]?.type||c.mediaType||c.type||"").trim().toLowerCase();return c.isVideo===!0||m==="video"||m.startsWith("video/")}function Zo(c,m={},C=""){return{id:c,url:Mr(m),type:m.type||"square",title:m.title||"",caption:m.caption||"",createdAt:m.createdAt,likes:m.likesCount??m.likes??0,comments:m.commentsCount??m.comments??0,isVideo:_r(m),ownerType:"user",ownerId:C||""}}function ln(c,m={},C=""){return{id:c,url:Mr(m),type:m.type||"square",title:m.title||"",caption:m.caption||"",createdAt:m.createdAt,likes:m.likesCount??m.likes??0,comments:m.commentsCount??m.comments??0,isVideo:_r(m),ownerType:"restaurant",ownerId:C||"",restaurantId:C||""}}function $r(c=[]){return(Array.isArray(c)?c:[]).map(m=>{const C=String(m?.restaurantId||m?.id||"").trim(),A=m?.isLive?"1":"0",N=String(m?.truthSource||m?.storyTruthSource||m?.storyTruth||"").trim().toLowerCase()||"canonical",z=String(m?.mediaType||m?.type||"").trim().toLowerCase(),B=String(m?.videoUrl||m?.imageUrl||m?.mediaUrl||m?.embedUrl||m?.url||"").trim(),V=m?.createdAt?.seconds!==void 0?`${m.createdAt.seconds}:${Number(m.createdAt?.nanoseconds)||0}`:String(m?.createdAt||m?.updatedAt||"").trim();return`${C}|${A}|${N}|${z}|${B}|${V}`}).join(",")}function dn({posts:c=[],force:m=!1,fastMode:C=!1,allowFeedFallback:A=!1,buildStoriesFromFeed:N,currentSignature:z=""}={}){if(!C||!A)return{updated:!1,signature:z,stories:[]};if(!Array.isArray(c)||!c.length)return{updated:!1,signature:z,stories:[]};if(typeof N!="function")return{updated:!1,signature:z,stories:[]};const B=N(c);if(!B.length)return{updated:!1,signature:z,stories:[]};const V=$r(B);return!m&&z===V?{updated:!1,signature:z,stories:[]}:{updated:!0,signature:V,stories:B}}const en=new Map([["xk","xk"],["kosove","xk"],["kosova","xk"],["kosovo","xk"],["al","al"],["shqiperi","al"],["shqiperia","al"],["albania","al"],["rs","rs"],["serbi","rs"],["serbia","rs"],["srbija","rs"]]),tn=Object.freeze({xk:"Kosove",al:"Shqiperi",rs:"Serbi"});function rn(c=""){return String(c||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9\s-]/g," ").replace(/\s+/g," ").trim()}function on(c=""){const m=rn(c);return m&&en.get(m)||""}function le(c,m){const C=Number(c),A=Number(m);return!Number.isFinite(C)||!Number.isFinite(A)||Math.abs(C)<1e-6&&Math.abs(A)<1e-6?null:Math.abs(C)<=90&&Math.abs(A)<=180?{lat:C,lng:A}:Math.abs(C)<=180&&Math.abs(A)<=90?{lat:A,lng:C}:null}function Gt(c={}){return!c||typeof c!="object"?null:le(c.lat,c.lng)||le(c.latitude,c.longitude)||le(c.gpsLat,c.gpsLng)||le(c.geo?.lat,c.geo?.lng)||le(c.geo?.latitude,c.geo?.longitude)||le(c.coords?.lat,c.coords?.lng)||le(c.coords?.latitude,c.coords?.longitude)||le(c.location?.lat,c.location?.lng)}function un({state:c=null,firebaseApi:m={},constants:C={},visibilityApi:A={},utilityApi:N={}}={}){const z=typeof m.collectionFn=="function"?m.collectionFn:null,B=typeof m.queryFn=="function"?m.queryFn:null,V=typeof m.orderByFn=="function"?m.orderByFn:null,ee=typeof m.limitFn=="function"?m.limitFn:null,de=typeof m.getDocsFn=="function"?m.getDocsFn:null,tt=typeof A.isForceHiddenUidFn=="function"?A.isForceHiddenUidFn:(()=>!1),rt=typeof A.isForceHiddenHandleFn=="function"?A.isForceHiddenHandleFn:(()=>!1),Bt=typeof A.isForceHiddenBusinessEntityFn=="function"?A.isForceHiddenBusinessEntityFn:(()=>!1),ot=typeof A.isPublicBusinessRecordFn=="function"?A.isPublicBusinessRecordFn:(()=>!0),nt=typeof N.formatRelativeFn=="function"?N.formatRelativeFn:(h=>String(h||"")),ze=typeof N.toDateSafeFn=="function"?N.toDateSafeFn:(h=>h);function F(h){const _=String(h||"").trim();if(!_)return!0;const E=String(c?.userProfile?.restaurantId||"").trim();if(E&&_===E)return!0;if(tt(_)||rt(_))return!1;const j=(c?.restaurants||[]).find(U=>String(U?.id||"")===_)||null;return j?ot(j):!0}function p(h){if(!h)return 0;try{return typeof h?.toDate=="function"?h.toDate()?.getTime?.()||0:typeof h=="number"?Number.isFinite(h)?h:0:h instanceof Date?h.getTime()||0:ze(h)?.getTime?.()||0}catch{return 0}}function te(h="",_=""){const E=String(h||"").trim();if(!E||!(c?.postEntityMap instanceof Map)||!c.postEntityMap.size)return null;const j=String(_||"").trim();let U=null,W=-1;return c.postEntityMap.forEach(k=>{if(!k||String(k?.id||"").trim()!==E)return;const re=String(k?.restaurantId||(k?.ownerType==="restaurant"?k?.ownerId:"")||k?.rid||"").trim(),ke=j&&re===j?1e6:0,Ee=[k?.caption,k?.content,k?.url,k?.image,k?.title].reduce((b,K)=>String(K||"").trim()?b+1:b,0),q=Math.max(p(k?.updatedAt),p(k?.updatedAtClient),p(k?.createdAtClient),p(k?.createdAt)),f=ke+q+Ee*10;(!U||f>W)&&(U=k,W=f)}),U}function be(h={}){const _=String(h?.rid||h?.restaurantId||"").trim();if(Bt({id:_,restaurantId:_,...h||{}})||!F(_))return null;const E=te(h?.id,_),j=(c?.restaurants||[]).find(Ge=>Ge?.id===_)||{},U=Gt(j)||Gt(h)||Gt(E),W=on(j?.countryCode||j?.country_code||j?.country||h?.countryCode||h?.country_code||h?.country||E?.countryCode||E?.country),k=W?tn[W]||"":String(j?.country||h?.country||E?.country||"").trim(),re=h?.thumbUrl||h?.mediaUrl||h?.media?.[0]?.thumbUrl||h?.media?.[0]?.url||"",ke=h?.logoUrl||h?.logo||h?.logoURL||"",Ee=String(E?.content||E?.caption||h?.caption||h?.content||h?.captionShort||"").trim(),q=String(E?.image||E?.url||h?.imageUrl||re||"").trim(),f=String(h?.id||E?.id||"").trim(),b=String(E?.mediaType||h?.mediaType||h?.media?.[0]?.type||"").trim().toLowerCase(),K=String(E?.url||h?.mediaUrl||h?.media?.[0]?.url||"").trim(),Re=b==="video"||h?.isVideo===!0||/\.(mp4|webm|mov|m3u8)(\?|$)/i.test(K),it=Re?K:"",at=String(h?.thumbUrl||h?.media?.[0]?.thumbUrl||E?.thumbUrl||"").trim();return{id:f,restaurantId:_,isVideo:Re,videoUrl:it,poster:at,business:h?.businessName||h?.restaurantName||j?.name||j?.restaurantName||"Business",logo:j?.logoUrl||j?.logo||ke||"",location:h?.city||j?.city||"",country:k,countryCode:W,lat:U?.lat??null,lng:U?.lng??null,content:Ee,image:q,likes:Number(h?.likesCount??h?.likes??E?.likes??0)||0,comments:Number(h?.commentsCount??h?.comments??E?.comments??0)||0,time:nt(ze(E?.createdAt||h?.createdAt)),createdAt:E?.createdAt||h?.createdAt,updatedAt:h?.updatedAt||E?.updatedAt||E?.updatedAtClient||h?.createdAt,category:h?.postType||"food",isLive:h?.isLive||!1,ownerType:"restaurant",ownerId:_,truthSource:"feed-projection",canonicalPath:String(h?.canonicalPath||(_&&f?`restaurants/${_}/socialPosts/${f}`:"")).trim()}}function ue(h=[]){return $r(h||[])}async function Pe(h){const _=String(h||"").trim();if(!_||!m.db||!z||!de)return[];try{const E=z(m.db,"users",_,"posts");let j=null;try{const k=[V("createdAt","desc")];ee&&C.fastLimits?.userPosts&&k.push(ee(C.fastLimits.userPosts));const re=B?B(E,...k):E;j=await de(re)}catch{const re=Number(C.fastLimits?.userPosts)||24;j=B&&ee?await de(B(E,ee(re))):await de(E)}const U=[];j?.forEach?.(k=>U.push({id:k.id,...k.data()}));const W=U.map(k=>Zo(k.id,{...k,url:k.url||k.mediaUrl||k.media?.[0]?.url||"",isVideo:k.isVideo??k.media?.[0]?.type==="video"},_)).filter(k=>k.url);return Jo(c,W)}catch{return[]}}return{canShowFeedRestaurantId:F,normalizeFeedPost:be,buildStoriesRowSignature:ue,loadUserPostsForUser:Pe}}function nn({posts:c=[],toDateSafe:m}={}){if(!Array.isArray(c)||typeof m!="function")return 0;let C=0;return c.forEach(A=>{const N=m(A?.createdAt)?.getTime?.()||0;N>C&&(C=N)}),C}function fn({posts:c=[],extraMeta:m={},toDateSafe:C,writeCache:A,feedCacheKey:N,feedFallbackLimit:z=0}={}){if(!Array.isArray(c)||typeof A!="function"||!N)return;const B=nn({posts:c,toDateSafe:C}),V=Math.max(0,Number(z)||0),ee=V>0?c.slice(0,V):c.slice();A(N,ee,{latestTs:B,...m})}export{Zo as a,$r as b,cn as c,sn as d,un as e,ln as n,Jo as p,dn as r,fn as s};
