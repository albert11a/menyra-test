...
`,g=Object.freeze({brand:"Mnyra GO",leadTitle:"Zbritje ose ofertë nga lokalet përreth teje",leadBody:"Ti thua vetëm sa veta jeni, çka doni dhe kur. Lokalet të kthejnë ofertën e tyre.",partyQuestion:"Sa veta jeni?",partyOne:"person",partyMany:"veta",step:"Hapi",stepBack:"Kthehu një hap prapa",next:"Vazhdo",categoryQuestion:"Çka dëshironi?",whenQuestion:"Kur?",whenLaterLabel:"Zgjidh ditën dhe orën",placeQuestion:"Ku?",placeLabel:"Qyteti",placeEmpty:"Shto qytetin tënd",cityPlaceholder:"Shkruaj qytetin",locationChange:"Ndrysho",locationSave:"Ruaj",submit:"Shiko ofertat",searching:"Po kërkojmë...",resultsHeadline:"lokale kanë oferta për ju",offering:"po ju ofron",forGroup:"për grupin tuaj",accept:"Prano ofertën",confirming:"Po konfirmohet...",sponsored:"Sponsored",onlyGo:"Vetëm me Mnyra GO",tableIncluded:"Tavolinë",emptyTitle:"Nuk gjetëm ofertë GO që përputhet tani.",emptySubtitle:"Provo me një orë tjetër ose me një grup tjetër.",doneTitle:"U krye",reservationConfirmed:"Tavolina është konfirmuar",claimConfirmed:"Oferta është e juaja",menu:"Shiko menunë",directions:"Udhëzime",code:"Kodi",cancel:"Anulo",cancelConfirm:"Dëshiron ta anulosh?",cancelYes:"Po, anuloje",cancelNo:"Jo",saveToAccount:"Ruaje në llogarinë tënde",signIn:"Hyr",errorTitle:"Mnyra GO është përkohësisht i padisponueshëm.",errorRetry:"Provo prapë",alternatives:"alternativa për ju",peopleSuffix:"persona",now:"Tani",back:"Ndrysho kërkimin",infoTitle:"Mirë të dihet"}),k="/apps/menyra-social/assets/go/",y=Object.freeze([Object.freeze({file:"story-1-unt.webp",headline:"A je unt?",text:"Trego sa veta jeni edhe çka po ju hahet."}),Object.freeze({file:"story-2-ku-me-dal.webp",headline:"S’po din ku me dal?",text:"Mos lyp lokal — lokalet që t’përshtaten t’gjejnë ty."}),Object.freeze({file:"story-3-shtrejt.webp",headline:"Edhe shumë shtrejt?",text:"Lokalet t’çojnë oferta me zbritje direkt — ti veç zgjedh."}),Object.freeze({file:"story-4-knaqu.webp",headline:"Ofertat t’vijn. Ti veç shko, knaqu.",text:"Zgjedhe ofertën që t’pëlqen, shko aty edhe knaqu."})]),w=Object.freeze([Object.freeze({icon:"gift",title:"Falas për ty",text:"Mnyra GO nuk kushton asgjë. Paguan vetëm atë që konsumon te lokali."}),Object.freeze({icon:"shield-check",title:"Pa llogari",text:"Nuk të duhet regjistrim. Kërkon, pranon ofertën dhe shkon."}),Object.freeze({icon:"ticket-percent",title:"Vetëm me Mnyra GO",text:"Këto oferta nuk janë në menu e as në rrjete sociale."}),Object.freeze({icon:"store",title:"Lokalet vendosin vetë",text:"Çdo lokal zgjedh çka ofron, për sa veta dhe në cilat orë."})]),G=Object.freeze({now:"zap",in30:"timer",in60:"clock",later:"calendar-clock"}),c=Object.freeze(["party","category","when","place"]),$=Object.freeze({party:"users",category:"utensils",when:"clock",place:"map-pin"});function n(a=""){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function f(a=""){const e=String(a||"").trim();return c.includes(e)?e:c[0]}function ne(a=""){const e=c.indexOf(f(a));return c[Math.min(c.length-1,e+1)]}function te(a=""){const e=c.indexOf(f(a));return c[Math.max(0,e-1)]}function u(a){const e=Math.trunc(Number(a));return Number.isFinite(e)?Math.min(m,Math.max(l,e)):A}function S(a,e=g){return u(a)===1?e.partyOne||g.partyOne:e.partyMany||g.partyMany}function z(a,e=g){return`${u(a)} ${S(a,e)}`}function T(a){const e=u(a),t=m-l;return Math.round((e-l)/t*100)}function M(a,{active:e=!1,attr:t="",value:r="",iconName:o="",wide:p=!1}={}){return`
    <button
      type="button"
      class="mnyra-go-page__chip${p?" mnyra-go-page__chip--wide":""}"
      ${t?`${t}="${n(r)}"`:""}
      aria-pressed="${e?"true":"false"}"
    >${o?i(o):""}<span>${n(a)}</span></button>
  `}function j(a,{nowMs:e=Date.now()}={}){const t=Date.parse(String(a||""));if(!Number.isFinite(t))return"";if(Math.abs(t-e)<900*1e3)return g.now;const r=new Date(t);return`Rreth ${String(r.getHours()).padStart(2,"0")}:${String(r.getMinutes()).padStart(2,"0")}`}function B(a="",e={},t=g){if(a==="party")return z(e.partySize,t);if(a==="category"){const r=b.find(o=>o.key===String(e.category||"all"));return r?r.label:""}if(a==="when"){const r=x.find(o=>o.key===String(e.when||"now"));return r?r.label:""}return a==="place"?String(e.city||"").trim()||t.placeEmpty:""}function D(a="",e={},t=g){const r=c.indexOf(a),o=c.slice(0,Math.max(0,r));return o.length?`
    <div class="mnyra-go-page__ask-done">
      ${o.map(p=>`
        <button type="button" class="mnyra-go-page__ask-tag" data-go-goto="${n(p)}">
          ${i($[p]||"check-check")}<span>${n(B(p,e,t))}</span>
        </button>
      `).join("")}
    </div>
  `:""}function P(a={},e=g){const t=u(a.partySize);return`
    <output class="mnyra-go-page__party-value" data-go-party-value for="mnyraGoParty">
      ${n(String(t))} <span>${n(S(t,e))}</span>
    </output>
    <input
      id="mnyraGoParty"
      type="range"
      ...
      step="1"
      value="${t}"
      style="--go-range-fill:${T(t)}%;"
      aria-label="${n(e.partyQuestion)}"
      aria-valuetext="${n(z(t,e))}"
    />
    <div class="mnyra-go-page__range-scale" aria-hidden="true">
      <span>${l}</span>
      <span>${m}</span>
    </div>
  `}function L(a={},e=g){const t=String(a.category||"all");return`
    <div class="mnyra-go-page__chips" role="group" aria-label="${n(e.categoryQuestion)}">
      ${b.map((r,o)=>M(r.label,{active:t===r.key,attr:"data-go-category",value:r.key,iconName:r.icon,wide:o===0})).join("")}
    </div>
  `}function N(a={},e=g){const t=String(a.when||"now");return`
    <div class="mnyra-go-page__chips" role="group" aria-label="${n(e.whenQuestion)}">
      ${x.map(r=>M(r.label,{active:t===r.key,attr:"data-go-when",value:r.key,iconName:G[r.key]||"clock"})).join("")}
    </div>
    ${t==="later"?`
      <p class="mnyra-go-page__field-label">${n(e.whenLaterLabel)}</p>
      <input
        type="datetime-local"
      ...
      />
    `:""}
  `}function I(a={},e=g){const t=String(a.city||"").trim();return a.editCity?`
    ...
    <div class="mnyra-go-page__place">
      ...
      <button type="button" class="mnyra-go-page__place-change" data-go-change-city>
        ${i("pencil")}${n(e.locationChange)}
      </button>
    </div>
  `}function R(a={},e=g){const t=f(a.step),r=c.indexOf(t),o=c.length,p=Math.round((r+1)/o*100);let s="",d="",h="";return t==="party"?(d=e.partyQuestion,s=P(a,e),h=`<button type="button" class="mnyra-go-page__cta" data-go-step-next>${n(e.next)}${i("chevron-right")}</button>`):t==="category"?(d=e.categoryQuestion,s=L(a,e)):t==="when"?(d=e.whenQuestion,s=N(a,e),String(a.when||"now")==="later"&&(h=`<button type="button" class="mnyra-go-page__cta" data-go-step-next>${n(e.next)}${i("chevron-right")}</button>`)):(d=e.placeQuestion,s=I(a,e),h=`<button type="button" class="mnyra-go-page__cta" data-go-submit>${i("search")}${n(e.submit)}</button>`),`
    <section class="mnyra-go-page__ask" data-go-step="${n(t)}">
      <header class="mnyra-go-page__ask-head">
        ${r>0?`<button type="button" class="mnyra-go-page__ask-back" data-go-step-back aria-label="${n(e.stepBack)}" title="${n(e.stepBack)}">${i("arrow-left")}</button>`:""}
        <div class="mnyra-go-page__ask-meta">
          <p class="mnyra-go-page__ask-count">${n(e.step)} ${r+1}/${o}</p>
      ...
      <h2 class="mnyra-go-page__q">${i($[t]||"users")}${n(d)}</h2>
      ${s}
      ${h}
    </section>
  `}function O(a={},{texts:e=g,busyOfferId:t="",nowMs:r=Date.now()}={}){const o=t&&t===a.offerId,p=Number.isFinite(Number(a.distanceKm))&&a.distanceKm!==null?`${Number(a.distanceKm).toFixed(1)} km`:"",s=a.isNow?e.now:j(a.expectedArrivalAt,{nowMs:r});return`
    <article class="mnyra-go-page__card" data-go-result="${n(a.offerId)}">
      <div class="mnyra-go-page__card-head">
        ${a.logoUrl?`<img class="mnyra-go-page__card-logo" src="${n(a.logoUrl)}" alt="" width="40" height="40" loading="lazy" decoding="async" />`:`<div class="mnyra-go-page__card-logo mnyra-go-page__card-logo--empty">${i("store")}</div>`}
      ...
      <div class="mnyra-go-page__card-meta">
        <span>${i("users")}${n(`${a.partySize} ${e.peopleSuffix}`)}</span>
        ${s?`<span>${i("clock")}${n(s)}</span>`:""}
        ${p?`<span>${i("map-pin")}${n(p)}</span>`:""}
        ${a.bookingType==="reservation"?`<span>${i("armchair")}${n(e.tableIncluded)}</span>`:""}
      </div>

      <p class="mnyra-go-page__card-only">${i("ticket-percent")}${n(e.onlyGo)}</p>
      ...
      <button
        ...
        ${o?"disabled":""}
      >${o?"":i("check-check")}${n(o?e.confirming:e.accept)}</button>
    </article>
  `}function C(a={},e=g){const t=Array.isArray(a.results)?a.results:[],r=Number(a.nowMs)||Date.now();return t.length?`
    <h2 class="mnyra-go-page__lead">${t.length} ${n(e.resultsHeadline)}</h2>
    ${a.notice?`<p class="mnyra-go-page__note">${n(a.notice)}</p>`:""}
    ${t.map(o=>O(o,{texts:e,busyOfferId:a.busyOfferId,nowMs:r})).join("")}
    <button type="button" class="mnyra-go-page__cta mnyra-go-page__cta--quiet" data-go-back>
      ${i("arrow-left")}${n(e.back)}
    </button>
  `:`
      <h2 class="mnyra-go-page__lead">${n(e.emptyTitle)}</h2>
      <p class="mnyra-go-page__hint">${n(e.emptySubtitle)}</p>
      <button type="button" class="mnyra-go-page__cta mnyra-go-page__cta--quiet" data-go-back>
        ${i("arrow-left")}${n(e.back)}
      </button>
    `}function q(a={},e=g){const t=a.booking||{},r=t.type==="reservation",o=Number(a.nowMs)||Date.now(),p=j(t.expectedArrivalAt,{nowMs:o}),s=String(a.statusLabel||"").trim()||(r?e.reservationConfirmed:e.claimConfirmed);return a.confirmCancel?`
    ...
    `:`
    <p class="mnyra-go-page__done">${n(e.doneTitle)} ${i("party-popper")}</p>
    <p class="mnyra-go-page__done-name">${n(t.businessName||"")}</p>
    <p class="mnyra-go-page__card-for">${n(t.benefitLabel||"")}</p>

    <div class="mnyra-go-page__card-meta">
      <span>${i("users")}${n(`${t.partySize||1} ${e.peopleSuffix}`)}</span>
      ${p?`<span>${i("clock")}${n(p)}</span>`:""}
      ${t.city?`<span>${i("map-pin")}${n(t.city)}</span>`:""}
    </div>

    <p class="mnyra-go-page__ok">${i("circle-check-big")}${n(s)}</p>

    ${t.shortCode?`
      <div class="mnyra-go-page__code">
        <p class="mnyra-go-page__code-label">${i("hash")}${n(e.code)}</p>
        ...
        >
          <figure class="mnyra-go-page__story-media">
            <img
              ...
              fetchpriority="${p?"high":"low"}"
              decoding="async"
              onerror="this.remove()"
            />
          </figure>
          <p class="mnyra-go-page__story-step">${n(String(o+1))} / ${n(String(t))}</p>
          <p class="mnyra-go-page__story-text">${n(r.text)}</p>
        </article>
      `}).join("")}
    </div>
  `}function H(a={},e=g){return`
    ...
    <section class="mnyra-go-page__info">
      ...
      ${w.map(t=>`
        <div class="mnyra-go-page__info-row">
          <span class="mnyra-go-page__info-ic">${i(t.icon)}</span>
          <div>
            <p class="mnyra-go-page__info-title-row">${n(t.title)}</p>
            <p class="mnyra-go-page__info-text">${n(t.text)}</p>
          </div>
        </div>
      `).join("")}
    </section>
  `}function re(a={}){const e={...g,...a.texts||{}},t=String(a.view||"search"),r=t==="search";let o="";return t==="loading"?o=`<div class="mnyra-go-page__empty">${n(e.searching)}</div>`:t==="results"?o=C(a,e):t==="booking"?o=q(a,e):t==="error"?o=K(a,e):o=H(a,e),`
    <div class="mnyra-go-page" data-go-page data-go-view="${n(t)}">
      ${r?`<div class="mnyra-go-page__top">${R(a.form||{},e)}</div>`:""}
      <div class="mnyra-go-page__bento" data-go-bento>${o}</div>
    </div>
  `}const oe=g,ie=y,pe=k,ge=w;export{ae as GO_PAGE_CSS,ge as GO_PAGE_INFO_ROWS,pe as GO_PAGE_STORY_BASE,ie as GO_PAGE_STORY_SLIDES,ee as GO_PAGE_STYLE_ELEMENT_ID,oe as GO_PAGE_TEXTS,c as GO_STEPS,u as clampGoPartySize,T as goPartyFillPercent,z as goPartyLabel,S as goPartyWord,ne as nextGoStep,te as previousGoStep,re as renderGoPageCore,f as resolveGoStep};
