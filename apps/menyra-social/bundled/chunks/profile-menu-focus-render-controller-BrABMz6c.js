        ...
        <div class="mb-6 bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-sm">
          ...
          <button type="button" data-hotel-card-details-open aria-expanded="${S?"true":"false"}" class="w-full flex items-start gap-4 p-4 rounded-[1.6rem] bg-slate-50 border border-slate-100 text-left active:scale-[0.99] transition-transform">
            <div class="w-16 h-16 rounded-2xl overflow-hidden bg-white shrink-0">
              <img src="${n(F||N)}" class="w-full h-full object-cover" loading="lazy" decoding="async" />
            </div>
          ...
          ${r&&!S?`<div class="text-center text-[10px] font-black uppercase tracking-widest mt-4 ${$?"text-rose-500":"text-slate-500"}">${n(r)}</div>`:""}
        </div>
        ...
        <div data-hotel-card-editor="${n(a)}" data-hotel-card-details-panel class="${S?"":"hidden "}bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-5 mb-6">
            ...
            <div class="grid grid-cols-1 gap-4">
              ...
              <div>
                ...
                <input id="hotelCardStartingPrice" type="text" value="${n(I)}" placeholder="145" inputmode="decimal" class="w-full mt-2 px-5 py-4 bg-slate-50 rounded-2xl text-sm font-bold border-none outline-none focus:ring-2 focus:ring-indigo-100" />
              </div>
            </div>
...
`))}</textarea>
    ...
    <section data-landing-swipe-root="true" class="relative w-full overflow-hidden font-sans" style="height: calc((var(--viewport-height, 1vh) * 100) - var(--smart-header-total-height, 4.5rem)); min-height: calc((var(--viewport-height, 1vh) * 100) - var(--smart-header-total-height, 4.5rem)); overscroll-behavior: none; -webkit-overflow-scrolling: auto; touch-action: none; user-select: none; background: #F8F9FA; --landing-panel-duration: 460ms; --landing-greeting-duration: 720ms; --landing-top-gap: 14px; --landing-swipe-bottom: 0.45rem;">
      ...
      <div data-landing-panel="0" class="absolute inset-0 z-50 flex flex-col items-start justify-center transition-transform ${a===0?"translate-y-0":"-translate-y-full pointer-events-none"}" style="background: #F8F9FA; color: #111827; padding-top: var(--landing-top-gap); opacity: ${a===0?"1":"0"}; transition-property: transform, opacity; transition-duration: var(--landing-panel-duration); transition-timing-function: cubic-bezier(0.23,1,0.32,1); will-change: transform, opacity;">
        ...
        <div class="flex flex-col items-start relative z-10 w-full" style="padding-left: 2.5rem; padding-right: 2.5rem;">
          <div class="relative w-full flex justify-start items-center mb-5" style="height: 40px;">
            ${t.map((R,h)=>{const A=h===s,T=h===(s-1+t.length)%t.length;return`
      ...
      <div data-landing-panel="2" class="absolute inset-0 transition-transform ${a<2?"translate-y-full":a===2?"translate-y-0":"-translate-y-full"}" style="background: #F8F9FA; opacity: ${a===2?"1":"0"}; pointer-events: ${a===2?"auto":"none"}; transition-property: transform, opacity; transition-duration: var(--landing-panel-duration); transition-timing-function: cubic-bezier(0.23,1,0.32,1); will-change: transform, opacity;">
        <div data-landing-panel-scroll="2" class="h-full overflow-y-auto overscroll-contain" style="-webkit-overflow-scrolling: touch; touch-action: pan-y; overscroll-behavior-y: none; padding-top: var(--landing-top-gap); padding-bottom: 0;">
          ${F?_e(e,$,{topTabOverride:"profile",tutorialMode:!0,contentTabOverride:"posts",landingHideContent:!1,collapseIdentity:!0,contentReveal:!0,landingMode:!0}):""}
        </div>
        ${z}
      </div>
  ...
  `}function Et(e={},t={}){const a=t.mode==="self"?"self":"public",s=t.disabledBlockClass||"",i=un(e,a),r=a==="self"?"avatar:self":`avatar:public:${i}`,o=t.avatarUrl||E(e.avatar||"","avatar",{stableKey:r}),l=t.avatarFit||He(!!e.restaurantId),c=String(d?.profileCardInfoOpen||"")===i,p=Number(d?.profileCardInfoHeights?.[i]||0),u=c&&Number.isFinite(p)&&p>0?`height:${Math.ceil(p)}px;`:"",f=t.avatarImgKeyAttr||(a==="self"?'data-img-key="avatar:self"':`data-img-key="avatar:public:${n(i)}"`),m=t.renderAvatarImage===!0?!!String(o||"").trim()&&!O(o):t.renderAvatarImage!==!1&&!!String(o||"").trim()&&!O(o)&&!!String(e?.avatar||"").trim(),b=!!t.identityPending,k=t.followersLabel??B(e.followers),v=D(e?.name)||"User",I=D(t.typeLabel||e?.customerType||e?.type||"Business"),P=D(e?.location||"-"),w=a==="public"?`${P} / ${I}`:P,S=t.bioHtml||n(e?.bio||"").replace(/\n/g,"<br>")||n(g("profile.noBio","Noch keine Bio.")),j=`business-cover:${i}`,F=ln(e,i),L=gn(e,{cacheKeys:F,stableKey:j,allowCacheFallback:t.allowTitleImageCacheFallback===!0}),$=bn(e),_=pn(e),z=Le(_?{buttonAttrs:`data-marketplace-open-map="${n(_)}"`,label:g("profile.openMap","Karte oeffnen"),iconName:"map"}:{href:$,label:g("profile.openMap","Karte oeffnen"),iconName:"map"}),R=Mt(e?.instagramUrl||e?.instagram||e?.insta||"","instagram"),h=Mt(e?.tiktokUrl||e?.tiktok||e?.tikTok||"","tiktok"),A=D(e?.phone||e?.telephone||e?.contactPhone||""),T=mn(A),U=D(e?.address||e?.locationLabel||[e?.place||e?.locationPlace,e?.location||e?.city].map(q=>D(q)).filter(Boolean).join(", ")),M=[Te({href:R,iconName:"instagram",eyebrow:"Instagram",value:e?.instagram||e?.instagramUrl||e?.insta||""}),Te({href:h,iconName:"music-2",eyebrow:"TikTok",value:e?.tiktok||e?.tiktokUrl||e?.tikTok||""})].filter(Boolean).join(""),H=a==="self"?`
    ...
    `;if(c){const q=[Te({href:T,iconName:"phone",eyebrow:g("profile.call","Anrufen"),value:A}),Te({href:$,iconName:"map-pin",eyebrow:g("profile.address","Adresse"),value:U||P}),M].filter(Boolean).join("");return`
      <div data-landing-tutorial-target="identity" data-business-profile-card="${n(i)}" class="bg-white rounded-[2.5rem] relative overflow-hidden z-10 border border-slate-100 shadow-sm ${s}" style="${u}min-height: var(--business-profile-card-min-height, 440px);display:grid;grid-template-columns:minmax(0,1fr);width:100%;max-width:100%;min-width:0;box-sizing:border-box;">
        ${hn({profileName:v,safeBio:S,metaLine:w,identityPending:b,followersLabel:k})}
  ...
  `}function _e(e={},t=[],{topTabOverride:a="",tutorialMode:s=!1,contentTabOverride:i="",landingHideContent:r=!1,collapseIdentity:o=!1,contentReveal:l=!1,landingMode:c=!1}={}){const p=Ua(e),u=!!e.privateAccount&&e.uid&&String(e.uid)!==String(d.user?.uid||"")&&!p,f=!!e.pendingFollowRequest&&!p,m=e.restaurantId?"Business":g("nav.user","User"),b=String(e.handle||gt(e.name||"user")).replace(/^@/,""),v=n(e.bio||"").replace(/\n/g,"<br>")||n(g("profile.noBio","Noch keine Bio.")),I=Ce(e),P=String(a||Ze(e)).trim().toLowerCase()||"profile",w=String(i||Ae(e)).trim().toLowerCase()||"posts",S=w==="menu",j=w==="checkins",F=t,$={...d?.profileView&&typeof d.profileView=="object"?d.profileView:{},profile:e,posts:Array.isArray(F)?F:[]},_=as(d,{profileView:$,profileTopTab:P,profileContentTab:w}),z=String(_?.header?.status||"").trim().toLowerCase()||"loading",R=String(_?.posts?.status||"").trim().toLowerCase()||"loading",h=e.uid||e.restaurantId||b||"public",A=`avatar:public:${h}`,T=String(e?.avatar||"").trim(),U=E(T,"avatar",{stableKey:A}),M=He(!!e.restaurantId),H=c?"":`data-img-key="avatar:public:${n(h)}"`,q=!T&&!!String(U||"").trim()&&!O(U),G=!!T||q&&xe(z),re=pt=>{if(pt==null)return!1;const ra=Number(pt);return Number.isFinite(ra)&&ra>=0},ut=G||re(e?.followers)||re(e?.following),J=xe(z)&&!ut,ie=!!String(U||"").trim()&&!O(U)&&G,Me=J?"...":B(e.followers),Ee=J?"...":B(e.following),Be=I?"pt-2":"pt-10",Ne=p?g("profile.following","Following"):f?g("profile.requested","Requested"):u?g("profile.request","Request"):g("profile.follow","Follow"),me=p?"bg-slate-100 text-slate-600 shadow-none border border-slate-200":f?"bg-amber-50 text-amber-700 shadow-none border border-amber-200":"bg-gradient-to-r from-slate-900 to-slate-800 text-white border border-transparent",be=s?"select-none":"app-main-content-safe",X=s?"pointer-events-none":"",Y=!o,na=!r,De=l?c?"transition-opacity duration-200":"animate-in fade-in duration-300":"",sa=w==="posts"&&F.length>0,Yn=w!=="posts"||sa||R==="empty"||R==="error",Zn=w==="posts"&&!sa&&R==="error";return!s&&(w==="posts"||w==="media")&&e?.restaurantId&&xe(R)&&da(e),`
    <div class="${be}" ${s?'data-landing-tutorial-surface="true"':""}>
      ${P==="profile"||P==="menu"?`
      ${Y?`
        <div class="app-content-inline pb-2 ${Be}">
          ${I?Et(e,{mode:"public",disabledBlockClass:X,avatarUrl:U,avatarFit:M,avatarImgKeyAttr:H,renderAvatarImage:ie,identityPending:J,followersLabel:Me,followLabel:Ne,followTone:me,isFollowing:p,hasPendingFollowRequest:f,isLocked:u,bioHtml:v,typeLabel:m,allowTitleImageCacheFallback:xe(z)||xe(R)}):`
          <div data-landing-tutorial-target="identity" class="bg-white rounded-[2.5rem] p-8 relative overflow-hidden z-10 border border-slate-100 ${X}">
            <div class="relative z-10">
              ...
              <div class="mb-8">
                ...
                ${I?"":`<p class="text-[11px] font-bold text-slate-400 uppercase tracking-[0.3em] mb-2">@${n(b)}</p>`}
          ...
          ${Yn?`
            ...
            `:`
              <div class="${d.profileViewMode==="grid"?"grid grid-cols-2 gap-4 app-content-inline grid-flow-dense":"flex flex-col gap-8 app-content-inline"} ${X} ${De}">
                ${Ge(F,d.profileViewMode,!1,{includeImageKeys:!c})}
              </div>
            `}
          `:`
            <div class="app-content-inline ${X}">
    ...
    <div ${i} class="bg-white p-3.5 rounded-[2.2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-5 group relative ${t==="profile"?"cursor-pointer":""}" style="padding:14px;border-radius:2.2rem;margin-bottom:20px;box-sizing:border-box;">
      <div class="w-full aspect-[16/9] rounded-[1.8rem] overflow-hidden bg-slate-100 mb-4 relative" style="aspect-ratio:16 / 9;border-radius:1.8rem;margin-bottom:16px;">
        ${p?`
          ...
          >
            ${c.map((v,I)=>{const P=t==="profile"?ue(e,{index:I}):"",w=ae(v||"",{mode:t,priorityIndex:a,slideIndex:I,stableKey:P,preferredSize:"medium",candidateSizes:["small","medium","large"],variant:"hero"}),S=I>0,j=S?N:w.safeImg,F=S?N:w.fallbackImg,L=S?w.loadingAttrs:w.imageAttrs,$=S?"":w.lazyAttrs||"",_=S?` data-menu-card-deferred-src="${n(w.safeImg)}"
                ...
                <div class="min-w-full h-full snap-center relative" data-menu-card-gallery-slide="${I}" style="min-width:100%;width:100%;height:100%;scroll-snap-align:center;">
                  <img src="${n(j)}" data-fallback-src="${n(F)}"${$}${_} class="w-full h-full object-cover select-none pointer-events-none" draggable="false" style="object-position:${V(e)};" ${L} decoding="async" />
                </div>
              `}).join("")}
          </div>
        `:`
          ${c.map((v,I)=>{const P=t==="profile"?ue(e,{index:I}):"",{safeImg:w,fallbackImg:S,imageAttrs:j,lazyAttrs:F}=ae(v||"",{mode:t,priorityIndex:a,slideIndex:I,stableKey:P,preferredSize:"medium",candidateSizes:["small","medium","large"],variant:"hero"});return`
              <div class="w-full h-full">
                <img src="${n(w)}" data-fallback-src="${n(S)}"${F} class="w-full h-full object-cover select-none pointer-events-none" draggable="false" style="object-position:${V(e)};" ${j} decoding="async" />
              </div>
            `}).join("")}
        `}
        <button
  ...
  `}function zn(e,t,{mode:a="profile",publicMenuSurfaceState:s=null,focusFallbackHtml:i=""}={}){const r=ze(Array.isArray(t)?t:[]),o=String(e?.restaurantId||"").trim(),l=a==="admin"||_a(o),c=s?.focus?.canRenderFocus?{items:Array.isArray(s.focus.items)?s.focus.items:[],enabled:!0}:o&&l?ye(o):{items:[],enabled:!1},p=c.enabled?(Array.isArray(c.items)?c.items:[]).map(h=>rt({...h,objectPosition:le(h)})):[],u=r.filter(h=>ne(h)==="testfirst_focus"&&!et(h)).map(h=>rt(h,{menuItemId:h.id||""})).filter(Boolean),f=new Set,m=[...p,...u].filter(h=>{const A=String(h.menuItemId||h.id||`${h.title}|${h.text}|${h.imageUrl}`);return!A||f.has(A)?!1:(f.add(A),!0)}),b=r.filter(h=>!et(h)),k=b.filter(h=>ne(h)!=="testfirst_focus"),v=k.length?k:b,I=k.length?m:[],P=v.filter(h=>ge(h)==="drink"),w=v.filter(h=>ge(h)!=="drink"),S=(h=[])=>{const A=[],T=[];return h.forEach(U=>{const M=ne(U);M==="testfirst_food"||M==="testfirst_special"&&tt(U)==="food"?T.push(U):A.push(U)}),{gridItems:A,foodItems:T}},j=(h,A=-1)=>ne(h)==="testfirst_special"?it(h,{mode:a,priorityIndex:A}):Wt(h,{mode:a,priorityIndex:A});let F=0;const L=()=>{const h=F;return F+=1,h},$=new Set,_=(h,A)=>!A.gridItems.length&&!A.foodItems.length?"":`
      <section class="menu-type-block relative" data-menu-type-block="${n(h)}">
        ...
        ${A.foodItems.length?`
          <div class="menu-category-section pb-6 pt-4" data-menu-type="${n(h)}">
            <div class="app-content-inline">
              ${ce(A.foodItems,T=>{const U=ne(T),M=L();return U==="testfirst_special"?it(T,{mode:a,size:"food",priorityIndex:M}):Yt(T,{mode:a,priorityIndex:M})},$)}
            </div>
          </div>
        `:""}
      </section>
  ...
  `:(Se({functionName:"renderMenuList",items:e,rawItems:e,filteredItems:e,renderDecision:"menu-list-no-products",source:t}),`
    ...
    <div class="mb-6 bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-sm">
      ...
      ${s.length?`
        <div class="space-y-3">
          ${s.map(b=>{const k=E(b.imageUrl||"","thumb"),v=O(k)?N:k,I=b.active!==!1?"Aktiv":"Inaktiv",P=b.active!==!1?"text-emerald-600":"text-slate-400";return`
              <div class="flex items-start gap-4 p-4 rounded-[1.6rem] bg-slate-50 border border-slate-100">
                <div class="w-16 h-16 rounded-2xl overflow-hidden bg-white shrink-0">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-[9px] font-black uppercase tracking-widest mt-2 ${P}">${I}</p>
                </div>
      ...
      <div class="grid grid-cols-1 gap-4">
        ...
        <div class="rounded-[1.8rem] border border-slate-100 bg-slate-50 p-4">
          <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">${n(B(F.length))}</span>
          </div>
          ${F.length?`
            <div class="grid grid-cols-1 gap-2">
              ${F.map($=>{const _=w.has($.id),z=$.imageUrl||N,R=String($.defaultImageRaw||$.imageCandidates[0]?.rawUrl||"").trim(),h=String($.cardImageUrl||"").trim(),A=String($.previewImageUrl||"").trim(),T=!!(A||h&&h!==R),U=A||(h&&!$.imageCandidates.some(M=>M.rawUrl===h)?h:"");return`
                    ...
                    ${_?`
                      <div class="mt-3 pt-3 border-t border-slate-100">
                        ...
                        <div class="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
                          ${$.imageCandidates.map((M,H)=>{const q=H===0,G=A?!1:q?!T:h===M.rawUrl;return`
  ...
  `}function ta(e,{restaurantId:t="",suppressLoading:a=!1,allowAutoEnsure:s=!0,requirePublicMenuTruth:i=!0}={}){const r=String(t||e?.canonicalRestaurantId||e?.restaurantId||"").trim();if(!r||!se(e))return"";const o=he(d,{profile:e,routePayload:d?.profileView?.routePayload,webDirectEntry:d?.__webDirectEntry,restaurantId:r});if(i&&o.menu.status!=="ready")return"";const l=!i||o.focus.canRenderFocus;if(s&&!d.focus.loading&&!l&&ve(Pt(e,r)),i&&!l)return"";const{items:c,loading:p}=l?{items:Array.isArray(o.focus.items)?o.focus.items:[],loading:o.focus.loading}:ye(r);if(!(l?!0:ye(r).enabled)||!c.length&&!p||a&&p&&!c.length)return"";if(p&&!c.length)return`
    ...
    `;const f=ka(c),m=c[f]||c[0],{safeImg:b,fallbackImg:k,imageAttrs:v,lazyAttrs:I}=ae(m.imageUrl||"",{mode:"profile",priorityIndex:0,preferredSize:"medium",candidateSizes:["small","medium","large"],variant:"hero",stableKey:m?.id?`focus-carousel:${r}:${String(m.id)}`:""}),P=m.text||"";return`
    <div id="focusCarousel" class="${Oe()} rounded-[2.5rem] p-6 border shadow-sm">
      ...
      <div class="relative rounded-[2rem] overflow-hidden border border-slate-100 bg-slate-50">
        ...
        `:`
          <img data-focus-media="image" data-focus-image src="${n(b)}" data-fallback-src="${n(k)}"${I} class="w-full h-56 object-cover" style="object-position:${le(m)};" ${v} decoding="async" />
        `}
      </div>
      <div class="mt-4">
      ...
      ${c.length>1?`
        <div class="flex items-center justify-center gap-2 mt-4">
          ${c.map((S,j)=>`
            <button type="button" data-focus-dot="${j}" class="w-2.5 h-2.5 rounded-full ${j===f?"bg-slate-900":"bg-slate-200"}"></button>
          `).join("")}
        </div>
      `:""}
    </div>
  ...
  `:c?Ue(l):`
    ...
    `;const l=Pt(e,o),c=Q(l),p=se(l)&&!c;p&&(r=he(d,{profile:l,routePayload:s,webDirectEntry:i,restaurantId:o}));const u=String(i?.canonicalRestaurantId||i?.restaurantId||"").trim(),f=new Set(r.targetIds),m=r.menu.status==="ready",b=r.focus.canRenderFocus,k=m&&p,v=r.focus.matches===!0&&r.focus.loading===!0,P=String(d?.profileView?.menuAccessSource||i?.menuAccessSource||s?.menuAccessSource||"").trim().toLowerCase()==="qr",w=i?.active===!0&&i?.webPriority===!0&&i?.menuFirst===!0&&String(d?.activeTab||"").trim().toLowerCase()==="profile"&&String(d?.profileTopTab||"").trim().toLowerCase()==="menu"&&(u===o||f.has(o)),S=w&&!P,j=["ready","empty","error"].includes(r.menu.status),F=w&&j,L=w&&(!k||r.menu.status!=="ready"),$=!k||r.focus.settled===!0||r.focus.confirmedEmpty===!0||r.menu.status!=="ready";a&&!F&&!j&&ca(l),a&&!L&&!$&&!v&&m&&(!S||j)&&ve(l);const z=r.menu.canRenderItems?ze(Bt(r.menu.items,{filter:"all",query:""})).filter(Y=>!et(Y)):[],R=r.menu.error||"",h=Xn(r.menu,z),{hasItems:A,hasError:T,isLoading:U,shouldRenderNoProducts:M}=h;Pa({profile:l,routePayload:s,surface:r,decision:h});const H={profile:l,routePayload:s,surface:r,decision:h,rawItems:r.menu.items,items:z,filteredItems:z,source:"public-menu"},q=Fa(r,z),G=z.filter(Y=>ge(Y)==="drink"),re=z.filter(Y=>ge(Y)!=="drink"),ut=0,J=G.length,ie=st(e),Me=ie||c,Ee=new Set;A&&o&&(ga(z,o),Ra(z,o));const Be=o&&b?(Array.isArray(r.focus.items)?r.focus.items:[]).map(Y=>rt({...Y,objectPosition:le(Y)})).filter(Boolean):[],Ne=r.focus.status==="empty"||r.focus.status==="error",me=p&&!b&&!Ne&&r.menu.status!=="empty"&&r.menu.status!=="error",be=Be.length?Qt(l,Be,{mode:t}):me?Fn({...H,reason:"focus-truth-pending"}):"",X=Me?be:ta(l,{restaurantId:o,suppressLoading:!0,allowAutoEnsure:m&&(!S||j),requirePublicMenuTruth:!0})||(me?jn({...H,reason:"focus-truth-pending"}):"");return ie?`
      <div class="app-main-content-safe"${q}>
        ${U?`
          ${be}
          ${Kt({...H,reason:"menu-loading"})}
        `:`
          ${A?zn(l,z,{mode:t,publicMenuSurfaceState:r,focusFallbackHtml:be}):T?`<div class="app-content-inline pt-6 text-center text-[10px] font-bold uppercase tracking-widest text-rose-500">${n(g("menu.loadError","Menu konnte nicht geladen werden"))}</div>`:M?(Se({...H,functionName:"renderProfileMenuView",renderDecision:"testfirst-no-products"}),`<div class="app-content-inline pt-6 text-center text-[10px] font-bold uppercase tracking-[0.3em] text-slate-300"${W({source:"public-menu:no-products"})}>${n(g("menu.noProducts","Keine Produkte"))}</div>`):Kt({...H,reason:"menu-not-confirmed-empty"})}
  ...
  `}function Wn(){const e=d.userProfile,t=oa(e),a=t?d.businessPosts:d.userPosts,s=String(d.user?.uid||e?.uid||"").trim(),i=String(e?.restaurantId||"").trim(),r=String(d.__userPostsLoadingUid||"").trim(),o=String(d.__businessPostsLoadingRestaurantId||"").trim(),l=String(d.__authBootstrapInFlightUid||"").trim(),c=!!s&&r===s,p=!!i&&o===i,u=!!s&&l===s,f=t?p||u&&!a.length:c||u&&!a.length,m=String(e.handle||gt(e.name||"user")).replace(/^@/,""),k=n(e.bio||"").replace(/\n/g,"<br>")||n(g("profile.noBio","Noch keine Bio.")),v=Ae(e),I=v==="menu",P=v==="checkins",w=a,S=E(e.avatar,"avatar"),j=He(t),F=Ze(e);return`
    <div class="app-main-content-safe">
      ...
      <div class="app-content-inline pb-2 ${t?"pt-2":"pt-10"}">
        ...
        ${t?Et(e,{mode:"self",avatarUrl:S,avatarFit:j,followersLabel:B(e.followers),bioHtml:k}):`
          ...
          <div class="relative z-10">
            <div class="flex justify-between items-start mb-8">
              <div id="profileAvatarTrigger" class="relative cursor-pointer group">
                <div class="relative w-[100px] h-[100px] rounded-[2rem] p-[3px] bg-gradient-to-br from-indigo-500 to-purple-500">
                  <img src="${n(S)}" data-fallback-src="${n(N)}" decoding="async" width="100" height="100" data-img-key="avatar:self" class="w-full h-full rounded-[1.8rem] ${j} border-2 border-white" />
                </div>
                ${e.isPremium?`
                  <div class="absolute -bottom-1 -right-1 bg-white rounded-full p-1.5 shadow-lg text-blue-500 border-2 border-slate-50">
                    ${x("badge-check","w-4 h-4 fill-blue-500 text-white")}
