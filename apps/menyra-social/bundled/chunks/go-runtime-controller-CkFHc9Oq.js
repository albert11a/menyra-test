...
`,u=Object.freeze({brand:"Mnyra GO",brandWord:"MNYRA",brandGo:"GO",close:"Mbyll",leadTitle:"Zbritje ose ofertë nga lokalet përreth teje",leadBody:"Ti thua vetëm sa veta jeni, çka doni dhe kur. Lokalet të kthejnë ofertën e tyre.",partyQuestion:"Sa veta jeni?",partyOne:"person",partyMany:"veta",step:"Hapi",stepBack:"Kthehu një hap prapa",next:"Vazhdo",categoryQuestion:"Çka dëshironi?",whenQuestion:"Kur?",whenLaterLabel:"Zgjidh ditën dhe orën",placeQuestion:"Ku?",placeLabel:"Qyteti",placeEmpty:"Shto qytetin tënd",cityPlaceholder:"Shkruaj qytetin",locationChange:"Ndrysho",locationSave:"Ruaj",submit:"Shiko ofertat",searching:"Po kërkojmë...",resultsHeadline:"lokale kanë oferta për ju",offering:"po ju ofron",forGroup:"për grupin tuaj",accept:"Prano ofertën",confirming:"Po konfirmohet...",sponsored:"Sponsored",onlyGo:"Vetëm me Mnyra GO",tableIncluded:"Tavolinë",emptyTitle:"Nuk gjetëm ofertë GO që përputhet tani.",emptySubtitle:"Provo me një orë tjetër ose me një grup tjetër.",doneTitle:"U krye",reservationConfirmed:"Tavolina është konfirmuar",claimConfirmed:"Oferta është e juaja",menu:"Shiko menunë",directions:"Udhëzime",code:"Kodi",cancel:"Anulo",cancelConfirm:"Dëshiron ta anulosh?",cancelYes:"Po, anuloje",cancelNo:"Jo",saveToAccount:"Ruaje në llogarinë tënde",signIn:"Hyr",errorTitle:"Mnyra GO është përkohësisht i padisponueshëm.",errorRetry:"Provo prapë",alternatives:"alternativa për ju",peopleSuffix:"persona",now:"Tani",back:"Ndrysho kërkimin",infoTitle:"Mirë të dihet"}),Se=Object.freeze([Object.freeze({icon:"users",step:"1",title:"Thuaj sa veta jeni",text:"Sa veta, çka doni dhe kur. Zgjat më pak se 10 sekonda."}),Object.freeze({icon:"badge-percent",step:"2",title:"Lokalet të bëjnë ofertë",text:"Lokalet përreth teje kthejnë zbritje ose diçka falas, vetëm për grupin tënd."}),Object.freeze({icon:"check-check",step:"3",title:"Zgjedh njërën",text:"E pranon ofertën me një prekje. Tavolina të mbetet e ruajtur."}),Object.freeze({icon:"party-popper",step:"4",title:"Shko dhe shijo",text:"Tregon kodin te lokali dhe oferta vlen aty për aty."})]),Me=Object.freeze([Object.freeze({icon:"gift",title:"Falas për ty",text:"Mnyra GO nuk kushton asgjë. Paguan vetëm atë që konsumon te lokali."}),Object.freeze({icon:"shield-check",title:"Pa llogari",text:"Nuk të duhet regjistrim. Kërkon, pranon ofertën dhe shkon."}),Object.freeze({icon:"ticket-percent",title:"Vetëm me Mnyra GO",text:"Këto oferta nuk janë në menu e as në rrjete sociale."}),Object.freeze({icon:"store",title:"Lokalet vendosin vetë",text:"Çdo lokal zgjedh çka ofron, për sa veta dhe në cilat orë."})]),ze=Object.freeze({now:"zap",in30:"timer",in60:"clock",later:"calendar-clock"}),h=Object.freeze(["party","category","when","place"]),je=Object.freeze({party:"users",category:"utensils",when:"clock",place:"map-pin"});function L(a=""){const e=String(a||"").trim();return h.includes(e)?e:h[0]}function J(a=""){const e=h.indexOf(L(a));return h[Math.min(h.length-1,e+1)]}function Oe(a=""){const e=h.indexOf(L(a));return h[Math.max(0,e-1)]}function n(a=""){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function S(a){const e=Math.trunc(Number(a));return Number.isFinite(e)?Math.min(E,Math.max(j,e)):me}function D(a,e=u){return S(a)===1?e.partyOne||u.partyOne:e.partyMany||u.partyMany}function R(a,e=u){return`${S(a)} ${D(a,e)}`}function re(a){const e=S(a),i=E-j;return Math.round((e-j)/i*100)}function oe(a,{active:e=!1,attr:i="",value:s="",iconName:p="",wide:g=!1}={}){return`
    <button
      type="button"
      class="mnyra-go__chip${g?" mnyra-go__chip--wide":""}"
      ${i?`${i}="${n(s)}"`:""}
      aria-pressed="${e?"true":"false"}"
    >${p?l(p):""}<span>${n(a)}</span></button>
  `}function ie(a,{nowMs:e=Date.now()}={}){const i=Date.parse(String(a||""));if(!Number.isFinite(i))return"";if(Math.abs(i-e)<900*1e3)return u.now;const s=new Date(i);return`Rreth ${String(s.getHours()).padStart(2,"0")}:${String(s.getMinutes()).padStart(2,"0")}`}function Ae(){return`
    <div class="mnyra-go__how" data-go-how>
      ${Se.map(a=>`
        <article class="mnyra-go__how-card">
          <span class="mnyra-go__how-plate">${l(a.icon)}</span>
  ...
  `}function Ge(a={},e=u){const i=String(a.city||"").trim();return a.editCity?`
      <div class="mnyra-go__place">
        <span class="mnyra-go__place-ic">${l("map-pin")}</span>
    ...
    <div class="mnyra-go__place">
      ...
      <button type="button" class="mnyra-go__place-change" data-go-change-city>
        ${l("pencil")}${n(e.locationChange)}
      </button>
    </div>
  `}function Ee(a="",e={},i=u){if(a==="party")return R(e.partySize,i);if(a==="category"){const s=ae.find(p=>p.key===String(e.category||"all"));return s?s.label:""}if(a==="when"){const s=ne.find(p=>p.key===String(e.when||"now"));return s?s.label:""}return a==="place"?String(e.city||"").trim()||i.placeEmpty:""}function Le(a="",e={},i=u){const s=h.indexOf(a),p=h.slice(0,Math.max(0,s));return p.length?`
    <div class="mnyra-go__ask-done">
      ${p.map(g=>`
        <button type="button" class="mnyra-go__ask-tag" data-go-goto="${n(g)}">
          ${l(je[g]||"check-check")}<span>${n(Ee(g,e,i))}</span>
        </button>
      `).join("")}
    </div>
  `:""}function Te(a={},e=u){const i=S(a.partySize);return`
    <output class="mnyra-go__party-value" data-go-party-value for="mnyraGoParty">
      ${n(String(i))} <span>${n(D(i,e))}</span>
    </output>
    <input
      id="mnyraGoParty"
      type="range"
      ...
      step="1"
      value="${i}"
      style="--go-range-fill:${re(i)}%;"
      aria-label="${n(e.partyQuestion)}"
      aria-valuetext="${n(R(i,e))}"
    />
    <div class="mnyra-go__range-scale" aria-hidden="true">
      <span>${j}</span>
      <span>${E}</span>
    </div>
  `}function Ie(a={},e=u){const i=String(a.category||"all");return`
    <div class="mnyra-go__chips" role="group" aria-label="${n(e.categoryQuestion)}">
      ${ae.map((s,p)=>oe(s.label,{active:i===s.key,attr:"data-go-category",value:s.key,iconName:s.icon,wide:p===0})).join("")}
    </div>
  `}function Be(a={},e=u){const i=String(a.when||"now");return`
    <div class="mnyra-go__chips" role="group" aria-label="${n(e.whenQuestion)}">
      ${ne.map(s=>oe(s.label,{active:i===s.key,attr:"data-go-when",value:s.key,iconName:ze[s.key]||"clock"})).join("")}
    </div>
    ${i==="later"?`
      ...
      />
    `:""}
  `}function Ne(a={},e=u){const i=L(a.step),s=h.indexOf(i),p=h.length,g=Math.round((s+1)/p*100);let f="",x="",v="users",w="";return i==="party"?(x=e.partyQuestion,v="users",f=Te(a,e),w=`<button type="button" class="mnyra-go__cta" data-go-step-next>${n(e.next)}${l("chevron-right")}</button>`):i==="category"?(x=e.categoryQuestion,v="utensils",f=Ie(a,e)):i==="when"?(x=e.whenQuestion,v="clock",f=Be(a,e),String(a.when||"now")==="later"&&(w=`<button type="button" class="mnyra-go__cta" data-go-step-next>${n(e.next)}${l("chevron-right")}</button>`)):(x=e.placeQuestion,v="map-pin",f=Ge(a,e),w=`<button type="button" class="mnyra-go__cta" data-go-submit>${l("search")}${n(e.submit)}</button>`),`
    <section class="mnyra-go__ask" data-go-step="${n(i)}">
      <header class="mnyra-go__ask-head">
        ${s>0?`<button type="button" class="mnyra-go__ask-back" data-go-step-back aria-label="${n(e.stepBack)}" title="${n(e.stepBack)}">${l("arrow-left")}</button>`:""}
        <div class="mnyra-go__ask-meta">
          <p class="mnyra-go__ask-count">${n(e.step)} ${s+1}/${p}</p>
          <div class="mnyra-go__ask-bar" role="progressbar" aria-valuemin="1" aria-valuemax="${p}" aria-valuenow="${s+1}">
            <span style="width:${g}%;"></span>
          </div>
        </div>
      </header>
      ...
      <h2 class="mnyra-go__q">${l(v)}${n(x)}</h2>
      ${f}
      ${w}
    </section>
  `}function De(a={},e=u){return`
  ...
  `}function se(a={},{texts:e=u,busyOfferId:i="",nowMs:s=Date.now()}={}){const p=i&&i===a.offerId,g=Number.isFinite(Number(a.distanceKm))&&a.distanceKm!==null?`${Number(a.distanceKm).toFixed(1)} km`:"",f=a.isNow?e.now:ie(a.expectedArrivalAt,{nowMs:s});return`
    <article class="mnyra-go__card" data-go-result="${n(a.offerId)}">
      <div class="mnyra-go__card-head">
        ${a.logoUrl?`<img class="mnyra-go__card-logo" src="${n(a.logoUrl)}" alt="" width="40" height="40" loading="lazy" decoding="async" />`:`<div class="mnyra-go__card-logo mnyra-go__card-logo--empty">${l("store")}</div>`}
      ...
      <div class="mnyra-go__card-meta">
        <span>${l("users")}${n(`${a.partySize} ${e.peopleSuffix}`)}</span>
        ${f?`<span>${l("clock")}${n(f)}</span>`:""}
        ${g?`<span>${l("map-pin")}${n(g)}</span>`:""}
        ${a.bookingType==="reservation"?`<span>${l("armchair")}${n(e.tableIncluded)}</span>`:""}
      </div>

      <p class="mnyra-go__card-only">${l("ticket-percent")}${n(e.onlyGo)}</p>
      ...
      <button
        ...
        ${p?"disabled":""}
      >${p?"":l("check-check")}${n(p?e.confirming:e.accept)}</button>
    </article>
  `}function Re(a={},e=u){const i=Array.isArray(a.results)?a.results:[],s=Number(a.nowMs)||Date.now();return i.length?`
    <div data-go-view="results">
      ...
      ${i.map(p=>se(p,{texts:e,busyOfferId:a.busyOfferId,nowMs:s})).join("")}
      <button type="button" class="mnyra-go__cta mnyra-go__cta--quiet" data-go-back>
        ${l("arrow-left")}${n(e.back)}
      </button>
    </div>
  `:`
      <div data-go-view="results">
    ...
    `}function Pe(a={},e=u){const i=a.booking||{},s=i.type==="reservation",p=Number(a.nowMs)||Date.now(),g=ie(i.expectedArrivalAt,{nowMs:p}),f=String(a.statusLabel||"").trim()||(s?e.reservationConfirmed:e.claimConfirmed);return a.confirmCancel?`
    ...
    `:`
    <div data-go-view="booking">
      <p class="mnyra-go__done">${n(e.doneTitle)} ${l("party-popper")}</p>
      <p class="mnyra-go__done-name">${n(i.businessName||"")}</p>
      <p class="mnyra-go__card-for">${n(i.benefitLabel||"")}</p>

      <div class="mnyra-go__card-meta">
        <span>${l("users")}${n(`${i.partySize||1} ${e.peopleSuffix}`)}</span>
        ${g?`<span>${l("clock")}${n(g)}</span>`:""}
        ${i.city?`<span>${l("map-pin")}${n(i.city)}</span>`:""}
      </div>

      <p class="mnyra-go__ok">${l("circle-check-big")}${n(f)}</p>

      ${i.shortCode?`
        <div class="mnyra-go__code">
          <p class="mnyra-go__code-label">${l("hash")}${n(e.code)}</p>
  ...
  `}function Ke(a={},e=u){const i=String(a.error||"").trim()||e.errorTitle;return`
    <div data-go-view="error">
      ...
      ${Array.isArray(a.alternatives)&&a.alternatives.length?`
        ...
        ${a.alternatives.map(s=>se(s,{texts:e,nowMs:a.nowMs})).join("")}
      `:`
        <button type="button" class="mnyra-go__cta" data-go-retry>
          ${l("rotate-ccw")}${n(e.errorRetry)}
        </button>
      `}
    </div>
  `}function He(a=u){return`
    <header class="mnyra-go__head">
      ...
      <button type="button" class="mnyra-go__x" data-go-close aria-label="${n(a.close)}" title="${n(a.close)}">${l("x")}</button>
    </header>
  `}function qe(a={}){const e={...u,...a.texts||{}},i=String(a.view||"search");let s="",p=!1;return i==="loading"?s=`<div class="mnyra-go__empty">${n(e.searching)}</div>`:i==="results"?s=Re(a,e):i==="booking"?s=Pe(a,e):i==="error"?s=Ke(a,e):(s=De(a.form||{},e),p=!0),`
    <div class="mnyra-go__sheet">
      ${He(e)}
      <div class="mnyra-go__body${p?" mnyra-go__body--search":""}" data-go-body>${s}</div>
    </div>
  `}const X="mnyraGoSticky";function Fe(a,e){return typeof a=="function"?a:e}function Ve(a){if(!a?.body)return null;try{const e=xe({documentObj:a});if(e)return e}catch{}return a.getElementById("overlayRoot")}function ee(a){if(a)try{be({documentObj:a})}catch{}}function We(a){if(!(!a||a.getElementById(U)))try{const e=a.createElement("style");e.id=U,e.textContent=$e,a.head?.appendChild(e)}catch{}}function ta({documentObj:a=null,windowObj:e=null,api:i=null,getCityFn:s=()=>"",getCoordsFn:p=()=>null,isSignedInFn:g=()=>!1,onAnalyticsFn:f=()=>{},openMenuFn:x=null,openSignInFn:v=null,nowFn:w=()=>Date.now()}={}){const _=a||(typeof document>"u"?null:document),T=e||(typeof window>"u"?null:window),M=i||ve(),k=Fe(f,()=>{}),t={open:!1,view:"search",form:{step:h[0],partySize:2,category:"all",when:"now",laterValue:"",city:"",editCity:!1},results:[],alternatives:[],booking:null,bookingToken:"",busyOfferId:"",confirmCancel:!1,error:"",notice:"",canSignIn:!1,nowMs:w()},O=new Map,ce=new Set(["resource-exhausted","failed-precondition","not-found","invalid-argument","already-exists","permission-denied","unauthenticated"]);function P(r,c){const o=String(c?.code||"").trim();ce.has(o)&&O.delete(r)}let y=null;function K(){if(!_)return null;if(y)return y;We(_),y=_.createElement("div"),y.id=ke,y.className="mnyra-go modal-overlay",y.setAttribute("data-go-modal",""),y.setAttribute("role","dialog"),y.setAttribute("aria-modal","true"),y.setAttribute("aria-label","Mnyra GO"),y.setAttribute("data-modal-surface",Y);try{y.style.setProperty("--modal-surface",Y)}catch{}return y}function m(){const r=K();if(!r)return;if(t.nowMs=w(),!t.open){try{r.remove()}catch{}ee(_),I();return}const c=Ve(_);c&&r.parentNode!==c&&c.appendChild(r),r.innerHTML=qe(t),ee(_),le(),I()}function le(){if(!(!t.open||!t.form.editCity))try{const r=y?.querySelector?.("[data-go-city-input]");if(!r||typeof r.focus!="function")return;r.focus({preventScroll:!0});const c=String(r.value||"").length;r.setSelectionRange?.(c,c)}catch{}}function I(){if(!_)return;let r=_.getElementById(X);const c=t.open?"":ye({activeBookings:G()});if(!c){r&&r.remove();return}r||(r=_.createElement("div"),r.id=X,_.body.appendChild(r)),r.innerHTML=c}function B(){t.open=!1,t.busyOfferId="",t.confirmCancel=!1,m()}function A(r){t.view="error",t.error=String(r?.message||"").trim(),t.alternatives=Array.isArray(r?.alternatives)?r.alternatives:[],t.busyOfferId="",m()}function H(){const r=t.form,c={now:0,in30:30,in60:60};let o=w();if(r.when==="later"&&r.laterValue){const b=Date.parse(r.laterValue);Number.isFinite(b)&&(o=b)}else o+=(c[r.when]||0)*60*1e3;const d=p();return{city:String(r.city||"").trim()||s(),partySize:S(r.partySize),category:r.category,requestedAt:o,lat:d?.lat,lng:d?.lng}}async function q(){t.open=!0,t.view="search",t.error="",t.notice="",t.form.city=t.form.city||s(),t.form.editCity=!1,t.form.step=h[0],t.canSignIn=!g(),m(),k("go_open",{}),M.ensureGuestSession().catch(()=>{})}async function F(){t.view="loading",t.error="",m();const r=H();k("go_search",{partySize:r.partySize,category:r.category});try{const c=await M.search(r);t.results=c.results,t.view="results",t.notice="",m(),k("go_results",{count:c.results.length})}catch(c){A(c)}}async function V(r="",c=""){if(!(!r||t.busyOfferId)){t.busyOfferId=r,t.error="",m(),O.has(r)||O.set(r,he(T?.crypto)),k("go_offer_accept",{offerId:r});try{const o=await M.createBooking({offerId:r,restaurantId:c,request:H(),idempotencyKey:O.get(r)}),d=o?.booking||null;if(!d)throw new Error("go-booking-missing");const b=String(o.bookingToken||"").trim()||G().find(C=>C.bookingId===d.id)?.bookingToken||"";fe({bookingId:d.id,bookingToken:b,shortCode:d.shortCode,restaurantId:d.restaurantId,businessName:d.businessName,benefitLabel:d.benefitLabel,type:d.type,status:d.status,partySize:d.partySize,expectedArrivalAt:d.expectedArrivalAt}),t.booking=d,t.bookingToken=b,t.view="booking",t.busyOfferId="",t.canSignIn=!g(),m(),k("go_booking_created",{offerId:r,type:d.type})}catch(o){if(o?.soldOut){t.view="error",t.error=o.message,t.alternatives=o.alternatives||[],t.busyOfferId="",P(r,o),m();return}P(r,o),A(o)}}}async function pe(r=""){const c=G().find(o=>o.bookingId===r)||G()[0]||null;if(!c)return q();t.open=!0,t.view="loading",t.confirmCancel=!1,m();try{const o=await M.getBooking(c.bookingToken);if(!o)throw new Error("go-booking-missing");_e(o),t.booking={...c,...o},t.bookingToken=c.bookingToken,t.view="booking",t.canSignIn=!g(),m()}catch(o){if(o?.code==="not-found"){Z(r),t.view="search",t.notice="",m();return}A(o)}}async function de(){const r=t.bookingToken;if(r){t.view="loading",m();try{const c=await M.cancelBooking(r);Z(t.booking?.id||""),k("go_cancel",{bookingId:t.booking?.id||""}),t.booking=c,t.confirmCancel=!1,t.view="search",t.form.city=t.form.city||s(),t.form.step=h[0],m()}catch(c){A(c)}}}function $(r={}){Object.assign(t.form,r),m()}function W(r={},{hold:c=!1}={}){Object.assign(t.form,r),c||(t.form.step=J(t.form.step)),m()}function ge(r=""){$({step:L(r),editCity:!1})}function ue(){const r=K();!r||r.dataset.goBound==="1"||(r.dataset.goBound="1",r.addEventListener("click",c=>{const o=c.target;if(!o||typeof o.closest!="function")return;if(o.closest("[data-go-close]"))return B();const d=o.closest("[data-go-category]");if(d)return W({category:d.getAttribute("data-go-category")||"all"});const b=o.closest("[data-go-when]");if(b){const z=b.getAttribute("data-go-when")||"now";return W({when:z},{hold:z==="later"})}if(o.closest("[data-go-step-next]"))return $({step:J(t.form.step),editCity:!1});if(o.closest("[data-go-step-back]"))return $({step:Oe(t.form.step),editCity:!1});const C=o.closest("[data-go-goto]");if(C)return ge(C.getAttribute("data-go-goto")||"");if(o.closest("[data-go-change-city]"))return $({editCity:!0});if(o.closest("[data-go-city-save]"))return $({editCity:!1});if(o.closest("[data-go-submit]")||o.closest("[data-go-retry]"))return F();if(o.closest("[data-go-back]"))return t.form.step=h[h.length-1],t.view="search",m();const N=o.closest("[data-go-accept]");if(N)return V(N.getAttribute("data-go-accept")||"",N.getAttribute("data-go-restaurant")||"");if(o.closest("[data-go-cancel-dismiss]"))return t.confirmCancel=!1,m();if(o.closest("[data-go-cancel-confirm]"))return de();if(o.closest("[data-go-cancel]"))return t.confirmCancel=!0,m();if(o.closest("[data-go-menu]")&&x)return x(t.booking?.restaurantId||"");if(o.closest("[data-go-signin]")&&v)return v();if(o.closest("[data-go-directions]")){const z=encodeURIComponent([t.booking?.businessName,t.booking?.city].filter(Boolean).join(" "));T&&z&&T.open(`https://www.google.com/maps/search/?api=1&query=${z}`,"_blank","noopener");return}}),r.addEventListener("input",c=>{const o=c.target;if(!(!o||typeof o.matches!="function")){if(o.matches("[data-go-party-range]")){const d=S(o.value);t.form.partySize=d;try{const b=r.querySelector?.("[data-go-party-value]");b&&(b.innerHTML=`${d} <span>${D(d)}</span>`),o.style?.setProperty?.("--go-range-fill",`${re(d)}%`),o.setAttribute?.("aria-valuetext",R(d))}catch{}return}o.matches("[data-go-city-input]")&&(t.form.city=o.value||"")}}),r.addEventListener("change",c=>{const o=c.target;!o||typeof o.matches!="function"||(o.matches("[data-go-when-input]")&&(t.form.laterValue=o.value||""),o.matches("[data-go-city-input]")&&(t.form.city=o.value||""))}),r.addEventListener("keydown",c=>{const o=c.target;c.key==="Enter"&&(!o||typeof o.matches!="function"||!o.matches("[data-go-city-input]")||(c.preventDefault(),t.form.city=o.value||"",$({editCity:!1})))}),_&&_.addEventListener("keydown",c=>{t.open&&c.key==="Escape"&&B()}))}return{state:t,open:(r="search",c="")=>(ue(),r==="booking"?pe(c):q()),close:B,refreshSticky:I,__render:m,__submitSearch:F,__acceptOffer:V}}export{ta as createGoRuntimeController};
