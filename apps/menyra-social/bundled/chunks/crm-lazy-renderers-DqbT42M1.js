import{m as ne}from"./loading-diagnostics-utils-D9lcQexT.js";function ie(n=""){const e=String(n||"").trim();if(!e)return"i panjohur";const l=new Date(e);return Number.isFinite(l.getTime())?l.toLocaleString("de-DE",{dateStyle:"short",timeStyle:"short"}):e}function Y(n="",e="#111827"){const l=String(n||"").trim();return/^#[0-9a-fA-F]{6}$/.test(l)?l:e}function re(n={}){const e=n?.landingScreenOne&&typeof n.landingScreenOne=="object"?n.landingScreenOne:{},l=Y(n?.businessNameColor||n?.landingBusinessNameColor||e.businessNameColor||"",""),t=l&&l.toLowerCase()!=="#111827"?l:"";return{part1:Y(n?.businessNameColorPart1||n?.landingBusinessNameColorPart1||e.businessNameColorPart1||l||"","#111827"),part2:Y(n?.businessNameColorPart2||n?.landingBusinessNameColorPart2||e.businessNameColorPart2||t||"","#4f46e5")}}function pe(n={}){const{state:e,icon:l,escapeHtml:t,getLeadSettingsConfig:P,CEO_COUNTRIES:w,LEAD_TYPE_ORDER:h,LEAD_TYPE_LABELS:y}=n,p=P();return`
  ...
  `}function ue(n={}){const{state:e,icon:l,escapeHtml:t,getLeadSettingsConfig:P,getOptimizedImageUrl:w,PLACEHOLDER_IMAGE:h,resolveCustomerType:y,normalizeLeadLocations:p,getLeadCountryCenter:c,getLeadMonthlyPrice:b,LEAD_TYPE_ORDER:g,LEAD_TYPE_LABELS:r,buildLeadAccountEmail:d,hasLeadLocationCoords:$,CEO_COUNTRIES:x,normalizeLeadCountry:I,resolveCurrencyCodeFromLeadCountry:C,buildLeadContactName:S}=n,a=e.leadModal.lead||{},U=P(),m=e.leadModal.logoPreview||a.logoUrl||"",L=m?w(m,"avatar"):h,T=e.leadModal.bestSpotLogoPreview||a.bestSpotLogoUrl||a.spotLogoUrl||m,D=T?w(T,"avatar"):h,M=e.leadModal.titleImagePreview||a.titleImageUrl||a.coverImageUrl||a.coverUrl||a.heroUrl||"",B=M?w(M,"medium"):h,o=e.leadModal.mode==="edit"&&!!a.id,O=!!e.leadModal.actionsOpen,k=!!e.leadModal.deleting,v=y(a.customerType||"cafe"),A=a.billingCycle==="yearly"?"yearly":"monthly",i=I(a.country||U.defaultCountry),u=String(typeof C=="function"&&C(i,"EUR")||"EUR").trim().toUpperCase()||"EUR",_=p(e.leadModal.locations,a.address||"",e.leadModal.coords||c(i)),E=b(v,U),R=E*12,j=A==="yearly"?R:E,z=re(a),K=a.specialEnabled===!0,V=e.leadModal.coords&&Number.isFinite(Number(e.leadModal.coords.lat))&&Number.isFinite(Number(e.leadModal.coords.lng))?{lat:Number(e.leadModal.coords.lat),lng:Number(e.leadModal.coords.lng)}:null;return`
  ...
  `}function xe(n={}){const{state:e,icon:l,escapeHtml:t,isCeoUser:P,renderCeoGuard:w,renderLeadSettingsView:h,renderLeadCreationView:y,normalizeSearchKey:p,normalizeLeadStatusKey:c,normalizeLeadScopeKey:b,createLeadScopeMap:g,sanitizeCeoCrmCounts:r,hasStoredCeoCrmCounts:d,resolveKnownScopeCountLabel:$,leadMatchesQuery:x,leadStatusTone:I,leadStatusLabel:C,buildLeadLandingPageUrl:S,getOptimizedImageUrl:a,PLACEHOLDER_IMAGE:U,renderOwnershipPills:m,renderCeoScopeTabs:L,LEAD_STATUS_ORDER:T,LEAD_STATUS_LABELS:D}=n;if(!P())return w("Leads");if(e.leads.view==="settings")return h();if(e.leads.view==="create")return y();const M=p(e.leads.query||""),B=c(e.leads.status||""),o=b(e.leads.scope),O=e.leads.pages||g(()=>[]);e.leads.hasMore||g(()=>!1);const k=e.leads.loaded||g(()=>!1),v=e.leads.knownCount||g(()=>0),A=e.leads.countExact||g(()=>!1),i=r(e.userProfile?.crmCounts||{}),u=d(e.userProfile?.crmCounts)?String(i.ownLeads):$(v.own,!!A.own,!!k.own),_=d(e.userProfile?.crmCounts)?String(i.staffLeads):$(v.staff,!!A.staff,!!k.staff),E=d(e.userProfile?.crmCounts)?String(i.archivedLeads):$(v.archived,!!A.archived,!!k.archived),R=(()=>{try{return typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}catch{return Date.now()}})();let j=Array.isArray(O[o])?O[o].slice():[];B&&o!=="archived"&&(j=j.filter(s=>c(s.status)===B));const z=(s,f=null)=>{const H=Array.isArray(s?.locations)?s.locations.map(G=>G?.address||G?.city||"").join(" "):"";return String(s?._searchKey||"").trim()||p([s?.businessName,s?.restaurantName,s?.name,f?.name,f?.restaurantName,s?.contactName,s?.phone,s?.email,s?.socialEmail,s?.instagram,s?.city,s?.address,s?.publicSlug,s?.landingSlug,f?.publicSlug,f?.landingSlug,s?.canonicalPublicPath,s?.status,C(s?.status),s?.customerType,H].filter(Boolean).join(" "))},K=j.map(s=>{const f=s.restaurantId?e.restaurants.find(G=>String(G.id)===String(s.restaurantId)):null,H=z(s,f);return{lead:s,rest:f,searchKey:H,matches:!M||x({...s,_searchKey:H},M)}}),V=K.filter(s=>s.matches).length,N=e.leads.loading?'<div class="text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 py-16">Leads po ngarkohen...</div>':K.length?`
        ...
        <div data-lead-row="true" data-lead-search-key="${t(H)}" class="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm" ${G?"":"hidden"}>
          <div class="flex items-center gap-3">
            <div class="w-12 h-12 rounded-2xl bg-slate-100 overflow-hidden flex items-center justify-center">
              <img src="${t(le)}" class="w-full h-full object-contain bg-white" />
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-black text-slate-900 truncate">${t(W)}</p>
              ${X?`<p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate">${t(X)}</p>`:""}
            </div>
            <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${J.bg} ${J.text}">${t(se)}</span>
          </div>
          ${oe}
          <div class="flex gap-2 mt-4">
            ...
            ${ae?`<a href="${t(ae)}" target="_blank" rel="noopener noreferrer" class="px-4 py-3 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest hover:bg-slate-800">Oferta</a>`:""}
    ...
    `:'<div class="text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 py-16">Nuk ka leads</div>',F=(()=>{try{return typeof performance<"u"&&typeof performance.now=="function"?performance.now():Date.now()}catch{return Date.now()}})();return ne("lead list render",{scope:o,count:K.length,items:V,elapsedMs:F-R}),`
  ...
  `}export{fe as renderCustomersView,ue as renderLeadCreationView,pe as renderLeadSettingsView,xe as renderLeadsView,be as renderStaffEditorView,ge as renderStaffView};
