...
`;function A(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Ne({businessName:e="",logoUrl:t="",imageUrl:a="",variant:n="",benefitLabel:i="",benefitView:o=null,sponsored:p=!1,meta:c=[],ctaLabel:b="",ctaIcon:y="check-check",ctaDisabled:g=!1,cardAttrs:u="",ctaAttrs:f="",texts:w=ve}={}){const z={...ve,...w||{}},R=(Array.isArray(c)?c:[]).filter(I=>I&&I.label),j=String(b||z.accept),$=o&&typeof o=="object"?o:{},v=String($.headline||i||""),M=String($.priceGo||""),_=String($.priceRegular||""),B=String(a||"").trim(),E=At({imageUrl:B,variant:n}),Z=E===le,O=B?`<img class="mnyra-go-page__card-photo" src="${A(B)}" alt="" loading="lazy" decoding="async" />`:"",G=`
    ...
    <p class="mnyra-go-page__card-benefit${M?" mnyra-go-page__card-benefit--title":""}">${A(v)}</p>
    ...
    ${M?`
      <div class="mnyra-go-page__card-prices">
        ...
        <span class="mnyra-go-page__card-price-go">${A(M)}</span>
      </div>
    `:""}
...
`;function Rt(e={},t={}){const a=t.escapeHtml,n=t.icon;return`
  ...
  `}function jt({overview:e={},deps:t={}}={}){const a=c=>Number.isFinite(Number(c))&&c!==null&&c!=="",n=c=>a(c)?String(Math.max(0,Math.trunc(Number(c)))):"",i=a(e?.openCents)?Math.max(0,Math.trunc(Number(e.openCents))):null,o=i===0;return`
  ...
  `}function Mt({tab:e="active",deps:t={}}={}){const a=t.escapeHtml,n=t.icon;return`
  ...
  `}function Et(e={},t={}){const a=t.escapeHtml,n=e.status==="paused"?r.paused:e.status==="archived"?r.archived:"";return`
    <div class="p-4 rounded-[1.6rem] bg-slate-50 border border-slate-100" data-go-offer="${s(a,e.id)}">
      ...
      <div style="pointer-events:none;" aria-hidden="true">
        ${Ne({businessName:"",imageUrl:e.imageUrl||"",variant:le,benefitLabel:e.benefitLabel||"",benefitView:Ce(e.benefit||{}),meta:[{icon:"users",label:je(e)},{icon:"clock",label:Me(e)}]})}
      </div>
  ...
  `}function Ot({offer:e={},businessName:t="",previewImageUrl:a="",deps:n={}}={}){const i=n.escapeHtml;return`
    <div data-go-offer-preview>
      ...
      -->
      <div style="pointer-events:none;" aria-hidden="true">
        ${Ne({businessName:t,imageUrl:a||e.imageUrl||"",benefitLabel:e.benefitLabel||"",benefitView:Ce(e.benefit||{}),meta:[{icon:"users",label:je(e)},{icon:"clock",label:Me(e)}]})}
      </div>
    </div>
...
`;function oe({attr:e="",unit:t="€",value:a="",placeholder:n="",mode:i="decimal",inputClass:o="",escapeHtml:p=null}={}){return`
  ...
  `}const we=Object.freeze([10,15,20,25]);function Nt({benefit:e={},percentCustom:t=!1,errorFor:a=()=>"",inputClass:n="",inputBase:i="",escapeHtml:o=null}={}){const p=f=>T(o,f),c=f=>{const w=a(f);return w?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="${s(o,f)}">${s(o,w)}</p>`:""};if(!ft.includes(e.kind))return`
      ...
      ${c("benefit")}
    `;const b=Number(e.percent)||0,y=t||b>0&&!we.includes(b),g=()=>{const f=bt(e.savingCents);if(!f)return"";const w=Math.round(Number(e.savingPercent)||0);return`
  ...
  `}function Lt(e=""){const t=String(e||"all").trim().toLowerCase();return t==="food"?["food"]:t==="coffee"||t==="drinks"||t==="dessert"?["drinks"]:["food","drinks"]}function Aa(e=[]){const t=Array.isArray(e)?e:[],a=t.includes("food"),n=t.includes("drinks");return a&&n?"all":a?"food":n?"drinks":""}function Ba({editor:e=null,businessName:t="",deps:a={}}={}){if(!e)return"";const n=a.escapeHtml,i=a.icon,o=e.draft||{},p=Array.isArray(e.errors)?e.errors:[],c=_=>p.find(B=>B.field===_)?.message||"",b=Array.isArray(o.partyRanges)?o.partyRanges:[],y=o.schedule?.mode==="windows"?"windows":"always",g=Array.isArray(o.schedule?.days)&&o.schedule.days.length?o.schedule.days:ye.slice(),u=_e.every(_=>b.includes(_.key)),f=Array.isArray(e.intents)?e.intents:Lt(o.category),w=o.benefit||{},z=e.mode==="edit",R="w-full go-offer-input bg-slate-50 border border-slate-100 rounded-2xl px-4 text-sm font-bold text-slate-900 outline-none focus:border-indigo-400",j=`mt-2 ${R}`,$='<div class="h-px bg-slate-100"></div>',v=_=>`<p class="mt-1 text-[11px] font-semibold text-slate-400">${s(n,_)}</p>`,M=pt(o).ok&&f.length>0;return`
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll px-6 py-5 space-y-5" data-go-editor-scroll>
          ...
          <div class="go-offer-section" data-go-section="benefit">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
              ${[[Fe,r.benefitPercent],[Ae,r.benefitBundle],[Be,r.benefitFree],[Ke,r.benefitSpecial]].map(([_,B])=>`
            ...
            <div class="mt-5" data-go-benefit-form>
              ${Nt({benefit:w,percentCustom:e.percentCustom===!0,errorFor:c,inputClass:j,inputBase:R,escapeHtml:n})}
            </div>
          </div>
          ...
          <div data-go-section="category">
            ...
            <div class="mt-3 space-y-2">
              ${[{key:"food",label:r.ifFood},{key:"drinks",label:r.ifDrinks}].map(_=>{const B=f.includes(_.key),E=ut.find(Z=>Z.key===_.key)?.hint||"";return`
          ...
          <div data-go-section="schedule">
            ...
            ${y==="windows"?`
              ...
              <div class="mt-4">
                ${T(n,r.daysQuestion)}
                <div class="mt-2 flex flex-wrap gap-2">
                  ${ye.map(_=>q(gt(_),{active:g.includes(_),attr:"data-go-offer-day",value:_,escapeHtml:n})).join("")}
                </div>
              </div>
              <div class="mt-4">
                ${T(n,r.hoursQuestion)}
  ...
  `}function Ka({restaurantName:e="",tab:t="active",overview:a={},search:n={},bookings:i=[],offers:o=[],settings:p={},paused:c=!1,loading:b=!1,error:y="",deps:g={}}={}){const u=g.escapeHtml,f=g.icon,w=v=>["accepted","activated"].includes(Re(v.status)),z=i.filter(w),R=i.filter(v=>!w(v)),j=o.filter(v=>v.status!=="archived");let $="";if(t==="offers")$=ee({eyebrow:r.brand,title:r.tabs.offers,sub:`${j.length} ${j.length===1?"oferte":"oferta"}`,action:`
      ...
      `,body:j.length?`<div class="space-y-3">${j.map(v=>Et(v,g)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${s(u,r.emptyTitle)}</div>`,deps:g});else if(t==="archive")$=ee({eyebrow:r.brand,title:r.tabs.archive,sub:`${R.length}`,body:R.length?`<div class="space-y-3">${R.map(v=>se(v,g)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${s(u,r.noHistory)}</div>`,deps:g});else if(t==="options"){const v=Le(p?.pausedUntil);$=ee({eyebrow:r.brand,title:r.tabs.options,body:`
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          ${c?`<button type="button" data-go-pause="0" class="go-pause px-4 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest">${s(u,r.resume)}</button>`:[{value:"30",label:"30 min"},{value:"60",label:"1 orë"},{value:"tomorrow",label:"Deri nesër"},{value:"-1",label:"Pa afat"}].map(M=>`
              <button type="button" data-go-pause="${M.value}"
                class="go-pause px-4 rounded-2xl bg-slate-50 border border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-600">${s(u,M.label)}</button>
            `).join("")}
        </div>
        <p class="mt-4 text-[10px] font-bold text-slate-400">${s(u,r.keepsRunning)}</p>
      `,deps:g})}else $=ee({eyebrow:r.brand,title:r.tabs.active,sub:`${z.length}`,body:`
        ${Ct({code:n.code,status:n.status,busy:n.busy,deps:g})}
        ${n.booking?`
          <div class="mb-4">${se(n.booking,g,{found:!0})}</div>
        `:""}
        ${b?`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-400">${s(u,r.loading)}</div>`:z.length?`<div class="space-y-3">${z.filter(v=>v.id!==n.booking?.id).map(v=>se(v,g)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${s(u,r.noBookings)}</div>`}
      `,deps:g});return`
...
`;function Ut(e=typeof document>"u"?null:document){if(!(!e||e.getElementById($e)))try{const t=e.createElement("style");t.id=$e,t.textContent=It,e.head?.appendChild(t)}catch{}}function k(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function P(e,t,a=""){if(typeof e!="function")return"";try{return e(t,a)||""}catch{return""}}const Ht=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function Zt({businessType:e="",isShopCatalog:t=!1}={}){if(t)return"shop";const a=String(e||"").trim().toLowerCase();return Ht.includes(a)?"hotel":"restaurant"}function Vt(e=new Date().getHours()){const t=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return t>=5&&t<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:t>=11&&t<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:t>=18&&t<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function Wt({name:e="",logoUrl:t="",hour:a=new Date().getHours(),iconFn:n}={}){const i=Vt(a),o=k(e||"Business");return`
    <div class="mnyra-dash__greet">
      <p class="mnyra-dash__greet-title">
        ...
        <span class="mnyra-dash__greet-logo">
          ${t?`<img src="${k(t)}" alt="${o}" title="${o}" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:`<span class="mnyra-dash__greet-logo-fallback" title="${o}">${P(n,"store","w-4 h-4")}</span>`}
        </span>
      </p>
  ...
  `:""}function aa({cards:e=[],iconFn:t}={}){const a=(Array.isArray(e)?e:[]).filter(i=>i&&i.key);if(!a.length)return"";const n=a.map((i,o)=>{const p=k(i.label||"");if(i.pending)return'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>';const c=o<2?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"';let b="";i.imageUrl?b=`<img class="mnyra-dash__hl-media" src="${k(i.imageUrl)}" alt="" ${c} decoding="async" onerror="this.style.display='none'" />`:i.videoUrl&&(b=`<video class="mnyra-dash__hl-media" src="${k(i.videoUrl)}#t=0.1" preload="metadata" muted playsinline disablepictureinpicture tabindex="-1" aria-hidden="true"></video>`);const y=`
      ...
      ${b}
    `,g=i.withEye?`<span class="mnyra-dash__hl-eye">${P(t,"eye","w-4 h-4")}</span>`:"";let u;i.locked?u=`<span class="mnyra-dash__hl-lock">${P(t,"lock","w-3 h-3")}Me pagesë</span>`:i.loading?u='<span class="mnyra-dash__hl-value mnyra-dash__hl-value--pending" aria-hidden="true"></span>':i.emptyText?u=`<span class="mnyra-dash__hl-empty">${k(i.emptyText)}</span>`:u=`<span class="mnyra-dash__hl-value">${g}${k(i.value||"0")}</span>`;let f;i.locked?f=`class="mnyra-dash__hl-card mnyra-dash__hl-card--locked" data-dashboard-metric-locked="${k(i.key)}"`:i.composer?f=`class="mnyra-dash__hl-card" data-dashboard-composer="${k(i.composer)}"`:f=`class="mnyra-dash__hl-card"${i.panelTab?` data-dashboard-panel-tab="${k(i.panelTab)}"`:""}`;const w=i.locked?`${p} – me pagesë`:`${p} ${i.emptyText||i.value||""}`.trim();return`
      <button type="button" ${f} data-dashboard-metric="${k(i.key)}" aria-label="${k(w)}">
        ${y}
    ...
    `}).join("");return`
    <div class="mnyra-dash__hl" data-dashboard-metrics="${k(na(a))}">
      ${n}
  ...
  `}function na(e=[]){return(Array.isArray(e)?e:[]).filter(t=>t&&t.key).map(t=>[t.key,t.label||"",t.value||"",t.emptyText||"",t.imageUrl||"",t.videoUrl||"",t.iconName||"",t.panelTab||"",t.composer||"",t.pending?"p":"",t.loading?"l":"",t.locked?"x":"",t.withEye?"e":""].join("~")).join("|")}const Ie=Object.freeze([Object.freeze({id:"funksionet",label:"Funksionet",iconName:"layout-grid"}),Object.freeze({id:"analitika",label:"Analitika",iconName:"bar-chart-3"}),Object.freeze({id:"opsionet",label:"Opsionet",iconName:"settings"})]);function ae(e=""){const t=String(e||"").trim().toLowerCase();return Ie.some(a=>a.id===t)?t:"funksionet"}function ra({activeTab:e="funksionet",iconFn:t}={}){const a=ae(e);return`<div class="mnyra-dash__tabs" role="tablist" data-dashboard-panel-tabs>${Ie.map(i=>{const o=i.id===a;return`
      ...
      >${P(t,i.iconName,"w-4 h-4")}<span class="mnyra-dash__tab-label">${k(i.label)}</span></button>
  ...
  `}function ia({posts:e=[],iconFn:t}={}){const a=Array.isArray(e)?e:[];let n="";return a.length?(n=a.map(i=>{const o=[i.dateLabel,`${W(i.likesCount||0)} Likes`,`${W(i.commentsCount||0)} Kommentare`];return Number(i.impressions||0)>0&&o.push(`${W(i.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${i.thumbUrl?`<img src="${k(i.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:P(t,i.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
  ...
  `}const pa="menyra_social_dashboard_cache_v1::",ze="menyra_social_composer_products_v1::",De=2500,Pe=1200,ua=6,ga=3,ma=Object.freeze({menuImageUrl:"/apps/menyra-social/assets/panel/menu-scan.jpg",qrImageUrl:"/apps/menyra-social/assets/panel/qr-stand.jpg"}),fa=Object.freeze({menuOpens:"Menü-Aufrufe",qrScans:"QR-Scans"});function K(e){const t=Number(e);return Number.isFinite(t)?t:0}function ba(e={}){const t=String(e.createdAtClient||"").trim();if(t){const n=new Date(t);if(!Number.isNaN(n.getTime()))return n}const a=e.createdAt;if(a&&typeof a.toDate=="function")try{const n=a.toDate();if(n instanceof Date&&!Number.isNaN(n.getTime()))return n}catch{}return null}function ya(e="",t={}){const a=Array.isArray(t.media)&&t.media.length?t.media[0]:{},n=String(a.type||t.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",i=String(a.thumbUrl||(n==="image"?a.url:"")||t.thumbUrl||"").trim(),o=n==="video"?String(a.url||t.mediaUrl||"").trim():"",p=ba(t);return{id:String(e||"").trim(),caption:String(t.caption||"").trim(),mediaType:n,thumbUrl:i,videoUrl:o,likesCount:K(t.likesCount),commentsCount:K(t.commentsCount),impressions:0,dateLabel:p?p.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:p?p.getTime():0}}function _a({days:e=[],todayKey:t="",rawPosts:a=[]}={}){const n=Array.isArray(e)?e:[],i=be(n),o=n.find(g=>String(g?.date||g?.id||"").trim()===String(t||"").trim()),p=be(o?[o]:[]),c=i.merged?.posts&&typeof i.merged.posts=="object"?i.merged.posts:{},b=(Array.isArray(a)?a:[]).map(g=>ya(g?.id,g?.data||{})).filter(g=>g.id).map(g=>({...g,impressions:K(c[g.id]?.impressions)})),y=b.slice().sort((g,u)=>u.createdAtMs-g.createdAtMs).slice(0,ga);return{day:String(t||"").trim(),week:i.summary,today:p.summary,posts:y,latestPost:xa(b)}}function xa(e=[]){const t=(Array.isArray(e)?e:[]).filter(a=>a&&a.id);return t.length?t.slice().sort((a,n)=>K(n.createdAtMs)-K(a.createdAtMs)||K(n.impressions)-K(a.impressions)||K(n.likesCount)-K(a.likesCount))[0]:null}function ka({profile:e={},restaurant:t={}}={}){return ht({profile:e,restaurant:t,feature:"qr"})}function va(e={}){const t=e&&typeof e=="object"?e:{};return String(t.titleImageUrl||t.coverImageUrl||t.coverUrl||t.heroUrl||t.bannerUrl||"").trim()}function wa({model:e=null,coverUrl:t="",subscribed:a=!1,assets:n={}}={}){const i=e?.today||{},o=!e,p=e?.latestPost||null,c=[];if(o)c.push({key:"latestPost",label:"Postimi fundit",pending:!0});else if(!p)c.push({key:"latestPost",label:"Postimi fundit",emptyText:"S'ka postim",iconName:"image",composer:"post"});else{const b=String(p.thumbUrl||"").trim();c.push({key:"latestPost",label:"Postimi fundit",value:W(K(p.impressions)),withEye:!0,imageUrl:b,videoUrl:b?"":String(p.videoUrl||"").trim(),iconName:"image",panelTab:"analitika"})}return c.push({key:"profileViews",label:"Vizitor n'profil",value:W(K(i.profileViews)),withEye:!0,loading:o,imageUrl:String(t||"").trim(),iconName:"user",panelTab:"analitika"}),c.push({key:"menuOpens",label:"Vizitor n'meny",value:W(K(i.menuOpens)),withEye:!0,loading:o&&a,locked:!a,imageUrl:String(n.menuImageUrl||"").trim(),iconName:"book-open",panelTab:"analitika"}),c.push({key:"qrScans",label:"Skanime n'tavolina",value:W(K(i.qrScans)),withEye:!0,loading:o&&a,locked:!a,imageUrl:String(n.qrImageUrl||"").trim(),iconName:"layout-grid",panelTab:"analitika"}),c}function ja({state:e,renderFn:t,documentObj:a,firestoreApi:n={},profileApi:i={},composerApi:o={},viewApi:p={},iconFn:c,storageObj:b}={}){const y=a||(typeof document>"u"?null:document),g=y?.defaultView||(typeof window>"u"?null:window),u=typeof t=="function"?t:()=>{},f=b||(typeof localStorage>"u"?null:localStorage),w=typeof i.getBusinessProfileTypeFn=="function"?i.getBusinessProfileTypeFn:(()=>""),z=typeof i.isShopCatalogProfileFn=="function"?i.isShopCatalogProfileFn:(()=>!1),R=typeof i.getRestaurantMetaByIdFn=="function"?i.getRestaurantMetaByIdFn:(()=>null),j=typeof i.resolveRestaurantLogoFn=="function"?i.resolveRestaurantLogoFn:(()=>""),$=typeof i.resolveOwnAvatarUrlFn=="function"?i.resolveOwnAvatarUrlFn:(()=>""),v=typeof p.renderAnalyticsViewFn=="function"?p.renderAnalyticsViewFn:(()=>""),M=typeof p.renderSettingsViewFn=="function"?p.renderSettingsViewFn:(()=>""),_=typeof p.warmAnalyticsFn=="function"?p.warmAnalyticsFn:(()=>{});let B=!1,E=0,Z=!1,O=null,G=null,L="",I=!1,ce=()=>null;const He=300;function ne(){const d=e?.userProfile||{};return Zt({businessType:w(d),isShopCatalog:z(d)})}function Ze(d=""){const l=R(d)||{};return Dt(l).map(h=>({id:h.id,name:h.title,price:h.price??"",category:h.beds||h.tag||"",type:"room",imageUrl:h.imageUrl||""}))}function Ve(d=""){if(!f)return null;try{const l=f.getItem(`${ze}${d}`);if(!l)return null;const h=JSON.parse(l),m=Array.isArray(h?.items)?h.items:null;return m&&m.length?m:null}catch{return null}}function We(d="",l=[]){if(f)try{f.setItem(`${ze}${d}`,JSON.stringify({savedAt:Date.now(),items:l}))}catch{}}async function qe(d=""){const{db:l,collectionFn:h,queryFn:m,limitFn:x,getDocsFn:S}=n;if(!l||typeof h!="function"||typeof S!="function")throw new Error("Produktet nuk u ngarkuan.");const U=h(l,"restaurants",d,"menuItems"),F=typeof m=="function"&&typeof x=="function"?m(U,x(He)):U,H=await S(F),D=[];return H.forEach(V=>{const J=ce(V?.id,V?.data?.()||{});J&&D.push(J)}),D.sort((V,J)=>V.name.localeCompare(J.name,"sq")),D}async function Qe(d="",l){const h=String(d||"").trim();if(!h)throw new Error("Produktet nuk u ngarkuan.");if(ne()==="hotel")return Ze(h);const m=qe(h).then(S=>(We(h,S),S)),x=Ve(h);return x?(typeof l=="function"?m.then(S=>l(S)).catch(()=>{}):m.catch(()=>{}),x):m}function he(){return O?Promise.resolve(O):(G||(G=dt(()=>import("./business-composer-controller-CQMwpCHB.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(d=>(ce=typeof d?.normalizeComposerProductCore=="function"?d.normalizeComposerProductCore:(()=>null),O=d.createBusinessComposerController({documentObj:y,windowObj:y?.defaultView||null,api:{getRestaurantIdFn:()=>Q(),getBusinessMetaFn:()=>{const l=Q();if(!l)return{name:"",logoUrl:"",city:""};const h=fe(l),m=R(l)||{};return{name:h.name,logoUrl:h.logoUrl,city:String(m.city||"").trim()}},loadProductsFn:(l,h)=>Qe(l,h),getBusinessKindFn:()=>ne(),uploadImageFn:o.uploadImageFn,uploadVideoFn:o.uploadVideoFn,captureVideoPosterFn:o.captureVideoPosterFn,createPostFn:o.createPostFn,createStoryFn:o.createStoryFn,formatPriceFn:o.formatPriceFn,getOptimizedImageUrlFn:o.getOptimizedImageUrlFn,escapeHtmlFn:o.escapeHtmlFn,iconFn:typeof c=="function"?c:void 0,afterPublishFn:async l=>{try{await Y({force:!0})}catch{}typeof o.afterPublishFn=="function"&&await o.afterPublishFn(l)}}}),O)).catch(d=>{throw G=null,console.error("[mnyra][dashboard] composer load failed",d),d})),G)}function pe(){const d=g?.navigator?.connection;return!d||typeof d!="object"?!1:d.saveData===!0?!0:/(^|-)2g$/.test(String(d.effectiveType||"").trim().toLowerCase())}function Ye(){if(I||O||!g||pe())return;I=!0;const d=()=>{if(he().catch(()=>{}),typeof o.prewarmFn=="function")try{o.prewarmFn()}catch{}};if(typeof g.requestIdleCallback=="function"){g.requestIdleCallback(d,{timeout:De});return}g.setTimeout?.(d,Pe)}function Je(){if(B||!g||pe())return;B=!0;const d=()=>{try{_()}catch{}};if(typeof g.requestIdleCallback=="function"){g.requestIdleCallback(d,{timeout:De});return}g.setTimeout?.(d,Pe)}function Xe(d="post"){const l=String(d||"").trim().toLowerCase(),h=l==="story"||l==="profile"?l:"post";if(typeof o.prewarmFn=="function")try{o.prewarmFn()}catch{}if(O){O.open(h);return}L=h,he().then(m=>{const x=L||h;L="",m?.open?.(x)}).catch(()=>{L=""})}function re(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:"",restaurantId:"",paywall:""}),e.dashboardView}function ue(d=""){const l=re(),h=String(d||"").trim();return String(l.restaurantId||"")===h||(l.restaurantId=h,l.model=null,l.status="idle",l.error="",l.loadedSignature="",l.paywall="",E+=1),l}function Q(){const d=e?.userProfile||{};return String(d.restaurantId||d.staffRestaurantId||"").trim()}let ge="";function et(){const d=String(e?.user?.uid||"").trim();!d||ge===d||typeof i.ensureBusinessProfileFn=="function"&&(ge=d,Promise.resolve().then(()=>i.ensureBusinessProfileFn()).catch(l=>{console.warn("[mnyra][panel] business profile could not be resolved",l)}).finally(()=>{String(e?.user?.uid||"").trim()===d&&u()}))}function tt(){const d=String(e?.user?.uid||"").trim();if(!d)return!1;const l=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||l===d}function me(d=""){return`${pa}${d}`}function at(d="",l=""){if(!f||!d)return null;try{const h=f.getItem(me(d));if(!h)return null;const m=JSON.parse(h);return!m||typeof m!="object"||String(m.day||"").trim()!==String(l||"").trim()||!m.model||typeof m.model!="object"?null:m.model}catch{return null}}function nt(d="",l=null){if(!(!f||!d||!l))try{f.setItem(me(d),JSON.stringify({day:l.day,model:l}))}catch{}}async function rt(d=""){const{db:l,collectionFn:h,queryFn:m,orderByFn:x,limitFn:S,getDocsFn:U}=n;if(!l||typeof h!="function"||typeof m!="function"||typeof x!="function"||typeof S!="function"||typeof U!="function")return[];const F=h(l,"restaurants",d,"socialPosts");return(await U(m(F,x("createdAt","desc"),S(ua)))).docs.map(D=>({id:D.id,data:D.data()||{}})).filter(D=>{const V=String(D.data.status||"active").trim().toLowerCase();return V!=="deleted"&&V!=="hidden"})}async function Y({force:d=!1}={}){const l=Q(),h=ue(l);if(!l)return;const m=lt({rangeKey:"7d"});if(!m)return;const x=`${l}::${m.toDay}`;if(!d&&h.loadedSignature===x&&h.status==="ready")return;if(!h.model){const F=at(l,m.toDay);F&&(h.model=F,h.status="ready",u())}E+=1;const S=E;h.model||(h.status="loading",h.error="",u());try{const F={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:l},[H,D]=await Promise.allSettled([ct({...F,fromDay:m.fromDay,toDay:m.toDay}),rt(l)]);if(S!==E)return;if(H.status==="rejected")throw H.reason;D.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",D.reason),h.model=_a({days:H.value,todayKey:m.toDay,rawPosts:D.status==="fulfilled"?D.value:[]}),h.status="ready",h.error="",h.loadedSignature=x,nt(l,h.model)}catch(F){if(S!==E)return;console.error("[mnyra][dashboard] load failed",F),h.model||(h.status="error",h.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}u()}function it(){Z||!y||(Z=!0,y.addEventListener("click",d=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(d.target?.closest?.("[data-dashboard-retry]")){Y({force:!0});return}if(d.target?.closest?.("[data-dashboard-paywall-close]")){d.preventDefault(),re().paywall="",u();return}const l=d.target?.closest?.("[data-dashboard-metric-locked]");if(l){d.preventDefault(),re().paywall=String(l.getAttribute("data-dashboard-metric-locked")||"").trim(),u();return}const h=d.target?.closest?.("[data-dashboard-composer]");if(h){d.preventDefault(),Xe(h.getAttribute("data-dashboard-composer"));return}const m=d.target?.closest?.("[data-dashboard-panel-tab]");if(m){d.preventDefault();const x=ae(m.getAttribute("data-dashboard-panel-tab"));if(x===ae(e?.dashboardPanelTab))return;e.dashboardPanelTab=x,u()}}catch{}}))}function fe(d=""){const l=e?.userProfile||{},h=d?R(d)||{}:{},m=String(h.name||h.restaurantName||l.name||"").trim()||"Business";let x="";try{x=String($()||"").trim()}catch{}if(!x)try{x=String(j(h)||"").trim()}catch{}return{name:m,logoUrl:x,kind:ne(),coverUrl:va(h),subscribed:ka({profile:l,restaurant:h})}}function st(d=""){try{if(!yt()||!d)return"";_t({restaurantId:d,onBadgeFn:()=>u()});const l=xt();return Bt({enabled:!0,unseenCount:l.unseen,activeOffers:l.activeOffers||0,todayBookings:l.today,iconFn:c})}catch{return""}}function ot(){Ut(y),it();const d=Q(),l=ue(d);let h="";if(!d)et(),h=tt()?la():ha();else{Ye(),Je();const m=fe(d),x=ae(e?.dashboardPanelTab);l.status==="idle"&&(l.status="loading",queueMicrotask(()=>{Y({force:!1})}));let S="";l.model?S=ia({posts:l.model.posts,iconFn:c}):l.status==="error"?S=ca({message:l.error}):S=sa();const U=`
        ${st(d)}
        ${qt({iconFn:c})}
        ${ta({iconFn:c,showEditor:!!d})}
        ${Qt({iconFn:c,showEditor:!!d})}
        ${Yt({iconFn:c,showEditor:!!d})}
        ${Xt({iconFn:c,kind:m.kind,showEditor:!!d})}
      `;let F;x==="analitika"?F=`
          <div class="mnyra-dash__embed">${v()}</div>
          ${S}
        `:x==="opsionet"?F=`<div class="mnyra-dash__embed">${M()}</div>`:F=U;const H=wa({model:l.model,coverUrl:m.coverUrl,subscribed:m.subscribed,assets:ma}),D=String(l.paywall||"").trim();h=`
        ${Wt({name:m.name,logoUrl:m.logoUrl,iconFn:c})}
        ${aa({cards:H,iconFn:c})}
        ${Ue(`
          ${ra({activeTab:x,iconFn:c})}
          ${F}
        `)}
    ...
    `}return Object.freeze({renderDashboardView:ot,loadDashboard:Y})}export{Te as G,vt as M,ja as a,Ka as b,wt as c,Aa as d,Ba as e,Ot as f,Lt as g,X as h,le as i,de as j,Ne as k,Dt as l,Pa as m,zt as n,Fa as o,Ra as r};
