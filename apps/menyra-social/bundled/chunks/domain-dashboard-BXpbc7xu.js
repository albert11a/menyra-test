...
`;function K(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Le({businessName:e="",logoUrl:t="",imageUrl:a="",variant:n="",benefitLabel:i="",benefitView:o=null,sponsored:p=!1,meta:c=[],ctaLabel:b="",ctaIcon:y="check-check",ctaDisabled:_=!1,cardAttrs:m="",ctaAttrs:u="",texts:x=$e}={}){const z={...$e,...x||{}},R=(Array.isArray(c)?c:[]).filter(I=>I&&I.label),E=String(b||z.accept),v=o&&typeof o=="object"?o:{},A=String(v.headline||i||""),S=String(v.priceGo||""),g=String(v.priceRegular||""),D=String(a||"").trim(),O=Rt({imageUrl:D,variant:n}),Z=O===he,T=D?`<img class="mnyra-go-page__card-photo" src="${K(D)}" alt="" loading="lazy" decoding="async" />`:"",N=`
    ...
    ${v.eyebrow?`<p class="mnyra-go-page__card-eyebrow">${K(v.eyebrow)}</p>`:""}
    <p class="mnyra-go-page__card-benefit${S?" mnyra-go-page__card-benefit--title":""}">${K(A)}</p>
    ${v.note?`<p class="mnyra-go-page__card-note">${K(v.note)}</p>`:""}
    ${S?`
      <div class="mnyra-go-page__card-prices">
        ${g?`<span class="mnyra-go-page__card-price-was">${K(g)}</span>`:""}
        <span class="mnyra-go-page__card-price-go">${K(S)}</span>
      </div>
    `:""}
    ${v.savingLabel?`<p class="mnyra-go-page__card-saving">${K(v.savingLabel)}</p>`:""}
    ${Z?"":`<p class="mnyra-go-page__card-for">${K(z.forGroup)}</p>`}
  `,L=`
    <div class="mnyra-go-page__card-meta">
      ${R.map(I=>`<span>${ee(I.icon||"")}${K(I.label)}</span>`).join("")}
    </div>
...
`;function Mt(e={},t={}){const a=t.escapeHtml,n=t.icon;return`
            ...
            <div class="go-tabs__pane" role="tablist" data-go-tab-pane="${b}"${b===o?"":' aria-hidden="true" inert'}>
              ${c.tabs.map(y=>`
                <button type="button" role="tab" aria-selected="${e===y?"true":"false"}" data-go-business-tab="${s(n,y)}"
                  ...
                  class="go-tab">${M(i,Ct[y],"w-4 h-4")}<span class="go-tab-label">${s(n,r.tabs[y])}</span></button>
              `).join("")}
            </div>
          `).join("")}
        </div>
  ...
  `}function te(e={},t={},{found:a=!1}={}){const n=t.escapeHtml,i=e.benefitLabel||e.snapshot?.benefitLabel||"",o=!e.businessSeenAt,p=e.partySizeVerified||e.partySizeRequested||e.partySize||1,c=le(e.status),b=Ue(e.acceptedAt),y=b?`${r.around} ${b}`:r.guestName;return`
  ...
  `}function Se({title:e="",note:t="",iconName:a="",deps:n={}}={}){const i=n.escapeHtml,o=n.icon;return q({eyebrow:r.brand,title:e,sub:r.soonHint,body:`
    ...
    `,deps:n})}function Gt(e={},t={}){const a=t.escapeHtml,n=e.status==="paused"?r.paused:e.status==="archived"?r.archived:"";return`
  ...
  `}function Nt({offer:e={},businessName:t="",previewImageUrl:a="",deps:n={}}={}){const i=n.escapeHtml;return`
...
`;function de({attr:e="",unit:t="€",value:a="",placeholder:n="",mode:i="decimal",inputClass:o="",escapeHtml:p=null}={}){return`
  ...
  `}const ze=Object.freeze([10,15,20,25]);function It({benefit:e={},percentCustom:t=!1,errorFor:a=()=>"",inputClass:n="",inputBase:i="",escapeHtml:o=null}={}){const p=u=>G(o,u),c=u=>{const x=a(u);return x?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="${s(o,u)}">${s(o,x)}</p>`:""};if(!yt.includes(e.kind))return`
      <p class="go-offer-saving font-bold text-slate-400">${s(o,r.benefitLegacy)}</p>
      ${c("benefit")}
    `;const b=Number(e.percent)||0,y=t||b>0&&!ze.includes(b),_=()=>{const u=_t(e.savingCents);if(!u)return"";const x=Math.round(Number(e.savingPercent)||0);return`
      <p class="mt-3 go-offer-saving font-black text-emerald-600" data-go-benefit-saving>
        ${s(o,r.saving)} ${s(o,u)}${x>0?` &middot; -${x}%`:""}
      </p>
    `},m=()=>`
    <div class="mt-3">
      ${p(r.priceRegular)}
      ${de({attr:"data-go-benefit-regular",value:xe(e.regularPriceCents),placeholder:r.pricePlaceholder,inputClass:i,escapeHtml:o})}
      ${c("regularPrice")}
    </div>
    <div class="mt-3">
      ${p(r.priceGo)}
      ${de({attr:"data-go-benefit-go",value:xe(e.goPriceCents),placeholder:r.pricePlaceholder,inputClass:i,escapeHtml:o})}
      ${c("goPrice")}
    </div>
    ${_()}
  `;if(e.kind===Re)return`
      ...
      <div class="mt-2 flex flex-wrap gap-2">
        ${ze.map(u=>Q(`${u}%`,{active:!y&&b===u,attr:"data-go-discount",value:String(u),escapeHtml:o})).join("")}
      ...
      <div class="mt-4">
        ${p(r.scopeQuestion)}
        <div class="mt-2 flex flex-wrap gap-2">
          ${[["all",r.scopeAll],["food",r.scopeFood],["drinks",r.scopeDrinks]].map(([u,x])=>Q(x,{active:(e.scope||"all")===u,attr:"data-go-discount-scope",value:u,escapeHtml:o})).join("")}
        </div>
        ${c("benefitScope")}
      </div>
    `;if(e.kind===Ae)return`
    ...
    `;if(e.kind===je){const u=String(e.conditionType||"");return`
      ...
      <div class="mt-4">
        ...
        <div class="mt-2 grid grid-cols-2 gap-2">
          ${[["food",r.conditionFood],["drink",r.conditionDrink],["any_order",r.conditionAny],["custom",r.conditionCustom]].map(([x,z])=>Q(z,{active:u===x,attr:"data-go-benefit-condition",value:x,escapeHtml:o})).join("")}
        </div>
        ${u==="custom"?`
          <div class="mt-3">
    ...
    `}return e.kind===Me?`
      ...
      ${c("benefitItem")}
      ${m()}
    `:""}function Ut({imageUrl:e="",photo:t={},escapeHtml:a=null,icon:n=null}={}){const i=String(t.status||""),o=i==="uploading",p=String(t.previewUrl||e||""),c=i==="error"?String(t.error||r.photoError):"",b=p?`
  ...
  `}function Ht(e=""){const t=String(e||"all").trim().toLowerCase();return t==="food"?["food"]:t==="coffee"||t==="drinks"||t==="dessert"?["drinks"]:["food","drinks"]}function ja(e=[]){const t=Array.isArray(e)?e:[],a=t.includes("food"),n=t.includes("drinks");return a&&n?"all":a?"food":n?"drinks":""}function Ma({editor:e=null,businessName:t="",deps:a={}}={}){if(!e)return"";const n=a.escapeHtml,i=a.icon,o=e.draft||{},p=Array.isArray(e.errors)?e.errors:[],c=g=>p.find(D=>D.field===g)?.message||"",b=Array.isArray(o.partyRanges)?o.partyRanges:[],y=o.schedule?.mode==="windows"?"windows":"always",_=Array.isArray(o.schedule?.days)&&o.schedule.days.length?o.schedule.days:ke.slice(),m=we.every(g=>b.includes(g.key)),u=Array.isArray(e.intents)?e.intents:Ht(o.category),x=o.benefit||{},z=e.mode==="edit",R="w-full go-offer-input bg-slate-50 border border-slate-100 rounded-2xl px-4 text-sm font-bold text-slate-900 outline-none focus:border-indigo-400",E=`mt-2 ${R}`,v='<div class="h-px bg-slate-100"></div>',A=g=>`<p class="mt-1 text-[11px] font-semibold text-slate-400">${s(n,g)}</p>`,S=gt(o).ok&&u.length>0;return`
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll px-6 py-5 space-y-5" data-go-editor-scroll>
          ...
          <div class="go-offer-section" data-go-section="benefit">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
              ${[[Re,r.benefitPercent],[Ae,r.benefitBundle],[je,r.benefitFree],[Me,r.benefitSpecial]].map(([g,D])=>`
                <button type="button" data-go-benefit-kind="${s(n,g)}"
                  aria-pressed="${x.kind===g?"true":"false"}"
                  class="go-offer-kind px-3 rounded-2xl text-xs font-black transition-colors ${x.kind===g?"bg-slate-900 text-white":"bg-slate-50 text-slate-600 border border-slate-100"}">
                  ${s(n,D)}
                </button>
              `).join("")}
            </div>
            <div class="mt-5" data-go-benefit-form>
              ${It({benefit:x,percentCustom:e.percentCustom===!0,errorFor:c,inputClass:E,inputBase:R,escapeHtml:n})}
            </div>
          </div>
          ...
          <div data-go-section="partyRanges">
            ...
            -->
            <div class="mt-3">
              ${ae(r.partyAll,{active:m,attr:"data-go-offer-party",value:"all",escapeHtml:n})}
            </div>
            <div class="mt-2 flex flex-wrap gap-2">
              ${we.map(g=>ae(g.label,{active:b.includes(g.key),attr:"data-go-offer-party",value:g.key,escapeHtml:n})).join("")}
            </div>
          ...
          <div data-go-section="schedule">
            ...
            ${y==="windows"?`
              ...
              <div class="mt-4">
                ${G(n,r.daysQuestion)}
                <div class="mt-2 flex flex-wrap gap-2">
                  ${ke.map(g=>Q(ft(g),{active:_.includes(g),attr:"data-go-offer-day",value:g,escapeHtml:n})).join("")}
                </div>
              </div>
              <div class="mt-4">
                ${G(n,r.hoursQuestion)}
          ...
          ${Nt({offer:o,businessName:t,previewImageUrl:e.photo?.previewUrl||"",deps:a})}
        </div>
  ...
  `}function Ea({restaurantName:e="",tab:t="active",group:a=0,overview:n={},search:i={},bookings:o=[],offers:p=[],settings:c={},paused:b=!1,loading:y=!1,error:_="",deps:m={}}={}){const u=m.escapeHtml,x=m.icon,z=g=>["accepted","activated"].includes(le(g.status)),R=o.filter(z),E=R.filter(g=>le(g.status)==="accepted"),v=o.filter(g=>!z(g)),A=p.filter(g=>g.status!=="archived");let S="";if(t==="offers")S=q({eyebrow:r.brand,title:r.tabs.offers,sub:`${A.length} ${A.length===1?"oferte":"oferta"}`,action:`
      ...
      `,body:A.length?`<div class="space-y-3">${A.map(g=>Gt(g,m)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${s(u,r.emptyTitle)}</div>`,deps:m});else if(t==="finalized")S=q({eyebrow:r.brand,title:r.tabs.finalized,sub:`${v.length}`,body:v.length?`<div class="space-y-3">${v.map(g=>te(g,m)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${s(u,r.noHistory)}</div>`,deps:m});else if(t==="pending")S=q({eyebrow:r.brand,title:r.tabs.pending,sub:`${E.length}`,body:y?`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-400">${s(u,r.loading)}</div>`:E.length?`<div class="space-y-3">${E.map(g=>te(g,m)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${s(u,r.noBookings)}</div>`,deps:m});else if(t==="stats")S=Se({title:r.tabs.stats,note:r.soonStats,iconName:"bar-chart-3",deps:m});else if(t==="payments")S=Se({title:r.tabs.payments,note:r.soonPayments,iconName:"wallet",deps:m});else if(t==="options"){const g=Ue(c?.pausedUntil);S=q({eyebrow:r.brand,title:r.tabs.options,body:`
        <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
          <div>
            <p class="text-xs font-black text-slate-800">${s(u,r.goOn)}</p>
            <p class="text-[10px] font-bold text-slate-400">${s(u,b?`${r.pausedUntil} ${g}`:"ON")}</p>
          </div>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          ${b?`<button type="button" data-go-pause="0" class="go-pause px-4 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest">${s(u,r.resume)}</button>`:[{value:"30",label:"30 min"},{value:"60",label:"1 orë"},{value:"tomorrow",label:"Deri nesër"},{value:"-1",label:"Pa afat"}].map(D=>`
      ...
      `,deps:m})}else S=q({eyebrow:r.brand,title:r.tabs.active,sub:`${R.length}`,body:`
        ${Tt({code:i.code,status:i.status,busy:i.busy,deps:m})}
        ${i.booking?`
          <div class="mb-4">${te(i.booking,m,{found:!0})}</div>
        `:""}
        ${y?`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-400">${s(u,r.loading)}</div>`:R.length?`<div class="space-y-3">${R.filter(g=>g.id!==i.booking?.id).map(g=>te(g,m)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${s(u,r.noBookings)}</div>`}
      `,deps:m});return`
  ...
  `}const Oa=r,De="mnyraDashboardStyles",Zt=`
.mnyra-dash {
...
`;function Vt(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(De)))try{const t=e.createElement("style");t.id=De,t.textContent=Zt,e.head?.appendChild(t)}catch{}}function w(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function B(e,t,a=""){if(typeof e!="function")return"";try{return e(t,a)||""}catch{return""}}const Wt=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function qt({businessType:e="",isShopCatalog:t=!1}={}){if(t)return"shop";const a=String(e||"").trim().toLowerCase();return Wt.includes(a)?"hotel":"restaurant"}function Qt(e=new Date().getHours()){const t=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return t>=5&&t<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:t>=11&&t<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:t>=18&&t<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function Yt({name:e="",logoUrl:t="",hour:a=new Date().getHours(),iconFn:n}={}){const i=Qt(a),o=w(e||"Business");return`
    <div class="mnyra-dash__greet">
      <p class="mnyra-dash__greet-title">
        ...
        <span class="mnyra-dash__greet-logo">
          ${t?`<img src="${w(t)}" alt="${o}" title="${o}" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:`<span class="mnyra-dash__greet-logo-fallback" title="${o}">${B(n,"store","w-4 h-4")}</span>`}
        </span>
      </p>
      <p class="mnyra-dash__greet-sub">${w(i.text)}</p>
    </div>
  ...
  `:""}const Pe=Object.freeze({restaurant:{accent:"Ndrysho",rest:"menunë",sub:"Shto produkte, kategori dhe çmime.",cta:"Menu"},shop:{accent:"Ndrysho",rest:"dyqanin",sub:"Shto produkte, kategori dhe stok.",cta:"Dyqani"},hotel:{accent:"Ndrysho",rest:"hotelin",sub:"Detajet, dhomat dhe çmimet e tua.",cta:"Hoteli"}});function ta(e="restaurant"){const t=String(e||"").trim().toLowerCase();return Pe[t]||Pe.restaurant}function aa({iconFn:e,kind:t="restaurant",showEditor:a=!0}={}){if(!a)return"";const n=ta(t);return`
    <button type="button" class="mnyra-dash__composer mnyra-dash__composer--tap mnyra-dash__composer--plane" data-dashboard-catalog-card data-nav="menu">
      <span class="mnyra-dash__composer-title"><span class="mnyra-dash__composer-accent">${w(n.accent)}</span> ${w(n.rest)}</span>
      <span class="mnyra-dash__composer-sub">${w(n.sub)}</span>
      <span class="mnyra-dash__composer-cta">
        <span class="mnyra-dash__composer-cta-icon">${B(e,"plus","w-4 h-4")}</span>
        <span class="mnyra-dash__composer-cta-label">${w(n.cta)}</span>
        <span class="mnyra-dash__composer-cta-chevron">${B(e,"chevron-right","w-4 h-4")}</span>
      </span>
    </button>
  ...
  `:""}function ia({cards:e=[],iconFn:t}={}){const a=(Array.isArray(e)?e:[]).filter(i=>i&&i.key);if(!a.length)return"";const n=a.map((i,o)=>{const p=w(i.label||"");if(i.pending)return'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>';const c=o<2?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"';let b="";i.imageUrl?b=`<img class="mnyra-dash__hl-media" src="${w(i.imageUrl)}" alt="" ${c} decoding="async" onerror="this.style.display='none'" />`:i.videoUrl&&(b=`<video class="mnyra-dash__hl-media" src="${w(i.videoUrl)}#t=0.1" preload="metadata" muted playsinline disablepictureinpicture tabindex="-1" aria-hidden="true"></video>`);const y=`
      <span class="mnyra-dash__hl-plate">${B(t,i.iconName||"image","w-6 h-6")}</span>
      ${b}
    `,_=i.withEye?`<span class="mnyra-dash__hl-eye">${B(t,"eye","w-4 h-4")}</span>`:"";let m;i.locked?m=`<span class="mnyra-dash__hl-lock">${B(t,"lock","w-3 h-3")}Me pagesë</span>`:i.loading?m='<span class="mnyra-dash__hl-value mnyra-dash__hl-value--pending" aria-hidden="true"></span>':i.emptyText?m=`<span class="mnyra-dash__hl-empty">${w(i.emptyText)}</span>`:m=`<span class="mnyra-dash__hl-value">${_}${w(i.value||"0")}</span>`;let u;i.locked?u=`class="mnyra-dash__hl-card mnyra-dash__hl-card--locked" data-dashboard-metric-locked="${w(i.key)}"`:i.composer?u=`class="mnyra-dash__hl-card" data-dashboard-composer="${w(i.composer)}"`:u=`class="mnyra-dash__hl-card"${i.panelTab?` data-dashboard-panel-tab="${w(i.panelTab)}"`:""}`;const x=i.locked?`${p} – me pagesë`:`${p} ${i.emptyText||i.value||""}`.trim();return`
      <button type="button" ${u} data-dashboard-metric="${w(i.key)}" aria-label="${w(x)}">
        ${y}
    ...
    `}).join("");return`
    <div class="mnyra-dash__hl" data-dashboard-metrics="${w(sa(a))}">
      ${n}
  ...
  `}function sa(e=[]){return(Array.isArray(e)?e:[]).filter(t=>t&&t.key).map(t=>[t.key,t.label||"",t.value||"",t.emptyText||"",t.imageUrl||"",t.videoUrl||"",t.iconName||"",t.panelTab||"",t.composer||"",t.pending?"p":"",t.loading?"l":"",t.locked?"x":"",t.withEye?"e":""].join("~")).join("|")}const He=Object.freeze([Object.freeze({id:"funksionet",label:"Funksionet",iconName:"layout-grid"}),Object.freeze({id:"analitika",label:"Analitika",iconName:"bar-chart-3"}),Object.freeze({id:"opsionet",label:"Opsionet",iconName:"settings"})]);function re(e=""){const t=String(e||"").trim().toLowerCase();return He.some(a=>a.id===t)?t:"funksionet"}function oa({activeTab:e="funksionet",iconFn:t}={}){const a=re(e);return`<div class="mnyra-dash__tabs" role="tablist" data-dashboard-panel-tabs>${He.map(i=>{const o=i.id===a;return`
      <button
        type="button"
        role="tab"
        data-dashboard-panel-tab="${w(i.id)}"
        aria-selected="${o?"true":"false"}"
        class="mnyra-dash__tab"
      >${B(t,i.iconName,"w-4 h-4")}<span class="mnyra-dash__tab-label">${w(i.label)}</span></button>
  ...
  `}function da({posts:e=[],iconFn:t}={}){const a=Array.isArray(e)?e:[];let n="";return a.length?(n=a.map(i=>{const o=[i.dateLabel,`${W(i.likesCount||0)} Likes`,`${W(i.commentsCount||0)} Kommentare`];return Number(i.impressions||0)>0&&o.push(`${W(i.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${i.thumbUrl?`<img src="${w(i.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:B(t,i.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
          <div class="mnyra-dash__post-main">
            <p class="mnyra-dash__post-caption">${w(i.caption||"Pa tekst")}</p>
            <p class="mnyra-dash__post-meta">${w(o.filter(Boolean).join(" · "))}</p>
          </div>
        </div>
  ...
  `}function ca({title:e=""}={}){return`
    <div class="mnyra-dash__paywall" data-dashboard-paywall role="dialog" aria-modal="true">
      <div class="mnyra-dash__paywall-card">
        <p class="mnyra-dash__paywall-title">${w(e||"Me pagesë")}</p>
  ...
  `}function ua({message:e=""}={}){return`
    <div class="mnyra-dash__section">
      <div class="mnyra-dash__state">
        ...
        <p class="mnyra-dash__state-body">${w(e||"Ju lutem kontrollo lidhjen dhe provo perseri.")}</p>
  ...
  `}const ma="menyra_social_dashboard_cache_v1::",Be="menyra_social_composer_products_v1::",Fe=2500,Ke=1200,fa=6,ba=3,ya=Object.freeze({menuImageUrl:"/apps/menyra-social/assets/panel/menu-scan.jpg",qrImageUrl:"/apps/menyra-social/assets/panel/qr-stand.jpg"}),_a=Object.freeze({menuOpens:"Menü-Aufrufe",qrScans:"QR-Scans"});function j(e){const t=Number(e);return Number.isFinite(t)?t:0}function ka(e={}){const t=String(e.createdAtClient||"").trim();if(t){const n=new Date(t);if(!Number.isNaN(n.getTime()))return n}const a=e.createdAt;if(a&&typeof a.toDate=="function")try{const n=a.toDate();if(n instanceof Date&&!Number.isNaN(n.getTime()))return n}catch{}return null}function wa(e="",t={}){const a=Array.isArray(t.media)&&t.media.length?t.media[0]:{},n=String(a.type||t.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",i=String(a.thumbUrl||(n==="image"?a.url:"")||t.thumbUrl||"").trim(),o=n==="video"?String(a.url||t.mediaUrl||"").trim():"",p=ka(t);return{id:String(e||"").trim(),caption:String(t.caption||"").trim(),mediaType:n,thumbUrl:i,videoUrl:o,likesCount:j(t.likesCount),commentsCount:j(t.commentsCount),impressions:0,dateLabel:p?p.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:p?p.getTime():0}}function xa({days:e=[],todayKey:t="",rawPosts:a=[]}={}){const n=Array.isArray(e)?e:[],i=_e(n),o=n.find(_=>String(_?.date||_?.id||"").trim()===String(t||"").trim()),p=_e(o?[o]:[]),c=i.merged?.posts&&typeof i.merged.posts=="object"?i.merged.posts:{},b=(Array.isArray(a)?a:[]).map(_=>wa(_?.id,_?.data||{})).filter(_=>_.id).map(_=>({..._,impressions:j(c[_.id]?.impressions)})),y=b.slice().sort((_,m)=>m.createdAtMs-_.createdAtMs).slice(0,ba);return{day:String(t||"").trim(),week:i.summary,today:p.summary,posts:y,latestPost:va(b)}}function va(e=[]){const t=(Array.isArray(e)?e:[]).filter(a=>a&&a.id);return t.length?t.slice().sort((a,n)=>j(n.createdAtMs)-j(a.createdAtMs)||j(n.impressions)-j(a.impressions)||j(n.likesCount)-j(a.likesCount))[0]:null}function $a({profile:e={},restaurant:t={}}={}){return ut({profile:e,restaurant:t,feature:"qr"})}function Sa(e={}){const t=e&&typeof e=="object"?e:{};return String(t.titleImageUrl||t.coverImageUrl||t.coverUrl||t.heroUrl||t.bannerUrl||"").trim()}function za({model:e=null,coverUrl:t="",subscribed:a=!1,assets:n={}}={}){const i=e?.today||{},o=!e,p=e?.latestPost||null,c=[];if(o)c.push({key:"latestPost",label:"Postimi fundit",pending:!0});else if(!p)c.push({key:"latestPost",label:"Postimi fundit",emptyText:"S'ka postim",iconName:"image",composer:"post"});else{const b=String(p.thumbUrl||"").trim();c.push({key:"latestPost",label:"Postimi fundit",value:W(j(p.impressions)),withEye:!0,imageUrl:b,videoUrl:b?"":String(p.videoUrl||"").trim(),iconName:"image",panelTab:"analitika"})}return c.push({key:"profileViews",label:"Vizitor n'profil",value:W(j(i.profileViews)),withEye:!0,loading:o,imageUrl:String(t||"").trim(),iconName:"user",panelTab:"analitika"}),c.push({key:"menuOpens",label:"Vizitor n'meny",value:W(j(i.menuOpens)),withEye:!0,loading:o&&a,locked:!a,imageUrl:String(n.menuImageUrl||"").trim(),iconName:"book-open",panelTab:"analitika"}),c.push({key:"qrScans",label:"Skanime n'tavolina",value:W(j(i.qrScans)),withEye:!0,loading:o&&a,locked:!a,imageUrl:String(n.qrImageUrl||"").trim(),iconName:"layout-grid",panelTab:"analitika"}),c}function Ta({state:e,renderFn:t,documentObj:a,firestoreApi:n={},profileApi:i={},composerApi:o={},viewApi:p={},iconFn:c,storageObj:b}={}){const y=a||(typeof document>"u"?null:document),_=y?.defaultView||(typeof window>"u"?null:window),m=typeof t=="function"?t:()=>{},u=b||(typeof localStorage>"u"?null:localStorage),x=typeof i.getBusinessProfileTypeFn=="function"?i.getBusinessProfileTypeFn:(()=>""),z=typeof i.isShopCatalogProfileFn=="function"?i.isShopCatalogProfileFn:(()=>!1),R=typeof i.getRestaurantMetaByIdFn=="function"?i.getRestaurantMetaByIdFn:(()=>null),E=typeof i.resolveRestaurantLogoFn=="function"?i.resolveRestaurantLogoFn:(()=>""),v=typeof i.resolveOwnAvatarUrlFn=="function"?i.resolveOwnAvatarUrlFn:(()=>""),A=typeof p.renderAnalyticsViewFn=="function"?p.renderAnalyticsViewFn:(()=>""),S=typeof p.renderSettingsViewFn=="function"?p.renderSettingsViewFn:(()=>""),g=typeof p.warmAnalyticsFn=="function"?p.warmAnalyticsFn:(()=>{});let D=!1,O=0,Z=!1,T=null,N=null,L="",I=!1,pe=()=>null;const Ve=300;function ie(){const d=e?.userProfile||{};return qt({businessType:x(d),isShopCatalog:z(d)})}function We(d=""){const l=R(d)||{};return Bt(l).map(h=>({id:h.id,name:h.title,price:h.price??"",category:h.beds||h.tag||"",type:"room",imageUrl:h.imageUrl||""}))}function qe(d=""){if(!u)return null;try{const l=u.getItem(`${Be}${d}`);if(!l)return null;const h=JSON.parse(l),f=Array.isArray(h?.items)?h.items:null;return f&&f.length?f:null}catch{return null}}function Qe(d="",l=[]){if(u)try{u.setItem(`${Be}${d}`,JSON.stringify({savedAt:Date.now(),items:l}))}catch{}}async function Ye(d=""){const{db:l,collectionFn:h,queryFn:f,limitFn:k,getDocsFn:$}=n;if(!l||typeof h!="function"||typeof $!="function")throw new Error("Produktet nuk u ngarkuan.");const U=h(l,"restaurants",d,"menuItems"),F=typeof f=="function"&&typeof k=="function"?f(U,k(Ve)):U,H=await $(F),P=[];return H.forEach(V=>{const X=pe(V?.id,V?.data?.()||{});X&&P.push(X)}),P.sort((V,X)=>V.name.localeCompare(X.name,"sq")),P}async function Je(d="",l){const h=String(d||"").trim();if(!h)throw new Error("Produktet nuk u ngarkuan.");if(ie()==="hotel")return We(h);const f=Ye(h).then($=>(Qe(h,$),$)),k=qe(h);return k?(typeof l=="function"?f.then($=>l($)).catch(()=>{}):f.catch(()=>{}),k):f}function ue(){return T?Promise.resolve(T):(N||(N=ct(()=>import("./business-composer-controller-SFSV1c3V.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(d=>(pe=typeof d?.normalizeComposerProductCore=="function"?d.normalizeComposerProductCore:(()=>null),T=d.createBusinessComposerController({documentObj:y,windowObj:y?.defaultView||null,api:{getRestaurantIdFn:()=>Y(),getBusinessMetaFn:()=>{const l=Y();if(!l)return{name:"",logoUrl:"",city:""};const h=ye(l),f=R(l)||{};return{name:h.name,logoUrl:h.logoUrl,city:String(f.city||"").trim()}},loadProductsFn:(l,h)=>Je(l,h),getBusinessKindFn:()=>ie(),uploadImageFn:o.uploadImageFn,uploadVideoFn:o.uploadVideoFn,captureVideoPosterFn:o.captureVideoPosterFn,createPostFn:o.createPostFn,createStoryFn:o.createStoryFn,formatPriceFn:o.formatPriceFn,getOptimizedImageUrlFn:o.getOptimizedImageUrlFn,escapeHtmlFn:o.escapeHtmlFn,iconFn:typeof c=="function"?c:void 0,afterPublishFn:async l=>{try{await J({force:!0})}catch{}typeof o.afterPublishFn=="function"&&await o.afterPublishFn(l)}}}),T)).catch(d=>{throw N=null,console.error("[mnyra][dashboard] composer load failed",d),d})),N)}function ge(){const d=_?.navigator?.connection;return!d||typeof d!="object"?!1:d.saveData===!0?!0:/(^|-)2g$/.test(String(d.effectiveType||"").trim().toLowerCase())}function Xe(){if(I||T||!_||ge())return;I=!0;const d=()=>{if(ue().catch(()=>{}),typeof o.prewarmFn=="function")try{o.prewarmFn()}catch{}};if(typeof _.requestIdleCallback=="function"){_.requestIdleCallback(d,{timeout:Fe});return}_.setTimeout?.(d,Ke)}function et(){if(D||!_||ge())return;D=!0;const d=()=>{try{g()}catch{}};if(typeof _.requestIdleCallback=="function"){_.requestIdleCallback(d,{timeout:Fe});return}_.setTimeout?.(d,Ke)}function tt(d="post"){const l=String(d||"").trim().toLowerCase(),h=l==="story"||l==="profile"?l:"post";if(typeof o.prewarmFn=="function")try{o.prewarmFn()}catch{}if(T){T.open(h);return}L=h,ue().then(f=>{const k=L||h;L="",f?.open?.(k)}).catch(()=>{L=""})}function se(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:"",restaurantId:"",paywall:""}),e.dashboardView}function me(d=""){const l=se(),h=String(d||"").trim();return String(l.restaurantId||"")===h||(l.restaurantId=h,l.model=null,l.status="idle",l.error="",l.loadedSignature="",l.paywall="",O+=1),l}function Y(){const d=e?.userProfile||{};return String(d.restaurantId||d.staffRestaurantId||"").trim()}let fe="";function at(){const d=String(e?.user?.uid||"").trim();!d||fe===d||typeof i.ensureBusinessProfileFn=="function"&&(fe=d,Promise.resolve().then(()=>i.ensureBusinessProfileFn()).catch(l=>{console.warn("[mnyra][panel] business profile could not be resolved",l)}).finally(()=>{String(e?.user?.uid||"").trim()===d&&m()}))}function nt(){const d=String(e?.user?.uid||"").trim();if(!d)return!1;const l=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||l===d}function be(d=""){return`${ma}${d}`}function rt(d="",l=""){if(!u||!d)return null;try{const h=u.getItem(be(d));if(!h)return null;const f=JSON.parse(h);return!f||typeof f!="object"||String(f.day||"").trim()!==String(l||"").trim()||!f.model||typeof f.model!="object"?null:f.model}catch{return null}}function it(d="",l=null){if(!(!u||!d||!l))try{u.setItem(be(d),JSON.stringify({day:l.day,model:l}))}catch{}}async function st(d=""){const{db:l,collectionFn:h,queryFn:f,orderByFn:k,limitFn:$,getDocsFn:U}=n;if(!l||typeof h!="function"||typeof f!="function"||typeof k!="function"||typeof $!="function"||typeof U!="function")return[];const F=h(l,"restaurants",d,"socialPosts");return(await U(f(F,k("createdAt","desc"),$(fa)))).docs.map(P=>({id:P.id,data:P.data()||{}})).filter(P=>{const V=String(P.data.status||"active").trim().toLowerCase();return V!=="deleted"&&V!=="hidden"})}async function J({force:d=!1}={}){const l=Y(),h=me(l);if(!l)return;const f=ht({rangeKey:"7d"});if(!f)return;const k=`${l}::${f.toDay}`;if(!d&&h.loadedSignature===k&&h.status==="ready")return;if(!h.model){const F=rt(l,f.toDay);F&&(h.model=F,h.status="ready",m())}O+=1;const $=O;h.model||(h.status="loading",h.error="",m());try{const F={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:l},[H,P]=await Promise.allSettled([pt({...F,fromDay:f.fromDay,toDay:f.toDay}),st(l)]);if($!==O)return;if(H.status==="rejected")throw H.reason;P.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",P.reason),h.model=xa({days:H.value,todayKey:f.toDay,rawPosts:P.status==="fulfilled"?P.value:[]}),h.status="ready",h.error="",h.loadedSignature=k,it(l,h.model)}catch(F){if($!==O)return;console.error("[mnyra][dashboard] load failed",F),h.model||(h.status="error",h.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}m()}function ot(){Z||!y||(Z=!0,y.addEventListener("click",d=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(d.target?.closest?.("[data-dashboard-retry]")){J({force:!0});return}if(d.target?.closest?.("[data-dashboard-paywall-close]")){d.preventDefault(),se().paywall="",m();return}const l=d.target?.closest?.("[data-dashboard-metric-locked]");if(l){d.preventDefault(),se().paywall=String(l.getAttribute("data-dashboard-metric-locked")||"").trim(),m();return}const h=d.target?.closest?.("[data-dashboard-composer]");if(h){d.preventDefault(),tt(h.getAttribute("data-dashboard-composer"));return}const f=d.target?.closest?.("[data-dashboard-panel-tab]");if(f){d.preventDefault();const k=re(f.getAttribute("data-dashboard-panel-tab"));if(k===re(e?.dashboardPanelTab))return;e.dashboardPanelTab=k,m()}}catch{}}))}function ye(d=""){const l=e?.userProfile||{},h=d?R(d)||{}:{},f=String(h.name||h.restaurantName||l.name||"").trim()||"Business";let k="";try{k=String(v()||"").trim()}catch{}if(!k)try{k=String(E(h)||"").trim()}catch{}return{name:f,logoUrl:k,kind:ie(),coverUrl:Sa(h),subscribed:$a({profile:l,restaurant:h})}}function dt(d=""){try{if(!kt()||!d)return"";wt({restaurantId:d,onBadgeFn:()=>m()});const l=xt();return At({enabled:!0,unseenCount:l.unseen,activeOffers:l.activeOffers||0,todayBookings:l.today,iconFn:c})}catch{return""}}function lt(){Vt(y),ot();const d=Y(),l=me(d);let h="";if(!d)at(),h=nt()?pa():ga();else{Xe(),et();const f=ye(d),k=re(e?.dashboardPanelTab);l.status==="idle"&&(l.status="loading",queueMicrotask(()=>{J({force:!1})}));let $="";l.model?$=da({posts:l.model.posts,iconFn:c}):l.status==="error"?$=ua({message:l.error}):$=la();const U=`
        ${dt(d)}
        ${Jt({iconFn:c})}
        ${ra({iconFn:c,showEditor:!!d})}
        ${Xt({iconFn:c,showEditor:!!d})}
        ${ea({iconFn:c,showEditor:!!d})}
        ${aa({iconFn:c,kind:f.kind,showEditor:!!d})}
      `;let F;k==="analitika"?F=`
          <div class="mnyra-dash__embed">${A()}</div>
          ${$}
        `:k==="opsionet"?F=`<div class="mnyra-dash__embed">${S()}</div>`:F=U;const H=za({model:l.model,coverUrl:f.coverUrl,subscribed:f.subscribed,assets:ya}),P=String(l.paywall||"").trim();h=`
        ${Yt({name:f.name,logoUrl:f.logoUrl,iconFn:c})}
        ${ia({cards:H,iconFn:c})}
        ${Ze(`
    ...
    `}return Object.freeze({renderDashboardView:lt,loadDashboard:J})}export{Oa as B,ne as G,$t as M,Ta as a,Ea as b,St as c,Aa as d,ja as e,Ma as f,Ht as g,Nt as h,Ne as i,ee as j,he as k,ce as l,Le as m,Pt as n,Bt as o,Ka as p,Ra as q,Ca as r};
