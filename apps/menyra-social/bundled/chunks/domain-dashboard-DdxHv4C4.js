const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["chunks/business-composer-controller-D6zMGtWy.js","chunks/domain-feed-social-eager-D9mkfYE3.js","chunks/domain-auth-B1kS5TG-.js","chunks/domain-public-profile-mLQti0eH.js","chunks/domain-media-eager-DAUyCk2O.js","chunks/domain-menu-eager-Cp6t4lbN.js","chunks/profile-post-card-markup-utils-HwqIiXgP.js"])))=>i.map(i=>d[i]);
import{_ as Et}from"./domain-auth-B1kS5TG-.js";import{f as ee,r as Bt,l as Kt,s as Be}from"./domain-analytics-i6lAJYIg.js";import{b as Tt}from"./domain-business-accounts-D8NpUhi6.js";import{i as Nt,e as Gt,a as It}from"./domain-feed-social-eager-D9mkfYE3.js";const Lt=20,Ut=8;function K(e=""){return e==null?"":String(e).trim()}function ye(e){if(e==null||e==="")return null;const a=Number(String(e).replace(",","."));return Number.isFinite(a)&&a>0?a:null}function Ht(e=Date.now(),a=Math.random()){const t=Math.max(0,Number(e)||0).toString(36),n=Math.floor(Math.max(0,Math.min(.999999,Number(a)||0))*36**6).toString(36).padStart(6,"0");return`room_${t}_${n}`}function Zt(e={}){const a=e&&typeof e=="object"?e:{},t=[...Array.isArray(a.images)?a.images:[],K(a.imageUrl??a.image??a.photoUrl)],n=[];return t.forEach(r=>{const i=K(r);i&&!n.includes(i)&&n.push(i)}),n.slice(0,Ut)}function Vt(e={},{index:a=0}={}){const t=e&&typeof e=="object"?e:{},n=ye(t.persons??t.guests??t.capacity),r=ye(t.size??t.sizeSqm??t.area),i=Zt(t);return{id:K(t.id)||Ht(Date.now()+a),title:K(t.title??t.name),description:K(t.description??t.text).slice(0,400),imageUrl:i[0]||"",images:i,price:ye(t.price??t.pricePerNight),currency:K(t.currency??t.currencyCode).toUpperCase()||"EUR",persons:n==null?null:Math.min(20,Math.round(n)),beds:K(t.beds??t.bedsLabel).slice(0,60),size:r==null?null:Math.min(500,Math.round(r)),tag:K(t.tag??t.badge).slice(0,40),active:t.active!==!1}}function Wt(e=[]){return(Array.isArray(e)?e:[]).slice(0,Lt).map((a,t)=>Vt(a,{index:t}))}function qt(e={}){return Wt((e&&typeof e=="object"?e:{}).hotelRooms).filter(t=>t.title)}function On(e={}){const a=[];return Number.isFinite(e?.persons)&&e.persons>0&&a.push({icon:"users",label:`${e.persons} persona`}),K(e?.beds)&&a.push({icon:"bed",label:K(e.beds)}),Number.isFinite(e?.size)&&e.size>0&&a.push({icon:"size",label:`${e.size} m²`}),a}function Rn(e={}){const a=Number(e?.price);if(!Number.isFinite(a)||a<=0)return"";const t=K(e?.currency).toUpperCase()||"EUR",n=Number.isInteger(a)?String(a):a.toFixed(2);return t==="EUR"?`€${n}`:`${n} ${t}`}const et="all",Qt=Object.freeze([{key:et,label:"Krejt",icon:"sparkles"},{key:"coffee",label:"Kafe",icon:"coffee"},{key:"drinks",label:"Pije",icon:"cup-soda"},{key:"food",label:"Ushqim",icon:"utensils"},{key:"dessert",label:"Ëmbëlsira",icon:"cake-slice"}]),Yt=Object.freeze(Qt.map(e=>e.key)),Jt="food",Xt="drinks",ea="unsure",tt=Object.freeze([Object.freeze({key:Jt,label:"Ushqim",hint:"Mëngjes, drekë, darkë, ëmbëlsirë",icon:"utensils",categories:Object.freeze(["food","dessert"])}),Object.freeze({key:Xt,label:"Pije",hint:"Kafe, lëngje dhe pije të tjera",icon:"cup-soda",categories:Object.freeze(["coffee","drinks"])}),Object.freeze({key:ea,label:"Nuk e di",hint:"Gjitha ofertat për rreth teje.",icon:"sparkles",categories:Object.freeze([])})]);Object.freeze(tt.map(e=>e.key));const En=1,Ke=10,Bn=2,re=Object.freeze([{key:"1-2",min:1,max:2,label:"1–2"},{key:"3-4",min:3,max:4,label:"3–4"},{key:"5-6",min:5,max:6,label:"5–6"},{key:"7+",min:7,max:99,label:"7+"}]),ta=Object.freeze([{key:"2-4",min:2,max:4,label:"2–4"},{key:"4-6",min:4,max:6,label:"4–6"},{key:"6+",min:6,max:99,label:"6+"}]);function Kn(e=[]){const a=Array.isArray(e)?e:e?[e]:[],t=[];return a.forEach(n=>{const r=ze(n);r&&re.forEach(i=>{i.min>r.max||i.max<r.min||t.includes(i.key)||t.push(i.key)})}),re.filter(n=>t.includes(n.key)).map(n=>n.key)}function ze(e=""){const a=String(e||"").trim().toLowerCase();return re.find(t=>t.key===a)||ta.find(t=>t.key===a)||null}const ie="Europe/Belgrade",at=1440,I=["mon","tue","wed","thu","fri","sat","sun"],aa=Object.freeze({mon:"Hën",tue:"Mar",wed:"Mër",thu:"Enj",fri:"Pre",sat:"Sht",sun:"Die"});function ve(e=""){const a=String(e||"").trim().toLowerCase();return aa[a]||a}function na(e=[]){const a=Array.isArray(e)?e:[],t=I.filter(r=>a.includes(r));if(!t.length||t.length===I.length)return"";const n=[];return t.forEach(r=>{const i=I.indexOf(r),d=n[n.length-1];if(d&&d.end===i-1){d.end=i;return}n.push({start:i,end:i})}),n.map(r=>{const i=ve(I[r.start]),d=ve(I[r.end]);return r.start===r.end?i:r.end===r.start+1?`${i}, ${d}`:`${i}–${d}`}).join(", ")}const Te=new Map;function nt(e){const a=String(e||"").trim()||ie,t=Te.get(a);if(t)return t;let n=null;try{n=new Intl.DateTimeFormat("en-GB",{timeZone:a,hour12:!1,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",weekday:"short"})}catch{n=nt(ie)}return Te.set(a,n),n}const ra={mon:"mon",tue:"tue",wed:"wed",thu:"thu",fri:"fri",sat:"sat",sun:"sun"};function Ae(e){if(e==null||e==="")return 0;if(typeof e=="number")return Number.isFinite(e)?e:0;if(e instanceof Date){const n=e.getTime();return Number.isNaN(n)?0:n}if(typeof e=="object"){if(typeof e.toDate=="function")try{const n=e.toDate(),r=n instanceof Date?n.getTime():NaN;return Number.isNaN(r)?0:r}catch{return 0}return Number.isFinite(Number(e.seconds))?Math.round(Number(e.seconds)*1e3):Number.isFinite(Number(e._seconds))?Math.round(Number(e._seconds)*1e3):0}const t=new Date(String(e)).getTime();return Number.isNaN(t)?0:t}function C(e){const a=Ae(e);return a?new Date(a).toISOString():""}function we(e,a=ie){const t=Ae(e)||Date.now(),n=nt(a).formatToParts(new Date(t)),r=m=>{const g=n.find(w=>w.type===m);return g?g.value:""},i=r("year"),d=r("month"),c=r("day"),b=r("hour")==="24"?"00":r("hour"),x=r("minute"),p=String(r("weekday")||"").slice(0,3).toLowerCase();return{ms:t,dayKey:i&&d&&c?`${i}-${d}-${c}`:"",weekday:ra[p]||"",minutes:(Number(b)||0)*60+(Number(x)||0),timeZone:String(a||"").trim()||ie}}function ia(e){const a=String(e??"").trim();if(!a)return-1;const t=a.match(/^(\d{1,2})\s*[:.：]?\s*(\d{2})?$/);if(!t)return-1;const n=Number(t[1]),r=t[2]===void 0?0:Number(t[2]);return!Number.isFinite(n)||!Number.isFinite(r)||n>24||r>59?-1:n*60+r}function ke(e){const a=Math.max(0,Math.round(Number(e)||0))%at,t=Math.floor(a/60),n=a%60;return`${String(t).padStart(2,"0")}:${String(n).padStart(2,"0")}`}function Ne(e){return typeof e=="number"?Number.isFinite(e)?Math.max(0,Math.round(e)):-1:ia(e)}function sa(e,a){const t=Ne(e);let n=Ne(a);return t<0||n<0||n===t?null:(n<t&&(n+=at),{start:t,end:n})}function oa(e=[]){const a=Array.isArray(e)?e:[],t=[];return a.forEach(n=>{if(!n||typeof n!="object")return;const r=sa(n.start??n.from,n.end??n.to);r&&t.push(r)}),da(t)}function da(e=[]){const a=(Array.isArray(e)?e:[]).filter(n=>n&&Number.isFinite(n.start)&&Number.isFinite(n.end)&&n.end>n.start).slice().sort((n,r)=>n.start-r.start||n.end-r.end),t=[];return a.forEach(n=>{const r=t[t.length-1];if(r&&n.start<=r.end){r.end=Math.max(r.end,n.end);return}t.push({start:n.start,end:n.end})}),t}const la="active",Ge="paused",Ie="archived",Le="go",ca="public",Q="percent",Y="bundle",ae="freeItem",ne="specialPrice",ha=Object.freeze([Q,Y,ae,ne]),rt=Object.freeze(["all","food","drinks"]),ua=Object.freeze(["food","drink","any_order","custom"]),pa=Object.freeze({all:"",food:"në ushqim",drinks:"në pije"}),ma=Object.freeze({food:"me porosi ushqimi",drink:"me porosi të pijes",any_order:"me çdo porosi"});function y(e="",a=240){return String(e??"").trim().slice(0,a)}function fa(e=""){const a=y(e,500);return a&&(a.startsWith("/")&&!a.startsWith("//")||/^https:\/\/[^\s]+$/i.test(a))?a:""}const $e=9999999;function ga(e){if(typeof e=="number")return!Number.isFinite(e)||e<=0?0:Math.min($e,Math.round(e*100));const a=String(e??"").trim().replace(/[^\d.,]/g,"");if(!a)return 0;const t=Math.max(a.lastIndexOf(","),a.lastIndexOf(".")),n=t<0?"":a.slice(t+1),i=n.length===1||n.length===2?`${a.slice(0,t).replace(/[.,]/g,"")}.${n}`:a.replace(/[.,]/g,""),d=Number.parseFloat(i);return!Number.isFinite(d)||d<=0?0:Math.min($e,Math.round(d*100))}function me(e=0){const a=Math.max(0,Math.trunc(Number(e)||0));return a?`${(a/100).toFixed(2).replace(".",",")} €`:""}function Ue(e=0){const a=Math.max(0,Math.trunc(Number(e)||0));return a?(a/100).toFixed(2).replace(".",","):""}function te(e,a=0){const t=Math.trunc(Number(e));return!Number.isFinite(t)||t<0?a:t}function ba(e=""){const a=String(e||"").trim().toLowerCase();return a===Ge?Ge:a===Ie?Ie:la}function ya(e=""){const a=String(e||"").trim().toLowerCase();return Yt.includes(a)?a:et}function it(e="",a=0){const t=String(e||"").trim().toLowerCase();return t==="percent"||t==="discount"?Q:t==="bundle"?Y:t==="freeitem"||t==="free_item"?ae:t==="specialprice"||t==="special_price"?ne:t==="table"?"table":t==="custom"?"custom":a>0?Q:"custom"}function Se(e,a){const t=Math.trunc(Number(e));return Number.isFinite(t)&&t>0?Math.min($e,t):ga(a)}function Pe(e={}){const a=e&&typeof e=="object"?e:{},t=Math.min(90,Math.max(0,te(a.percent??a.discountPercent,0))),n=it(a.kind||a.type||a.offerType,t),r=String(a.scope||a.discountScope||"").trim().toLowerCase(),i=String(a.conditionType||a.condition||"").trim().toLowerCase(),d=Se(a.regularPriceCents,a.regularPrice),c=Se(a.goPriceCents,a.goPrice),b=c>0&&d>c?d-c:0,x={kind:n,percent:t,scope:rt.includes(r)?r:"all",itemId:y(a.itemId,180),itemName:y(a.itemName||a.item||a.bundleTitle||a.productName||a.freeItem,160),priceText:y(a.priceText||a.price,60),regularPriceCents:d,goPriceCents:c,savingCents:b,savingPercent:b>0&&d>0?Math.round(b/d*1e4)/100:0,conditionType:ua.includes(i)?i:"",customCondition:y(a.customCondition,120),text:y(a.text,160)};return x.label=_a(x),x}function st(e=""){const a=String(e||"").trim().toLowerCase();return pa[a]||""}function ot(e={}){const a=e&&typeof e=="object"?e:{},t=String(a.conditionType||"").trim().toLowerCase();return t==="custom"?y(a.customCondition,120):ma[t]||""}function dt(e={}){return me(Se(e.goPriceCents,e.goPrice))||y(e.priceText,60)}function _a(e={}){const a=e&&typeof e=="object"?e:{},t=y(a.text,160);if(t)return t;const n=te(a.percent,0),r=it(a.kind,n),i=y(a.itemName,160);if(r===Q)return n<=0?"":[`-${n}%`,st(a.scope)].filter(Boolean).join(" ");if(r===ae)return i?[`${i} FALAS`,ot(a)].filter(Boolean).join(" "):"";if(r===Y||r===ne){const d=dt(a);return i?[i,d].filter(Boolean).join(" "):d}return r==="table"?"Tavolinë e rezervuar":""}function xa(e={}){const a=e&&typeof e=="object"&&e.label!==void 0?e:Pe(e),t={kind:a.kind||"",eyebrow:"",headline:a.label||"",note:"",priceRegular:"",priceGo:"",savingLabel:""},n=y(a.itemName,160);if(a.kind===Q){const r=te(a.percent,0);return t.headline=r>0?`-${r}%`:"",t.note=st(a.scope),t}if(a.kind===ae)return t.headline=n?`${n} FALAS`:"",t.note=ot(a),t;if(a.kind===Y||a.kind===ne){t.eyebrow=a.kind===Y?"Paketë GO":"Çmim special GO",t.headline=n,t.priceRegular=me(a.regularPriceCents),t.priceGo=dt(a);const r=me(a.savingCents);return t.savingLabel=r?`Kursen ${r}`:"",t}return t}function lt(e){const a=Array.isArray(e)?e:e?[e]:[],t=[];return a.forEach(n=>{const r=String(n||"").trim().toLowerCase();ze(r)&&!t.includes(r)&&t.push(r)}),t.length?t:re.map(n=>n.key)}function va(e=[]){const a=lt(e);let t=Number.POSITIVE_INFINITY,n=0;return a.forEach(r=>{const i=ze(r);i&&(t=Math.min(t,i.min),n=Math.max(n,i.max))}),!Number.isFinite(t)||!n?{min:1,max:99}:{min:t,max:n}}function wa(e={}){const a=e&&typeof e=="object"?e:{},t=String(a.mode||"").trim().toLowerCase()==="windows"?"windows":"always",n=[];(Array.isArray(a.days)?a.days:[]).forEach(d=>{const c=String(d||"").trim().toLowerCase();I.includes(c)&&!n.includes(c)&&n.push(c)});const i=oa((Array.isArray(a.windows)?a.windows:[]).map(d=>({start:d?.start??d?.from,end:d?.end??d?.to})));return t==="always"||!n.length&&!i.length?{mode:"always",days:I.slice(),windows:[]}:{mode:"windows",days:n.length?n:I.slice(),windows:i}}function ka(e={}){const a=e&&typeof e=="object"?e:{},t=i=>{const d=y(i,10);return/^\d{4}-\d{2}-\d{2}$/.test(d)?d:""},n=t(a.startDate||a.from),r=t(a.endDate||a.to);return n&&r&&r<n?{startDate:r,endDate:n}:{startDate:n,endDate:r}}function $a(e={}){const a=e&&typeof e=="object"?e:{};return{dailyGroups:te(a.dailyGroups??a.maxGroupsPerDay,0),totalRedemptions:te(a.totalRedemptions??a.maxRedemptions,0)}}function Sa(e){const a=Array.isArray(e)?e:[],t=[];return a.forEach(n=>{const r=String(n||"").trim().toLowerCase();(r===Le||r===ca)&&!t.includes(r)&&t.push(r)}),t.length?t:[Le]}function fe(e={},a=""){const t=e&&typeof e=="object"?e:{},n=lt(t.partyRanges||t.partySizes),r=va(n),i=Pe(t.benefit);return{id:y(t.id||a,180),restaurantId:y(t.restaurantId,180),locationId:y(t.locationId,180)||"main",title:y(t.title,120),description:y(t.description||t.text,400),terms:y(t.terms||t.conditions,400),imageUrl:fa(t.imageUrl||t.image?.url||t.photoUrl),benefit:i,benefitLabel:i.label,category:ya(t.category),partyRanges:n,minParty:r.min,maxParty:r.max,schedule:wa(t.schedule),dateRange:ka(t.dateRange),limits:$a(t.limits),channels:Sa(t.channels),status:ba(t.status),sponsored:t.sponsored===!0||t.sponsored?.active===!0,sponsoredUntil:C(t.sponsored?.until),priceLevel:Math.min(4,Math.max(0,te(t.priceLevel,0))),redeemedCount:te(t.redeemedCount,0),createdAt:C(t.createdAt),updatedAt:C(t.updatedAt)}}function Tn(e={},{serverTimestamp:a=null}={}){const t=fe(e),n={restaurantId:t.restaurantId,locationId:t.locationId,title:t.title,description:t.description,terms:t.terms,imageUrl:t.imageUrl,benefit:t.benefit,benefitLabel:t.benefitLabel,category:t.category,partyRanges:t.partyRanges,minParty:t.minParty,maxParty:t.maxParty,schedule:t.schedule,dateRange:t.dateRange,limits:t.limits,channels:t.channels,status:t.status,sponsored:t.sponsored,priceLevel:t.priceLevel};return a&&(n.updatedAt=a,t.createdAt||(n.createdAt=a)),n}function za(e={}){const a=e&&typeof e=="object"&&e.label!==void 0?e:Pe(e),t=[],n=y(a.itemName,160),r=(i="")=>{n||t.push({field:"benefitItem",message:i}),!(a.priceText&&!a.regularPriceCents&&!a.goPriceCents)&&(a.regularPriceCents||t.push({field:"regularPrice",message:"Shkruaj çmimin normal."}),a.goPriceCents||t.push({field:"goPrice",message:"Shkruaj çmimin GO."}),a.regularPriceCents&&a.goPriceCents&&a.goPriceCents>=a.regularPriceCents&&t.push({field:"goPrice",message:"Çmimi GO duhet të jetë më i ulët se çmimi normal."}))};return a.kind===Q?(a.percent<=0&&t.push({field:"benefitPercent",message:"Shkruaj zbritjen."}),rt.includes(a.scope)||t.push({field:"benefitScope",message:"Zgjidh ku vlen zbritja."}),t):a.kind===ae?(n||t.push({field:"benefitItem",message:"Shkruaj çka merr klienti falas."}),a.conditionType?a.conditionType==="custom"&&!a.customCondition&&t.push({field:"benefitCondition",message:"Shkruaj kushtin e ofertës."}):t.push({field:"benefitCondition",message:"Zgjidh kushtin e ofertës."}),t):a.kind===Y?(r("Shkruaj çka përfshin paketa."),t):a.kind===ne?(r("Shkruaj produktin."),t):(a.label||t.push({field:"benefit",message:"Shkruaj çka po ofron."}),t)}function Aa(e={}){const a=fe(e),t=[];return a.restaurantId||t.push({field:"restaurantId",message:"Lokali mungon."}),t.push(...za(a.benefit)),a.partyRanges.length||t.push({field:"partyRanges",message:"Zgjidh sa persona."}),a.schedule.mode==="windows"&&(a.schedule.days.length||t.push({field:"schedule",message:"Zgjidh ditët."}),a.schedule.windows.length||t.push({field:"schedule",message:"Zgjidh orarin."})),{ok:t.length===0,errors:t,offer:a}}function ct(e={}){const t=(e&&e.schedule?e:fe(e)).schedule;if(t.mode==="always")return"Gjithmonë";const n=na(t.days),r=t.windows.map(i=>`${ke(i.start)}-${ke(i.end)}`).join(", ");return[n,r].filter(Boolean).join(" · ")}function ht(e={}){const a=e&&e.partyRanges?e:fe(e);return a.minParty<=1&&a.maxParty>=Ke?"Të gjithë":`${a.maxParty>=Ke?`${a.minParty}+`:`${a.minParty}–${a.maxParty}`} persona`}const ut=Object.freeze({x:[["path",{d:"M18 6 6 18"}],["path",{d:"m6 6 12 12"}]],users:[["path",{d:"M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"}],["circle",{cx:"9",cy:"7",r:"4"}],["path",{d:"M22 21v-2a4 4 0 0 0-3-3.87"}],["path",{d:"M16 3.13a4 4 0 0 1 0 7.75"}]],sparkles:[["path",{d:"M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"}],["path",{d:"M20 3v4"}],["path",{d:"M22 5h-4"}],["path",{d:"M4 17v2"}],["path",{d:"M5 18H3"}]],coffee:[["path",{d:"M10 2v2"}],["path",{d:"M14 2v2"}],["path",{d:"M16 8a1 1 0 0 1 1 1v8a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4V9a1 1 0 0 1 1-1h14a4 4 0 1 1 0 8h-1"}],["path",{d:"M6 2v2"}]],"cup-soda":[["path",{d:"m6 8 1.75 12.28a2 2 0 0 0 2 1.72h4.54a2 2 0 0 0 2-1.72L18 8"}],["path",{d:"M5 8h14"}],["path",{d:"M7 15a6.47 6.47 0 0 1 5 0 6.47 6.47 0 0 0 5 0"}],["path",{d:"m12 8 1-6h2"}]],utensils:[["path",{d:"M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"}],["path",{d:"M7 2v20"}],["path",{d:"M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3Zm0 0v7"}]],"cake-slice":[["circle",{cx:"9",cy:"7",r:"2"}],["path",{d:"M7.2 7.9 3 11v9c0 .6.4 1 1 1h16c.6 0 1-.4 1-1v-9c0-2-3-6-7-8l-3.6 2.6"}],["path",{d:"M16 13H3"}],["path",{d:"M16 17H3"}]],zap:[["path",{d:"M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"}]],timer:[["line",{x1:"10",x2:"14",y1:"2",y2:"2"}],["line",{x1:"12",x2:"15",y1:"14",y2:"11"}],["circle",{cx:"12",cy:"14",r:"8"}]],clock:[["circle",{cx:"12",cy:"12",r:"10"}],["polyline",{points:"12 6 12 12 16 14"}]],"calendar-clock":[["path",{d:"M21 7.5V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h3.5"}],["path",{d:"M16 2v4"}],["path",{d:"M8 2v4"}],["path",{d:"M3 10h5"}],["path",{d:"M17.5 17.5 16 16.3V14"}],["circle",{cx:"16",cy:"16",r:"6"}]],"map-pin":[["path",{d:"M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"}],["circle",{cx:"12",cy:"10",r:"3"}]],search:[["circle",{cx:"11",cy:"11",r:"8"}],["path",{d:"m21 21-4.3-4.3"}]],"badge-percent":[["path",{d:"M3.85 8.62a4 4 0 0 1 4.78-4.77 4 4 0 0 1 6.74 0 4 4 0 0 1 4.78 4.78 4 4 0 0 1 0 6.74 4 4 0 0 1-4.77 4.78 4 4 0 0 1-6.75 0 4 4 0 0 1-4.78-4.77 4 4 0 0 1 0-6.76Z"}],["path",{d:"m15 9-6 6"}],["path",{d:"M9 9h.01"}],["path",{d:"M15 15h.01"}]],"check-check":[["path",{d:"M18 6 7 17l-5-5"}],["path",{d:"m22 10-7.5 7.5L13 16"}]],"party-popper":[["path",{d:"M5.8 11.3 2 22l10.7-3.79"}],["path",{d:"M4 3h.01"}],["path",{d:"M22 8h.01"}],["path",{d:"M15 2h.01"}],["path",{d:"M22 20h.01"}],["path",{d:"m22 2-2.24.75a2.9 2.9 0 0 0-1.96 3.12c.1.86-.57 1.63-1.45 1.63h-.38c-.86 0-1.6.6-1.76 1.44L14 10"}],["path",{d:"m22 13-.82-.33c-.86-.34-1.82.2-1.98 1.11c-.11.7-.72 1.22-1.43 1.22H17"}],["path",{d:"m11 2 .33.82c.34.86-.2 1.82-1.11 1.98C9.52 4.9 9 5.52 9 6.23V7"}],["path",{d:"M11 13c1.93 1.93 2.83 4.17 2 5-.83.83-3.07-.07-5-2-1.93-1.93-2.83-4.17-2-5 .83-.83 3.07.07 5 2Z"}]],gift:[["rect",{x:"3",y:"8",width:"18",height:"4",rx:"1"}],["path",{d:"M12 8v13"}],["path",{d:"M19 12v7a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2v-7"}],["path",{d:"M7.5 8a2.5 2.5 0 0 1 0-5A4.8 8 0 0 1 12 8a4.8 8 0 0 1 4.5-5 2.5 2.5 0 0 1 0 5"}]],"shield-check":[["path",{d:"M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"}],["path",{d:"m9 12 2 2 4-4"}]],"ticket-percent":[["path",{d:"M2 9a3 3 0 1 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 1 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"}],["path",{d:"M9 9h.01"}],["path",{d:"m15 9-6 6"}],["path",{d:"M15 15h.01"}]],store:[["path",{d:"m2 7 4.41-4.41A2 2 0 0 1 7.83 2h8.34a2 2 0 0 1 1.42.59L22 7"}],["path",{d:"M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"}],["path",{d:"M15 22v-4a2 2 0 0 0-2-2h-2a2 2 0 0 0-2 2v4"}],["path",{d:"M2 7h20"}],["path",{d:"M22 7v3a2 2 0 0 1-2 2a2.7 2.7 0 0 1-1.59-.63.7.7 0 0 0-.82 0A2.7 2.7 0 0 1 16 12a2.7 2.7 0 0 1-1.59-.63.7.7 0 0 0-.82 0A2.7 2.7 0 0 1 12 12a2.7 2.7 0 0 1-1.59-.63.7.7 0 0 0-.82 0A2.7 2.7 0 0 1 8 12a2.7 2.7 0 0 1-1.59-.63.7.7 0 0 0-.82 0A2.7 2.7 0 0 1 4 12a2 2 0 0 1-2-2V7"}]],pencil:[["path",{d:"M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"}],["path",{d:"m15 5 4 4"}]],armchair:[["path",{d:"M19 9V6a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v3"}],["path",{d:"M3 16a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-5a2 2 0 0 0-4 0v1.5a.5.5 0 0 1-.5.5h-9a.5.5 0 0 1-.5-.5V11a2 2 0 0 0-4 0z"}],["path",{d:"M5 18v2"}],["path",{d:"M19 18v2"}]],"circle-check-big":[["path",{d:"M21.801 10A10 10 0 1 1 17 3.335"}],["path",{d:"m9 11 3 3L22 4"}]],"book-open":[["path",{d:"M12 7v14"}],["path",{d:"M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"}]],navigation:[["polygon",{points:"3 11 22 2 13 21 11 13 3 11"}]],"log-in":[["path",{d:"M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"}],["polyline",{points:"10 17 15 12 10 7"}],["line",{x1:"15",x2:"3",y1:"12",y2:"12"}]],"rotate-ccw":[["path",{d:"M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"}],["path",{d:"M3 3v5h5"}]],"arrow-left":[["path",{d:"m12 19-7-7 7-7"}],["path",{d:"M19 12H5"}]],"chevron-right":[["path",{d:"m9 18 6-6-6-6"}]],"triangle-alert":[["path",{d:"m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"}],["path",{d:"M12 9v4"}],["path",{d:"M12 17h.01"}]],link:[["path",{d:"M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"}],["path",{d:"M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"}]],copy:[["rect",{width:"14",height:"14",x:"8",y:"8",rx:"2",ry:"2"}],["path",{d:"M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"}]],check:[["path",{d:"M20 6 9 17l-5-5"}]],hash:[["line",{x1:"4",x2:"20",y1:"9",y2:"9"}],["line",{x1:"4",x2:"20",y1:"15",y2:"15"}],["line",{x1:"10",x2:"8",y1:"3",y2:"21"}],["line",{x1:"16",x2:"14",y1:"3",y2:"21"}]],ban:[["circle",{cx:"12",cy:"12",r:"10"}],["path",{d:"m4.9 4.9 14.2 14.2"}]]}),Pa=Object.freeze({xmlns:"http://www.w3.org/2000/svg",width:"24",height:"24",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round","aria-hidden":"true",focusable:"false"});function He(e={}){return Object.entries(e).map(([a,t])=>` ${a}="${t}"`).join("")}Object.freeze(Object.keys(ut));function ce(e="",a=""){const t=ut[String(e||"").trim()];if(!t)return"";const n=t.map(([i,d])=>`<${i}${He(d)}></${i}>`).join(""),r=String(a||"").trim();return`<svg${He(Pa)}${r?` class="${r}"`:""}>${n}</svg>`}const Ze=Object.freeze({offering:"po ju ofron",forGroup:"për grupin tuaj",accept:"Prano ofertën",sponsored:"Sponsored",onlyGo:"Vetëm me Mnyra GO",tableIncluded:"Tavolinë",peopleSuffix:"persona"}),pt="clean",De="hero",mt="compact",Da=[pt,De,mt];function ja({imageUrl:e="",variant:a=""}={}){if(!String(e||"").trim())return pt;const t=String(a||"").trim().toLowerCase();return Da.includes(t)?t:De}const Fa=`
...
`;function F(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Ca({businessName:e="",logoUrl:a="",imageUrl:t="",variant:n="",benefitLabel:r="",benefitView:i=null,sponsored:d=!1,meta:c=[],ctaLabel:b="",ctaIcon:x="check-check",ctaDisabled:p=!1,cardAttrs:m="",ctaAttrs:g="",texts:w=Ze}={}){const A={...Ze,...w||{}},M=(Array.isArray(c)?c:[]).filter(V=>V&&V.label),O=String(b||A.accept),_=i&&typeof i=="object"?i:{},B=String(_.headline||r||""),U=String(_.priceGo||""),v=String(_.priceRegular||""),R=String(t||"").trim(),T=ja({imageUrl:R,variant:n}),J=T===mt,N=R?`<img class="mnyra-go-page__card-photo" src="${F(R)}" alt="" loading="lazy" decoding="async" />`:"",H=`
    <div class="mnyra-go-page__card-head">
      ${a?`<img class="mnyra-go-page__card-logo" src="${F(a)}" alt="" width="40" height="40" loading="lazy" decoding="async" />`:`<div class="mnyra-go-page__card-logo mnyra-go-page__card-logo--empty">${ce("store")}</div>`}
    ...
    <p class="mnyra-go-page__card-benefit${U?" mnyra-go-page__card-benefit--title":""}">${F(B)}</p>
    ...
    ${U?`
      <div class="mnyra-go-page__card-prices">
        ...
        <span class="mnyra-go-page__card-price-go">${F(U)}</span>
      </div>
    `:""}
  ...
  `,Z=`
    <div class="mnyra-go-page__card-meta">
      ${M.map(V=>`<span>${ce(V.icon||"")}${F(V.label)}</span>`).join("")}
    </div>
  ...
  `}const S=Object.freeze({accepted:"accepted",activated:"activated",finalized:"finalized",cancelled:"cancelled",expired:"expired"}),Ve=Object.freeze({confirmed:S.accepted,checked_in:S.finalized,completed:S.finalized,cancelled_by_user:S.cancelled,cancelled_by_business:S.cancelled,not_arrived:S.expired});Object.freeze([S.accepted,S.activated]);Object.freeze({[S.accepted]:[S.activated,S.cancelled,S.expired],[S.activated]:[S.finalized,S.expired],[S.finalized]:[],[S.cancelled]:[],[S.expired]:[]});function ft(e=""){const a=String(e||"").trim().toLowerCase();return Object.values(S).includes(a)?a:Ve[a]?Ve[a]:S.accepted}function Nn({atMs:e=Date.now(),timeZone:a=ie}={}){return we(e,a).dayKey}function Ma(e={},a=""){const t=e&&typeof e=="object"?e:{},n=t.snapshot&&typeof t.snapshot=="object"?t.snapshot:{};return{id:y(t.id||a,180),restaurantId:y(t.restaurantId||n.restaurantId,180),locationId:y(t.locationId||n.locationId,180)||"main",offerId:y(t.offerId||n.offerId,180),guestId:y(t.guestId,180),uid:y(t.uid,180),shortCode:y(t.shortCode,12).toUpperCase(),status:ft(t.status),partySizeRequested:Math.max(1,Math.trunc(Number(t.partySizeRequested||t.partySize||n.partySize)||1)),partySizeVerified:Number.isFinite(Number(t.partySizeVerified))&&Number(t.partySizeVerified)>0?Math.trunc(Number(t.partySizeVerified)):null,dayKey:y(t.dayKey,10),timeZone:y(t.timeZone,60)||ie,snapshot:n,businessName:y(n.businessName,120),benefitLabel:y(n.benefitLabel,160),logoUrl:y(n.logoUrl,500),businessSeenAt:C(t.businessSeenAt),acceptedAt:C(t.acceptedAt||t.createdAt),activationDeadline:C(t.activationDeadline),activatedAt:C(t.activatedAt),finalizationDeadline:C(t.finalizationDeadline),finalizedAt:C(t.finalizedAt||t.completedAt),cancelledAt:C(t.cancelledAt),expiredAt:C(t.expiredAt),expiredReason:y(t.expiredReason,40),commissionVersion:y(t.commissionVersion,40),commission:t.commission&&typeof t.commission=="object"?{version:y(t.commission.version,40),currency:y(t.commission.currency,8),partySize:Math.max(1,Math.trunc(Number(t.commission.partySize)||1)),amountCents:Math.max(0,Math.trunc(Number(t.commission.amountCents)||0)),status:y(t.commission.status,20),confirmedAt:C(t.commission.confirmedAt)}:null,createdAt:C(t.createdAt),updatedAt:C(t.updatedAt)}}function Oa(e={}){const a=ft(e?.status);return a===S.accepted?"Ka pranuar":a===S.activated?"Aktivizuar":a===S.finalized?"Finalizuar":a===S.cancelled?"Anuluar nga klienti":"Skaduar"}function Gn(e={},{nowMs:a=Date.now()}={}){const t=Ma(e),n=Ae(t.activationDeadline);if(!n)return"";const r=t.timeZone,i=we(n,r),d=we(a,r),c=ke(i.minutes);return i.dayKey===d.dayKey?`E vlefshme deri sot, ${c}`:`E vlefshme deri nesër, ${c}`}function Ra(e=0){const a=Math.max(0,Math.trunc(Number(e)||0)),t=Math.trunc(a/100),n=String(a%100).padStart(2,"0");return`${t},${n} €`}const s=Object.freeze({brand:"Mnyra GO",mark:"⚡",editor:"Editori",brandMnyra:"MNYRA",brandGo:"GO",emptyTitle:"Merr klientë kur ata janë gati të dalin.",emptyAction:"Aktivizo ofertën e parë",cardIdle:"Krijo oferta për klientët që kërkojnë tani.",cardManage:"Menaxho GO",tabs:{active:"Aktiv",offers:"Ofertat",archive:"Arkiv",options:"Opsionet"},statNew:"Të reja",statActive:"Aktive",statToday:"Sot",guests:"Mysafirë",goOn:"GO Aktiv",pause:"Pauzo GO",resume:"Aktivizo GO",pausedUntil:"Pauzuar deri",createOffer:"Ofertë e re GO",editorHint:"Krijoje ofertën një herë. Mnyra ua shfaq automatikisht klientëve që përputhen.",scanOffer:"Skano ofertën",seenToday:"Ofertën e kanë parë sot",acceptedToday:"E kanë pranuar sot",editOffer:"Ndrysho ofertën",preview:"Kështu e sheh klienti",activate:"Aktivizo",save:"Ruaj ofertën",saving:"Po ruhet...",close:"Mbyll",edit:"Edit",offering:"po ju ofron",forGroup:"për grupin tuaj",accept:"Prano ofertën",finalizeTitle:"Finalizo ofertën",benefitQuestion:"Çka po ofron?",benefitHint:"Zgjidh çfarë dëshiron t'i ofrosh klientit.",benefitPercent:"Zbritje %",benefitBundle:"Paketë GO",benefitFree:"Falas",benefitSpecial:"Çmim special",benefitLegacy:"Zgjidh llojin e ofertës.",discountQuestion:"Sa zbritje po ofron?",discountOther:"Tjetër",discountPlaceholder:"Shkruaj zbritjen",scopeQuestion:"Ku vlen zbritja?",scopeAll:"Krejt fatura",scopeFood:"Ushqim",scopeDrinks:"Pije",bundleQuestion:"Çka përfshin paketa?",bundlePlaceholder:"p.sh. 2 Burger + 2 Pije",freeQuestion:"Çka merr falas?",freePlaceholder:"p.sh. 1 Pije",conditionQuestion:"Kur e merr falas?",conditionFood:"Me ushqim",conditionDrink:"Me pije",conditionAny:"Me çdo porosi",conditionCustom:"Tjetër",customConditionQuestion:"Shkruaj kushtin",customConditionPlaceholder:"p.sh. kur porosit 2 pizza",productQuestion:"Cili produkt?",productPlaceholder:"p.sh. Pizza Margherita",priceRegular:"Çmimi normal",priceGo:"Çmimi GO",pricePlaceholder:"0,00",saving:"Kursen",photoQuestion:"Foto e ofertës",photoHint:"Shto një foto që klienti ta shohë ofertën menjëherë.",photoOptional:"Opsionale",photoAdd:"Shto një foto",photoSource:"Nga telefoni ose kamera",photoChange:"Ndrysho",photoRemove:"Hiq",photoUploading:"Po ngarkohet...",photoError:"Fotoja nuk u ngarkua. Provo prapë.",partyQuestion:"Për sa persona vlen?",partyHint:"Zgjidh për çfarë madhësie të grupit vlen oferta.",partyAll:"Të gjithë",categoryQuestion:"Kur të shfaqet oferta?",categoryHint:"Zgjidh kur kjo ofertë i përshtatet kërkimit të klientit.",ifFood:"Nëse kërkohet ushqim",ifDrinks:"Nëse kërkohet kafe / pije",scheduleQuestion:"Kur vlen oferta?",scheduleHint:"Zgjidh kur klientët mund ta përdorin ofertën.",always:"Gjithmonë",specificHours:"Orar specifik",daysQuestion:"Ditët",hoursQuestion:"Orari",hoursFrom:"Nga",hoursTo:"Deri",limitsTitle:"Kufijtë",slotGroups:"Grupe për 30 min",slotGuests:"Mysafirë për 30 min",dailyGroups:"Grupe në ditë",totalRedemptions:"Sa herë gjithsej",noLimit:"0 = pa kufi",paused:"Pauzuar",archived:"Arkivuar",archive:"Arkivo",noBookings:"Ende asnjë klient sot.",noHistory:"Ende asnjë histori.",loading:"Po ngarkohet...",guestName:"Mnyra Guest",around:"Rreth",finalize:"Finalizo",needsActivation:"Klienti duhet ta aktivizojë ofertën.",search:"Kërko",searching:"Po kërkoj...",codePlaceholder:"Kodi i klientit",codeNotFound:"Ky kod nuk u gjet.",partyAtTable:"Sa persona janë",commission:"Provizioni",keepsRunning:"Rezervimet ekzistuese mbeten. Vetëm të rejat ndalen.",onlyBusiness:"Ky funksion eshte vetem per profile biznesi.",loadingBusiness:"Biznesi po ngarkohet..."});function o(e,a=""){return typeof e=="function"?e(a):String(a??"")}function L(e,a="",t="w-4 h-4"){return typeof e=="function"?e(a,t):""}function gt(e=""){const a=Date.parse(String(e||""));if(!Number.isFinite(a))return"";const t=new Date(a);return`${String(t.getHours()).padStart(2,"0")}:${String(t.getMinutes()).padStart(2,"0")}`}function Ea({enabled:e=!1,unseenCount:a=0,activeOffers:t=0,todayBookings:n=0,iconFn:r=null,texts:i={}}={}){if(!e)return"";const d={...s,...i||{}},c=Math.max(0,Math.trunc(Number(a)||0)),b=t>0||n>0,x=b?`${t} oferta aktive · ${n} rezervime sot`:d.cardIdle;return`
    <button type="button" class="mnyra-dash__composer mnyra-dash__composer--tap mnyra-dash__composer--plane" data-go-business-card data-nav="gobiznes">
      <span class="mnyra-dash__composer-title">
        ...
        ${c>0?`<span class="mnyra-dash__composer-badge" aria-label="${c} ${d.statNew}">${c}</span>`:""}
      </span>
...
`;function Ka(e={},a={}){const t=a.escapeHtml,n=a.icon,r=e.imageUrl?`<img class="go-hl__media" src="${o(t,e.imageUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:"",i=e.action?`<span class="go-hl__action">${o(t,e.action)}</span>`:`
      <span class="go-hl__label">${o(t,e.label)}</span>
      <span class="go-hl__value">${o(t,e.value)}</span>
    `,d=e.action||`${e.label} ${e.value}`;return`
    <button type="button" class="go-hl__card" ${e.attr||""} data-go-highlight="${o(t,e.key)}"
      aria-label="${o(t,d)}">
      <span class="go-hl__plate ${o(t,e.tone||"text-slate-400")}">${L(n,e.icon,"w-6 h-6")}</span>
      ${r}
      <span class="go-hl__body">${i}</span>
    </button>
  `}function Ta({stats:e={},deps:a={}}={}){return`
    <div class="go-hl" data-go-highlights>
      ${[{key:"scan",action:s.scanOffer,icon:"camera",tone:"text-indigo-600",attr:"data-go-scan"},{key:"seen",label:s.seenToday,value:Number(e.impressions)||0,icon:"eye",tone:"text-indigo-600"},{key:"accepted",label:s.acceptedToday,value:Number(e.accepted)||0,icon:"check-check",tone:"text-emerald-600"}].map(n=>Ka(n,a)).join("")}
      <span class="go-hl__tail" aria-hidden="true"></span>
    </div>
  `}function Na({tab:e="active",deps:a={}}={}){const t=a.escapeHtml,n=a.icon;return`
    <div class="go-tabs" role="tablist" data-go-tabs>
      ${[["active",s.tabs.active,"zap"],["offers",s.tabs.offers,"tag"],["archive",s.tabs.archive,"archive"],["options",s.tabs.options,"settings"]].map(([i,d,c])=>`
        <button type="button" role="tab" aria-selected="${e===i?"true":"false"}" data-go-business-tab="${i}"
          class="go-tab">${L(n,c,"w-4 h-4")}<span class="go-tab-label">${o(t,d)}</span></button>
      `).join("")}
    </div>
  `}function _e(e={},a={},{found:t=!1}={}){const n=a.escapeHtml,r=e.benefitLabel||e.snapshot?.benefitLabel||"",i=!e.businessSeenAt,d=e.partySizeVerified||e.partySizeRequested||1,c=gt(e.acceptedAt),b=c?`${s.around} ${c}`:s.guestName;return`
    <div class="p-4 rounded-[1.6rem] border ${t?"go-booking--found bg-white":i?"bg-indigo-50/50 border-indigo-100":"bg-slate-50 border-slate-100"}"
      ...
      <div class="go-booking-meta mt-3 flex flex-wrap items-center text-xs font-bold text-slate-600">
        <span>👥 ${o(n,`${d} ${s.guests}`)}</span>
        ${r?`<span>🎁 ${o(n,r)}</span>`:""}
      </div>
      ${t&&e.status==="activated"?`
        <div class="mt-4">
          <!--
          ...
          <button type="button" data-go-booking-finalize data-go-booking-id="${o(n,e.id)}"
      ...
      ${e.commission?`
        ...
        <p class="mt-3 pt-3 border-t border-slate-200/70 text-[10px] font-black uppercase tracking-widest text-slate-400">
          ${o(n,s.commission)} · ${o(n,Ra(e.commission.amountCents))}
        </p>
      `:""}
    </div>
  `}function Ga({code:e="",status:a="",busy:t=!1,deps:n={}}={}){const r=n.escapeHtml,i=n.icon;return`
    <div class="mb-4" data-go-code-search>
      <div class="go-code-box flex items-center gap-2 p-1.5 rounded-2xl border border-slate-200 bg-white">
        <span class="pl-2 text-slate-400">${L(i,"search","w-4 h-4")}</span>
        ...
        <button type="button" data-go-code-submit ${t?"disabled":""}
          ...
          ${o(r,t?s.searching:s.search)}
        </button>
      </div>
      ${a?`<p class="mt-2 text-[10px] font-bold text-rose-500">${o(r,a)}</p>`:""}
    </div>
  `}function he({eyebrow:e="",title:a="",sub:t="",action:n="",body:r="",deps:i={}}={}){const d=i.escapeHtml;return`
    <div class="mb-6 bg-white rounded-[2.5rem] p-6 border border-slate-100 shadow-sm">
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <h3 class="text-xl font-black italic tracking-tighter">${o(d,a)}</h3>
          ${t?`<p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">${o(d,t)}</p>`:""}
        </div>
        ${n}
      </div>
      ${r}
    </div>
  `}function Ia(e={},a={}){const t=a.escapeHtml,n=e.status==="paused"?s.paused:e.status==="archived"?s.archived:"";return`
    <div class="flex items-start gap-4 p-4 rounded-[1.6rem] bg-slate-50 border border-slate-100" data-go-offer="${o(t,e.id)}">
      ...
      <div class="flex flex-col gap-2">
        <button type="button" data-go-offer-edit="${o(t,e.id)}"
          class="px-3 py-1.5 rounded-xl bg-white text-[10px] font-black uppercase tracking-widest text-slate-600 border border-slate-200">${o(t,s.edit)}</button>
        <button type="button" data-go-offer-toggle="${o(t,e.id)}"
          class="px-3 py-1.5 rounded-xl bg-white text-[10px] font-black uppercase tracking-widest text-slate-600 border border-slate-200">${o(t,e.status==="active"?s.paused:s.activate)}</button>
        <button type="button" data-go-offer-archive="${o(t,e.id)}"
          class="px-3 py-1.5 rounded-xl bg-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-400">${o(t,s.archive)}</button>
      </div>
    </div>
  `}function La({offer:e={},businessName:a="",previewImageUrl:t="",deps:n={}}={}){const r=n.escapeHtml;return`
    <div data-go-offer-preview>
      ...
      <div style="pointer-events:none;" aria-hidden="true">
        ${Ca({businessName:a,imageUrl:t||e.imageUrl||"",benefitLabel:e.benefitLabel||"",benefitView:xa(e.benefit||{}),meta:[{icon:"users",label:ht(e)},{icon:"clock",label:ct(e)}]})}
      </div>
    </div>
  `}function G(e,a="",t=""){return`<label class="text-[10px] font-black uppercase tracking-widest text-slate-400"${t?` for="${o(e,t)}"`:""}>${o(e,a)}</label>`}function ue(e,{active:a=!1,attr:t="",value:n="",escapeHtml:r=null}={}){return`
    <button type="button" ${t?`${t}="${o(r,n)}"`:""} aria-pressed="${a?"true":"false"}"
      class="go-offer-chip px-4 rounded-2xl text-xs font-black transition-colors ${a?"bg-slate-900 text-white":"bg-slate-50 text-slate-600 border border-slate-100"}">
      ${o(r,e)}
    </button>
  `}const Ua=`
...
`;function xe({attr:e="",unit:a="€",value:t="",placeholder:n="",mode:r="decimal",inputClass:i="",escapeHtml:d=null}={}){return`
    <div class="go-offer-price">
      <input type="text" ${e} inputmode="${o(d,r)}" autocomplete="off"
        placeholder="${o(d,n)}" value="${o(d,t)}" class="${i}" />
      <span class="go-offer-price__unit">${o(d,a)}</span>
    </div>
  `}function se(e,{active:a=!1,attr:t="",value:n="",escapeHtml:r=null}={}){return`
    <button type="button" ${t?`${t}="${o(r,n)}"`:""} aria-pressed="${a?"true":"false"}"
      class="go-offer-pill rounded-xl font-black transition-colors ${a?"bg-slate-900 text-white":"bg-slate-50 text-slate-600 border border-slate-100"}">
      ${o(r,e)}
    </button>
  `}const We=Object.freeze([10,15,20,25]);function Ha({benefit:e={},percentCustom:a=!1,errorFor:t=()=>"",inputClass:n="",inputBase:r="",escapeHtml:i=null}={}){const d=g=>G(i,g),c=g=>{const w=t(g);return w?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="${o(i,g)}">${o(i,w)}</p>`:""};if(!ha.includes(e.kind))return`
      <p class="go-offer-saving font-bold text-slate-400">${o(i,s.benefitLegacy)}</p>
      ${c("benefit")}
    `;const b=Number(e.percent)||0,x=a||b>0&&!We.includes(b),p=()=>{const g=me(e.savingCents);if(!g)return"";const w=Math.round(Number(e.savingPercent)||0);return`
  ...
  `;if(e.kind===Q)return`
      ...
      <div class="mt-2 flex flex-wrap gap-2">
        ${We.map(g=>se(`${g}%`,{active:!x&&b===g,attr:"data-go-discount",value:String(g),escapeHtml:i})).join("")}
    ...
    `}return e.kind===ne?`
      ...
      ${c("benefitItem")}
      ${m()}
    `:""}function Za({imageUrl:e="",photo:a={},escapeHtml:t=null,icon:n=null}={}){const r=String(a.status||""),i=r==="uploading",d=String(a.previewUrl||e||""),c=r==="error"?String(a.error||s.photoError):"",b=d?`
      <div class="go-offer-photo__frame${i?" go-offer-photo__frame--busy":""}">
        <img class="go-offer-photo__img" src="${o(t,d)}" alt="" decoding="async" />
        ${i?`<span class="go-offer-photo__busy">${o(t,s.photoUploading)}</span>`:""}
      </div>
      <div class="go-offer-photo__actions">
        <button type="button" class="go-offer-photo__action" data-go-offer-photo-pick>${o(t,s.photoChange)}</button>
        <button type="button" class="go-offer-photo__action go-offer-photo__action--remove" data-go-offer-photo-remove>${o(t,s.photoRemove)}</button>
      </div>
    `:`
      <button type="button" class="go-offer-photo" data-go-offer-photo-pick>
        <span class="go-offer-photo__plus">${L(n,"plus","w-5 h-5")}</span>
        <span class="go-offer-photo__title">${o(t,s.photoAdd)}</span>
        <span class="go-offer-photo__sub">${o(t,s.photoSource)}</span>
      </button>
    `;return`
    <div data-go-section="photo">
      ...
      <p class="mt-1 text-[11px] font-semibold text-slate-400">
        ${o(t,s.photoHint)}
        <span class="text-slate-300">&middot; ${o(t,s.photoOptional)}</span>
      </p>
      <!--
      ...
      ${c?`<p class="mt-2 text-[11px] font-bold text-rose-500">${o(t,c)}</p>`:""}
    </div>
  `}function Va(e=""){const a=String(e||"all").trim().toLowerCase();return a==="food"?["food"]:a==="coffee"||a==="drinks"||a==="dessert"?["drinks"]:["food","drinks"]}function In(e=[]){const a=Array.isArray(e)?e:[],t=a.includes("food"),n=a.includes("drinks");return t&&n?"all":t?"food":n?"drinks":""}function Ln({editor:e=null,businessName:a="",deps:t={}}={}){if(!e)return"";const n=t.escapeHtml,r=t.icon,i=e.draft||{},d=Array.isArray(e.errors)?e.errors:[],c=v=>d.find(R=>R.field===v)?.message||"",b=Array.isArray(i.partyRanges)?i.partyRanges:[],x=i.schedule?.mode==="windows"?"windows":"always",p=Array.isArray(i.schedule?.days)&&i.schedule.days.length?i.schedule.days:I.slice(),m=re.every(v=>b.includes(v.key)),g=Array.isArray(e.intents)?e.intents:Va(i.category),w=i.benefit||{},A=e.mode==="edit",M="w-full go-offer-input bg-slate-50 border border-slate-100 rounded-2xl px-4 text-sm font-bold text-slate-900 outline-none focus:border-indigo-400",O=`mt-2 ${M}`,_='<div class="h-px bg-slate-100"></div>',B=v=>`<p class="mt-1 text-[11px] font-semibold text-slate-400">${o(n,v)}</p>`,U=Aa(i).ok&&g.length>0;return`
      ...
      <div class="modal-frame">
        ...
        <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll px-6 py-5 space-y-5" data-go-editor-scroll>
          ...
          <div class="go-offer-section" data-go-section="benefit">
            ${G(n,s.benefitQuestion)}
            ${B(s.benefitHint)}
            <div class="mt-4 grid grid-cols-2 gap-2">
              ${[[Q,s.benefitPercent],[Y,s.benefitBundle],[ae,s.benefitFree],[ne,s.benefitSpecial]].map(([v,R])=>`
            ...
            <div class="mt-5" data-go-benefit-form>
              ${Ha({benefit:w,percentCustom:e.percentCustom===!0,errorFor:c,inputClass:O,inputBase:M,escapeHtml:n})}
            </div>
          </div>
          ...
          -->
          ${Za({imageUrl:i.imageUrl||"",photo:e.photo||{},escapeHtml:n,icon:r})}
          ...
          <div data-go-section="partyRanges">
            ...
            ${c("partyRanges")?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="partyRanges">${o(n,c("partyRanges"))}</p>`:""}
          </div>

          ${_}

          <div data-go-section="category">
            ${G(n,s.categoryQuestion)}
            ${B(s.categoryHint)}
            <div class="mt-3 space-y-2">
              ${[{key:"food",label:s.ifFood},{key:"drinks",label:s.ifDrinks}].map(v=>{const R=g.includes(v.key),T=tt.find(J=>J.key===v.key)?.hint||"";return`
                  <button type="button" data-go-offer-intent="${o(n,v.key)}" aria-pressed="${R?"true":"false"}"
                    class="w-full text-left go-offer-answer px-4 py-3 rounded-2xl border transition-colors ${R?"bg-slate-900 border-slate-900 text-white":"bg-slate-50 border-slate-100 text-slate-600"}">
                    <span class="block text-xs font-black">${o(n,v.label)}</span>
                    <span class="block text-[11px] font-semibold go-offer-answer__hint">${o(n,T)}</span>
                  </button>
                `}).join("")}
            </div>
            ${c("category")?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="category">${o(n,c("category"))}</p>`:""}
          </div>

          ${_}

          <div data-go-section="schedule">
            ${G(n,s.scheduleQuestion)}
            ${B(s.scheduleHint)}
            ...
            ${x==="windows"?`
              ...
              <div class="mt-4">
                ...
                <div class="mt-2 grid grid-cols-2 gap-3">
                  <div>
                    ${G(n,s.hoursFrom,"goOfferFrom")}
                    <input id="goOfferFrom" type="time" data-go-offer-from value="${o(n,e.windowFrom||"14:00")}" class="${O}" />
                  </div>
                  <div>
                    ${G(n,s.hoursTo,"goOfferTo")}
                    <input id="goOfferTo" type="time" data-go-offer-to value="${o(n,e.windowTo||"18:00")}" class="${O}" />
                  </div>
                </div>
              </div>
            `:""}
            ${c("schedule")?`<p class="mt-2 text-[11px] font-bold text-rose-500" data-go-error="schedule">${o(n,c("schedule"))}</p>`:""}
          </div>

          ${_}

          ${La({offer:i,businessName:a,previewImageUrl:e.photo?.previewUrl||"",deps:t})}
        </div>
        ...
        <div class="px-6 pb-6 pt-4 border-t border-slate-100 bg-white modal-footer-safe">
          ...
          <button type="button" data-go-offer-save ${e.saving?"disabled":""}
            ...
            class="w-full py-4 rounded-[1.8rem] text-white font-black text-xs uppercase tracking-widest active:scale-95 transition-all go-offer-save${U?" go-offer-save--ready":""}">
  ...
  `}function Un({restaurantName:e="",tab:a="active",stats:t={},search:n={},bookings:r=[],offers:i=[],settings:d={},paused:c=!1,loading:b=!1,error:x="",deps:p={}}={}){const m=p.escapeHtml,g=p.icon,w=r.filter(_=>["accepted","activated"].includes(_.status)),A=r.filter(_=>!["accepted","activated"].includes(_.status)),M=i.filter(_=>_.status!=="archived");let O="";if(a==="offers")O=he({eyebrow:s.brand,title:s.tabs.offers,sub:`${M.length} ${M.length===1?"oferte":"oferta"}`,action:`
      ...
      `,body:M.length?`<div class="space-y-3">${M.map(_=>Ia(_,p)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(m,s.emptyTitle)}</div>`,deps:p});else if(a==="archive")O=he({eyebrow:s.brand,title:s.tabs.archive,sub:`${A.length}`,body:A.length?`<div class="space-y-3">${A.map(_=>_e(_,p)).join("")}</div>`:`<div class="text-center py-10 text-[10px] font-bold uppercase tracking-widest text-slate-300">${o(m,s.noHistory)}</div>`,deps:p});else if(a==="options"){const _=gt(d?.pausedUntil);O=he({eyebrow:s.brand,title:s.tabs.options,body:`
        <div class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
          <div>
            <p class="text-xs font-black text-slate-800">${o(m,s.goOn)}</p>
            <p class="text-[10px] font-bold text-slate-400">${o(m,c?`${s.pausedUntil} ${_}`:"ON")}</p>
          </div>
          <span class="text-[9px] font-black uppercase tracking-widest ${c?"text-amber-600":"text-emerald-600"}">
            ${o(m,c?s.paused:s.statActive)}
          </span>
        </div>
        <div class="mt-4 flex flex-wrap gap-2">
          ${c?`<button type="button" data-go-pause="0" class="go-pause px-4 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest">${o(m,s.resume)}</button>`:[{value:"30",label:"30 min"},{value:"60",label:"1 orë"},{value:"tomorrow",label:"Deri nesër"},{value:"-1",label:"Pa afat"}].map(B=>`
      ...
      `,deps:p})}else O=he({eyebrow:s.brand,title:s.tabs.active,sub:`${w.length}`,body:`
        ${Ga({code:n.code,status:n.status,busy:n.busy,deps:p})}
        ${n.booking?`
  ...
  `}const qe="mnyraDashboardStyles",Wa=`
.mnyra-dash {
...
`;function qa(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(qe)))try{const a=e.createElement("style");a.id=qe,a.textContent=Wa,e.head?.appendChild(a)}catch{}}function $(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function D(e,a,t=""){if(typeof e!="function")return"";try{return e(a,t)||""}catch{return""}}const Qa=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function Ya({businessType:e="",isShopCatalog:a=!1}={}){if(a)return"shop";const t=String(e||"").trim().toLowerCase();return Qa.includes(t)?"hotel":"restaurant"}function Ja(e=new Date().getHours()){const a=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return a>=5&&a<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:a>=11&&a<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:a>=18&&a<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function Xa({name:e="",logoUrl:a="",hour:t=new Date().getHours(),iconFn:n}={}){const r=Ja(t),i=$(e||"Business");return`
    <div class="mnyra-dash__greet">
      <p class="mnyra-dash__greet-title">
        ...
        <span class="mnyra-dash__greet-logo">
          ${a?`<img src="${$(a)}" alt="${i}" title="${i}" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:`<span class="mnyra-dash__greet-logo-fallback" title="${i}">${D(n,"store","w-4 h-4")}</span>`}
        </span>
      </p>
      <p class="mnyra-dash__greet-sub">${$(r.text)}</p>
    </div>
  `}function en({iconFn:e}={}){return`
  ...
  `}function tn({iconFn:e,showEditor:a=!0}={}){return a?`
  ...
  `:""}function an({iconFn:e,showEditor:a=!0}={}){return a?`
  ...
  `:""}const Qe=Object.freeze({restaurant:{accent:"Ndrysho",rest:"menunë",sub:"Shto produkte, kategori dhe çmime.",cta:"Menu"},shop:{accent:"Ndrysho",rest:"dyqanin",sub:"Shto produkte, kategori dhe stok.",cta:"Dyqani"},hotel:{accent:"Ndrysho",rest:"hotelin",sub:"Detajet, dhomat dhe çmimet e tua.",cta:"Hoteli"}});function nn(e="restaurant"){const a=String(e||"").trim().toLowerCase();return Qe[a]||Qe.restaurant}function rn({iconFn:e,kind:a="restaurant",showEditor:t=!0}={}){if(!t)return"";const n=nn(a);return`
    <button type="button" class="mnyra-dash__composer mnyra-dash__composer--tap mnyra-dash__composer--plane" data-dashboard-catalog-card data-nav="menu">
      <span class="mnyra-dash__composer-title"><span class="mnyra-dash__composer-accent">${$(n.accent)}</span> ${$(n.rest)}</span>
      <span class="mnyra-dash__composer-sub">${$(n.sub)}</span>
      <span class="mnyra-dash__composer-cta">
        <span class="mnyra-dash__composer-cta-icon">${D(e,"plus","w-4 h-4")}</span>
        <span class="mnyra-dash__composer-cta-label">${$(n.cta)}</span>
        <span class="mnyra-dash__composer-cta-chevron">${D(e,"chevron-right","w-4 h-4")}</span>
      </span>
    </button>
  `}const sn="/waiter?from=panel";function on({iconFn:e,showEditor:a=!0}={}){return a?`
    <a href="${sn}" class="mnyra-dash__composer mnyra-dash__composer--tap mnyra-dash__composer--waiter" data-dashboard-waiter-card>
  ...
  `:""}function dn({cards:e=[],iconFn:a}={}){const t=(Array.isArray(e)?e:[]).filter(r=>r&&r.key);if(!t.length)return"";const n=t.map((r,i)=>{const d=$(r.label||"");if(r.pending)return'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>';const c=i<2?'loading="eager" fetchpriority="high"':'loading="lazy" fetchpriority="low"';let b="";r.imageUrl?b=`<img class="mnyra-dash__hl-media" src="${$(r.imageUrl)}" alt="" ${c} decoding="async" onerror="this.style.display='none'" />`:r.videoUrl&&(b=`<video class="mnyra-dash__hl-media" src="${$(r.videoUrl)}#t=0.1" preload="metadata" muted playsinline disablepictureinpicture tabindex="-1" aria-hidden="true"></video>`);const x=`
      <span class="mnyra-dash__hl-plate">${D(a,r.iconName||"image","w-6 h-6")}</span>
      ${b}
    `,p=r.withEye?`<span class="mnyra-dash__hl-eye">${D(a,"eye","w-4 h-4")}</span>`:"";let m;r.locked?m=`<span class="mnyra-dash__hl-lock">${D(a,"lock","w-3 h-3")}Me pagesë</span>`:r.loading?m='<span class="mnyra-dash__hl-value mnyra-dash__hl-value--pending" aria-hidden="true"></span>':r.emptyText?m=`<span class="mnyra-dash__hl-empty">${$(r.emptyText)}</span>`:m=`<span class="mnyra-dash__hl-value">${p}${$(r.value||"0")}</span>`;let g;r.locked?g=`class="mnyra-dash__hl-card mnyra-dash__hl-card--locked" data-dashboard-metric-locked="${$(r.key)}"`:r.composer?g=`class="mnyra-dash__hl-card" data-dashboard-composer="${$(r.composer)}"`:g=`class="mnyra-dash__hl-card"${r.panelTab?` data-dashboard-panel-tab="${$(r.panelTab)}"`:""}`;const w=r.locked?`${d} – me pagesë`:`${d} ${r.emptyText||r.value||""}`.trim();return`
      <button type="button" ${g} data-dashboard-metric="${$(r.key)}" aria-label="${$(w)}">
        ${x}
    ...
    `}).join("");return`
    <div class="mnyra-dash__hl" data-dashboard-metrics="${$(ln(t))}">
      ${n}
      <span class="mnyra-dash__hl-tail" aria-hidden="true"></span>
    </div>
  `}function ln(e=[]){return(Array.isArray(e)?e:[]).filter(a=>a&&a.key).map(a=>[a.key,a.label||"",a.value||"",a.emptyText||"",a.imageUrl||"",a.videoUrl||"",a.iconName||"",a.panelTab||"",a.composer||"",a.pending?"p":"",a.loading?"l":"",a.locked?"x":"",a.withEye?"e":""].join("~")).join("|")}const bt=Object.freeze([Object.freeze({id:"funksionet",label:"Funksionet",iconName:"layout-grid"}),Object.freeze({id:"analitika",label:"Analitika",iconName:"bar-chart-3"}),Object.freeze({id:"opsionet",label:"Opsionet",iconName:"settings"})]);function pe(e=""){const a=String(e||"").trim().toLowerCase();return bt.some(t=>t.id===a)?a:"funksionet"}function cn({activeTab:e="funksionet",iconFn:a}={}){const t=pe(e);return`<div class="mnyra-dash__tabs" role="tablist" data-dashboard-panel-tabs>${bt.map(r=>{const i=r.id===t;return`
      <button
        type="button"
        role="tab"
        data-dashboard-panel-tab="${$(r.id)}"
        aria-selected="${i?"true":"false"}"
        class="mnyra-dash__tab"
      >${D(a,r.iconName,"w-4 h-4")}<span class="mnyra-dash__tab-label">${$(r.label)}</span></button>
    `}).join("")}</div>`}function yt(e=""){return`
    <div class="mnyra-dash__bento" data-dashboard-bento>
      ${e}
    </div>
  `}function hn({posts:e=[],iconFn:a}={}){const t=Array.isArray(e)?e:[];let n="";return t.length?(n=t.map(r=>{const i=[r.dateLabel,`${ee(r.likesCount||0)} Likes`,`${ee(r.commentsCount||0)} Kommentare`];return Number(r.impressions||0)>0&&i.push(`${ee(r.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${r.thumbUrl?`<img src="${$(r.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:D(a,r.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
          <div class="mnyra-dash__post-main">
            <p class="mnyra-dash__post-caption">${$(r.caption||"Pa tekst")}</p>
            <p class="mnyra-dash__post-meta">${$(i.filter(Boolean).join(" · "))}</p>
          </div>
        </div>
  ...
  `}function mn(){return'<div class="mnyra-dash__skeleton" style="min-height:44px; border-radius:14px; margin: 4px 0 16px;"></div>'}function fn(){const e=Array.from({length:4},()=>'<div class="mnyra-dash__hl-card mnyra-dash__hl-card--pending" aria-hidden="true"></div>').join(""),a=Array.from({length:4},(t,n)=>`<div class="mnyra-dash__skeleton" style="min-height:132px; border-radius:var(--dash-card-radius); margin-top:${n===0?32:22}px;"></div>`).join("");return`
    ${mn()}
  ...
  `}function gn({message:e=""}={}){return`
    <div class="mnyra-dash__section">
      <div class="mnyra-dash__state">
        ...
        <p class="mnyra-dash__state-body">${$(e||"Ju lutem kontrollo lidhjen dhe provo perseri.")}</p>
  ...
  `}const yn="menyra_social_dashboard_cache_v1::",Ye="menyra_social_composer_products_v1::",Je=2500,Xe=1200,_n=6,xn=3,vn=Object.freeze({menuImageUrl:"/apps/menyra-social/assets/panel/menu-scan.jpg",qrImageUrl:"/apps/menyra-social/assets/panel/qr-stand.jpg"}),wn=Object.freeze({menuOpens:"Menü-Aufrufe",qrScans:"QR-Scans"});function E(e){const a=Number(e);return Number.isFinite(a)?a:0}function kn(e={}){const a=String(e.createdAtClient||"").trim();if(a){const n=new Date(a);if(!Number.isNaN(n.getTime()))return n}const t=e.createdAt;if(t&&typeof t.toDate=="function")try{const n=t.toDate();if(n instanceof Date&&!Number.isNaN(n.getTime()))return n}catch{}return null}function $n(e="",a={}){const t=Array.isArray(a.media)&&a.media.length?a.media[0]:{},n=String(t.type||a.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",r=String(t.thumbUrl||(n==="image"?t.url:"")||a.thumbUrl||"").trim(),i=n==="video"?String(t.url||a.mediaUrl||"").trim():"",d=kn(a);return{id:String(e||"").trim(),caption:String(a.caption||"").trim(),mediaType:n,thumbUrl:r,videoUrl:i,likesCount:E(a.likesCount),commentsCount:E(a.commentsCount),impressions:0,dateLabel:d?d.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:d?d.getTime():0}}function Sn({days:e=[],todayKey:a="",rawPosts:t=[]}={}){const n=Array.isArray(e)?e:[],r=Be(n),i=n.find(p=>String(p?.date||p?.id||"").trim()===String(a||"").trim()),d=Be(i?[i]:[]),c=r.merged?.posts&&typeof r.merged.posts=="object"?r.merged.posts:{},b=(Array.isArray(t)?t:[]).map(p=>$n(p?.id,p?.data||{})).filter(p=>p.id).map(p=>({...p,impressions:E(c[p.id]?.impressions)})),x=b.slice().sort((p,m)=>m.createdAtMs-p.createdAtMs).slice(0,xn);return{day:String(a||"").trim(),week:r.summary,today:d.summary,posts:x,latestPost:zn(b)}}function zn(e=[]){const a=(Array.isArray(e)?e:[]).filter(t=>t&&t.id);return a.length?a.slice().sort((t,n)=>E(n.createdAtMs)-E(t.createdAtMs)||E(n.impressions)-E(t.impressions)||E(n.likesCount)-E(t.likesCount))[0]:null}function An({profile:e={},restaurant:a={}}={}){return Tt({profile:e,restaurant:a,feature:"qr"})}function Pn(e={}){const a=e&&typeof e=="object"?e:{};return String(a.titleImageUrl||a.coverImageUrl||a.coverUrl||a.heroUrl||a.bannerUrl||"").trim()}function Dn({model:e=null,coverUrl:a="",subscribed:t=!1,assets:n={}}={}){const r=e?.today||{},i=!e,d=e?.latestPost||null,c=[];if(i)c.push({key:"latestPost",label:"Postimi fundit",pending:!0});else if(!d)c.push({key:"latestPost",label:"Postimi fundit",emptyText:"S'ka postim",iconName:"image",composer:"post"});else{const b=String(d.thumbUrl||"").trim();c.push({key:"latestPost",label:"Postimi fundit",value:ee(E(d.impressions)),withEye:!0,imageUrl:b,videoUrl:b?"":String(d.videoUrl||"").trim(),iconName:"image",panelTab:"analitika"})}return c.push({key:"profileViews",label:"Vizitor n'profil",value:ee(E(r.profileViews)),withEye:!0,loading:i,imageUrl:String(a||"").trim(),iconName:"user",panelTab:"analitika"}),c.push({key:"menuOpens",label:"Vizitor n'meny",value:ee(E(r.menuOpens)),withEye:!0,loading:i&&t,locked:!t,imageUrl:String(n.menuImageUrl||"").trim(),iconName:"book-open",panelTab:"analitika"}),c.push({key:"qrScans",label:"Skanime n'tavolina",value:ee(E(r.qrScans)),withEye:!0,loading:i&&t,locked:!t,imageUrl:String(n.qrImageUrl||"").trim(),iconName:"layout-grid",panelTab:"analitika"}),c}function Zn({state:e,renderFn:a,documentObj:t,firestoreApi:n={},profileApi:r={},composerApi:i={},viewApi:d={},iconFn:c,storageObj:b}={}){const x=t||(typeof document>"u"?null:document),p=x?.defaultView||(typeof window>"u"?null:window),m=typeof a=="function"?a:()=>{},g=b||(typeof localStorage>"u"?null:localStorage),w=typeof r.getBusinessProfileTypeFn=="function"?r.getBusinessProfileTypeFn:(()=>""),A=typeof r.isShopCatalogProfileFn=="function"?r.isShopCatalogProfileFn:(()=>!1),M=typeof r.getRestaurantMetaByIdFn=="function"?r.getRestaurantMetaByIdFn:(()=>null),O=typeof r.resolveRestaurantLogoFn=="function"?r.resolveRestaurantLogoFn:(()=>""),_=typeof r.resolveOwnAvatarUrlFn=="function"?r.resolveOwnAvatarUrlFn:(()=>""),B=typeof d.renderAnalyticsViewFn=="function"?d.renderAnalyticsViewFn:(()=>""),U=typeof d.renderSettingsViewFn=="function"?d.renderSettingsViewFn:(()=>""),v=typeof d.warmAnalyticsFn=="function"?d.warmAnalyticsFn:(()=>{});let R=!1,T=0,J=!1,N=null,H=null,Z="",V=!1,je=()=>null;const _t=300;function ge(){const l=e?.userProfile||{};return Ya({businessType:w(l),isShopCatalog:A(l)})}function xt(l=""){const h=M(l)||{};return qt(h).map(u=>({id:u.id,name:u.title,price:u.price??"",category:u.beds||u.tag||"",type:"room",imageUrl:u.imageUrl||""}))}function vt(l=""){if(!g)return null;try{const h=g.getItem(`${Ye}${l}`);if(!h)return null;const u=JSON.parse(h),f=Array.isArray(u?.items)?u.items:null;return f&&f.length?f:null}catch{return null}}function wt(l="",h=[]){if(g)try{g.setItem(`${Ye}${l}`,JSON.stringify({savedAt:Date.now(),items:h}))}catch{}}async function kt(l=""){const{db:h,collectionFn:u,queryFn:f,limitFn:k,getDocsFn:z}=n;if(!h||typeof u!="function"||typeof z!="function")throw new Error("Produktet nuk u ngarkuan.");const W=u(h,"restaurants",l,"menuItems"),j=typeof f=="function"&&typeof k=="function"?f(W,k(_t)):W,q=await z(j),P=[];return q.forEach(X=>{const le=je(X?.id,X?.data?.()||{});le&&P.push(le)}),P.sort((X,le)=>X.name.localeCompare(le.name,"sq")),P}async function $t(l="",h){const u=String(l||"").trim();if(!u)throw new Error("Produktet nuk u ngarkuan.");if(ge()==="hotel")return xt(u);const f=kt(u).then(z=>(wt(u,z),z)),k=vt(u);return k?(typeof h=="function"?f.then(z=>h(z)).catch(()=>{}):f.catch(()=>{}),k):f}function Fe(){return N?Promise.resolve(N):(H||(H=Et(()=>import("./business-composer-controller-D6zMGtWy.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(l=>(je=typeof l?.normalizeComposerProductCore=="function"?l.normalizeComposerProductCore:(()=>null),N=l.createBusinessComposerController({documentObj:x,windowObj:x?.defaultView||null,api:{getRestaurantIdFn:()=>oe(),getBusinessMetaFn:()=>{const h=oe();if(!h)return{name:"",logoUrl:"",city:""};const u=Ee(h),f=M(h)||{};return{name:u.name,logoUrl:u.logoUrl,city:String(f.city||"").trim()}},loadProductsFn:(h,u)=>$t(h,u),getBusinessKindFn:()=>ge(),uploadImageFn:i.uploadImageFn,uploadVideoFn:i.uploadVideoFn,captureVideoPosterFn:i.captureVideoPosterFn,createPostFn:i.createPostFn,createStoryFn:i.createStoryFn,formatPriceFn:i.formatPriceFn,getOptimizedImageUrlFn:i.getOptimizedImageUrlFn,escapeHtmlFn:i.escapeHtmlFn,iconFn:typeof c=="function"?c:void 0,afterPublishFn:async h=>{try{await de({force:!0})}catch{}typeof i.afterPublishFn=="function"&&await i.afterPublishFn(h)}}}),N)).catch(l=>{throw H=null,console.error("[mnyra][dashboard] composer load failed",l),l})),H)}function Ce(){const l=p?.navigator?.connection;return!l||typeof l!="object"?!1:l.saveData===!0?!0:/(^|-)2g$/.test(String(l.effectiveType||"").trim().toLowerCase())}function St(){if(V||N||!p||Ce())return;V=!0;const l=()=>{if(Fe().catch(()=>{}),typeof i.prewarmFn=="function")try{i.prewarmFn()}catch{}};if(typeof p.requestIdleCallback=="function"){p.requestIdleCallback(l,{timeout:Je});return}p.setTimeout?.(l,Xe)}function zt(){if(R||!p||Ce())return;R=!0;const l=()=>{try{v()}catch{}};if(typeof p.requestIdleCallback=="function"){p.requestIdleCallback(l,{timeout:Je});return}p.setTimeout?.(l,Xe)}function At(l="post"){const h=String(l||"").trim().toLowerCase(),u=h==="story"||h==="profile"?h:"post";if(typeof i.prewarmFn=="function")try{i.prewarmFn()}catch{}if(N){N.open(u);return}Z=u,Fe().then(f=>{const k=Z||u;Z="",f?.open?.(k)}).catch(()=>{Z=""})}function be(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:"",restaurantId:"",paywall:""}),e.dashboardView}function Me(l=""){const h=be(),u=String(l||"").trim();return String(h.restaurantId||"")===u||(h.restaurantId=u,h.model=null,h.status="idle",h.error="",h.loadedSignature="",h.paywall="",T+=1),h}function oe(){const l=e?.userProfile||{};return String(l.restaurantId||l.staffRestaurantId||"").trim()}let Oe="";function Pt(){const l=String(e?.user?.uid||"").trim();!l||Oe===l||typeof r.ensureBusinessProfileFn=="function"&&(Oe=l,Promise.resolve().then(()=>r.ensureBusinessProfileFn()).catch(h=>{console.warn("[mnyra][panel] business profile could not be resolved",h)}).finally(()=>{String(e?.user?.uid||"").trim()===l&&m()}))}function Dt(){const l=String(e?.user?.uid||"").trim();if(!l)return!1;const h=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||h===l}function Re(l=""){return`${yn}${l}`}function jt(l="",h=""){if(!g||!l)return null;try{const u=g.getItem(Re(l));if(!u)return null;const f=JSON.parse(u);return!f||typeof f!="object"||String(f.day||"").trim()!==String(h||"").trim()||!f.model||typeof f.model!="object"?null:f.model}catch{return null}}function Ft(l="",h=null){if(!(!g||!l||!h))try{g.setItem(Re(l),JSON.stringify({day:h.day,model:h}))}catch{}}async function Ct(l=""){const{db:h,collectionFn:u,queryFn:f,orderByFn:k,limitFn:z,getDocsFn:W}=n;if(!h||typeof u!="function"||typeof f!="function"||typeof k!="function"||typeof z!="function"||typeof W!="function")return[];const j=u(h,"restaurants",l,"socialPosts");return(await W(f(j,k("createdAt","desc"),z(_n)))).docs.map(P=>({id:P.id,data:P.data()||{}})).filter(P=>{const X=String(P.data.status||"active").trim().toLowerCase();return X!=="deleted"&&X!=="hidden"})}async function de({force:l=!1}={}){const h=oe(),u=Me(h);if(!h)return;const f=Bt({rangeKey:"7d"});if(!f)return;const k=`${h}::${f.toDay}`;if(!l&&u.loadedSignature===k&&u.status==="ready")return;if(!u.model){const j=jt(h,f.toDay);j&&(u.model=j,u.status="ready",m())}T+=1;const z=T;u.model||(u.status="loading",u.error="",m());try{const j={db:n.db,collectionFn:n.collectionFn,queryFn:n.queryFn,whereFn:n.whereFn,documentIdFn:n.documentIdFn,getDocsFn:n.getDocsFn,restaurantId:h},[q,P]=await Promise.allSettled([Kt({...j,fromDay:f.fromDay,toDay:f.toDay}),Ct(h)]);if(z!==T)return;if(q.status==="rejected")throw q.reason;P.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",P.reason),u.model=Sn({days:q.value,todayKey:f.toDay,rawPosts:P.status==="fulfilled"?P.value:[]}),u.status="ready",u.error="",u.loadedSignature=k,Ft(h,u.model)}catch(j){if(z!==T)return;console.error("[mnyra][dashboard] load failed",j),u.model||(u.status="error",u.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}m()}function Mt(){J||!x||(J=!0,x.addEventListener("click",l=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(l.target?.closest?.("[data-dashboard-retry]")){de({force:!0});return}if(l.target?.closest?.("[data-dashboard-paywall-close]")){l.preventDefault(),be().paywall="",m();return}const h=l.target?.closest?.("[data-dashboard-metric-locked]");if(h){l.preventDefault(),be().paywall=String(h.getAttribute("data-dashboard-metric-locked")||"").trim(),m();return}const u=l.target?.closest?.("[data-dashboard-composer]");if(u){l.preventDefault(),At(u.getAttribute("data-dashboard-composer"));return}const f=l.target?.closest?.("[data-dashboard-panel-tab]");if(f){l.preventDefault();const k=pe(f.getAttribute("data-dashboard-panel-tab"));if(k===pe(e?.dashboardPanelTab))return;e.dashboardPanelTab=k,m()}}catch{}}))}function Ee(l=""){const h=e?.userProfile||{},u=l?M(l)||{}:{},f=String(u.name||u.restaurantName||h.name||"").trim()||"Business";let k="";try{k=String(_()||"").trim()}catch{}if(!k)try{k=String(O(u)||"").trim()}catch{}return{name:f,logoUrl:k,kind:ge(),coverUrl:Pn(u),subscribed:An({profile:h,restaurant:u})}}function Ot(l=""){try{if(!Nt()||!l)return"";Gt({restaurantId:l,onBadgeFn:()=>m()});const h=It();return Ea({enabled:!0,unseenCount:h.unseen,activeOffers:h.activeOffers||0,todayBookings:h.today,iconFn:c})}catch{return""}}function Rt(){qa(x),Mt();const l=oe(),h=Me(l);let u="";if(!l)Pt(),u=Dt()?fn():bn();else{St(),zt();const f=Ee(l),k=pe(e?.dashboardPanelTab);h.status==="idle"&&(h.status="loading",queueMicrotask(()=>{de({force:!1})}));let z="";h.model?z=hn({posts:h.model.posts,iconFn:c}):h.status==="error"?z=gn({message:h.error}):z=un();const W=`
        ${Ot(l)}
        ${en({iconFn:c})}
        ${on({iconFn:c,showEditor:!!l})}
        ${tn({iconFn:c,showEditor:!!l})}
        ${an({iconFn:c,showEditor:!!l})}
        ${rn({iconFn:c,kind:f.kind,showEditor:!!l})}
      `;let j;k==="analitika"?j=`
          <div class="mnyra-dash__embed">${B()}</div>
          ${z}
        `:k==="opsionet"?j=`<div class="mnyra-dash__embed">${U()}</div>`:j=W;const q=Dn({model:h.model,coverUrl:f.coverUrl,subscribed:f.subscribed,assets:vn}),P=String(h.paywall||"").trim();u=`
        ${Xa({name:f.name,logoUrl:f.logoUrl,iconFn:c})}
        ${dn({cards:q,iconFn:c})}
        ${yt(`
          ${cn({activeTab:k,iconFn:c})}
          ${j}
        `)}
        ${P?pn({title:wn[P]||"Me pagesë"}):""}
      `}return`
      <section class="mnyra-dash" data-dashboard-root>
        ${u}
      </section>
    `}return Object.freeze({renderDashboardView:Rt,loadDashboard:de})}export{Ca as A,Gn as B,qt as C,On as D,Rn as E,re as G,Ut as M,Zn as a,fe as b,Ht as c,Va as d,Un as e,ke as f,Kn as g,I as h,In as i,Ln as j,La as k,Ma as l,Nn as m,Wt as n,Fa as o,ga as p,Bn as q,Hn as r,Ke as s,Tn as t,En as u,Aa as v,ce as w,tt as x,mt as y,De as z};
