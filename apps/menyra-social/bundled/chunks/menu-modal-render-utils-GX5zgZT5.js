  ...
  `,pe=`
    <div class="flex-1 overflow-y-auto no-scrollbar modal-scroll px-6 py-5 space-y-4">
      ...
      <div class="relative rounded-[2.5rem] overflow-hidden border border-slate-100 bg-slate-50">
        ${Ce?`
          <video id="menuItemHeroVideo" src="${r(_e)}" ${Ae?`poster="${r(Ae)}"`:""} class="w-full h-52 object-cover" style="object-position:${w.x}% ${w.y}%;" muted loop playsinline autoplay preload="metadata"></video>
