import{f as x,r as G,l as Q,s as j}from"./domain-analytics-BLDCuXpL.js";const R="mnyraDashboardStyles",Y=`
.mnyra-dash {
  --dash-surface: #ffffff;
  --dash-plane: #f8fafc;
  --dash-ink: #0f172a;
...
`;function Z(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(R)))try{const t=e.createElement("style");t.id=R,t.textContent=Y,e.head?.appendChild(t)}catch{}}function u(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function P(e,t,o=""){if(typeof e!="function")return"";try{return e(t,o)||""}catch{return""}}const W=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function X({businessType:e="",isShopCatalog:t=!1}={}){if(t)return"shop";const o=String(e||"").trim().toLowerCase();return W.includes(o)?"hotel":"restaurant"}function ee({kind:e="restaurant",isOwner:t=!1,canAccessOrders:o=!1}={}){const a=[{nav:"upload",uploadIntent:"chooser",iconName:"plus",label:"Neuer Beitrag",sub:"Foto oder Video posten"},{nav:"upload",uploadIntent:"story",iconName:"camera",label:"Story",sub:"24h sichtbar"}];return e==="hotel"?a.push({nav:"menu",iconName:"bed-double",label:"Hotel & Zimmer",sub:"Details, Zimmer, Angebote"}):e==="shop"?a.push({nav:"menu",iconName:"shopping-bag",label:"Shop bearbeiten",sub:"Produkte & Lager"}):a.push({nav:"menu",iconName:"utensils",label:"Menü bearbeiten",sub:"Produkte & Kategorien"}),a.push({nav:"menu",iconName:"megaphone",label:"Angebote & Werbung",sub:"Im Editor verwalten"}),e!=="hotel"&&o&&a.push({nav:"orders",iconName:"shopping-cart",label:"Bestellungen",sub:"Eingang & Status"}),a.push({nav:"analytics",iconName:"bar-chart-3",label:"Analytics",sub:"Alle Statistiken"}),t&&a.push({nav:"businessAccounts",iconName:"users-round",label:"Team & Staff",sub:"Zugänge verwalten"}),a.push({nav:"settings",iconName:"settings",label:"Einstellungen",sub:"Profil & Kontakt"}),a}function te(e="restaurant"){const t=[{key:"profileViews",label:"Profilaufrufe"},{key:"postImpressions",label:"Beitrags-Reichweite"},{key:"contactClicks",label:"Kontakt-Klicks"}];return e==="shop"?t.concat([{key:"ordersCompleted",label:"Bestellungen"},{key:"revenue",label:"Umsatz",unit:"€"},{key:"productViews",label:"Produkt-Aufrufe"}]):e==="hotel"?t.concat([{key:"uniqueVisitors",label:"Besucher"},{key:"postLikes",label:"Likes"},{key:"feedImpressions",label:"Feed-Reichweite"}]):t.concat([{key:"ordersCompleted",label:"Bestellungen"},{key:"revenue",label:"Umsatz",unit:"€"},{key:"qrScans",label:"QR-Scans"}])}function T(e=0,t=""){const o=x(e);return t?`${o} ${t}`:o}function ae(e=new Date().getHours()){const t=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return t>=5&&t<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:t>=11&&t<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:t>=18&&t<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function ne({name:e="",logoUrl:t="",hour:o=new Date().getHours(),iconFn:a}={}){const n=ae(o);return`
  ...
  `}function se({actions:e=[],iconFn:t}={}){return`
  ...
  `}function re({kpiDefs:e=[],week:t={},today:o={}}={}){return`
    ...
    <div class="mnyra-dash__kpi">
      ...
      <p class="mnyra-dash__kpi-value">${u(T(t?.[n.key]||0,n.unit||""))}</p>
      <p class="mnyra-dash__kpi-today">Heute: ${u(T(o?.[n.key]||0,n.unit||""))}</p>
    </div>
  `).join("")}</div>
    </div>
  `}function oe({posts:e=[],iconFn:t}={}){const o=Array.isArray(e)?e:[];let a="";return o.length?(a=o.map(n=>{const c=[n.dateLabel,`${x(n.likesCount||0)} Likes`,`${x(n.commentsCount||0)} Kommentare`];return Number(n.impressions||0)>0&&c.push(`${x(n.impressions)} Reichweite (7 T.)`),`
  ...
  `}function ie(){return'<div class="mnyra-dash__skeleton" style="min-height:56px; border-radius:18px; margin: 4px 2px 16px;"></div>'}function de({message:e=""}={}){return`
    <div class="mnyra-dash__section">
  ...
  `}const ce="menyra_social_dashboard_cache_v1::",ue=6,me=3;function D(e){const t=Number(e);return Number.isFinite(t)?t:0}function he(e={}){const t=String(e.createdAtClient||"").trim();if(t){const a=new Date(t);if(!Number.isNaN(a.getTime()))return a}const o=e.createdAt;if(o&&typeof o.toDate=="function")try{const a=o.toDate();if(a instanceof Date&&!Number.isNaN(a.getTime()))return a}catch{}return null}function pe(e="",t={}){const o=Array.isArray(t.media)&&t.media.length?t.media[0]:{},a=String(o.type||t.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",n=String(o.thumbUrl||(a==="image"?o.url:"")||t.thumbUrl||"").trim(),c=he(t);return{id:String(e||"").trim(),caption:String(t.caption||"").trim(),mediaType:a,thumbUrl:n,likesCount:D(t.likesCount),commentsCount:D(t.commentsCount),impressions:0,dateLabel:c?c.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:c?c.getTime():0}}function ye({days:e=[],todayKey:t="",rawPosts:o=[]}={}){const a=Array.isArray(e)?e:[],n=j(a),c=a.find(l=>String(l?.date||l?.id||"").trim()===String(t||"").trim()),k=j(c?[c]:[]),g=n.merged?.posts&&typeof n.merged.posts=="object"?n.merged.posts:{},b=(Array.isArray(o)?o:[]).map(l=>pe(l?.id,l?.data||{})).filter(l=>l.id).sort((l,w)=>w.createdAtMs-l.createdAtMs).slice(0,me).map(l=>({...l,impressions:D(g[l.id]?.impressions)}));return{day:String(t||"").trim(),week:n.summary,today:k.summary,posts:b}}function ge({state:e,renderFn:t,documentObj:o,firestoreApi:a={},profileApi:n={},iconFn:c,storageObj:k}={}){const g=o||(typeof document>"u"?null:document),b=typeof t=="function"?t:()=>{},l=k||(typeof localStorage>"u"?null:localStorage),w=typeof n.getBusinessProfileTypeFn=="function"?n.getBusinessProfileTypeFn:(()=>""),I=typeof n.isShopCatalogProfileFn=="function"?n.isShopCatalogProfileFn:(()=>!1),A=typeof n.isBusinessOwnerProfileFn=="function"?n.isBusinessOwnerProfileFn:(()=>!1),L=typeof n.canAccessRestaurantOrdersFn=="function"?n.canAccessRestaurantOrdersFn:(()=>!1),E=typeof n.getRestaurantMetaByIdFn=="function"?n.getRestaurantMetaByIdFn:(()=>null),M=typeof n.resolveRestaurantLogoFn=="function"?n.resolveRestaurantLogoFn:(()=>"");let _=0,$=!1;function C(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:""}),e.dashboardView}function B(){const r=e?.userProfile||{};return String(r.restaurantId||r.staffRestaurantId||"").trim()}function O(){const r=String(e?.user?.uid||"").trim();if(!r)return!1;const s=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||s===r}function F(r=""){return`${ce}${r}`}function K(r="",s=""){if(!l||!r)return null;try{const d=l.getItem(F(r));if(!d)return null;const i=JSON.parse(d);return!i||typeof i!="object"||String(i.day||"").trim()!==String(s||"").trim()||!i.model||typeof i.model!="object"?null:i.model}catch{return null}}function V(r="",s=null){if(!(!l||!r||!s))try{l.setItem(F(r),JSON.stringify({day:s.day,model:s}))}catch{}}async function H(r=""){const{db:s,collectionFn:d,queryFn:i,orderByFn:h,limitFn:m,getDocsFn:y}=a;if(!s||typeof d!="function"||typeof i!="function"||typeof h!="function"||typeof m!="function"||typeof y!="function")return[];const f=d(s,"restaurants",r,"socialPosts");return(await y(i(f,h("createdAt","desc"),m(ue)))).docs.map(p=>({id:p.id,data:p.data()||{}})).filter(p=>{const N=String(p.data.status||"active").trim().toLowerCase();return N!=="deleted"&&N!=="hidden"})}async function S({force:r=!1}={}){const s=C(),d=B();if(!d)return;const i=G({rangeKey:"7d"});if(!i)return;const h=`${d}::${i.toDay}`;if(!r&&s.loadedSignature===h&&s.status==="ready")return;if(!s.model){const f=K(d,i.toDay);f&&(s.model=f,s.status="ready",b())}_+=1;const m=_;s.model||(s.status="loading",s.error="",b());try{const f={db:a.db,collectionFn:a.collectionFn,queryFn:a.queryFn,whereFn:a.whereFn,documentIdFn:a.documentIdFn,getDocsFn:a.getDocsFn,restaurantId:d},[v,p]=await Promise.allSettled([Q({...f,fromDay:i.fromDay,toDay:i.toDay}),H(d)]);if(m!==_)return;if(v.status==="rejected")throw v.reason;p.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",p.reason),s.model=ye({days:v.value,todayKey:i.toDay,rawPosts:p.status==="fulfilled"?p.value:[]}),s.status="ready",s.error="",s.loadedSignature=h,V(d,s.model)}catch(f){if(m!==_)return;console.error("[mnyra][dashboard] load failed",f),s.model||(s.status="error",s.error="Bitte prüfe deine Verbindung und versuche es erneut.")}b()}function U(){$||!g||($=!0,g.addEventListener("click",r=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;r.target?.closest?.("[data-dashboard-retry]")&&S({force:!0})}catch{}}))}function q(r=""){const s=e?.userProfile||{},d=r?E(r)||{}:{},i=w(s),h=String(d.name||d.restaurantName||s.name||"").trim()||"Business";let m="";try{m=String(M(d)||"").trim()}catch{}return m||(m=String(s.avatar||"").trim()),{name:h,logoUrl:m,kind:X({businessType:i,isShopCatalog:I(s)})}}function J(){Z(g),U();const r=C(),s=B();let d="";if(!s)d=O()?`${ie()}${z({kpiCount:6})}`:le();else{const i=q(s),h=ee({kind:i.kind,isOwner:A(e?.userProfile),canAccessOrders:L(e?.userProfile)}),m=te(i.kind);r.status==="idle"&&(r.status="loading",queueMicrotask(()=>{S({force:!1})}));let y="";r.model?y=`
          ${re({kpiDefs:m,week:r.model.week,today:r.model.today})}
          ${oe({posts:r.model.posts,iconFn:c})}
        `:r.status==="error"?y=de({message:r.error}):y=z({kpiCount:m.length}),d=`
        ${ne({name:i.name,logoUrl:i.logoUrl,iconFn:c})}
        ${se({actions:h,iconFn:c})}
        ${y}
      `}return`
      <section class="p-4 pb-28 mnyra-dash" data-dashboard-root>
        ${d}
      </section>
    `}return Object.freeze({renderDashboardView:J,loadDashboard:S})}export{ge as c};
