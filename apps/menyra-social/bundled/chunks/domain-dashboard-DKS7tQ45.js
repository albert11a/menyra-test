const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["chunks/business-composer-controller-9aWMxrys.js","chunks/domain-feed-social-eager-CKjKgOnA.js","chunks/domain-auth-Aq-4Vdvh.js","chunks/domain-public-profile-mLQti0eH.js","chunks/domain-media-eager-DAUyCk2O.js","chunks/domain-menu-eager-Cn6TTvVS.js","chunks/profile-post-card-markup-utils-HwqIiXgP.js"])))=>i.map(i=>d[i]);
import{_ as ye}from"./domain-auth-Aq-4Vdvh.js";import{f as I,r as be,l as _e,s as V}from"./domain-analytics-jv5B-kA2.js";const ve=20,ke=8;function g(e=""){return e==null?"":String(e).trim()}function N(e){if(e==null||e==="")return null;const t=Number(String(e).replace(",","."));return Number.isFinite(t)&&t>0?t:null}function xe(e=Date.now(),t=Math.random()){const n=Math.max(0,Number(e)||0).toString(36),s=Math.floor(Math.max(0,Math.min(.999999,Number(t)||0))*36**6).toString(36).padStart(6,"0");return`room_${n}_${s}`}function we(e={}){const t=e&&typeof e=="object"?e:{},n=[...Array.isArray(t.images)?t.images:[],g(t.imageUrl??t.image??t.photoUrl)],s=[];return n.forEach(o=>{const l=g(o);l&&!s.includes(l)&&s.push(l)}),s.slice(0,ke)}function Se(e={},{index:t=0}={}){const n=e&&typeof e=="object"?e:{},s=N(n.persons??n.guests??n.capacity),o=N(n.size??n.sizeSqm??n.area),l=we(n);return{id:g(n.id)||xe(Date.now()+t),title:g(n.title??n.name),description:g(n.description??n.text).slice(0,400),imageUrl:l[0]||"",images:l,price:N(n.price??n.pricePerNight),currency:g(n.currency??n.currencyCode).toUpperCase()||"EUR",persons:s==null?null:Math.min(20,Math.round(s)),beds:g(n.beds??n.bedsLabel).slice(0,60),size:o==null?null:Math.min(500,Math.round(o)),tag:g(n.tag??n.badge).slice(0,40),active:n.active!==!1}}function Pe(e=[]){return(Array.isArray(e)?e:[]).slice(0,ve).map((t,n)=>Se(t,{index:n}))}function Fe(e={}){return Pe((e&&typeof e=="object"?e:{}).hotelRooms).filter(n=>n.title)}function Ze(e={}){const t=[];return Number.isFinite(e?.persons)&&e.persons>0&&t.push({icon:"users",label:`${e.persons} persona`}),g(e?.beds)&&t.push({icon:"bed",label:g(e.beds)}),Number.isFinite(e?.size)&&e.size>0&&t.push({icon:"size",label:`${e.size} m²`}),t}function et(e={}){const t=Number(e?.price);if(!Number.isFinite(t)||t<=0)return"";const n=g(e?.currency).toUpperCase()||"EUR",s=Number.isInteger(t)?String(t):t.toFixed(2);return n==="EUR"?`€${s}`:`${s} ${n}`}const q="mnyraDashboardStyles",ze=`
.mnyra-dash {
...
`;function Ce(e=typeof document>"u"?null:document){if(!(!e||e.getElementById(q)))try{const t=e.createElement("style");t.id=q,t.textContent=ze,e.head?.appendChild(t)}catch{}}function m(e=""){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function P(e,t,n=""){if(typeof e!="function")return"";try{return e(t,n)||""}catch{return""}}const De=Object.freeze(["hotel","motel","hostel","resort","accommodation","travel"]);function Re({businessType:e="",isShopCatalog:t=!1}={}){if(t)return"shop";const n=String(e||"").trim().toLowerCase();return De.includes(n)?"hotel":"restaurant"}function je({kind:e="restaurant",isOwner:t=!1,canAccessOrders:n=!1}={}){const s=[{nav:"upload",uploadIntent:"chooser",iconName:"plus",label:"Neuer Beitrag",sub:"Posto foto ose video"},{nav:"upload",uploadIntent:"story",iconName:"camera",label:"Story",sub:"E dukshme 24h"}];return e==="hotel"?s.push({nav:"menu",iconName:"bed-double",label:"Hotel & Dhoma",sub:"Detaje, dhoma, oferta"}):e==="shop"?s.push({nav:"menu",iconName:"shopping-bag",label:"Ndrysho dyqanin",sub:"Produkte & Stok"}):s.push({nav:"menu",iconName:"utensils",label:"Ndrysho menune",sub:"Produkte & Kategorien"}),s.push({nav:"menu",iconName:"megaphone",label:"Oferta & Reklama",sub:"Im Editor verwalten"}),e!=="hotel"&&n&&s.push({nav:"orders",iconName:"shopping-cart",label:"Porosite",sub:"Hyrje & Status"}),s.push({nav:"analytics",iconName:"bar-chart-3",label:"Analytics",sub:"Te gjitha statistikat"}),t&&s.push({nav:"businessAccounts",iconName:"users-round",label:"Team & Staff",sub:"Zugänge verwalten"}),s.push({nav:"settings",iconName:"settings",label:"Cilesimet",sub:"Profili & Kontakti"}),s}function Ie(e="restaurant"){const t=[{key:"profileViews",label:"Profilaufrufe"},{key:"postImpressions",label:"Shtrirja e postimeve"},{key:"contactClicks",label:"Kontakt-Klicks"}];return e==="shop"?t.concat([{key:"ordersCompleted",label:"Porosite"},{key:"revenue",label:"Umsatz",unit:"€"},{key:"productViews",label:"Produkt-Aufrufe"}]):e==="hotel"?t.concat([{key:"uniqueVisitors",label:"Vizitore"},{key:"postLikes",label:"Likes"},{key:"feedImpressions",label:"Shtrirja ne feed"}]):t.concat([{key:"ordersCompleted",label:"Porosite"},{key:"revenue",label:"Umsatz",unit:"€"},{key:"qrScans",label:"QR-Scans"}])}function G(e=0,t=""){const n=I(e);return t?`${n} ${t}`:n}function $e(e=new Date().getHours()){const t=Number.isFinite(Number(e))?(Math.trunc(Number(e))%24+24)%24:12;return t>=5&&t<=10?{dayPart:"mengjes",text:"Ju urojmë një mëngjes të mbarë!"}:t>=11&&t<=17?{dayPart:"dite",text:"Ju urojmë një ditë të mbarë!"}:t>=18&&t<=21?{dayPart:"mbremje",text:"Ju urojmë një mbrëmje të mbarë!"}:{dayPart:"nate",text:"Ju urojmë një natë të mbarë!"}}function Be({name:e="",logoUrl:t="",hour:n=new Date().getHours(),iconFn:s}={}){const o=$e(n);return`
  ...
  `}function Ne({actions:e=[],iconFn:t}={}){return`<div class="mnyra-dash__actions">${(Array.isArray(e)?e:[]).map(s=>{const o=s.uploadIntent?` data-upload-intent="${m(s.uploadIntent)}"`:"";return`
      <button type="button" class="mnyra-dash__action" data-nav="${m(s.nav)}"${o}>
        <span class="mnyra-dash__action-icon">${P(t,s.iconName,"w-4 h-4")}</span>
        <span>
  ...
  `}function Ke({kpiDefs:e=[],week:t={},today:n={}}={}){return`
  ...
  `}function Me({posts:e=[],iconFn:t}={}){const n=Array.isArray(e)?e:[];let s="";return n.length?(s=n.map(o=>{const l=[o.dateLabel,`${I(o.likesCount||0)} Likes`,`${I(o.commentsCount||0)} Kommentare`];return Number(o.impressions||0)>0&&l.push(`${I(o.impressions)} shtrirje (7 dite)`),`
        <div class="mnyra-dash__post">
          <div class="mnyra-dash__post-thumb">
            ${o.thumbUrl?`<img src="${m(o.thumbUrl)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'" />`:P(t,o.mediaType==="video"?"play":"image","w-5 h-5")}
          </div>
  ...
  `}function Oe(){return'<div class="mnyra-dash__skeleton" style="min-height:44px; border-radius:14px; margin: 4px 0 16px;"></div>'}function Le({message:e=""}={}){return`
  ...
  `}const Ue="menyra_social_dashboard_cache_v1::",Q="menyra_social_composer_products_v1::",Ae=2500,Ve=1200,qe=6,Ge=3;function T(e){const t=Number(e);return Number.isFinite(t)?t:0}function Je(e={}){const t=String(e.createdAtClient||"").trim();if(t){const s=new Date(t);if(!Number.isNaN(s.getTime()))return s}const n=e.createdAt;if(n&&typeof n.toDate=="function")try{const s=n.toDate();if(s instanceof Date&&!Number.isNaN(s.getTime()))return s}catch{}return null}function Qe(e="",t={}){const n=Array.isArray(t.media)&&t.media.length?t.media[0]:{},s=String(n.type||t.mediaType||"image").trim().toLowerCase()==="video"?"video":"image",o=String(n.thumbUrl||(s==="image"?n.url:"")||t.thumbUrl||"").trim(),l=Je(t);return{id:String(e||"").trim(),caption:String(t.caption||"").trim(),mediaType:s,thumbUrl:o,likesCount:T(t.likesCount),commentsCount:T(t.commentsCount),impressions:0,dateLabel:l?l.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"",createdAtMs:l?l.getTime():0}}function We({days:e=[],todayKey:t="",rawPosts:n=[]}={}){const s=Array.isArray(e)?e:[],o=V(s),l=s.find(u=>String(u?.date||u?.id||"").trim()===String(t||"").trim()),b=V(l?[l]:[]),$=o.merged?.posts&&typeof o.merged.posts=="object"?o.merged.posts:{},_=(Array.isArray(n)?n:[]).map(u=>Qe(u?.id,u?.data||{})).filter(u=>u.id).sort((u,S)=>S.createdAtMs-u.createdAtMs).slice(0,Ge).map(u=>({...u,impressions:T($[u.id]?.impressions)}));return{day:String(t||"").trim(),week:o.summary,today:b.summary,posts:_}}function tt({state:e,renderFn:t,documentObj:n,firestoreApi:s={},profileApi:o={},composerApi:l={},iconFn:b,storageObj:$}={}){const _=n||(typeof document>"u"?null:document),u=_?.defaultView||(typeof window>"u"?null:window),S=typeof t=="function"?t:()=>{},v=$||(typeof localStorage>"u"?null:localStorage),W=typeof o.getBusinessProfileTypeFn=="function"?o.getBusinessProfileTypeFn:(()=>""),X=typeof o.isShopCatalogProfileFn=="function"?o.isShopCatalogProfileFn:(()=>!1),Y=typeof o.isBusinessOwnerProfileFn=="function"?o.isBusinessOwnerProfileFn:(()=>!1),Z=typeof o.canAccessRestaurantOrdersFn=="function"?o.canAccessRestaurantOrdersFn:(()=>!1),B=typeof o.getRestaurantMetaByIdFn=="function"?o.getRestaurantMetaByIdFn:(()=>null),ee=typeof o.resolveRestaurantLogoFn=="function"?o.resolveRestaurantLogoFn:(()=>""),te=typeof o.resolveOwnAvatarUrlFn=="function"?o.resolveOwnAvatarUrlFn:(()=>"");let F=0,K=!1,k=null,z=null,C="",M=!1,O=()=>null;const ae=300;function E(){const a=e?.userProfile||{};return Re({businessType:W(a),isShopCatalog:X(a)})}function ne(a=""){const r=B(a)||{};return Fe(r).map(i=>({id:i.id,name:i.title,price:i.price??"",category:i.beds||i.tag||"",type:"room",imageUrl:i.imageUrl||""}))}function re(a=""){if(!v)return null;try{const r=v.getItem(`${Q}${a}`);if(!r)return null;const i=JSON.parse(r),d=Array.isArray(i?.items)?i.items:null;return d&&d.length?d:null}catch{return null}}function se(a="",r=[]){if(v)try{v.setItem(`${Q}${a}`,JSON.stringify({savedAt:Date.now(),items:r}))}catch{}}async function ie(a=""){const{db:r,collectionFn:i,queryFn:d,limitFn:c,getDocsFn:h}=s;if(!r||typeof i!="function"||typeof h!="function")throw new Error("Produktet nuk u ngarkuan.");const f=i(r,"restaurants",a,"menuItems"),y=typeof d=="function"&&typeof c=="function"?d(f,c(ae)):f,w=await h(y),p=[];return w.forEach(x=>{const j=O(x?.id,x?.data?.()||{});j&&p.push(j)}),p.sort((x,j)=>x.name.localeCompare(j.name,"sq")),p}async function oe(a="",r){const i=String(a||"").trim();if(!i)throw new Error("Produktet nuk u ngarkuan.");if(E()==="hotel")return ne(i);const d=ie(i).then(h=>(se(i,h),h)),c=re(i);return c?(typeof r=="function"?d.then(h=>r(h)).catch(()=>{}):d.catch(()=>{}),c):d}function L(){return k?Promise.resolve(k):(z||(z=ye(()=>import("./business-composer-controller-9aWMxrys.js"),__vite__mapDeps([0,1,2,3,4,5,6])).then(a=>(O=typeof a?.normalizeComposerProductCore=="function"?a.normalizeComposerProductCore:(()=>null),k=a.createBusinessComposerController({documentObj:_,windowObj:_?.defaultView||null,api:{getRestaurantIdFn:()=>D(),getBusinessMetaFn:()=>{const r=D();if(!r)return{name:"",logoUrl:"",city:""};const i=A(r),d=B(r)||{};return{name:i.name,logoUrl:i.logoUrl,city:String(d.city||"").trim()}},loadProductsFn:(r,i)=>oe(r,i),getBusinessKindFn:()=>E(),uploadImageFn:l.uploadImageFn,uploadVideoFn:l.uploadVideoFn,captureVideoPosterFn:l.captureVideoPosterFn,createPostFn:l.createPostFn,createStoryFn:l.createStoryFn,formatPriceFn:l.formatPriceFn,getOptimizedImageUrlFn:l.getOptimizedImageUrlFn,escapeHtmlFn:l.escapeHtmlFn,iconFn:typeof b=="function"?b:void 0,afterPublishFn:async r=>{try{await R({force:!0})}catch{}typeof l.afterPublishFn=="function"&&await l.afterPublishFn(r)}}}),k)).catch(a=>{throw z=null,console.error("[mnyra][dashboard] composer load failed",a),a})),z)}function de(){const a=u?.navigator?.connection;return!a||typeof a!="object"?!1:a.saveData===!0?!0:/(^|-)2g$/.test(String(a.effectiveType||"").trim().toLowerCase())}function le(){if(M||k||!u||de())return;M=!0;const a=()=>{if(L().catch(()=>{}),typeof l.prewarmFn=="function")try{l.prewarmFn()}catch{}};if(typeof u.requestIdleCallback=="function"){u.requestIdleCallback(a,{timeout:Ae});return}u.setTimeout?.(a,Ve)}function ce(a="post"){const r=String(a||"").trim().toLowerCase(),i=r==="story"||r==="profile"?r:"post";if(typeof l.prewarmFn=="function")try{l.prewarmFn()}catch{}if(k){k.open(i);return}C=i,L().then(d=>{const c=C||i;C="",d?.open?.(c)}).catch(()=>{C=""})}function H(){return(!e.dashboardView||typeof e.dashboardView!="object")&&(e.dashboardView={status:"idle",error:"",model:null,loadedSignature:""}),e.dashboardView}function D(){const a=e?.userProfile||{};return String(a.restaurantId||a.staffRestaurantId||"").trim()}function ue(){const a=String(e?.user?.uid||"").trim();if(!a)return!1;const r=String(e?.__authBootstrapInFlightUid||"").trim();return!!e?.__authProfileLoadPromise||r===a}function U(a=""){return`${Ue}${a}`}function he(a="",r=""){if(!v||!a)return null;try{const i=v.getItem(U(a));if(!i)return null;const d=JSON.parse(i);return!d||typeof d!="object"||String(d.day||"").trim()!==String(r||"").trim()||!d.model||typeof d.model!="object"?null:d.model}catch{return null}}function me(a="",r=null){if(!(!v||!a||!r))try{v.setItem(U(a),JSON.stringify({day:r.day,model:r}))}catch{}}async function pe(a=""){const{db:r,collectionFn:i,queryFn:d,orderByFn:c,limitFn:h,getDocsFn:f}=s;if(!r||typeof i!="function"||typeof d!="function"||typeof c!="function"||typeof h!="function"||typeof f!="function")return[];const y=i(r,"restaurants",a,"socialPosts");return(await f(d(y,c("createdAt","desc"),h(qe)))).docs.map(p=>({id:p.id,data:p.data()||{}})).filter(p=>{const x=String(p.data.status||"active").trim().toLowerCase();return x!=="deleted"&&x!=="hidden"})}async function R({force:a=!1}={}){const r=H(),i=D();if(!i)return;const d=be({rangeKey:"7d"});if(!d)return;const c=`${i}::${d.toDay}`;if(!a&&r.loadedSignature===c&&r.status==="ready")return;if(!r.model){const y=he(i,d.toDay);y&&(r.model=y,r.status="ready",S())}F+=1;const h=F;r.model||(r.status="loading",r.error="",S());try{const y={db:s.db,collectionFn:s.collectionFn,queryFn:s.queryFn,whereFn:s.whereFn,documentIdFn:s.documentIdFn,getDocsFn:s.getDocsFn,restaurantId:i},[w,p]=await Promise.allSettled([_e({...y,fromDay:d.fromDay,toDay:d.toDay}),pe(i)]);if(h!==F)return;if(w.status==="rejected")throw w.reason;p.status==="rejected"&&console.error("[mnyra][dashboard] recent posts load failed",p.reason),r.model=We({days:w.value,todayKey:d.toDay,rawPosts:p.status==="fulfilled"?p.value:[]}),r.status="ready",r.error="",r.loadedSignature=c,me(i,r.model)}catch(y){if(h!==F)return;console.error("[mnyra][dashboard] load failed",y),r.model||(r.status="error",r.error="Ju lutem kontrollo lidhjen dhe provo perseri.")}S()}function fe(){K||!_||(K=!0,_.addEventListener("click",a=>{try{if(String(e?.activeTab||"").trim().toLowerCase()!=="dashboard")return;if(a.target?.closest?.("[data-dashboard-retry]")){R({force:!0});return}const r=a.target?.closest?.("[data-dashboard-composer]");r&&(a.preventDefault(),ce(r.getAttribute("data-dashboard-composer")))}catch{}}))}function A(a=""){const r=e?.userProfile||{},i=a?B(a)||{}:{},d=String(i.name||i.restaurantName||r.name||"").trim()||"Business";let c="";try{c=String(te()||"").trim()}catch{}if(!c)try{c=String(ee(i)||"").trim()}catch{}return{name:d,logoUrl:c,kind:E()}}function ge(){Ce(_),fe();const a=H(),r=D();let i="";if(!r)i=ue()?`${Oe()}${J({kpiCount:6})}`:He();else{le();const d=A(r),c=je({kind:d.kind,isOwner:Y(e?.userProfile),canAccessOrders:Z(e?.userProfile)}),h=Ie(d.kind);a.status==="idle"&&(a.status="loading",queueMicrotask(()=>{R({force:!1})}));let f="";a.model?f=`
          ${Ke({kpiDefs:h,week:a.model.week,today:a.model.today})}
          ${Me({posts:a.model.posts,iconFn:b})}
        `:a.status==="error"?f=Le({message:a.error}):f=J({kpiCount:h.length}),i=`
        ${Be({name:d.name,logoUrl:d.logoUrl,iconFn:b})}
        ${Ee({iconFn:b})}
        ${Te(`
          ${Ne({actions:c,iconFn:b})}
          ${f}
        `)}
      `}return`
    ...
    `}return Object.freeze({renderDashboardView:ge,loadDashboard:R})}export{ke as M,tt as a,Fe as b,xe as c,Ze as d,et as f,Pe as n};
