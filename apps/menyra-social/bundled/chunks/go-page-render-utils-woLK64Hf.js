...
`,s=Object.freeze({brand:"Mnyra GO",partyQuestion:"Sa veta jeni?",partyOne:"person",partyMany:"veta",step:"Hapi",stepBack:"Kthehu një hap prapa",next:"Vazhdo",categoryQuestion:"Çka dëshironi?",whenQuestion:"Kur?",whenLaterLabel:"Zgjidh ditën dhe orën",placeQuestion:"Ku?",placeLabel:"Qyteti",placeEmpty:"Shto qytetin tënd",cityPlaceholder:"Shkruaj qytetin",locationChange:"Ndrysho",locationSave:"Ruaj",submit:"Shiko ofertat",searching:"Po kërkojmë...",resultsHeadline:"lokale kanë oferta për ju",offering:"po ju ofron",forGroup:"për grupin tuaj",accept:"Prano ofertën",confirming:"Po konfirmohet...",sponsored:"Sponsored",onlyGo:"Vetëm me Mnyra GO",tableIncluded:"Tavolinë",emptyTitle:"Nuk gjetëm ofertë GO që përputhet tani.",emptySubtitle:"Provo me një orë tjetër ose me një grup tjetër.",doneTitle:"U krye",reservationConfirmed:"Tavolina është konfirmuar",claimConfirmed:"Oferta është e juaja",menu:"Shiko menunë",directions:"Udhëzime",code:"Kodi",cancel:"Anulo",cancelConfirm:"Dëshiron ta anulosh?",cancelYes:"Po, anuloje",cancelNo:"Jo",saveToAccount:"Ruaje në llogarinë tënde",signIn:"Hyr",errorTitle:"Mnyra GO është përkohësisht i padisponueshëm.",errorRetry:"Provo prapë",alternatives:"alternativa për ju",peopleSuffix:"persona",now:"Tani",back:"Ndrysho kërkimin"}),w="/apps/menyra-social/assets/go/",k=Object.freeze([Object.freeze({file:"story-1-uritur.webp",alt:"Vajzë ulur në tavolinë, e uritur",side:"right",headline:Object.freeze(["A je ",Object.freeze({accent:"unt?"})]),text:Object.freeze(["Hape Mnyra GO edhe thuaj veç ",Object.freeze({accent:"sa veta jeni"})," edhe çka po ju hahet."])}),Object.freeze({file:"story-2-kerkim.webp",alt:"Vajzë që shikon telefonin, tuj kërku lokal",side:"left",headline:Object.freeze(["Ku me ",Object.freeze({accent:"shku?"})]),text:Object.freeze(["Po kërkon restorant a kafe, po s’po din ku? ",Object.freeze({accent:"Mos kërko"})," — lokalet t’gjejnë ty."])}),Object.freeze({file:"story-3-oferta.webp",alt:"Vajzë e gëzueme me telefon në dorë",side:"right",headline:Object.freeze(["Ofertat ",Object.freeze({accent:"t’vijn."})]),text:Object.freeze(["Lokalet përreth teje t’çojnë zbritje a diçka falas, ",Object.freeze({accent:"veç për grupin tënd"}),"."])}),Object.freeze({file:"story-4-tavolina.webp",alt:"Dy shoqe në tavolinë të lokalit",side:"right",headline:null,text:Object.freeze(["Zgjedh njënën, shko edhe ",Object.freeze({accent:"knaqu."})," Kaq âsht Mnyra GO."])})]);function _(a=[],e="mnyra-go-page__story-accent"){return(Array.isArray(a)?a:[a]).map(n=>{if(typeof n=="string")return t(n);const r=typeof n?.accent=="string"?n.accent:"";return r?`<span class="${e}">${t(r)}</span>`:""}).join("")}function ne(a=[]){return(Array.isArray(a)?a:[a]).map(e=>typeof e=="string"?e:String(e?.accent||"")).join("")}const E=Object.freeze({now:"zap",in30:"timer",in60:"clock",later:"calendar-clock"}),p=Object.freeze(["party","category","when","place"]),$=Object.freeze({party:"users",category:"utensils",when:"clock",place:"map-pin"});function t(a=""){return String(a??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function f(a=""){const e=String(a||"").trim();return p.includes(e)?e:p[0]}function te(a=""){const e=p.indexOf(f(a));return p[Math.min(p.length-1,e+1)]}function re(a=""){const e=p.indexOf(f(a));return p[Math.max(0,e-1)]}function u(a){const e=Math.trunc(Number(a));return Number.isFinite(e)?Math.min(m,Math.max(l,e)):A}function z(a,e=s){return u(a)===1?e.partyOne||s.partyOne:e.partyMany||s.partyMany}function S(a,e=s){return`${u(a)} ${z(a,e)}`}function D(a){const e=u(a),n=m-l;return Math.round((e-l)/n*100)}function M(a,{active:e=!1,attr:n="",value:r="",iconName:i="",wide:g=!1}={}){return`
    <button
      type="button"
      class="mnyra-go-page__chip${g?" mnyra-go-page__chip--wide":""}"
      ${n?`${n}="${t(r)}"`:""}
      aria-pressed="${e?"true":"false"}"
    >${i?o(i):""}<span>${t(a)}</span></button>
  `}function j(a,{nowMs:e=Date.now()}={}){const n=Date.parse(String(a||""));if(!Number.isFinite(n))return"";if(Math.abs(n-e)<900*1e3)return s.now;const r=new Date(n);return`Rreth ${String(r.getHours()).padStart(2,"0")}:${String(r.getMinutes()).padStart(2,"0")}`}function G(a="",e={},n=s){if(a==="party")return S(e.partySize,n);if(a==="category"){const r=b.find(i=>i.key===String(e.category||"all"));return r?r.label:""}if(a==="when"){const r=v.find(i=>i.key===String(e.when||"now"));return r?r.label:""}return a==="place"?String(e.city||"").trim()||n.placeEmpty:""}function F(a="",e={},n=s){const r=p.indexOf(a),i=p.slice(0,Math.max(0,r));return i.length?`
    <div class="mnyra-go-page__ask-done">
      ${i.map(g=>`
        <button type="button" class="mnyra-go-page__ask-tag" data-go-goto="${t(g)}">
          ${o($[g]||"check-check")}<span>${t(G(g,e,n))}</span>
        </button>
      `).join("")}
    </div>
  `:""}function T(a={},e=s){const n=u(a.partySize);return`
    <output class="mnyra-go-page__party-value" data-go-party-value for="mnyraGoParty">
      ${t(String(n))} <span>${t(z(n,e))}</span>
    </output>
    <input
      id="mnyraGoParty"
      type="range"
      ...
      step="1"
      value="${n}"
      style="--go-range-fill:${D(n)}%;"
      aria-label="${t(e.partyQuestion)}"
      aria-valuetext="${t(S(n,e))}"
    />
    <div class="mnyra-go-page__range-scale" aria-hidden="true">
      <span>${l}</span>
      <span>${m}</span>
    </div>
  `}function K(a={},e=s){const n=String(a.category||"all");return`
    <div class="mnyra-go-page__chips" role="group" aria-label="${t(e.categoryQuestion)}">
      ${b.map((r,i)=>M(r.label,{active:n===r.key,attr:"data-go-category",value:r.key,iconName:r.icon,wide:i===0})).join("")}
    </div>
  `}function P(a={},e=s){const n=String(a.when||"now");return`
    <div class="mnyra-go-page__chips" role="group" aria-label="${t(e.whenQuestion)}">
      ${v.map(r=>M(r.label,{active:n===r.key,attr:"data-go-when",value:r.key,iconName:E[r.key]||"clock"})).join("")}
    </div>
    ${n==="later"?`
      <p class="mnyra-go-page__field-label">${t(e.whenLaterLabel)}</p>
      <input
        type="datetime-local"
        class="mnyra-go-page__field"
        data-go-when-input
        value="${t(a.laterValue||"")}"
        aria-label="${t(e.whenLaterLabel)}"
      />
    `:""}
  `}function R(a={},e=s){const n=String(a.city||"").trim();return a.editCity?`
      <div class="mnyra-go-page__place">
        ...
        <span class="mnyra-go-page__place-body">
          <input
            ...
            data-go-city-input
            value="${t(n)}"
            placeholder="${t(e.cityPlaceholder)}"
            aria-label="${t(e.placeQuestion)}"
    ...
    <div class="mnyra-go-page__place">
      <span class="mnyra-go-page__place-ic">${o("map-pin")}</span>
      <span class="mnyra-go-page__place-body">
        <span class="mnyra-go-page__place-label">${t(e.placeLabel)}</span>
        <p class="mnyra-go-page__place-city">${t(n||e.placeEmpty)}</p>
      </span>
      <button type="button" class="mnyra-go-page__place-change" data-go-change-city>
        ${o("pencil")}${t(e.locationChange)}
      </button>
    </div>
  `}function L(a={},e=s){const n=f(a.step),r=p.indexOf(n),i=p.length,g=Math.round((r+1)/i*100);let d="",c="",h="";return n==="party"?(c=e.partyQuestion,d=T(a,e),h=`<button type="button" class="mnyra-go-page__cta" data-go-step-next>${t(e.next)}${o("chevron-right")}</button>`):n==="category"?(c=e.categoryQuestion,d=K(a,e)):n==="when"?(c=e.whenQuestion,d=P(a,e),String(a.when||"now")==="later"&&(h=`<button type="button" class="mnyra-go-page__cta" data-go-step-next>${t(e.next)}${o("chevron-right")}</button>`)):(c=e.placeQuestion,d=R(a,e),h=`<button type="button" class="mnyra-go-page__cta" data-go-submit>${o("search")}${t(e.submit)}</button>`),`
    <section class="mnyra-go-page__ask" data-go-step="${t(n)}">
      <header class="mnyra-go-page__ask-head">
        ${r>0?`<button type="button" class="mnyra-go-page__ask-back" data-go-step-back aria-label="${t(e.stepBack)}" title="${t(e.stepBack)}">${o("arrow-left")}</button>`:""}
        <div class="mnyra-go-page__ask-meta">
          <p class="mnyra-go-page__ask-count">${t(e.step)} ${r+1}/${i}</p>
          <div class="mnyra-go-page__ask-bar" role="progressbar" aria-valuemin="1" aria-valuemax="${i}" aria-valuenow="${r+1}">
      ...
      <h2 class="mnyra-go-page__q">${o($[n]||"users")}${t(c)}</h2>
      ${d}
      ${h}
    </section>
  `}function O(a={},{texts:e=s,busyOfferId:n="",nowMs:r=Date.now()}={}){const i=n&&n===a.offerId,g=Number.isFinite(Number(a.distanceKm))&&a.distanceKm!==null?`${Number(a.distanceKm).toFixed(1)} km`:"",d=a.isNow?e.now:j(a.expectedArrivalAt,{nowMs:r});return`
    <article class="mnyra-go-page__card" data-go-result="${t(a.offerId)}">
      <div class="mnyra-go-page__card-head">
        ${a.logoUrl?`<img class="mnyra-go-page__card-logo" src="${t(a.logoUrl)}" alt="" width="40" height="40" loading="lazy" decoding="async" />`:`<div class="mnyra-go-page__card-logo mnyra-go-page__card-logo--empty">${o("store")}</div>`}
        <div class="mnyra-go-page__card-names">
          <p class="mnyra-go-page__card-who">${t(a.businessName)} <span>${t(e.offering)}</span></p>
          ${a.sponsored?`<p class="mnyra-go-page__card-sponsored">${t(e.sponsored)}</p>`:""}
        </div>
      </div>

      <p class="mnyra-go-page__card-benefit">${t(a.benefitLabel)}</p>
      <p class="mnyra-go-page__card-for">${t(e.forGroup)}</p>

      <div class="mnyra-go-page__card-meta">
        <span>${o("users")}${t(`${a.partySize} ${e.peopleSuffix}`)}</span>
        ${d?`<span>${o("clock")}${t(d)}</span>`:""}
        ${g?`<span>${o("map-pin")}${t(g)}</span>`:""}
        ${a.bookingType==="reservation"?`<span>${o("armchair")}${t(e.tableIncluded)}</span>`:""}
      </div>

      <p class="mnyra-go-page__card-only">${o("ticket-percent")}${t(e.onlyGo)}</p>

      <button
        type="button"
        class="mnyra-go-page__cta"
        data-go-accept="${t(a.offerId)}"
        data-go-restaurant="${t(a.restaurantId)}"
        ${i?"disabled":""}
      >${i?"":o("check-check")}${t(i?e.confirming:e.accept)}</button>
    </article>
  `}function I(a={},e=s){const n=Array.isArray(a.results)?a.results:[],r=Number(a.nowMs)||Date.now();return n.length?`
    <h2 class="mnyra-go-page__lead">${n.length} ${t(e.resultsHeadline)}</h2>
    ${a.notice?`<p class="mnyra-go-page__note">${t(a.notice)}</p>`:""}
    ${n.map(i=>O(i,{texts:e,busyOfferId:a.busyOfferId,nowMs:r})).join("")}
    <button type="button" class="mnyra-go-page__cta mnyra-go-page__cta--quiet" data-go-back>
      ${o("arrow-left")}${t(e.back)}
    </button>
  `:`
      <h2 class="mnyra-go-page__lead">${t(e.emptyTitle)}</h2>
      <p class="mnyra-go-page__hint">${t(e.emptySubtitle)}</p>
      <button type="button" class="mnyra-go-page__cta mnyra-go-page__cta--quiet" data-go-back>
        ${o("arrow-left")}${t(e.back)}
      </button>
    `}function C(a={},e=s){const n=a.booking||{},r=n.type==="reservation",i=Number(a.nowMs)||Date.now(),g=j(n.expectedArrivalAt,{nowMs:i}),d=String(a.statusLabel||"").trim()||(r?e.reservationConfirmed:e.claimConfirmed);return a.confirmCancel?`
      <h2 class="mnyra-go-page__lead">${t(e.cancelConfirm)}</h2>
      <div class="mnyra-go-page__row">
        <button type="button" class="mnyra-go-page__cta" data-go-cancel-confirm>
          ${o("ban")}${t(e.cancelYes)}
        </button>
        <button type="button" class="mnyra-go-page__cta mnyra-go-page__cta--quiet" data-go-cancel-dismiss>${t(e.cancelNo)}</button>
      </div>
    `:`
    <p class="mnyra-go-page__done">${t(e.doneTitle)} ${o("party-popper")}</p>
    <p class="mnyra-go-page__done-name">${t(n.businessName||"")}</p>
    <p class="mnyra-go-page__card-for">${t(n.benefitLabel||"")}</p>

    <div class="mnyra-go-page__card-meta">
      <span>${o("users")}${t(`${n.partySize||1} ${e.peopleSuffix}`)}</span>
      ${g?`<span>${o("clock")}${t(g)}</span>`:""}
      ${n.city?`<span>${o("map-pin")}${t(n.city)}</span>`:""}
    </div>

    <p class="mnyra-go-page__ok">${o("circle-check-big")}${t(d)}</p>

    ${n.shortCode?`
      <div class="mnyra-go-page__code">
        <p class="mnyra-go-page__code-label">${o("hash")}${t(e.code)}</p>
        <p class="mnyra-go-page__code-value">${t(n.shortCode)}</p>
      </div>
    `:""}

    <div class="mnyra-go-page__row">
      <button type="button" class="mnyra-go-page__cta mnyra-go-page__cta--quiet" data-go-menu>
        ${o("book-open")}${t(e.menu)}
      </button>
      <button type="button" class="mnyra-go-page__cta mnyra-go-page__cta--quiet" data-go-directions>
        ${o("navigation")}${t(e.directions)}
      </button>
    </div>

    ${a.canSignIn?`
      <button type="button" class="mnyra-go-page__cta mnyra-go-page__cta--quiet" data-go-signin>
        ${o("log-in")}${t(e.saveToAccount)} · ${t(e.signIn)}
      </button>
    `:""}

    <button type="button" class="mnyra-go-page__ghost" data-go-cancel>${t(e.cancel)}</button>
  `}function H(a={},e=s){const n=String(a.error||"").trim()||e.errorTitle;return`
    <h2 class="mnyra-go-page__lead">${t(n)}</h2>
    ${Array.isArray(a.alternatives)&&a.alternatives.length?`
      <p class="mnyra-go-page__hint">${a.alternatives.length} ${t(e.alternatives)}</p>
      ${a.alternatives.map(r=>O(r,{texts:e,nowMs:a.nowMs})).join("")}
    `:`
      <button type="button" class="mnyra-go-page__cta" data-go-retry>
        ${o("rotate-ccw")}${t(e.errorRetry)}
      </button>
    `}
  `}function N(a=[]){const e=Array.isArray(a)?a:[];return`
    <div class="mnyra-go-page__story" data-go-story>
      ${k.map((n,r)=>{const i=r===0;return`
        <article
          class="mnyra-go-page__story-slide"
          data-go-story-slide="${r}"
          ${e.includes(r)?'data-go-story-in="1"':""}
        >
          <figure class="mnyra-go-page__story-media" data-go-story-side="${t(n.side||"right")}">
            <img
              src="${t(w+n.file)}"
              alt="${t(n.alt||"")}"
              width="1600"
              ...
              onerror="this.remove()"
            />
            ${n.headline?`<figcaption class="mnyra-go-page__story-headline">${_(n.headline,"mnyra-go-page__story-headline-accent")}</figcaption>`:""}
          </figure>
          <p class="mnyra-go-page__story-text">${_(n.text)}</p>
        </article>
      `}).join("")}
    </div>
  `}function Z(a={}){return N(a.storyShown)}function ie(a={}){const e={...s,...a.texts||{}},n=String(a.view||"search"),r=n==="search";let i="";return n==="loading"?i=`<div class="mnyra-go-page__empty">${t(e.searching)}</div>`:n==="results"?i=I(a,e):n==="booking"?i=C(a,e):n==="error"?i=H(a,e):i=Z(a),`
    <div class="mnyra-go-page" data-go-page data-go-view="${t(n)}">
      ${r?`<div class="mnyra-go-page__top">${L(a.form||{},e)}</div>`:""}
      <div class="mnyra-go-page__bento" data-go-bento>${i}</div>
    </div>
  `}const oe=s,se=k,ge=w;export{ae as GO_PAGE_CSS,ge as GO_PAGE_STORY_BASE,se as GO_PAGE_STORY_SLIDES,ee as GO_PAGE_STYLE_ELEMENT_ID,oe as GO_PAGE_TEXTS,p as GO_STEPS,u as clampGoPartySize,D as goPartyFillPercent,S as goPartyLabel,z as goPartyWord,ne as goStoryPlainText,te as nextGoStep,re as previousGoStep,ie as renderGoPageCore,f as resolveGoStep};
