...
const socialRoot = document.getElementById("socialRoot");
...
const STORAGE_KEYS = {
  profile: "menyra_social2_profile_v1",
  settings: "menyra_social2_settings_v1",
  notifications: "menyra_social2_notifications_v1",
  following: "menyra_social2_following_v1",
  postMeta: "menyra_social2_post_meta_v1",
  checkins: "menyra_social2_checkins_v1"
};
...
const ADMIN_VIEW_CONFIG = {
  ceo: {
    label: "CEO Dashboard",
    htmlUrl: "/apps/menyra-ceo/dashboard.html",
    basePath: "/apps/menyra-ceo/"
  },
  staff: {
    label: "Staff Dashboard",
    htmlUrl: "/apps/menyra-staff/dashboard.html",
    basePath: "/apps/menyra-staff/"
  },
  owner: {
    label: "Owner Admin",
    htmlUrl: "/apps/menyra-owner/index.html",
    basePath: "/apps/menyra-owner/"
  }
};

let adminShellReady = false;
let currentAdminRole = "";
const baseBodyClass = document.body ? document.body.className : "";
...
const state = {
  ...
  adminViews: {
    ceo: { mounted: false, assetsLoaded: false, loading: false },
    staff: { mounted: false, assetsLoaded: false, loading: false },
    owner: { mounted: false, assetsLoaded: false, loading: false }
  },
  followingHandles: [],
  profileModal: {
    open: false,
...
function buildRoleContext(roles, restaurantId) {
  const list = normalizeRoleList(roles);
  const set = new Set(list);
  const isOwner = set.has("owner") || set.has("admin");
  const isManager = set.has("manager");
  const isCeo = set.has("ceo");
  const isStaff = set.has("staff");
  const isBusiness = isOwner || isManager || set.has("business");
...
async function loadRoleContext(user) {
  if (!user) return;
  const roles = new Set(normalizeRoleList(state.userProfile.roles || state.userProfile.role || ""));
  if (state.userProfile.role === "business") roles.add("business");
  let restaurantId = state.userProfile.restaurantId || "";

  const [ceoSnap, staffSnap, staffIndexSnap] = await Promise.all([
    getDoc(doc(db, "superadmins", user.uid)).catch(() => null),
    getDoc(doc(db, "staffAdmins", user.uid)).catch(() => null),
    getDoc(doc(db, "staffIndex", user.uid)).catch(() => null)
  ]);

  if (ceoSnap?.exists?.()) roles.add("ceo");
  if (staffSnap?.exists?.()) roles.add("staff");

  if (!restaurantId && staffIndexSnap?.exists?.()) {
    const data = staffIndexSnap.data() || {};
    const list = Array.isArray(data.restaurantIds) ? data.restaurantIds : [];
    restaurantId = list[0] || data.restaurantId || "";
  }

  if (restaurantId) {
    const staffRef = doc(db, "restaurants", restaurantId, "staff", user.uid);
    const staffDoc = await getDoc(staffRef).catch(() => null);
    if (staffDoc?.exists?.()) {
      const staffData = staffDoc.data() || {};
      normalizeRoleList(staffData.roles || staffData.role || "").forEach((r) => roles.add(r));
    }
  }

  const context = buildRoleContext(Array.from(roles), restaurantId);
  state.roleContext = context;
  state.userProfile.roles = context.roles;
  if (context.isBusiness) {
    state.userProfile.role = "business";
  }
  if (context.restaurantId && !state.userProfile.restaurantId) {
    state.userProfile.restaurantId = context.restaurantId;
  }
  if (!canShowProfile() && state.activeTab === "profile") {
...
function isBusinessAccount() {
  return state.roleContext.isBusiness || state.userProfile.role === "business";
}

function isStaffOnly() {
  return state.roleContext.isStaff && !state.roleContext.isBusiness && !state.roleContext.isCeo;
}

function canShowProfile() {
  return !state.roleContext.isCeo && !isStaffOnly();
}
...
function hasAdminAccess(role) {
  if (role === "ceo") return state.roleContext.isCeo;
  if (role === "staff") return state.roleContext.isStaff;
  if (role === "owner") return state.roleContext.isBusiness;
  return false;
}
...
function resolveAssetUrl(value, basePath) {
  ...
  try {
    return new URL(raw, window.location.origin + basePath).toString();
  } catch {
    return raw;
  }
}

async function loadAdminTemplate(role) {
  const view = state.adminViews[role];
  if (view?.template) return view.template;
  const config = ADMIN_VIEW_CONFIG[role];
  if (!config) return null;

  const res = await fetch(config.htmlUrl, { cache: "no-store" });
  if (!res.ok) throw new Error("Admin view load failed.");
  const html = await res.text();
  const doc = new DOMParser().parseFromString(html, "text/html");

  const links = Array.from(doc.querySelectorAll("link[rel=\"stylesheet\"]"))
    .map((link) => resolveAssetUrl(link.getAttribute("href"), config.basePath))
    .filter(Boolean);
  ...
  doc.querySelectorAll("script").forEach((script) => {
    ...
    if (src && src.includes("pwa.js")) return;
    scripts.push({
      src: src ? resolveAssetUrl(src, config.basePath) : "",
  ...
  if (view) view.template = template;
  return template;
}

function applyAdminAssets(role, assets) {
  if (!assets) return;
  const head = document.head;
  assets.links.forEach((href) => {
    if (head.querySelector(`link[data-admin-role="${role}"][href="${href}"]`)) return;
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = href;
    link.dataset.adminRole = role;
    head.appendChild(link);
  });
  assets.styles.forEach((css, idx) => {
    if (head.querySelector(`style[data-admin-role="${role}"][data-admin-style="${idx}"]`)) return;
    const style = document.createElement("style");
    style.dataset.adminRole = role;
    style.dataset.adminStyle = String(idx);
    style.textContent = css;
    head.appendChild(style);
  });
}

function removeAdminAssets(role) {
  if (!role) return;
  document.querySelectorAll(`[data-admin-role="${role}"]`).forEach((node) => node.remove());
  if (state.adminViews[role]) state.adminViews[role].assetsLoaded = false;
}

function applyAdminBodyClass(className) {
  const next = [baseBodyClass, className].filter(Boolean).join(" ").trim();
  if (document.body) document.body.className = next;
}

function restoreBodyClass() {
  if (document.body) document.body.className = baseBodyClass;
}

function fixRelativeUrls(root, basePath) {
  if (!root) return;
  root.querySelectorAll("[src]").forEach((el) => {
    const value = el.getAttribute("src");
    const next = resolveAssetUrl(value, basePath);
    if (next && next !== value) el.setAttribute("src", next);
  });
...
function ensureAdminShell() {
  if (adminShellReady || !adminRoot) return;
  adminRoot.innerHTML = `
    <div class="min-h-screen bg-slate-50 text-slate-900">
      <div class="sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b border-slate-100">
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <button id="adminBackBtn" class="px-4 py-2 rounded-2xl bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest">Zurueck</button>
            <div>
              <div class="text-[9px] font-black uppercase tracking-widest text-indigo-600">MENYRA Social 2</div>
              <div id="adminTitle" class="text-xl font-black italic uppercase tracking-tighter text-slate-900">Admin</div>
            </div>
          </div>
          <div class="text-[10px] font-bold uppercase tracking-widest text-slate-400">System</div>
        </div>
      </div>
      <div class="max-w-6xl mx-auto px-6 py-6">
        <div id="adminViewCeo" class="hidden"></div>
        <div id="adminViewStaff" class="hidden"></div>
        <div id="adminViewOwner" class="hidden"></div>
      </div>
    </div>
  `;
...
function setAdminViewVisible(role) {
  const map = {
    ceo: "adminViewCeo",
    staff: "adminViewStaff",
    owner: "adminViewOwner"
  };
  const titleEl = document.getElementById("adminTitle");
  if (titleEl) titleEl.textContent = ADMIN_VIEW_CONFIG[role]?.label || "Admin";
  Object.entries(map).forEach(([key, id]) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.classList.toggle("hidden", key !== role);
  });
}

async function mountAdminView(role) {
  const config = ADMIN_VIEW_CONFIG[role];
  if (!config) return;
  ensureAdminShell();
  setAdminViewVisible(role);

  const mountId = role === "ceo" ? "adminViewCeo" : role === "staff" ? "adminViewStaff" : "adminViewOwner";
  const mount = document.getElementById(mountId);
  if (!mount) return;

  const viewState = state.adminViews[role];
  if (viewState?.template?.bodyClass) {
    applyAdminBodyClass(viewState.template.bodyClass);
  }
  if (viewState.loading) return;
  viewState.loading = true;
  try {
    if (!viewState.mounted) {
      mount.innerHTML = `<div class="p-10 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">Lade ${config.label}...</div>`;
      const template = await loadAdminTemplate(role);
      if (!template) return;
      mount.innerHTML = template.body || "";
      fixRelativeUrls(mount, template.basePath);
      applyAdminAssets(role, template.assets);
      applyAdminBodyClass(template.bodyClass);
      viewState.assetsLoaded = true;
      ...
      for (const script of template.scripts) {
        if (script.src) {
          if (script.type === "module") {
            ...
            if (!document.querySelector(`script[data-admin-role="${role}"][src="${script.src}"]`)) {
              const tag = document.createElement("script");
              tag.src = script.src;
              tag.dataset.adminRole = role;
              document.body.appendChild(tag);
            }
          }
        } else if (script.inline) {
          const inlineTag = document.createElement("script");
          inlineTag.dataset.adminRole = role;
          inlineTag.textContent = script.inline;
          document.body.appendChild(inlineTag);
        }
      }

      viewState.mounted = true;
    } else if (!viewState.assetsLoaded) {
  ...
  if (currentAdminRole && currentAdminRole !== role) {
    removeAdminAssets(currentAdminRole);
  }
    ...
    <div class="min-h-screen bg-slate-50 flex flex-col p-8 font-sans animate-in">
      <div class="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full">
        <div class="mb-10 text-center">
          ...
          <p class="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2">Social 2 Login</p>
        </div>
...
function renderDrawer() {
  ...
  const adminItems = [];
  if (state.roleContext.isCeo) adminItems.push({ id: "admin-ceo", label: "CEO", icon: "shield" });
  if (state.roleContext.isStaff) adminItems.push({ id: "admin-staff", label: "Staff", icon: "briefcase" });
  if (state.roleContext.isBusiness) adminItems.push({ id: "admin-owner", label: "Owner", icon: "award" });
  const mainItems = [
    { id: "feed", label: "Feed", icon: "home" },
    { id: "map", label: "Karte", icon: "map" },
    ...(canShowProfile() ? [{ id: "profile", label: "Profil", icon: "user" }] : []),
    ...
    <div class="fixed inset-0 z-50 transition-all duration-500 ${state.drawerOpen ? "visible" : "invisible"}">
      ...
      <div class="absolute left-0 top-0 bottom-0 w-80 bg-white shadow-2xl transition-transform duration-500 p-8 flex flex-col ${state.drawerOpen ? "translate-x-0" : "-translate-x-full"}">
        ...
        <nav class="space-y-2 flex-1">
          ${adminItems.length ? `
            <div class="pt-2">
              <div class="px-2 pb-2 text-[9px] font-black uppercase tracking-widest text-slate-300">Admin</div>
              <div class="space-y-2">${adminItems.map(renderNavButton).join("")}</div>
            </div>
          ` : ""}
          <div class="pt-2">
            <div class="px-2 pb-2 text-[9px] font-black uppercase tracking-widest text-slate-300">Social</div>
            <div class="space-y-2">${mainItems.map(renderNavButton).join("")}</div>
          </div>
        </nav>
...
function renderProfileView() {
  if (!canShowProfile()) {
    return `
      <div class="p-8 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">
        Profil nicht verfuegbar.
      </div>
    `;
  }
  const profile = state.userProfile;
  const posts = isBusinessAccount() ? state.businessPosts : state.userPosts;
  return `
    <div class="p-8 animate-in slide-in-from-bottom-10 duration-700 pb-24">
      ...
      <div class="flex flex-col items-center text-center mb-10">
        ...
        <p class="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-1">${isBusinessAccount() ? "Business Account" : "Explorer"} / ${escapeHtml(profile.location || "-")}</p>
...
function renderUploadView() {
  ...
  if (!canUpload()) {
    return `
      <div class="p-8 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">
        Upload ist fuer deinen Account deaktiviert.
      </div>
    `;
  }
  return `
    <div class="p-6 animate-in slide-in-from-bottom-10 duration-700 min-h-[70vh] flex flex-col">
      ...
      ` : `
        <div id="uploadFileTrigger" class="flex-1 flex flex-col items-center justify-center rounded-[3rem] border-4 border-dashed p-8 text-center cursor-pointer transition-all border-slate-200 bg-white">
          ...
          <p class="text-sm font-medium text-slate-500">Posten als ${isBusinessAccount() ? "Business (Feed)" : "User (Profil)"}</p>
        </div>
      `}
    </div>
  `;
...
function renderMain() {
  let view = "";
  const safeTab = state.activeTab === "profile" && !canShowProfile() ? "feed" : state.activeTab;
  if (safeTab === "feed") view = renderFeedView();
  if (safeTab === "map") view = renderMapView();
  if (safeTab === "profile") view = renderProfileView();
  if (safeTab === "settings") view = renderSettingsView();
  if (safeTab === "notifications") view = renderNotificationsView();
  if (safeTab === "upload") view = renderUploadView();
...
function renderLoading() {
  return `
    <div class="min-h-screen flex items-center justify-center text-slate-400 text-sm font-bold">
      Lade MENYRA Social 2...
    </div>
  `;
}

function render() {
  const adminRole = resolveAdminRole(state.activeTab);
  if (!adminRole) restoreBodyClass();
  if (!state.sessionReady) {
    if (socialRoot) {
      socialRoot.classList.remove("hidden");
      socialRoot.innerHTML = renderLoading();
    }
    if (adminRoot) adminRoot.classList.add("hidden");
  } else if (!state.user) {
    if (socialRoot) {
      socialRoot.classList.remove("hidden");
      socialRoot.innerHTML = renderAuthScreen();
      bindAuthEvents();
    }
    if (adminRoot) adminRoot.classList.add("hidden");
  } else if (adminRole) {
    if (!hasAdminAccess(adminRole)) {
      state.activeTab = "feed";
      render();
      return;
    }
    if (socialRoot) socialRoot.classList.add("hidden");
    if (adminRoot) adminRoot.classList.remove("hidden");
    mountAdminView(adminRole).catch((err) => console.warn(err));
  } else {
    if (currentAdminRole) {
      removeAdminAssets(currentAdminRole);
      currentAdminRole = "";
    ...
    if (document.documentElement) {
      document.documentElement.classList.remove("is-pwa", "is-dark");
    }
    if (adminRoot) adminRoot.classList.add("hidden");
    if (socialRoot) {
      socialRoot.classList.remove("hidden");
      socialRoot.innerHTML = renderMain();
      bindAppEvents();
    }
  }
  ...
  if (state.user && !adminRole && state.activeTab === "map") {
    window.setTimeout(() => {
      initLeafletIfNeeded();
      updateMapSheet();
    }, 0);
...
function bindAppEvents() {
  const root = socialRoot || document;
  ...
  [logoutBtn, settingsLogout].forEach((btn) => {
    if (btn) {
      btn.addEventListener("click", async () => {
        ...
        state.likesModal = { open: false, postId: "" };
        state.selectedBusiness = null;
        state.roleContext = { roles: [], restaurantId: "", isCeo: false, isStaff: false, isOwner: false, isManager: false, isBusiness: false };
        state.adminViews = {
          ceo: { mounted: false, assetsLoaded: false, loading: false },
          staff: { mounted: false, assetsLoaded: false, loading: false },
          owner: { mounted: false, assetsLoaded: false, loading: false }
  ...
  root.querySelectorAll("[data-nav]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (!tab) return;
      if (tab === "profile" && !canShowProfile()) return;
      if (tab === "upload" && !canUpload()) return;
      setState({
        activeTab: tab,
        drawerOpen: false,
        settingsView: "main",
...
async function updateRestaurantSelection(restaurantId) {
  ...
  state.roleContext.restaurantId = restaurantId || state.roleContext.restaurantId || "";
  safeStorage.setItem(STORAGE_KEYS.profile, JSON.stringify(state.userProfile));
  render();
  try {
    await setDoc(doc(db, "users", state.user.uid), {
