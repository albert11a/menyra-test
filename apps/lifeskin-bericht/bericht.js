...
class Bericht {
  ...
  #einblenden(rolle) {
    ...
    // DIE BEWEGUNG SOLL MAN SEHEN - also beginnt sie AM RAND, nicht davor.
    //
    // Hier stand einmal eine halbe Fensterhoehe Vorlauf. Das war der
    // falsche Schluss aus einer richtigen Messung: Der Abschnitt war damit
    // fertig, BEVOR er ins Bild kam, und gesehen hat man von der Bewegung
    // gar nichts mehr - langsam gelesen so wenig wie schnell gewischt.
    //
    // Jetzt loest sie knapp am unteren Rand aus. Sie laeuft damit
    // vollstaendig innerhalb des sichtbaren Bereichs ab, und was man
    // sieht, ist der Abschnitt, der ankommt.
    ...
    for (const block of bloecke) {
      ...
      kinder.forEach((kind, i) => {
        ...
        // Hoechstens fuenf Stufen. Die Kaskade soll sichtbar sein, aber
        // die letzte Zeile darf nicht eine halbe Sekunde nach der ersten
        // kommen - beim schnellen Wischen waere sie dann die Einzige, die
        // niemand sieht.
    ...
    // KEINE AUSNAHME FUER SCHNELLES SCROLLEN.
    //
    // Hier stand eine: Ueber einem gemessenen Tempo erschien der Abschnitt
    // ohne Uebergang, und laufende Bewegungen wurden abgebrochen. Das
    // loeste ein anderes Problem als das vorliegende - es sorgte dafuer,
    // dass beim Wischen nichts BLASS ist, und nahm dabei genau das weg,
    // was man sehen soll. Wer schnell wischt, sieht die Bewegung eben
    // angeschnitten; das ist mehr als keine.
    const pruefen = () => {
      let offen = 0;
      for (const block of bloecke) {
