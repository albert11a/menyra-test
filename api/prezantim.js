// Landing 2 - die persoenliche Praesentation des jeweiligen Lokals.
//
// Dieselbe Aufgabe wie api/oferta.js, fuer die andere Seite. Bewusst eine
// eigene Datei und keine gemeinsame: Die beiden Landings teilen keine Zeile,
// damit eine Aenderung an der einen die andere nicht mitnehmen kann. Was hier
// steht, ist Auslieferung - gelesen wird nur, geschrieben nichts.
//
...
const SHELL_PATH = "apps/menyra-social/lead-landing-2/index.html";
...
function buildTags(profile, url) {
  ...
  parts.push("Profili juaj ne Mnyra eshte gati.");
...
function buildRestaurantHint(profile) {
  ...
  return id ? `<meta name="l2-restaurant" content="${esc(id)}" />` : "";
}
...
<head>
  ...
  <link rel="stylesheet" href="/apps/menyra-social/lead-landing-2/landing2-styles.css" />
</head>
<body>
  <main id="l2Root">
    <div class="l2-boot"><p class="l2-boot__text">Po ngarkohet...</p></div>
  </main>
  <script type="module" src="/apps/menyra-social/lead-landing-2/landing2-app.js"></script>
</body>
</html>`;
...
function readSlug(req) {
  ...
  const match = url.match(/\/prezantim\/([^/?#]+)/i);
  if (!match) return "";
  try {
...
export default async function prezantimHandler(req, res) {
  const slug = readSlug(req);
  const url = `${SITE}/prezantim/${encodeURIComponent(slug)}`;
