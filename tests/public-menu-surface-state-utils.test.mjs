...
test("public menu surface exposes error only for a matching public error truth", () => {
  const loadingSurface = resolveVisiblePublicMenuSurfaceState(
    {
      menu: {
        restaurantId: "restaurant-a",
        source: "public",
  ...
  const errorSurface = resolveVisiblePublicMenuSurfaceState(
    {
      menu: {
        restaurantId: "restaurant-a",
        source: "public",
        ...
        items: [],
        loading: false,
        error: "Menu konnte nicht geladen werden.",
      },
    },
    { profile },
  );

  assert.equal(loadingSurface.menu.status, "loading");
  assert.equal(loadingSurface.menu.error, "");
  assert.equal(errorSurface.menu.status, "error");
  assert.equal(errorSurface.menu.error, "Menu konnte nicht geladen werden.");
});
  ...
  const surface = resolveVisiblePublicMenuSurfaceState(
    {
      ...
      focus: {
        ...
        truthState: "seeded",
        items: [
          { id: "focus-1", targetMenuItemId: "missing-item", title: "Missing" },
        ],
        loading: false,
      },
    },
