...
// Generated from ../../../shared/go/go-guest-identity-core.js. Do not edit manually.
...
const GO_GUEST_TOKEN_VERSION = "g1";
const GO_GUEST_TOKEN_SEPARATOR = ".";
...
const GO_GUEST_TOKEN_STORAGE_KEY = "mnyra_go_guest_token_v1";
const GO_GUEST_COOKIE_NAME = "mnyra_go_guest";

// Ab wann ein Gast mehr zeigen muss als seinen Token. Bis dahin: keine
// zusaetzliche Huerde fuer niemanden (Punkt 36).
const GO_TRUST_LEVEL_NORMAL = "normal";
const GO_TRUST_LEVEL_WATCH = "watch";
const GO_TRUST_LEVEL_VERIFY = "verify";
 ...
 */
function buildGoGuestToken({ guestId = "", secret = "" } = {}) {
...
function parseGoGuestToken(token = "") {
 ...
 * nach aussen.
 */
function buildGoBookingToken({ bookingId = "", secret = "" } = {}) {
...
function parseGoBookingToken(token = "") {
  const parts = cleanPart(token, 400).split(GO_GUEST_TOKEN_SEPARATOR);
 ...
 */
function resolveGoGuestTrustLevel(stats = {}, { nowMs = Date.now() } = {}) {
...
function goGuestActionAllowed(level = GO_TRUST_LEVEL_NORMAL, { isSignedIn = false } = {}) {
 ...
 */
function applyGoGuestActivity(stats = {}, { kind = "", nowMs = Date.now() } = {}) {
  const source = stats && typeof stats === "object" ? { ...stats } : {};
...
module.exports = {
  GO_GUEST_TOKEN_VERSION,
  GO_GUEST_TOKEN_SEPARATOR,
  GO_GUEST_TOKEN_STORAGE_KEY,
  GO_GUEST_COOKIE_NAME,
