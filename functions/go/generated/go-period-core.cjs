...
// Generated from ../../../shared/go/go-period-core.js. Do not edit manually.
...
const GO_PERIODS = Object.freeze([
...
const GO_PERIOD_KEYS = Object.freeze(GO_PERIODS.map((entry) => entry.key));
const GO_PERIOD_DEFAULT = "sot";

function normalizeGoPeriod(value = "") {
  const key = String(value || "").trim().toLowerCase();
  return GO_PERIOD_KEYS.includes(key) ? key : GO_PERIOD_DEFAULT;
}

function goPeriodLabel(value = "") {
 ...
 * oben etwas wegnimmt.
 */
function buildGoCohortFunnel({ bookings = [], fromMs = 0, toMs = Date.now() } = {}) {
  let accepted = 0;
  let activated = 0;
  let finalized = 0;
 ...
 * abgerechnet.
 */
function countGoVisitors({ bookings = [], fromMs = 0, toMs = Date.now() } = {}) {
  let visits = 0;
  let visitors = 0;
