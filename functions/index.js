...
/* ------------------------------------------------- Lead-Landing: Messung */

// Was der Wirt auf seiner Landing tut, wird hier aufgezeichnet: welche Wische
// er gesehen hat, wie lange er auf jedem war, und wie er die drei Fragen
// beantwortet hat.
//
// Warum ueber eine Funktion und nicht direkt aus der Seite: Die Landing liest
// bisher ausschliesslich und hat kein Schreibrecht - genau das trennt sie von
// der echten App. Die Firestore-Regeln erlauben Schreiben auf restaurants/**
// ohnehin nur Inhaber und CEO. Haette man dafuer eine Regel geoeffnet, koennte
// jeder Sitzungen erfinden. Hier schreibt der Server mit Admin-Rechten, die
// Seite selbst bleibt ohne.
//
// Geschickt wird immer der ganze Stand, nicht die Aenderung seit dem letzten
// Mal. Geht eine Sendung verloren - und beim Schliessen der Seite geht schon
// mal eine verloren -, traegt die naechste alles nach. Deshalb auch merge:
// Dieselbe Sitzung darf beliebig oft ankommen.
const LANDING_STEP_LIMIT = 40;
const LANDING_SESSION_MAX_MS = 6 * 60 * 60 * 1000;

function landingText(value, max = 120) {
  return typeof value === "string" ? value.trim().slice(0, max) : "";
}

function landingMillis(value) {
  const number = Number(value);
  if (!Number.isFinite(number) || number < 0) return 0;
  return Math.min(Math.round(number), LANDING_SESSION_MAX_MS);
}

// Nur bekannte Formen kommen durch: Schluessel kurz und schlicht, Werte
// Zahlen. Ohne das koennte jemand beliebige Felder in das Dokument schreiben.
function landingSteps(raw) {
  const source = raw && typeof raw === "object" ? raw : {};
  const out = {};
  Object.keys(source).slice(0, LANDING_STEP_LIMIT).forEach((key) => {
    const name = landingText(key, 48);
    if (!name || !/^[a-z0-9-]+$/.test(name)) return;
    out[name] = landingMillis(source[key]);
  });
  return out;
}

function landingAnswers(raw) {
  const source = raw && typeof raw === "object" ? raw : {};
  const out = {};
  Object.keys(source).slice(0, 12).forEach((key) => {
    const name = landingText(key, 24);
    const value = landingText(source[key], 24);
    if (!name || !/^[a-z0-9]+$/.test(name)) return;
...
async function landingRestaurantIdFromSlug(rawSlug) {
  const slug = landingText(rawSlug, 72);
  if (!slug) return "";
  const db = admin.firestore();

  const routeSnap = await db.collection("publicRoutes").doc(slug).get();
  if (routeSnap.exists) {
    const found = landingText(routeSnap.get("restaurantId"))
      || landingText(routeSnap.get("canonicalRestaurantId"));
  ...
  for (const field of ["publicSlug", "landingSlug"]) {
    const query = await db.collection("restaurants").where(field, "==", slug).limit(1).get();
    if (!query.empty) return query.docs[0].id;
  }
  return "";
}

exports.landingTrack = functions
  .region("us-central1")
  .https.onRequest(async (req, res) => {
    sendCors(res, req);
    if (req.method === "OPTIONS") {
    ...
    // sendBeacon schickt beim Schliessen der Seite - dabei kommt der Koerper
    // je nach Browser als Text und nicht als geparstes JSON an.
    let body = req.body;
    ...
    const sessionId = landingText(body.sessionId, 64);
    ...
    try {
      const restaurantId = await landingRestaurantIdFromSlug(body.slug);
      if (!restaurantId) {
        res.status(404).json({ ok: false, error: "Unknown restaurant" });
        return;
      }

      const steps = landingSteps(body.steps);
      const answers = landingAnswers(body.answers);
      const outcome = ["yes", "no", ""].includes(landingText(body.outcome, 8))
        ? landingText(body.outcome, 8)
        : "";

      const ref = admin.firestore()
        .collection("restaurants").doc(restaurantId)
        .collection("landingSessions").doc(sessionId);

      const payload = {
        slug: landingText(body.slug, 72),
        steps,
        stepCount: Object.keys(steps).length,
        answers,
        answerCount: Object.keys(answers).length,
        outcome,
        totalMs: landingMillis(body.totalMs),
        updatedAt: FieldValue.serverTimestamp()
      };
      ...
      // geschlossen wurde - und eine Sitzung von gestern saehe aus wie eine
      // von eben.
      const existing = await ref.get();
      if (!existing.exists) payload.startedAt = FieldValue.serverTimestamp();

      await ref.set(payload, { merge: true });

      res.status(200).json({ ok: true });
    } catch (error) {
      logFunctionWarn("landing.track", { status: "failed", message: String(error && error.message) });
