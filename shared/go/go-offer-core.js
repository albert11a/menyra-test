...
export function normalizeGoBenefit(raw = {}) {
  ...
  const benefit = {
    ...
    text: cleanGoText(source.text || source.label, 160)
  };
  benefit.label = benefit.text || buildGoBenefitLabel(benefit);
  return benefit;
}
...
export function buildGoBenefitLabel(benefit = {}) {
  ...
  const text = cleanGoText(source.text || source.label, 160);
  if (text) return text;
