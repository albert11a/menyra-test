...
- Zurueck kommt die Zeile nicht in einem Bild, sondern indem sie ihren Platz aufzieht (`playMainHeaderTabsReveal()`, Klasse `html.smart-header-tabs-revealing`, 260ms). Ausgeblendet ist sie per `display:none` ganz aus dem Layout, der Content steht also um ihre Hoehe weiter oben. Unterwegs gleicht das ein Scroll aus (`scrollTo(scrollY + rowHeight)`), am Seitenanfang gibt es dafuer keinen Weg mehr - dort waere das Zurueckholen ein Ruck. Genau der faellt gegen das ruhige Wiederauftauchen beim Hochscrollen auf, wo die Zeile ihren Platz nie verloren hat.
- Aufgezogen wird die innere `.smart-header-tabs-row`, nicht die Tab-Zeile selbst: deren Schattenkante haengt als `::after` unter der Unterkante und wuerde von einem `overflow` auf der Zeile weggeschnitten.
