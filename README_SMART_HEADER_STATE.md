...
- Und zwar **fahrend**, nicht springend: `html.smart-header-tabs-animating` legt fuer `MAIN_HEADER_TABS_COLLAPSE_ANIM_MS` einen Uebergang auf die Hoehe (dazu `overflow: hidden`, und die eigene Schattenkante tritt solange hinter `.smart-header-underline` zurueck). Die Pill-Reihe faehrt dabei dieselbe Strecke nach oben, die die Zeile an Hoehe verliert (`--smart-header-tabs-row-height`, gleiche Dauer und Kurve): beim Scrollen wandert sie schliesslich hinter die Leiste, waehrend nur die Hoehe zu schrumpfen die Pills stehenliesse und von unten abschneiden wuerde - dieselbe Zeit, aber sichtbar eine andere Bewegung. Gemessen wird die Hoehe deshalb an der Pill-Reihe und nicht an der Zeile: mitten in der Fahrt haette ein Blick auf die Zeile deren halbe Hoehe als "die" Hoehe festgehalten und die Fahrt zugleich abgebrochen. Beschnitten wird auch im zugemachten Zustand selbst - sonst quillt die Pill-Reihe aus der Zeile ohne Hoehe heraus und liegt ohne jeden Abstand auf dem Text darunter: die Zeile ist "zu" und trotzdem zu sehen. `tests/smart-header-tabs-collapsed-clip.test.mjs` haelt beides fest. Ohne Pfeil bewegt sich die Zeile immer mit dem Finger; ein Sprung um ihre ganze Hoehe in einem Bild sah daneben hart aus. Die Klasse liegt nur waehrend der Fahrt: der stille Layout-Wechsel weiter unten muss sofort sitzen, weil dort der Scroll-Ausgleich im selben Bild dagegenhaelt.
