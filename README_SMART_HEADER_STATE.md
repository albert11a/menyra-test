...
- Beim Einfahren **von aussen** wird die Zeile erst still hinter die Leiste gestellt (`sliding` aus, `stuck` und `tucked` an), dann erzwingt `forceMainHeaderTabsReflow()` den Startpunkt und erst danach faehrt sie mit Uebergang hervor - sonst faengt die Fahrt beim Ziel an und ist nicht zu sehen.
- **Faehrt sie schon** (der Nutzer tippt schnell hin und her), wird nur die Richtung umgedreht: der Uebergang bleibt liegen und rechnet von der Stelle weiter, an der die Zeile gerade steht. Ihn dort abzuschalten hat sie erst hart hinter die Leiste springen lassen und von dort neu anfahren - genau das Ruckeln beim schnellen Auf und Zu.
...
- Gefahren wird die Seite dabei vom **Browser selbst** (`animateMainHeaderTabsScrollTo()` -> `scrollTo({ behavior: "smooth" })`). Das ist dieselbe Mechanik wie ein Wisch, laeuft ausserhalb des Hauptfadens - was der Pfeil oben macht, sieht deshalb aus wie Scrollen und fuehlt sich auch so an. Eine selbstgebaute Fahrt, die jeden Frame `scrollTop` schreibt, kann das nicht: ueber die kurze Strecke einer Zeilenhoehe bewegen sich die letzten Frames um Bruchteile eines Pixels, der Browser rundet auf ganze - und die Bewegung stockt sichtbar.
- Die Laufzeit schaut der Fahrt nur zu und schreibt im Normalfall gar nichts; jeder eigene Schreibvorgang wuerde sie abwuergen. Eingegriffen wird nur dort, wo der Browser allein nicht durchkommt: der Render-Pfad setzt die Scroll-Position bei jedem Re-Render auf den gemerkten Wert zurueck (`setViewportScrollTop()` / `scheduleViewportScrollTop()`). Laeuft die Strecke zum Ziel wieder auseinander (`MAIN_HEADER_TABS_SCROLL_REGRESSION_PX`) oder steht sie still, ohne angekommen zu sein (`MAIN_HEADER_TABS_SCROLL_STALL_MS`), wird die Fahrt neu angesetzt; nach der Ankunft wird das Ziel noch `MAIN_HEADER_TABS_SCROLL_HOLD_MS` lang gehalten. Ein `pointerdown`/`touchstart`/`wheel`/`keydown` bricht sofort alles ab - die Hand des Nutzers hat Vorrang.
