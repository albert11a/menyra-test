...
**Kein Vorfahr der Kopfzeile darf schneiden - und einer hat es getan (Stand 08.08.).** `#app` trug `overflow-x: clip` mit `overflow-y: visible` daneben. Die Absicht steht wortwoertlich da: waagerecht schneiden, senkrecht nicht. **WebKit haelt das nicht ein und schneidet bei `overflow-x: clip` auf beiden Achsen** - in jeder anderen Engine korrekt, auf WebKit seit Jahren offen und bis heute nicht behoben ([Apple Developer Forums #745729](https://developer.apple.com/forums/thread/745729), bestaetigt von Safari 18.3 bis macOS 26.3). Auf iOS gibt es keine andere Engine; Safari, Chrome und Edge sind alle WebKit.

Damit war der gesamte Aufwand oben auf dem Geraet wirkungslos, sobald der Ueberstand ueber den Dokumentanfang hinausragte: `#app` hat ihn abgeschnitten - **beide** Griffe, den Ueberstand an der Box wie die 320px an `.smart-header-shell::before`. Betroffen ist der oberste Scroll-Bereich (die Flaeche reicht rund 520px hinauf) und damit jede Seite, die kuerzer ist als dieser Ueberstand: kurze Business-Profile, leere Menues, das Location-Gate. Weiter unten im Dokument - etwa am Ende eines langen Feeds - liegt die Oberkante von `#app` laengst ausserhalb des Bildes, dort hat nie etwas geschnitten.

Der waagerechte Schnitt sitzt deshalb jetzt an `<main>` (`.app-main-scroll`), unterhalb der Kopfzeile. Dort steht ohnehin alles, was seitlich ausbrechen kann (Karussells, breite Kacheln, lange Namen); die Kopfzeile ist ein Geschwister davor und liegt nicht mehr darin. Die Modals (`fixed inset-0`) schneidet das nicht - ihr Bezugsrahmen ist das Bild, nicht `<main>`; Beleg ist die App selbst, sie standen bis dahin in `#app`, das genauso geschnitten hat, und waren auf dem Geraet nie gekappt.

`overflow-x: hidden` ist an beiden Stellen die falsche Abkuerzung: neben einem sichtbaren `overflow-y` rechnet die andere Achse auf `auto`, aus dem Element wird ein eigener Scroll-Container - und die klebende Kopfzeile haengt dann an ihm statt am Bild. `tests/smart-header-overscan-clip.test.mjs` haelt alle vier Punkte fest: `#app` schneidet nicht, der Schnitt existiert weiterhin genau einmal an `.app-main-scroll`, er wird nie auf `hidden` heruntergestuft, und die Kopfzeile wird ausserhalb von `<main>` gerendert.

**Warum das am Schreibtisch nicht auffaellt - und im Messgeraet auch nicht.** Chromium und Firefox setzen `overflow-x: clip` korrekt um; am Rechner sieht der Ueberstand also genau richtig aus. Und das Overlay aus `?debug-header=1` liest `getBoundingClientRect().top`: ein Versatz, der erst beim Malen entsteht, taucht dort gar nicht auf - die Zahl bleibt bei 0, waehrend der Bildschirm den Streifen zeigt. Das gilt auch fuer die Playwright-Laeufe, die Geometrie pruefen, nicht das Bild. Fuer diese Klasse Fehler bleibt die Bildschirmaufnahme vom Geraet das einzige gueltige Instrument.
