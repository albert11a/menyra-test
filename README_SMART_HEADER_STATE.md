...
- Ebenso raus ist die Mindesthoehe, die dem Hauptbereich frueher einen Scroll-Weg fuer den Pfeil garantierte. Sie hing an `--viewport-height`, und das aendert sich auf iOS **waehrend des Scrollens**, weil die Adressleiste einfaehrt (`visualViewport`-`scroll`/`resize`). Das Dokument wuchs dann unter dem Finger - der zweite Grund, warum die Seite nach einem Neuladen beim Scrollen sprang.

**3. Was von ihr zu sehen ist, wird gemessen und nie gerechnet.**

- `measureMainHeaderTabsRow()` misst, wie weit die Zeile unter der Leiste hervorschaut (`zeile.bottom - leiste.bottom`), dazu wie weit sie ohne Pfeil und ohne Kleben hervorschauen wuerde (`hoehe - scrollY` - ihr Platz im Dokument beginnt genau an der Unterkante der Leiste).
- Vorher wurde `scrollY < gerundete Zeilenhoehe` gerechnet. Die Zeile ist aber krumm hoch (40.67px) und Scroll-Positionen sind auf dem Geraet gebrochen: blieb die Seite einen Bruchteil unter dem gerundeten Wert stehen, galt die laengst verschwundene Zeile weiter als sichtbar - **und der Pfeil machte oben wieder zu, statt aufzumachen.**

Daraus ergeben sich genau zwei Bewegungen, beide reine `transform`-Fahrten - die Seite scrollt bei keiner davon:

- **ZU / AUF** (`html.smart-header-tabs-closed`): die Zeile faehrt um ihre eigene Hoehe nach oben hinter die Leiste und wieder hervor. Ihr Platz im Dokument bleibt stehen, die Seite bewegt sich nicht, der Abstand bleibt derselbe. Weil der Versatz konstant ist, gilt er auf **jeder** Scroll-Position: die zugemachte Zeile liegt oben wie unterwegs hinter der Leiste. Zu bleibt deshalb zu, bis der Nutzer wieder tippt - der ausdrueckliche Wunsch schlaegt die Scroll-Automatik.
- **GEHOLT / LOSGELASSEN** (`html.smart-header-tabs-stuck`, dazu `-tucked` fuer die Fahrt): weiter unten, wo ihr Platz laengst weggescrollt ist, klebt sie per `position: sticky` unter der Leiste. Sticky belegt exakt denselben Layout-Platz wie relative - die Leseposition steht auf den Pixel. Ein ausdruecklich geholtes Ziel hebt ein frueheres "zu" auf.

Dazwischen macht der Browser die Arbeit allein: offen sitzt die Zeile im Fluss unter der Leiste und scrollt hinter sie weg. Kein Zustand, kein Timer, kein Ausgleich - der Scroll-Listener aendert nur dann etwas, wenn die Zeile geklebt ist (Runterscrollen laesst sie los, ganz oben hoert das Kleben nahtlos auf).
...
- **Mitten in einer Fahrt** zaehlt ihr Ziel (`mainHeaderTabsIntent`) und nicht der Zwischenstand: ein Tipp soll die Bewegung umdrehen. Wuerde dort gemessen, laese der Tipp die halb hervorgefahrene Zeile als "zu sehen" und machte sie ein zweites Mal zu. Dasselbe Ziel dreht auch den Chevron sofort.
...
- Geklebt malt diese Kante, und `.smart-header-underline` tritt zurueck (`html.smart-header-tabs-stuck`).
- Zugemacht ist es umgekehrt: dort muss die Kante unter der Leiste bleiben, weil die Zeile weiter unten in der Seite ja weggescrollt ist. Damit die beiden sich nicht doppeln, faehrt die zugemachte Zeile um ihre Hoehe **plus die Kantenhoehe** (`--smart-header-edge-height`) - dann liegt auch ihr eigener Schatten hinter der Leiste.
- Beide Kanten sind derselbe Verlauf aus derselben Hoehe; der Test haelt das fest.
