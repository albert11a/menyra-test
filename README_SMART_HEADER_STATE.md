...
- Deshalb besteht der Pfeil auf seinem Ziel: `scrollMainHeaderTabsTo()` setzt die Position hart (`setViewportScrollTop()`, kein `behavior: "smooth"`) und danach nochmal ueber `MAIN_HEADER_TABS_SCROLL_ASSERT_DELAYS_MS` hinweg - wer zuletzt schreibt, gewinnt. Ein `pointerdown`/`touchstart`/`wheel`/`keydown` bricht das sofort ab: die Hand des Nutzers hat Vorrang.
- Erst wenn die Seite danach immer noch nicht oben steht, heftet `verifyMainHeaderTabsReveal()` die Zeile unter die Leiste. Das ist die **Notloesung**, nicht der Weg: geklebt liegt die Zeile ueber dem Inhalt, und am Seitenanfang faellt genau das auf - der Abstand unter den Pills fehlt, obwohl niemand gescrollt hat. Deshalb wird dort zuerst nochmal hart nach oben gescrollt und nur beim Scheitern geklebt. Ist die Seite inzwischen weiter nach unten gegangen, war das der Finger des Nutzers - dann haelt sich die Nachschau ganz heraus.
...
- Zu und wieder auf muss deshalb genau zum Startbild zurueckfuehren: Seite bei 0, Zeile an ihrem Platz, nichts geklebt. Festgehalten wird das in `tests/smart-header-tabs-toggle.test.mjs` - dazu der direkte Vergleich "mit Pfeil" gegen "ohne Pfeil", der Re-Render mitten im Pfeil-Scroll und der Fall, in dem der Browser den Scroll ganz verweigert.
- Nachgemessen wird auf der echten Safari-Engine (Playwright WebKit, iPhone-Profil), weil die Seite nur mobil benutzt wird.
