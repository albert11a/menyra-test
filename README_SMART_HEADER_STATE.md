...
- Wirklich weg ist er trotzdem: `visibility` springt am Ende der Fahrt auf `hidden` (Verzoegerung in Hoehe der Fahrtdauer) - nicht anfassbar, nicht anspringbar, nicht vorgelesen. Beim Erscheinen umgekehrt, sofort.
- Die Regeln sind auf `.smart-header-actions` eingegrenzt, damit sie die Utility-Klasse `.flex` und die Breiten-Regel fuer schmale Geraete sicher schlagen. Auf schmalen Geraeten setzt die Regel nur noch den **Wert** (`--smart-header-collapse-width`), nicht die Breite selbst - sonst schluege sie die Fahrt.
